...
wunjo_subdirs="plugins renderer data src src/ui"

# Extract displayed strings from JSON layouts
for file in data/layouts/*.json; do
    if [ -f "$file" ]; then
        name_string=$(jq '.wunjoInfo[] | .displayName' $file)
...
$EXTRACTRC --strip-cdata --tag=name --tag=description --tag=label --tag=comment --tag=paramlistdisplay data/transitions/*.xml data/transitions/frei0r/*.xml data/effects/*.xml data/effects/frei0r/*.xml data/effects/avfilter/*.xml data/effects/ladspa/*.xml data/effects/sox/*.xml data/generators/*.xml data/profiles.xml data/effectscategory.rc >> rc.cpp
$EXTRACTRC `find $wunjo_subdirs -name \*.rc -a ! -name encodingprofiles.rc -a ! -name camcorderfilters.rc -a ! -name externalproxies.rc -o -name \*.ui` >> rc.cpp

$XGETTEXT `find $wunjo_subdirs -name \*.cpp -o -name \*.h -o -name \*.qml` *.cpp -o $podir/wunjo.pot
rm -f rc.cpp
