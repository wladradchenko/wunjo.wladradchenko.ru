...
TEST_CASE("Simple Mix", "[SameTrackMix]")
{
    ...
    REQUIRE(pCore->getDurationFromString(WunjoSettings::mix_duration()) == 25);
    ...
    QString binId = WunjoTests::createProducerWithSound(pCore->getProjectProfile(), binModel, 100);

    // Create video clip
    QString binId2 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 50, false);
    ...
    int mixDuration = pCore->getDurationFromString(WunjoSettings::mix_duration());
    ...
    auto state0 = [&]() {
        ...
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->mixCount() == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 0);
    };
    ...
    auto state1 = [&]() {
        ...
        REQUIRE(timeline->getClipPlaytime(cid2) > 10);
        REQUIRE(timeline->getClipPosition(cid2) < 110);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid3)->mixCount() == 1);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 1);
    };
    ...
    SECTION("Create mix on color clips and move main (right side) clip")
    {
        ...
        REQUIRE(timeline->getClipSubPlaylistIndex(cid4) == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixCount() == 0);
        undoStack->undo();
        state2();
        undoStack->undo();
        state0();
    ...
    SECTION("Create mix on color clips and move some to another track")
    {
        ...
        REQUIRE(timeline->getClipSubPlaylistIndex(cid5) == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixCount() == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 2);
    ...
    SECTION("Create mix on color clips and group move")
    {
        ...
        REQUIRE(timeline->requestClipMove(cid4, tid4, 600));
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixCount() == 1);
        undoStack->undo();
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 1);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixCount() == 0);
        state2();
    ...
    SECTION("Create mix and move AV clips")
    {
        ...
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid3)->mixCount() == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 0);
        undoStack->undo();
        state1();
        // Undo mix
    ...
    SECTION("Create chained mixes on AV clips")
    {
        ...
        REQUIRE(timeline->mixClip(cid1));
        state1b();
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid2) == false);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid3)->mixIsReversed(audio2) == false);

        // Create mix between cid2 and cid5
        REQUIRE(timeline->mixClip(cid2));
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid2) == false);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid5) == true);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid3)->mixIsReversed(audio2) == false);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid3)->mixIsReversed(audio5) == true);
        // Undo cid5 mix
        undoStack->undo();
    ...
    SECTION("Test chained mixes on color clips")
    {
        ...
        REQUIRE(timeline->getClipSubPlaylistIndex(cid7) == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 3);
    ...
    SECTION("Test chained mixes and check mix direction")
    {
        ...
        auto mix0 = [&]() {
            ...
            REQUIRE(timeline->getClipSubPlaylistIndex(cid7) == 0);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 1);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid5) == false);
        };
        ...
        auto mix1 = [&]() {
            ...
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 2);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid5) == false);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid6) == true);
        };
        ...
        auto mix2 = [&]() {
            ...
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid5) == false);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid6) == true);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid7) == false);
        };
        ...
        auto mix3 = [&]() {
            ...
            REQUIRE(timeline->getClipSubPlaylistIndex(cid6) == 1);
            REQUIRE(timeline->getClipSubPlaylistIndex(cid7) == 0);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 4);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid4) == false);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid5) == true);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid6) == false);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixIsReversed(cid7) == true);
        };
    ...
    SECTION("Test mixes track move and direction")
    {
        ...
        auto mix3b = [&]() {
            ...
            REQUIRE(timeline->getClipSubPlaylistIndex(cid7) == 0);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid2)->mixCount() == 0);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixCount() == 4);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixIsReversed(cid4) == false);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixIsReversed(cid5) == true);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixIsReversed(cid6) == false);
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid4)->mixIsReversed(cid7) == true);
        };
