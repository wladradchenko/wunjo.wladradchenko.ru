...
TEST_CASE("Basic creation/deletion of a track", "[TrackModel]")
{
    ...
    std::shared_ptr<TimelineItemModel> tim = WunjoTests::createTimelineModel(document.uuid(), undoStack);
...
TEST_CASE("Adding multiple A/V tracks", "[TrackModel]")
{
    ...
    SECTION("Check AV track ordering")
    {
        ...
        for (int ix = 0; ix < max; ix++) {
            ...
            auto trackModel = WunjoTests::getTrackById_const(timeline, tid);
            if (ix < 5) {
...
TEST_CASE("Basic creation/deletion of a clip", "[ClipModel]")
{
    ...
    QString binId = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
    QString binId2 = WunjoTests::createProducer(pCore->getProjectProfile(), "green", binModel);
...
TEST_CASE("Clip manipulation", "[ClipModel]")
{
    ...
    QString binId3 = WunjoTests::createProducer(pCore->getProjectProfile(), "green", binModel);
    QString binId_unlimited = WunjoTests::createProducer(pCore->getProjectProfile(), "green", binModel, 20, false);
    ...
    SECTION("Group and selection")
    {
        ...
        auto state = [&]() {
            ...
            int gid = WunjoTests::groupsModel(timeline)->getRootId(cid1);
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid) == std::unordered_set<int>{cid1, cid2});
        };
        state();
...
TEST_CASE("Undo and Redo", "[ClipModel]")
{
    ...
    SECTION("requestCreateClip")
    {
        // an invalid clip id shouldn't get created
        {
            ...
            REQUIRE_FALSE(WunjoTests::requestClipCreation(timeline, "impossible bin id", temp, PlaylistState::VideoOnly, 1, 1., false, undo, redo));
        }
        ...
        int cid3;
        {
            Fun undo = []() { return true; };
            Fun redo = []() { return true; };
            REQUIRE(WunjoTests::requestClipCreation(timeline, binId3, cid3, PlaylistState::VideoOnly, 1, 1., false, undo, redo));
        ...
        int cid4;
        {
            Fun undo = []() { return true; };
            Fun redo = []() { return true; };
            REQUIRE(WunjoTests::requestClipCreation(timeline, binId4, cid4, PlaylistState::VideoOnly, 1, 1., false, undo, redo));
            pCore->pushUndo(undo, redo, QString());
        }
    ...
    {
        QString binId3 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
    ...
    SECTION("Clip creation and resize")
    {
...

        {
            std::function<bool(void)> undo = []() { return true; };
            std::function<bool(void)> redo = []() { return true; };
            REQUIRE(WunjoTests::requestClipCreation(timeline, binId, cid6, PlaylistState::VideoOnly, 1, 1., false, undo, redo));
            pCore->pushUndo(undo, redo, QString());
        }
        int l = timeline->getClipPlaytime(cid6);
...
TEST_CASE("Snapping", "[Snapping]")
{
    ...
    // We mock the project class so that the undoStack function returns our undoStack
    WunjoDoc document(undoStack);
    Mock<WunjoDoc> docMock(document);
    ...
    QString binId = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 50);
    QString binId2 = WunjoTests::createProducer(pCore->getProjectProfile(), "blue", binModel);
    ...
    SECTION("getBlankSizeNearClip")
    {
        ...
        // before
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, false) == 0);
        // after
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, true) == INT_MAX);
        REQUIRE(timeline->requestClipMove(cid1, tid1, 10));
        // before
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, false) == 10);
        // after
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, true) == INT_MAX);
        REQUIRE(timeline->requestClipMove(cid2, tid1, 25 + length));
        // before
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, false) == 10);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid2, false) == 15);
        // after
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, true) == 15);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid2, true) == INT_MAX);

        REQUIRE(timeline->requestClipMove(cid2, tid1, 10 + length));
        // before
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, false) == 10);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid2, false) == 0);
        // after
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid1, true) == 0);
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->getBlankSizeNearClip(cid2, true) == INT_MAX);
    }
...
TEST_CASE("Operations under locked tracks", "[Locked]")
{
    ...
    SECTION("Locked track can't receive insertion")
    {
        ...
        REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->isLocked());
        REQUIRE(timeline->getClipsCount() == 0);
        REQUIRE(timeline->checkConsistency());
        int cid1 = -1;
        ...
        REQUIRE_FALSE(WunjoTests::getTrackById_const(timeline, tid1)->isLocked());
        REQUIRE(timeline->getClipsCount() == 0);
        REQUIRE(timeline->checkConsistency());
        REQUIRE(timeline->requestClipInsertion(binId, tid1, 2, cid1));
        REQUIRE(timeline->getClipsCount() == 1);
...
TEST_CASE("New WunjoDoc activeTrack", "WunjoDoc")
{
    auto binModel = pCore->projectItemModel();
    binModel->clean();
    ...
    // Bug 442545: WunjoDoc created with 0 video tracks causes a crash at
