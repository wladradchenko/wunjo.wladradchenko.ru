...
{
    QString path = sourcesPath + "/dataset/test-mix.wmproj";
    ...
    SECTION("Asset functions")
    {
        ...
        QStringList filters = WunjoTests::getAssetsServiceIds(doc, QStringLiteral("filter"));
        CHECK(filters == QStringList({"audiolevel", "avfilter.fieldorder", "panner", "volume"}));
        qDebug() << filters;

        WunjoTests::removeAssetsById(doc, QStringLiteral("filter"), QStringList({"volume"}));
        filters = WunjoTests::getAssetsServiceIds(doc, QStringLiteral("filter"));

        CHECK(filters == QStringList({"audiolevel", "avfilter.fieldorder", "panner"}));

        QStringList transitions = WunjoTests::getAssetsServiceIds(doc, QStringLiteral("transition"));
    ...
    {
        CHECK(WunjoTests::isMltBuildInLuma(QStringLiteral("luma05.pgm")));
        CHECK_FALSE(WunjoTests::isMltBuildInLuma(QStringLiteral("luma05.png")));
        CHECK_FALSE(WunjoTests::isMltBuildInLuma(QStringLiteral("my-luma05.pgm")));
        CHECK_FALSE(WunjoTests::isMltBuildInLuma(QStringLiteral("luma87.pgm")));
    }
}

TEST_CASE("Missing project items", "[DocumentChecker]")
{
    QString path = sourcesPath + "/dataset/missing-proxy.wmproj";
