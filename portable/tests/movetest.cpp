...
TEST_CASE("Cut undo/redo", "[MoveClips]")
{
    ...
    QString binId2 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 100, false);
    ...
    SECTION("Ensure all clip instances on a track use the same producer")
    {
        ...
        Mlt::Producer prod1 = *(WunjoTests::getClipPtr(timeline, cid1));
        Mlt::Producer prod2 = *(WunjoTests::getClipPtr(timeline, cid2));
        ...
        Mlt::Producer prod3 = *(WunjoTests::getClipPtr(timeline, cid3));
        Mlt::Producer prod4 = *(WunjoTests::getClipPtr(timeline, cid4));
        // Clips on different tracks should not use the same producer
        ...
        prod3 = *(WunjoTests::getClipPtr(timeline, cid3));
        prod4 = *(WunjoTests::getClipPtr(timeline, cid4));
        // Clips on different tracks should not use the same producer
        REQUIRE(!prod3.same_clip(prod4));
        // Clips on same track should use the same producer
        REQUIRE(prod1.same_clip(prod3));
