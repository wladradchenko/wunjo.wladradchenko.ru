...
TEST_CASE("Test saving sequences elements", "[TSG]")
{
    ...
    SECTION("Group save")
    {
        ...
        QString saveFile = dir.absoluteFilePath(QStringLiteral("test.wmproj"));
        ...
        REQUIRE(WunjoTests::groupsCount(timeline) == 1);
        timeline.reset();
        pCore->projectManager()->closeCurrentDocument(false, false);
    }
    SECTION("Save / Load multiple sequences")
    {
        ...
        WunjoTests::setAudioTargets(timeline2, audioInfo);
        WunjoTests::setVideoTargets(timeline2, tid1);
        ...
        REQUIRE(WunjoTests::groupsCount(timeline2) == 2);
        ...
        DocOpenResult openResults2 = WunjoDoc::Open(openURL, QDir::temp().path(), undoGroup, false, nullptr);
        REQUIRE(openResults2.isSuccessful() == true);

        std::unique_ptr<WunjoDoc> openedDoc2 = openResults2.getDocument();
