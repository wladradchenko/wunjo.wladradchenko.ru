...
TEST_CASE("Marker model", "[MarkerListModel]")
{
    ...
    SECTION("Basic Manipulation")
    {
        ...
        // add some back
        list.emplace_back(GenTime(1.7), QLatin1String("test marker6"), WunjoSettings::default_marker_type(), GenTime(0));
...
TEST_CASE("Fps change guides", "[FpsChange]")
{
    ...
    SECTION("Guide position preserved in seconds after fps increase")
    {
        ...
        WunjoDoc document(undoStack);
        pCore->projectManager()->testSetDocument(&document);
        WunjoTests::updateTimeline(false, QString(), QString(), QDateTime::currentDateTime(), 0);
        auto timeline = document.getTimeline(document.uuid());
        pCore->projectManager()->testSetActiveTimeline(timeline);
        ...
        WunjoTests::updateProjectProfile(&document, false);
        fps = pCore->getCurrentFps();
        GenTime::setFps(fps);
