...
TEST_CASE("Functional test of the group hierarchy", "[GroupsModel]")
{
    ...
    WunjoTests::resetNextId();

    GroupsModel *groups = WunjoTests::groupsModel(timeline);
    ...
    WunjoTests::destructGroupItem(timeline, 8, false, undo, redo);
...
TEST_CASE("Orphan groups deletion", "[GroupsModel]")
{
    ...
    WunjoTests::destructGroupItem(timeline, 0, true, undo, redo);
    ...
    {
        WunjoTests::destructGroupItem(timeline, 1, false, undo, redo);

        REQUIRE(groups->getLeaves(gid3) == std::unordered_set<int>({gid1, 2, 3}));
        REQUIRE(groups->checkConsistency(false));
        WunjoTests::destructGroupItem(timeline, gid1, true, undo, redo);
    ...
    {
        WunjoTests::destructGroupItem(timeline, 1, true, undo, redo);

        REQUIRE(groups->getLeaves(gid3) == std::unordered_set<int>({2, 3}));
        REQUIRE(groups->checkConsistency(false));

        WunjoTests::destructGroupItem(timeline, 2, true, undo, redo);

        REQUIRE(groups->getLeaves(gid3) == std::unordered_set<int>({3}));
        REQUIRE(groups->checkConsistency(false));

        REQUIRE(WunjoTests::downLinks(timeline, gid3) > 0);
        WunjoTests::destructGroupItem(timeline, 3, true, undo, redo);
        REQUIRE(WunjoTests::downLinks(timeline, gid3) == 0);
        REQUIRE(WunjoTests::downLinksSize(timeline) == 0);
        REQUIRE(groups->checkConsistency(false));
    }
    pCore->projectManager()->closeCurrentDocument(false, false);
}
...
TEST_CASE("Integration with timeline", "[GroupsModel]")
{
    ...
    WunjoTests::finishTimelineConstruct(timeline);
    ...
    WunjoTests::finishTimelineConstruct(timeline2);
    mockedDoc.addTimeline(uuid2, timeline2);

    RESET(timMock2);*/

    QString binId = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
    QString binId2 = WunjoTests::createProducerWithSound(pCore->getProjectProfile(), binModel);
    ...
    for (int i = 0; i < 4; i++) {
        if (i % 2 == 0) {
            ...
            WunjoTests::forceClipAudio(timeline, clips.back());
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        if (i % 2 == 0) {
            ...
            WunjoTests::forceClipAudio(timeline2, clips2.back());
        }
    }
    ...
    SECTION("Basic Creation and export/import from json")
    {
        auto check_roots = [&](int r1, int r2, int r3, int r4) {
            REQUIRE(WunjoTests::groupsModel(timeline)->getRootId(clips[0]) == r1);
            REQUIRE(WunjoTests::groupsModel(timeline)->getRootId(clips[1]) == r2);
            REQUIRE(WunjoTests::groupsModel(timeline)->getRootId(clips[2]) == r3);
            REQUIRE(WunjoTests::groupsModel(timeline)->getRootId(clips[3]) == r4);
        };
        ...
        rec_check = [&](int gid) {
            ...
            if (WunjoTests::groupsModel(timeline2)->isLeaf(gid)) {
                ...
                auto children = WunjoTests::groupsModel(timeline2)->getDirectChildren(gid);
                ...
                for (int c : corresp) {
                    ...
                    parents.insert(WunjoTests::groupUpLink(timeline)[c]);
                }
        ...
        auto checkJsonParsing = [&]() {
            ...
            for (int i = 0; i < 4; i++) {
                while (WunjoTests::groupsModel(timeline2)->getRootId(clips2[i]) != clips2[i]) {
                    WunjoTests::groupsModel(timeline2)->ungroupItem(clips2[i], undo, redo);
                }
            }
            // we do the export then import
            REQUIRE(WunjoTests::groupsModel(timeline2)->fromJson(WunjoTests::groupsModel(timeline)->toJson()));
            std::unordered_map<int, int> roots;
            for (int i = 0; i < 4; i++) {
                int r = WunjoTests::groupsModel(timeline2)->getRootId(clips2[i]);
                if (roots.count(r) == 0) {
                    roots[r] = rec_check(r);
                    REQUIRE(roots[r] != -1);
                }
            ...
            for (int i = 0; i < 4; i++) {
                int r = WunjoTests::groupsModel(timeline)->getRootId(clips[i]);
                int r2 = WunjoTests::groupsModel(timeline2)->getRootId(clips2[i]);
                REQUIRE(roots[r2] == r);
            }
            pCore->projectManager()->setActiveTimeline(uuid1);
        ...
        auto state1 = [&]() {
            gid1 = WunjoTests::groupsModel(timeline)->getRootId(clips[0]);
            check_roots(gid1, gid1, clips[2], clips[3]);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid1) == GroupType::Normal);
            REQUIRE(WunjoTests::groupsModel(timeline)->getSubtree(gid1) == std::unordered_set<int>({gid1, clips[0], clips[1]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid1) == std::unordered_set<int>({clips[0], clips[1]}));
        ...
        auto state2 = [&]() {
            gid2 = WunjoTests::groupsModel(timeline)->getRootId(clips[2]);
            check_roots(gid1, gid1, gid2, gid2);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid1) == GroupType::Normal);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid2) == GroupType::AVSplit);
            REQUIRE(WunjoTests::groupsModel(timeline)->getSubtree(gid2) == std::unordered_set<int>({gid2, clips[2], clips[3]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid2) == std::unordered_set<int>({clips[2], clips[3]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getSubtree(gid1) == std::unordered_set<int>({gid1, clips[0], clips[1]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid1) == std::unordered_set<int>({clips[0], clips[1]}));
        ...
        auto state3 = [&]() {
            ...
            gid3 = WunjoTests::groupsModel(timeline)->getRootId(clips[0]);
            check_roots(gid3, gid3, gid3, gid3);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid1) == GroupType::Normal);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid2) == GroupType::AVSplit);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid3) == GroupType::Normal);
            REQUIRE(WunjoTests::groupsModel(timeline)->getSubtree(gid3) ==
                    std::unordered_set<int>({gid1, clips[0], clips[1], gid3, gid2, clips[2], clips[3]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid3) == std::unordered_set<int>({clips[2], clips[3], clips[0], clips[1]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getSubtree(gid2) == std::unordered_set<int>({gid2, clips[2], clips[3]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid2) == std::unordered_set<int>({clips[2], clips[3]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getSubtree(gid1) == std::unordered_set<int>({gid1, clips[0], clips[1]}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid1) == std::unordered_set<int>({clips[0], clips[1]}));
    ...
    SECTION("Group deletion undo")
    {
        ...
        auto state0 = [&]() {
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->checkConsistency());
        ...
        auto state = [&](int gid1, int gid2, int gid3) {
            state0();
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid1) == GroupType::AVSplit);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid2) == GroupType::AVSplit);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(gid3) == GroupType::Normal);
            REQUIRE(timeline->checkConsistency());
        };
        ...
        for (int i = 0; i < 4; i++) {
            ...
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid1)->checkConsistency());
            REQUIRE(WunjoTests::getTrackById_const(timeline, tid3)->checkConsistency());
            REQUIRE(timeline->checkConsistency());
...
TEST_CASE("Complex Functions", "[GroupsModel]")
{
    ...
    SECTION("MergeSingleGroups")
    {
        Fun undo = []() { return true; };
        Fun redo = []() { return true; };
        REQUIRE(WunjoTests::groupUpLink(timeline).size() == 0);
    ...
    SECTION("Split leaf")
    {
        ...
        auto test_leaf = [&]() {
            ...
            REQUIRE(WunjoTests::groupUpLink(timeline).size() == 1);
            REQUIRE(groups->checkConsistency());
        };
        test_leaf();
    ...
    SECTION("Splitting preserves group type")
    {
        ...
        WunjoTests::setGroupType(timeline, 4, GroupType::AVSplit);
        WunjoTests::setGroupType(timeline, 5, GroupType::AVSplit);
        WunjoTests::setGroupType(timeline, 6, GroupType::Normal);

        auto test_tree = [&]() {
            REQUIRE(WunjoTests::groupUpLink(timeline).size() == 7);
            for (int i = 0; i <= 6; i++) {
        ...
        auto test_tree2 = [&]() {
            // REQUIRE(WunjoTests::groupUpLink(timeline).size() == 6);
