...
TEST_CASE("Basic tree testing", "[TreeModel]")
{
    ...
    REQUIRE(WunjoTests::checkModelConsistency(model));
    ...
    SECTION("Item creation Test")
    {
        ...
        REQUIRE(WunjoTests::modelSize(model) == 3);
        ...
        REQUIRE(WunjoTests::modelSize(model) == 4);
    ...
    SECTION("Deregistration tests")
    {
        ...
        auto state = [&]() {
            ...
            REQUIRE(WunjoTests::modelSize(model) == 6);
        ...
        REQUIRE(WunjoTests::modelSize(model) == 5);
...
TEST_CASE("Effect filter text-matching logic")
{
    ...
    SECTION("Handles basic alphanum search")
    {
        ...
        CHECK(WunjoTests::effectFilterName(filter, item) == true);

        // should not search across spaces
        filter.setFilterName(true, "Thisis");
        CHECK(WunjoTests::effectFilterName(filter, item) == false);
