...
TEST_CASE("Simple trimming operations", "[Trimming]")
{
    ...
    WunjoDoc document(undoStack, {1, 3});
    pCore->projectManager()->testSetDocument(&document);
    QDateTime documentDate = QDateTime::currentDateTime();
    WunjoTests::updateTimeline(false, QString(), QString(), documentDate, 0);
    auto timeline = document.getTimeline(document.uuid());
    pCore->projectManager()->testSetActiveTimeline(timeline);

    QString binId = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
    QString binId2 = WunjoTests::createProducer(pCore->getProjectProfile(), "blue", binModel);
    QString binId3 = WunjoTests::createProducerWithSound(pCore->getProjectProfile(), binModel);
    ...
    WunjoTests::makeFiniteClipEnd(timeline, cid1);
    WunjoTests::makeFiniteClipEnd(timeline, cid2);
    WunjoTests::makeFiniteClipEnd(timeline, cid3);
    WunjoTests::makeFiniteClipEnd(timeline, cid4);
    WunjoTests::makeFiniteClipEnd(timeline, cid5);
    WunjoTests::makeFiniteClipEnd(timeline, cid6);
    WunjoTests::makeFiniteClipEnd(timeline, cid7);
    ...
    SECTION("Clip cutting")
    {
        ...
        auto state = [&]() {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 2);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == l - 4);
        };
        state();
        ...
        auto state2 = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid3) == l + l - 5);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 2);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == 5);
            REQUIRE(WunjoTests::getClipPtr(timeline, splitted)->getIn() == 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, splitted)->getOut() == l - 4);
        };
        state2();
    ...
    SECTION("Cut and resize")
    {
        ...
        WunjoTests::makeFiniteClipEnd(timeline, splitted);
        auto state2 = [&]() {
            REQUIRE(timeline->checkConsistency());
            REQUIRE(timeline->getClipTrackId(cid1) == tid1);
    ...
    SECTION("Clip cutting 2")
    {
        ...
        auto state = [&]() {
            ...
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(gid1) == std::unordered_set<int>({cid1, cid4}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(gid2) == std::unordered_set<int>({cid2, cid5}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(gid3) == std::unordered_set<int>({cid3, cid6}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(gid4) == std::unordered_set<int>({gid1, gid2, gid3, cid7}));
        ...
        auto check_groups = [&]() {
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(gid2) == std::unordered_set<int>({splitted, splitted2}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(gid3) == std::unordered_set<int>({cid3, cid6}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(gid4) == std::unordered_set<int>({gid2, gid3, cid7}));
            REQUIRE(timeline->getGroupElements(cid3) == std::unordered_set<int>({splitted, splitted2, cid3, cid6, cid7}));

            int g1b = WunjoTests::groupUpLink(timeline)[cid1];
            int g2b = WunjoTests::groupUpLink(timeline)[cid2];
            int g4b = WunjoTests::groupsModel(timeline)->getRootId(cid1);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1b) == std::unordered_set<int>({cid1, cid4}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g2b) == std::unordered_set<int>({cid2, cid5}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g4b) == std::unordered_set<int>({g1b, g2b}));
    ...
    SECTION("Simple audio split")
    {
        ...
        auto state2 = [&]() {
            ...
            int g1 = WunjoTests::groupsModel(timeline)->getDirectAncestor(audio1);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1) == std::unordered_set<int>({audio1, splitted1}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g1) == GroupType::AVSplit);
        };
        state2();
    ...
    SECTION("Split audio on a selection")
    {
        ...
        auto state2 = [&]() {
            ...
            int g1 = WunjoTests::groupsModel(timeline)->getDirectAncestor(audio1);
            int g2 = WunjoTests::groupsModel(timeline)->getDirectAncestor(audio2);
            int g3 = WunjoTests::groupsModel(timeline)->getDirectAncestor(audio3);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1) == std::unordered_set<int>({audio1, splitted1}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g2) == std::unordered_set<int>({audio2, splitted2}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g3) == std::unordered_set<int>({audio3, splitted3}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g1) == GroupType::AVSplit);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g2) == GroupType::AVSplit);
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g3) == GroupType::AVSplit);
        };
        state2();
    ...
    SECTION("Cut should preserve AV groups")
    {
        ...
        WunjoTests::setAudioTargets(timeline, audioInfo);
        WunjoTests::setVideoTargets(timeline, tid5);

        int cid6 = -1;
        int pos = 3;
        REQUIRE(timeline->requestClipInsertion(binId3, tid5, pos, cid6, true, true, false));
        int cid7 = WunjoTests::groupsModel(timeline)->getSplitPartner(cid6);
        ...
        auto state = [&]() {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid6)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid7)->clipState() == PlaylistState::AudioOnly);

            // we check that the av group was correctly created
            REQUIRE(timeline->getGroupElements(cid6) == std::unordered_set<int>({cid6, cid7}));
            int g1 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid6);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1) == std::unordered_set<int>({cid6, cid7}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g1) == GroupType::AVSplit);
        };
        state();
        ...
        auto state2 = [&]() {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid6)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid7)->clipState() == PlaylistState::AudioOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid8)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid9)->clipState() == PlaylistState::AudioOnly);
            ...
            int g2 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid8);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g2) == std::unordered_set<int>({cid8, cid9}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g2) == GroupType::AVSplit);
        };
        state2();
...
TEST_CASE("Spacer operations", "[Spacer]")
{
    ...
    std::shared_ptr<DocUndoStack> undoStack = std::make_shared<DocUndoStack>(nullptr);
    pCore->setCurrentProfile(QStringLiteral("dv_pal"));
    WunjoDoc document(undoStack, {0, 1});
    pCore->projectManager()->testSetDocument(&document);
    QDateTime documentDate = QDateTime::currentDateTime();
    WunjoTests::updateTimeline(false, QString(), QString(), documentDate, 0);
    auto timeline = document.getTimeline(document.uuid());
    ...
    SECTION("Simple one track space insertion")
    {
        ...
        auto state = [&]() {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l - 1);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == l - 1);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid3)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid3)->getOut() == l - 1);
        };
        state();
...
TEST_CASE("Insert/delete", "[Trimming2]")
{
    ...
    QString binId = WunjoTests::createProducerWithSound(pCore->getProjectProfile(), binModel);
    ...
    audioInfo.insert(1, QStringLiteral("stream1"));
    WunjoTests::setAudioTargets(timeline, audioInfo);
    WunjoTests::setVideoTargets(timeline, tid1);
    ...
    SECTION("Remove Space should preserve groups")
    {
        ...
        int cid2 = WunjoTests::groupsModel(timeline)->getSplitPartner(cid1);
        ...
        auto state = [&](int pos) {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->clipState() == PlaylistState::AudioOnly);
            // we check that the av group was correctly created
            REQUIRE(timeline->getGroupElements(cid1) == std::unordered_set<int>({cid1, cid2}));
            int g1 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid1);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1) == std::unordered_set<int>({cid1, cid2}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g1) == GroupType::AVSplit);
        };
        state(3);
    ...
    SECTION("Insert zone should preserve groups")
    {
        ...
        WunjoTests::setAudioTimelineTargets(timeline, aTarget);
        WunjoTests::setVideoTargets(timeline, tid1);
        // Make tracks active
        timeline->setTrackProperty(tid1, QStringLiteral("wunjo:timeline_active"), QStringLiteral("1"));
        timeline->setTrackProperty(tid2, QStringLiteral("wunjo:timeline_active"), QStringLiteral("1"));
        REQUIRE(TimelineFunctions::insertZone(timeline, {tid1, tid2}, binId, 3 + 2, {l / 4, 3 * l / 4}, false));
        aTarget.clear();
        WunjoTests::setAudioTimelineTargets(timeline, aTarget);
        WunjoTests::setVideoTargets(timeline, -1);
        ...
        auto state2 = [&]() {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->clipState() == PlaylistState::AudioOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid3)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->clipState() == PlaylistState::AudioOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid5)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid6)->clipState() == PlaylistState::AudioOnly);
            // we check that the av group was correctly created
            REQUIRE(timeline->getGroupElements(cid1) == std::unordered_set<int>({cid1, cid2}));
            int g1 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid1);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1) == std::unordered_set<int>({cid1, cid2}));
            ...
            int g2 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid3);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g2) == std::unordered_set<int>({cid3, cid4}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g2) == GroupType::AVSplit);

            int g3 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid5);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g3) == std::unordered_set<int>({cid5, cid6}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g3) == GroupType::AVSplit);
        };
        state2();
        undoStack->undo();
...
TEST_CASE("Copy/paste", "[CP]")
{
    ...
    QString binId = WunjoTests::createProducerWithSound(pCore->getProjectProfile(), binModel);
    QString binId2 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
    ...
    SECTION("Copy paste groups")
    {
        ...
        int cid3 = timeline->getClipByPosition(tid1, 3 + 2 * l + 1);
        REQUIRE(cid3 != -1);
        int cid4 = WunjoTests::groupsModel(timeline)->getSplitPartner(cid3);
        auto state2 = [&](int count1, int count2) {
            ...
            // we check that the av group was correctly created
            REQUIRE(timeline->getGroupElements(cid3) == std::unordered_set<int>({cid3, cid4}));
            int g1 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid3);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1) == std::unordered_set<int>({cid3, cid4}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g1) == GroupType::AVSplit);
        };
        state2(2, 0);
        ...
        REQUIRE(cid5 != -1);
        int cid6 = WunjoTests::groupsModel(timeline)->getSplitPartner(cid5);
        REQUIRE(cid6 != -1);
        int cid7 = timeline->getClipByPosition(tid1b, 2 * l + 1);
        REQUIRE(cid7 != -1);
        int cid8 = WunjoTests::groupsModel(timeline)->getSplitPartner(cid7);
        REQUIRE(cid8 != -1);
        auto state3 = [&](int count1, int count2) {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid5)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid6)->clipState() == PlaylistState::AudioOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid7)->clipState() == PlaylistState::VideoOnly);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid8)->clipState() == PlaylistState::AudioOnly);
            // we check that the av group was correctly created
            REQUIRE(timeline->getGroupElements(cid5) == std::unordered_set<int>({cid5, cid6}));
            int g1 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid5);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g1) == std::unordered_set<int>({cid5, cid6}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g1) == GroupType::AVSplit);

            REQUIRE(timeline->getGroupElements(cid7) == std::unordered_set<int>({cid7, cid8}));
            int g2 = WunjoTests::groupsModel(timeline)->getDirectAncestor(cid7);
            REQUIRE(WunjoTests::groupsModel(timeline)->getDirectChildren(g2) == std::unordered_set<int>({cid7, cid8}));
            REQUIRE(WunjoTests::groupsModel(timeline)->getType(g2) == GroupType::AVSplit);
        };
        state3(2, 2);
    ...
    SECTION("Paste when tracks get deleted")
    {
        ...
        REQUIRE(cid3 != -1);
        cid4 = WunjoTests::groupsModel(timeline)->getSplitPartner(cid3);
        state2(tid2b);
    }
    pCore->projectManager()->closeCurrentDocument(false, false);
}
...
TEST_CASE("Advanced trimming operations: Slip", "[TrimmingSlip]")
{
    ...
    // Here we do some trickery to enable testing.
    pCore->setCurrentProfile(QStringLiteral("dv_pal"));
    WunjoDoc document(undoStack, {0, 2});
    pCore->projectManager()->testSetDocument(&document);
    QDateTime documentDate = QDateTime::currentDateTime();
    WunjoTests::updateTimeline(false, QString(), QString(), documentDate, 0);
    auto timeline = document.getTimeline(document.uuid());
    ...
    SECTION("Slip single cut clip")
    {
        ...
        auto state = [&]() {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l - 6);
        };
        state();
        ...
        auto state2 = [&]() {
            ...
            REQUIRE(timeline->getClipPlaytime(cid1) == l - 11);
            REQUIRE(timeline->getClipPosition(cid1) == 5 + 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 3);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l - 9);
        };
        state2();
    ...
    SECTION("Slip beyond borders")
    {
        ...
        auto state2 = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid1) == 5 + 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l - 12);
        };
        state2();
        ...
        auto state3 = [&]() {
            ...
            REQUIRE(timeline->getClipPlaytime(cid1) == l - 11);
            REQUIRE(timeline->getClipPosition(cid1) == 5 + 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 11);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l - 1);
        };
        state3();
    ...
    SECTION("Slip group")
    {
        ...
        auto state = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid1) == 5 + 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l1 - 6);
            REQUIRE(timeline->getClipTrackId(cid1) == tid1);

            REQUIRE(timeline->getClipPlaytime(cid2) == l2 - 11);
            REQUIRE(timeline->getClipPosition(cid2) == 50 + 11);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 11);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == l2 - 1);
            REQUIRE(timeline->getClipTrackId(cid2) == tid1);

            REQUIRE(timeline->getClipPlaytime(cid3) == l3 - 5);
            REQUIRE(timeline->getClipPosition(cid3) == 25);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid3)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid3)->getOut() == l3 - 6);
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getIn() == 8);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getOut() == l4 - 10);
            REQUIRE(timeline->getClipTrackId(cid4) == tid2);

            REQUIRE(timeline->getClipPlaytime(cid5) == l5);
            REQUIRE(timeline->getClipPosition(cid5) == 60);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid5)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid5)->getOut() == l5 - 1);
            REQUIRE(timeline->getClipTrackId(cid5) == tid2);

            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid1) == std::unordered_set<int>({cid1, cid2, cid3, cid4, cid5}));
        };
        state();
        ...
        auto state2 = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid1) == 5 + 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 3);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l1 - 9);
            REQUIRE(timeline->getClipTrackId(cid1) == tid1);

            REQUIRE(timeline->getClipPlaytime(cid2) == l2 - 11);
            REQUIRE(timeline->getClipPosition(cid2) == 50 + 11);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 8);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == l2 - 4);
            REQUIRE(timeline->getClipTrackId(cid2) == tid1);
            ...
            REQUIRE(timeline->getClipPlaytime(cid4) == l4 - 17);
            REQUIRE(timeline->getClipPosition(cid4) == 8);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getIn() == 5);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getOut() == l4 - 13);
            REQUIRE(timeline->getClipTrackId(cid4) == tid2);
        ...
        auto state3 = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid1) == 5 + 6);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l1 - 12);
            REQUIRE(timeline->getClipTrackId(cid1) == tid1);

            REQUIRE(timeline->getClipPlaytime(cid2) == l2 - 11);
            REQUIRE(timeline->getClipPosition(cid2) == 50 + 11);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == l2 - 12);
            REQUIRE(timeline->getClipTrackId(cid2) == tid1);
            ...
            REQUIRE(timeline->getClipPlaytime(cid4) == l4 - 17);
            REQUIRE(timeline->getClipPosition(cid4) == 8);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getOut() == l4 - 18);
            REQUIRE(timeline->getClipTrackId(cid4) == tid2);
...
TEST_CASE("Advanced trimming operations: Ripple", "[TrimmingRipple]")
{
    ...
    SECTION("Ripple resize single fullsized clip (longer)")
    {
        ...
        auto state = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid1) == 5);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid1)->getOut() == l1 - 1);
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid3)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid3)->getOut() == l3 - 1);
        };
        state();
    ...
    SECTION("Ripple resize single fullsized clip (shorter)")
    {
        ...
        auto stateB = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid2) == 50);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 0);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == l2 - 6);
        ...
        auto stateC = [&]() {
            ...
            REQUIRE(timeline->getClipPosition(cid2) == 50);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getIn() == 3);
            REQUIRE(WunjoTests::getClipPtr(timeline, cid2)->getOut() == l2 - 6);
    ...
    SECTION("Ripple resize fullsized multitrack group (shorter)")
    {
        ...
        auto stateA1 = [&]() {
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getIn() == WunjoTests::getClipPtr(timeline, cid1)->getIn());
            REQUIRE(WunjoTests::getClipPtr(timeline, cid4)->getOut() == WunjoTests::getClipPtr(timeline, cid1)->getOut());
        };
        auto stateA2 = [&]() {
            REQUIRE(timeline->checkConsistency());
            REQUIRE(timeline->getClipPlaytime(cid2) == l2);
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid5)->getIn() == WunjoTests::getClipPtr(timeline, cid2)->getIn());
            REQUIRE(WunjoTests::getClipPtr(timeline, cid5)->getOut() == WunjoTests::getClipPtr(timeline, cid2)->getOut());
            ...
            REQUIRE(WunjoTests::getClipPtr(timeline, cid6)->getIn() == WunjoTests::getClipPtr(timeline, cid3)->getIn());
            REQUIRE(WunjoTests::getClipPtr(timeline, cid6)->getOut() == WunjoTests::getClipPtr(timeline, cid3)->getOut());
        };
        stateA1();
        stateA2();
    ...
    SECTION("Ripple resize fullsized single track group (shorter)")
    {
        ...
        auto stateA1 = [&]() {
            ...
            REQUIRE(WunjoTests::groupsModel(timeline)->getLeaves(gid1) == std::unordered_set<int>({cid1, cid2, cid3}));
        };
        auto stateA2 = [&]() {
            REQUIRE(timeline->checkConsistency());
            REQUIRE(timeline->getClipPlaytime(cid2) == l2);
