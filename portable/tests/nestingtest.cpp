...
TEST_CASE("Save File With 2 Sequences", "[SF2]")
{
    ...
    SECTION("Simple insert and save")
    {
        ...
        // Save and close
        pCore->projectManager()->testSaveFileAs(dir.absoluteFilePath(QStringLiteral("test-nest.wmproj")));
        Q_ASSERT(QFile::exists(dir.absoluteFilePath(QStringLiteral("test-nest.wmproj"))));
        pCore->projectManager()->closeCurrentDocument(false, false);
    }
    ...
    SECTION("Open and check in/out points")
    {
        // Open document
        WunjoTests::resetNextId();
        QString saveFile = QDir::temp().absoluteFilePath(QStringLiteral("test-nest.wmproj"));
    ...
    SECTION("Reopen and check in/out points")
    {
        ...
        WunjoTests::resetNextId();
        Q_ASSERT(QFile::exists(QDir::temp().absoluteFilePath(QStringLiteral("test-nest.wmproj"))));
        QString saveFile = QDir::temp().absoluteFilePath(QStringLiteral("test-nest.wmproj"));
        QUrl openURL = QUrl::fromLocalFile(saveFile);
        ...
        QDir dir = QDir::temp();
        QFile::remove(dir.absoluteFilePath(QStringLiteral("test-nest.wmproj")));
        // binModel->clean();
...
TEST_CASE("Save File, Reopen and check for corruption", "[SF3]")
{
    ...
    {
        QString path = sourcesPath + "/dataset/test-nesting-effects.wmproj";
...
TEST_CASE("Save File And Check Sequence Effects", "[SF2]")
{
    ...
    SECTION("Simple insert, add master effect and save")
    {
        ...
        // Save and close
        REQUIRE(pCore->projectManager()->testSaveFileAs(dir.absoluteFilePath(QStringLiteral("test-nest2.wmproj"))));
        pCore->projectManager()->closeCurrentDocument(false, false);
    }
    ...
    SECTION("Open and check that effects are restored")
    {
        ...
        // We mock the project class so that the undoStack function returns our undoStack, and our mocked document
        WunjoTests::resetNextId();
        QString saveFile = QDir::temp().absoluteFilePath(QStringLiteral("test-nest2.wmproj"));
        QUrl openURL = QUrl::fromLocalFile(saveFile);
        ...
        pCore->projectManager()->closeCurrentDocument(false, false);
        QFile::remove(QDir::temp().absoluteFilePath(QStringLiteral("test-nest2.wmproj")));
    }
}
...
TEST_CASE("Check nested sequences on opening", "[NEST]")
{
    ...
    SECTION("Open and simply save file")
    {
        QString path = sourcesPath + "/dataset/test-nesting.wmproj";
        Q_ASSERT(QFile::exists(sourcesPath));
        QUrl openURL = QUrl::fromLocalFile(path);
