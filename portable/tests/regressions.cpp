...
TEST_CASE("Regression")
{
    ...
    WunjoDoc &mockedDoc = docMock.get();
    ...
    std::shared_ptr<TimelineItemModel> tim = WunjoTests::createTimelineModel(mockedDoc.uuid(), undoStack);
    ...
    undoStack->redo();
    undoStack->undo();
    QString binId0 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
    int c = ClipModel::construct(timeline, binId0, -1, PlaylistState::VideoOnly);
    WunjoTests::makeFiniteClipEnd(timeline, c);

    TrackModel::construct(timeline);
    REQUIRE(WunjoTests::getTrackById_const(timeline, 1)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 1)->checkConsistency());
    ...
    TrackModel::construct(timeline);
    REQUIRE(WunjoTests::getTrackById_const(timeline, 1)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    undoStack->undo();
    REQUIRE(WunjoTests::getTrackById_const(timeline, 1)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    undoStack->redo();
...
TEST_CASE("Regression2")
{
    ...
    undoStack->redo();
    TrackModel::construct(timeline);
    REQUIRE(WunjoTests::getTrackById_const(timeline, 0)->checkConsistency());
    undoStack->undo();
    REQUIRE(WunjoTests::getTrackById_const(timeline, 0)->checkConsistency());
    {
        QString binId0 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
        bool ok = timeline->requestClipInsertion(binId0, 0, 10, dummy_id);
        WunjoTests::makeFiniteClipEnd(timeline, dummy_id);
        REQUIRE(ok);
    }
    REQUIRE(WunjoTests::getTrackById_const(timeline, 0)->checkConsistency());
    undoStack->undo();
    ...
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    {
        ...
        WunjoTests::makeFiniteClipEnd(timeline, 3);
        REQUIRE(ok);
    }
    REQUIRE(WunjoTests::getTrackById_const(timeline, 0)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    ...
    REQUIRE(WunjoTests::getTrackById_const(timeline, 0)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 4)->checkConsistency());
    {
        QString binId0 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel);
        int c = ClipModel::construct(timeline, binId0, -1, PlaylistState::VideoOnly);
        WunjoTests::makeFiniteClipEnd(timeline, c);
    ...
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 4)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 6)->checkConsistency());
    {
...
TEST_CASE("Regression 3")
{
    ...
    REQUIRE(WunjoTests::getTrackById_const(timeline, 1)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 3)->checkConsistency());
    ...
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 3)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 7)->checkConsistency());
    TrackModel::construct(timeline);
    REQUIRE(WunjoTests::getTrackById_const(timeline, 1)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 3)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 7)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 8)->checkConsistency());
    TrackModel::construct(timeline);
    REQUIRE(WunjoTests::getTrackById_const(timeline, 1)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 2)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 3)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 7)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 8)->checkConsistency());
    REQUIRE(WunjoTests::getTrackById_const(timeline, 9)->checkConsistency());
...
TEST_CASE("FuzzBug1")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        WunjoTests::finishTimelineConstruct(timeline_0);
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 20, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
...
TEST_CASE("FuzzBug2")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "d", binModel, 0, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
...
TEST_CASE("FuzzBug3")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        WunjoTests::createProducerWithSound(pCore->getProjectProfile(), binModel);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
        ...
        WunjoTests::setAudioTargets(timeline_0, audioInfo);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
...
TEST_CASE("FuzzBug4")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        WunjoTests::finishTimelineConstruct(timeline_0);
        pCore->projectManager()->testSetActiveTimeline(timeline_0);
        WunjoTests::setVideoTargets(timeline_0, 1);
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 2, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "blue", binModel, 20, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "gseen", binModel, 20, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        const QString producerWithAudio = WunjoTests::createProducerWithSound(pCore->getProjectProfile(), binModel);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
...
TEST_CASE("FuzzBug5")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        std::shared_ptr<TimelineItemModel> tim_1 = WunjoTests::createTimelineModel(uuid2, undoStack);
        Mock<TimelineItemModel> timMock_1(*tim_1.get());
        auto timeline_1 = std::shared_ptr<TimelineItemModel>(&timMock_1.get(), [](...) {});
        WunjoTests::finishTimelineConstruct(timeline_1);
        ...
        QMap<int, QString> audioInfo;
        audioInfo.insert(1, QStringLiteral("stream1"));
        WunjoTests::setAudioTargets(timeline_1, audioInfo);
        WunjoTests::setVideoTargets(timeline_1, 5);
        QMap<int, int> audioTarget;
        audioTarget.insert(6, 1);
        WunjoTests::setAudioTimelineTargets(timeline_1, audioTarget);

        /*timeline_1->setTrackProperty(4, "wunjo:timeline_active", QStringLiteral("1"));
        timeline_1->setTrackProperty(5, "wunjo:timeline_active", QStringLiteral("1"));
        timeline_1->setTrackProperty(6, "wunjo:timeline_active", QStringLiteral("1"));*/
...
TEST_CASE("FuzzBug6")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        pCore->projectManager()->testSetActiveTimeline(timeline_1);
        REQUIRE(timeline_1->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "b", binModel, 20, true);
        pCore->projectManager()->testSetActiveTimeline(timeline_0);
        REQUIRE(timeline_0->checkConsistency());
        pCore->projectManager()->testSetActiveTimeline(timeline_1);
        REQUIRE(timeline_1->checkConsistency());
...
TEST_CASE("FuzzBug7")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        pCore->projectManager()->testSetActiveTimeline(timeline_1);
        REQUIRE(timeline_1->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "r5", binModel, 2, true);
        pCore->projectManager()->testSetActiveTimeline(timeline_0);
        REQUIRE(timeline_0->checkConsistency());
        pCore->projectManager()->testSetActiveTimeline(timeline_1);
        REQUIRE(timeline_1->checkConsistency());
...
TEST_CASE("FuzzBug8")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "red20", binModel, 1, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
...
TEST_CASE("FuzzBug9")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "60", binModel, 1, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
...
TEST_CASE("FuzzBug10")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 50, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
...
TEST_CASE("FuzzBug11")
{
    ...
    WunjoTests::resetNextId();
    {
        ...
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "blue", binModel, 0, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
        REQUIRE(timeline_0->checkConsistency());
        WunjoTests::createProducer(pCore->getProjectProfile(), "green", binModel, 20, true);
        REQUIRE(timeline_0->checkConsistency());
        undoStack->undo();
        REQUIRE(timeline_0->checkConsistency());
        undoStack->redo();
