...
TEST_CASE("Save File", "[SF]")
{
    ...
    SECTION("Simple insert and save")
    {
        ...
        QString binId2 = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 20, false);
        ...
        state();
        REQUIRE(timeline->checkConsistency());
        pCore->projectManager()->testSaveFileAs(dir.absoluteFilePath(QStringLiteral("test.wmproj")));
        // Undo resize
        undoStack->undo();
        // Undo first insert
        undoStack->undo();
    ...
    SECTION("Reopen and check in/out points")
    {
        ...
        WunjoTests::resetNextId();
        QString saveFile = QDir::temp().absoluteFilePath(QStringLiteral("test.wmproj"));
        QUrl openURL = QUrl::fromLocalFile(saveFile);

        QUndoGroup *undoGroup = new QUndoGroup();
        undoGroup->addStack(undoStack.get());
        DocOpenResult openResults = WunjoDoc::Open(openURL, QDir::temp().path(), undoGroup, false, nullptr);
        REQUIRE(openResults.isSuccessful() == true);

        std::unique_ptr<WunjoDoc> openedDoc = openResults.getDocument();
        ...
        pCore->projectManager()->closeCurrentDocument(false, false);
        QDir dir = QDir::temp();
        QFile::remove(dir.absoluteFilePath(QStringLiteral("test.wmproj")));
    }
    SECTION("Open a file with AV clips")
    {
        // Create new document
        QString path = sourcesPath + "/dataset/av.wmproj";
...
TEST_CASE("Check File Corruption", "[CFC]")
{
    ...
    {
        // open
        QString path = sourcesPath + "/dataset/clip-ids.wmproj";
        QUrl openURL = QUrl::fromLocalFile(path);
        ...
        std::unique_ptr<WunjoDoc> openedDoc = openResults.getDocument();
        // Mock<WunjoDoc> docMock(*openedDoc.get());
...
TEST_CASE("Non-BMP Unicode", "[NONBMP]")
{
    ...
    // A Wunjo bug (bugzilla bug 435768) caused characters outside the Basic
    ...
    // If Wunjo drops the emoji, then the result is just "testtest" which can
    // be checked for.

    const QString emojiTestString = QString::fromUtf8("test\xF0\x9F\x8D\x99test");
    std::shared_ptr<DocUndoStack> undoStack = std::make_shared<DocUndoStack>(nullptr);

    QString saveFileName = QDir::temp().absoluteFilePath(QStringLiteral("test-nonbmp.wmproj"));
    ...
    SECTION("Save title with special chars")
    {
        ...
        QString binId2 = WunjoTests::createTextProducer(pCore->getProjectProfile(), binModel, titleXml, emojiTestString, 150);
        ...
        // try opening the file as a Wunjodoc and check that the title hasn't
        // lost the emoji

        QUrl openURL = QUrl::fromLocalFile(saveFileName);
        QUndoGroup *undoGroup = new QUndoGroup();
        undoGroup->addStack(undoStack.get());
        DocOpenResult openResults = WunjoDoc::Open(openURL, QDir::temp().path(),
            undoGroup, false, nullptr);
        REQUIRE(openResults.isSuccessful() == true);

        std::unique_ptr<WunjoDoc> openedDoc = openResults.getDocument();
        QDomDocument newDoc = WunjoTests::getDocument(openedDoc.get());
        auto producers = newDoc.elementsByTagName(QStringLiteral("producer"));
        QDomElement textTitle;
        for (int i = 0; i < producers.size(); i++) {
            auto wunjoid = WunjoTests::getProperty(producers.at(i).toElement(), QStringLiteral("wunjo:id"));
            if (wunjoid != nullptr && wunjoid->text() == binId2) {
                textTitle = producers.at(i).toElement();
                break;
            }
        }
        auto clipname = WunjoTests::getProperty(textTitle, QStringLiteral("wunjo:clipname"));
        REQUIRE(clipname != nullptr);
        CHECK(clipname->text() == emojiTestString);
        auto xmldata = WunjoTests::getProperty(textTitle, QStringLiteral("xmldata"));
    ...
    SECTION("Save project and check profile")
    {
        ...
        REQUIRE(file.open(QIODevice::ReadOnly));
        QByteArray contents = file.readAll();
        QString contentCheck("<property name=\"wunjo:docproperties.profile\">atsc_1080p_25</property>");
...
TEST_CASE("Opening Mix", "[OPENMIX]")
{
    ...
    {
        // We mock the project class so that the undoStack function returns our undoStack, and our mocked document
        QUrl openURL = QUrl::fromLocalFile(sourcesPath + "/dataset/test-mix.wmproj");
        ...
        REQUIRE(WunjoTests::getTrackById_const(timeline, mixtrackId)->mixCount() == 2);
        int mixtrackId2 = timeline->getTrackIndexFromPosition(3);
        REQUIRE(WunjoTests::getTrackById_const(timeline, mixtrackId2)->mixCount() == 1);
...
TEST_CASE("Opening File With Keyframes", "[OPENKFRS]")
{
    ...
    {
        // Create new document
        QString path = sourcesPath + "/dataset/test-keyframes.wmproj";
        QUrl openURL = QUrl::fromLocalFile(path);
