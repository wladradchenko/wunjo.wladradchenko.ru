...
TEST_CASE("Cache insert-remove", "[Cache]")
{
    ...
    // Create bin clip
    QString binId = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 20, false);
...
TEST_CASE("getAudioKey() should dereference `ok` param", "ThumbnailCache") {
    ...
    SECTION("Request invalid id")
    {
        ...
        //     if (*ok) {
        bool ok = true;
        WunjoTests::getAudioKey(QStringLiteral("nonexistent-key"), &ok);
        REQUIRE(ok == false);
    }
    SECTION("Request valid id")
    {
        bool ok = false;
        WunjoTests::getAudioKey(binId, &ok);
        REQUIRE(ok == true);
    }
    pCore->projectManager()->closeCurrentDocument(false, false);
}
