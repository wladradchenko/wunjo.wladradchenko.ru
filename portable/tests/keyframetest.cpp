...
{
    auto m2 = WunjoTests::cloneModel(m);
...
TEST_CASE("Keyframe model", "[KeyframeModel]")
{
    ...
    const QString binId = WunjoTests::createProducer(pCore->getProjectProfile(), "red", binModel, 100, false);
    ...
    SECTION("Add/remove + undo")
    {
        ...
        REQUIRE(WunjoTests::addKeyframe(model, GenTime(1.1), KeyframeType::Linear, 42));
        auto state1 = [&]() {
        ...
        REQUIRE(WunjoTests::addKeyframe(model, GenTime(12.6), KeyframeType::Discrete, 33));
        auto state2 = [&]() {
        ...
        REQUIRE(WunjoTests::removeKeyframe(model, GenTime(1.1)));
        auto state3 = [&]() {
            REQUIRE(model->rowCount() == 2);
            REQUIRE(check_anim_identity(model));
            REQUIRE(model->hasKeyframe(GenTime(12.6)));
        ...
        REQUIRE(WunjoTests::removeAllKeyframes(model));
        state0();
        REQUIRE(WunjoTests::removeAllKeyframes(model));
        state0();
        undoStack->undo();
