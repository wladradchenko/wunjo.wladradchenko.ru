...
Wunjo uses MLT for all video/audio effects/filters.
...
## Important notes
* Effects can be blacklisted in `wunjo/data/blacklisted_effects.txt`. All effects with a custom XML GUI need to be blacklisted

* Effects can be added to "Main effects" list in `wunjo/data/preferred_effects.txt`

* Effects can be assigned to an effect category in `wunjo/data/effectscategory.rc`.

* Wunjo parses the effect folder at each startup, so that if you have an XML file describing a new effect,
just copy it to your `~/.kde/share/apps/wunjo/effects/` folder and restart Wunjo to enable the new effect.
...
| `id`         | internal wunjo id, can be anything, but must be unique for each effect |
...
| `dependency` | _(optional)_ ) MLT ("mlt_service") name of an effect or composition this asset depends on. If the dependency is not available this asset will not be available in Wunjo too|
...
| attribute name | description    |
| :------------- | :------------- |
| `name`       | a `wunjo:` prefixed name that will be used internally and in project file to save animation |
...
| `newstuff` | _(optional)_ KNewStuff config file (usually placed in `wunjo/data` and added to `wunjo/src/uiresources.qrc` so the value looks like `":data/wunjo_wipes.knsrc"`). If this is empty no download button is shown|
