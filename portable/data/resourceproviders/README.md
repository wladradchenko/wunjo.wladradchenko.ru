...
Instruction written 2021 by Julius Künzel as part of Wunjo (www.wunjo.online)
## Overview
...
| clientkey | String | If OAuth2 or `%clientkey%` is used | The client key to access the api. </br>_Wunjo has some keys build in:  `%pixabay_apikey%`, `%freesound_apikey%` and `%pexels_apikey%` will be replaced by a key for the certain provider._ |
...
| licenseUrl | String | | Url to the license to be opened in a external browser. If you use a [template](#templates), always use link to the english license version. Wunjo can generate License names out of links to Creative Commons, Pexels and Pixabay Licenses for all other links the License name is "Unknown License". |
