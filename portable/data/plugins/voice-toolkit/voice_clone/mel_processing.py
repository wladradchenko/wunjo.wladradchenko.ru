...
def spectrogram_torch(y, n_fft, sampling_rate, hop_size, win_size, center=False):
    ...
    spec_real = torch.view_as_real(spec)  # Update
    spec = torch.sqrt(spec_real.pow(2).sum(-1) + 1e-6)
    return spec
...
def spectrogram_torch_conv(y, n_fft, sampling_rate, hop_size, win_size, center=False):
    ...
    spec1_real = torch.view_as_real(spec1)  # Update
    assert torch.allclose(spec1_real, spec2, atol=1e-4)
