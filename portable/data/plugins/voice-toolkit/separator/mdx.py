"""Separating a voice from what is behind it — the MDX architecture.

Ported from Wunjo v2 (portable/src/sound_processing/separator/mdx/model.py),
which took it from audio-separator/UVR. Two things changed on the way in:

  * stems are written with soundfile instead of pydub, which was carrying an
    int16 interleave and a second dependency to produce a plain wav;
  * the second pass through the model is only run when something asks for it.
    v2 always demixed twice — once for the stem, once in "match mix" mode —
    and then threw the second result away unless `invert_using_spec` was on,
    which it never was. Every separation cost twice what it needed to.
"""
...
class CommonSeparator:
    ...
    def write_audio(self, stem_path: str, stem_source):
        """Write a separated stem, at the rate it was separated at."""
        ...
        # (samples, channels) is what soundfile writes, which is the shape the
        # stems already have here.
        soundfile.write(stem_path, stem_source, self.sample_rate)
    ...
    def separate(self, audio_file_path, progress_callback=None):
        ...
        if not isinstance(self.secondary_source, np.ndarray):
            if self.invert_using_spec:
                ...
                # Subtraction needs no second pass through the model: what is
                # left of the mix once the stem is taken out *is* the other stem.
