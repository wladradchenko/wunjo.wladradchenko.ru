...
class Cropper(object):
    def __init__(self, **kwargs) -> None:
        ...
        # No face detector: the Face Region effect says where the face is,
        # and the landmark model below turns that box into real points.
    ...
    def crop_single_image(self, obj, **kwargs):
        ...
        pts = self.face_analysis_wrapper.get(cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR), crop=kwargs.get('crop'))[0].kps
    ...
    def calc_lmk_source_image(self, img_rgb_: np.ndarray, crop_cfg: CropConfig, idx: int = 0):
        ...
        lmk = self.face_analysis_wrapper.get(img_bgr, crop=crop_cfg.crop)[0].kps
        lmk = self.human_landmark_runner.run(img_rgb, lmk)

        return lmk

    def clear_face(self):
        # nothing is remembered between frames any more
        pass
