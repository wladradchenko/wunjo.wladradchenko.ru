...
class LivePortrait:
    def __init__(self, checkpoint_F: str, checkpoint_M: str, checkpoint_G: str, checkpoint_W: str, checkpoint_S: str,
        ...
        # The condition was the wrong way round: passing a number kept the
        # built-in 1280 and only passing None ever changed anything, so whatever
        # the caller asked for was thrown away and every source was quietly
        # shrunk to 1280 on its longest side.
        if source_max_dim:
            self.inference_cfg.source_max_dim = int(source_max_dim)
    ...
    def update_video_portrait_parameters(self, frames_files, pipeline, output: str, fps: int = 25, eye_ratio: list = None,
        ...
        for i, img_src in tqdm(enumerate(frames_files), desc="Writing...", total=len(frames_files), unit='frames', file=sys.stderr):
