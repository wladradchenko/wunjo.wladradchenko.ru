...
class FaceSwapProcessing:
    ...
    def __init__(self, model_path, face_swap_model_path, similarface = False, device="cpu", progress_callback=None):
        ...
        # Where the face is comes from the Face Region effect; the points the
        # swap model needs are read off it with the landmark model, on the crop.
        from regionface import RegionFaces

        self.face_analysis_wrapper = RegionFaces(
            landmark_model=os.path.join(model_path, "landmark.onnx"),
    ...
    def load(self, face_swap_model_path):
        ...
        from regionface import InSwapper

        return InSwapper(face_swap_model_path, provider)
    ...
    def face_detect_with_alignment_crop(self, image_files, face_fields):
        ...
        :param face_fields: crop target face — one dict, or one per frame when
            the region was keyframed and the face moves across the shot
        :return:
        """
        predictions = []
        per_frame = face_fields if isinstance(face_fields, (list, tuple)) else None

        for n, image_file in enumerate(tqdm(image_files)):
            crop = (per_frame[n] if n < len(per_frame) else per_frame[-1]) if per_frame else face_fields
