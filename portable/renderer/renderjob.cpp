...
RenderJob::RenderJob(const QString &render, const QString &scenelist, const QString &target, int pid, int in, int out, const QString &subtitleFile,
    ...
    , m_dest(target)
    , m_progress(0)
    , m_wunjosocket(new QLocalSocket(this))
    ...
    , m_renderProcess(&m_looper)
{
    ...
    // Disable VDPAU so that rendering will work even if there is a Wunjo instance using VDPAU
    ...
    if (!m_logfile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qCWarning(WUNJO_RENDERER_LOG) << "Unable to log to" << m_logfile.fileName();
    } else {
    ...
    , m_renderProcess(&m_looper)
{
    ...
    QString servername = QStringLiteral("online.wunjo.make-%1").arg(m_pid);
    m_wunjosocket->connectToServer(servername);
    if (m_wunjosocket->waitForConnected(1000)) {
        QJsonObject method, args;
        args["status"] = -2;
        args["error"] = errorMessage;
        method["setRenderingFinished"] = args;
        m_wunjosocket->write(QJsonDocument(method).toJson());
        m_wunjosocket->flush();
        m_looper.quit();
    } else {
    }
...
RenderJob::~RenderJob()
{
    if (m_wunjosocket->state() == QLocalSocket::ConnectedState) {
        m_wunjosocket->disconnectFromServer();
    }
    delete m_wunjosocket;
    m_logfile.close();
}
...
void RenderJob::sendFinish(int status, const QString &error)
{
    if (m_wunjosocket->state() == QLocalSocket::ConnectedState) {
        ...
        if (!QFile::exists(m_dest)) {
            qCDebug(WUNJO_RENDERER_LOG) << "Rendering to" << m_dest << "finished. Status:" << status << "Errors: Result file does not exist!!!";
            qCDebug(WUNJO_RENDERER_LOG) << "JOB OUTPUT:" << m_renderProcess.readAllStandardOutput();
        } else {
            qCDebug(WUNJO_RENDERER_LOG) << "Rendering to" << m_dest << "finished. Status:" << status << "Errors:" << error;
        }
    }
}
...
void RenderJob::updateProgress()
{
    if (m_wunjosocket->state() == QLocalSocket::ConnectedState) {
        ...
        // Linux, try to reconnect if we have a Wunjo instance
        qCWarning(WUNJO_RENDERER_LOG) << "Connection to Wunjo instance lost, trying to reconnect";
        QStringList files = QDir::temp().entryList({QStringLiteral("online.wunjo.make*")}, QDir::System);
        qCDebug(WUNJO_RENDERER_LOG) << "Searching for possible instances, results:" << files;
        if (!files.isEmpty()) {
            qCDebug(WUNJO_RENDERER_LOG) << "Found possible running instance:" << files.first();
            bool ok;
            int pid = files.first().section(QLatin1Char('-'), -1).toInt(&ok);
            qCDebug(WUNJO_RENDERER_LOG) << "Found possible running instance PID:" << pid;
            if (ok && pid > -1) {
                m_pid = pid;
                QString servername = QStringLiteral("online.wunjo.make-%1").arg(m_pid);
                m_wunjosocket->connectToServer(servername);
        ...
        qCDebug(WUNJO_RENDERER_LOG) << "Progress:" << m_progress << "%,"
                                       << "frame" << m_frame;
    }
#endif
...
void RenderJob::start()
{
    ...
    if (m_pid > -1) {
        connect(m_wunjosocket, &QLocalSocket::connected, this, [this]() {
            QJsonObject obj;
            obj["url"] = m_dest;
            m_wunjosocket->write(QJsonDocument(obj).toJson());
            m_wunjosocket->flush();
            QJsonObject method, args;
            args["url"] = m_dest;
            args["progress"] = 0;
        ...
        if (m_wunjosocket->waitForConnected(1000)) {
            ...
            qCDebug(WUNJO_RENDERER_LOG) << "==== RENDER SOCKET NOT CONNECTED";
        }
        connect(m_wunjosocket, &QLocalSocket::readyRead, this, &RenderJob::gotMessage);
    }
...
{
    if (m_wunjosocket) {
        const QByteArray msg = m_wunjosocket->readAll();
        if (msg == "abort") {
            slotAbort();
        }
    }
        ...
        if (m_dualpass) {
            ...
            if (!m_subtitleFile.isEmpty()) {
                ...
                if (!ffmpegExe.isEmpty()) {
                    ...
                    qCDebug(WUNJO_RENDERER_LOG) << "::: JOB ARGS: " << args;
                    m_progress = 0;
...
void RenderJob::slotCheckSubtitleProcess(int exitCode, QProcess::ExitStatus exitStatus)
{
    if (exitStatus == QProcess::CrashExit || !QFile::exists(m_temporaryRenderFile)) {
        // rendering crashed
        qCCritical(WUNJO_RENDERER_LOG) << "Subtitle process exited unexpectedly:" << exitStatus << "/" << exitCode << "; Does file exist?"
