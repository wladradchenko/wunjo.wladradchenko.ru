/*
    this file is part of Wunjo, the Libre Video Editor by KDE
...
void OtioExport::exportTimeline(const std::shared_ptr<TimelineItemModel> &timeline, const QString &fileName)
{
    ...
    otioMetadata["version"] = std::string(WUNJO_VERSION);
    otioTimeline->metadata()["wunjo"] = otioMetadata;
                             ...
                             OTIO_NS::SerializableObject::Retainer<OTIO_NS::Timeline> &otioTimeline)
{
    ...
    const QString trackName = track->getProperty(QStringLiteral("wunjo:track_name")).toString();
                            ...
                            OTIO_NS::SerializableObject::Retainer<OTIO_NS::Track> &otioTrack)
{
    ...
    if (std::shared_ptr<ProjectClip> projectClip = pCore->projectItemModel()->getClipByBinID(clipBinId)) {
        ...
        if (projectClip->hasUrl()) {
            ...
            // standardized, so we use the "wunjo" namespace/prefix to
            // indicate these values are specific to wunjo.
            ...
            otioParameters["wunjo"] = parameters;
            otioMediaReference = new OTIO_NS::GeneratorReference(projectClip->name().toStdString(), "wunjo:SolidColor", otioAvailableRange, otioParameters);
        }
    }
                              ...
                              const OTIO_NS::SerializableObject::Retainer<OTIO_NS::Item> &otioItem)
{
    ...
    otioMarker->metadata()["wunjo"] = otioMetadata;
