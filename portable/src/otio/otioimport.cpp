...
void OtioImport::importFile(const QString &fileName, bool newDocument)
{
    ...
    for (const auto &otioClip : importData->otioTimeline->find_clips()) {
        if (const auto &otioExternalReference = dynamic_cast<OTIO_NS::ExternalReference *>(otioClip->media_reference())) {
            ...
            if (otioGeneratorReference->generator_kind() == "wunjo:SolidColor") {
                ...
                auto i = otioParameters.find("wunjo");
    ...
    if (profile.isEmpty()) {
        ...
        bool adjusted = false;
        std::pair<int, int> fraction = WunjoDoc::getFpsFraction(otioFps, &adjusted);
                              ...
                              const std::shared_ptr<MarkerListModel> &markerModel)
{
    ...
    int type = -1;
    if (otioMarker->metadata().has_key("wunjo") && otioMarker->metadata()["wunjo"].type() == typeid(OTIO_NS::AnyDictionary)) {
        auto otioMetadata = std::any_cast<OTIO_NS::AnyDictionary>(otioMarker->metadata()["wunjo"]);
