...
MltConnection::MltConnection(const QString &mltPath)
{
    ...
    WunjoSettings::setProducerslist(producersList);
...
QString MltConnection::locateMltProfilePath(const QString &libName, const QString &pathHint)
{
    ...
    // stored setting should not be considered on windows as MLT is distributed with each new Wunjo version
    profilePath = WunjoSettings::mltpath();
    qDebug() << "Checking path restored from WunjoSettings::mltpath():" << profilePath;
    ...
    // try to automatically guess MLT path if installed with the same prefix as wunjo with default data path
    profilePath = QDir::cleanPath(qApp->applicationDirPath() + QStringLiteral("/../share/%1/profiles").arg(libName));
    qDebug() << "Checking path relative to wunjo executable (" << qApp->applicationDirPath() << ") ./../share/%LIBNAME%/profiles:" << profilePath;
    ...
    qDebug() << "Checking path relative to wunjo executable (" << qApp->applicationDirPath() << ") ./../Resources/%LIBNAME%/profiles:" << profilePath;
...
#if (!(defined(Q_OS_WIN) || defined(Q_OS_MAC)))
    // stored setting should not be considered on windows as MLT is distributed with each new Wunjo version
    meltPath = WunjoSettings::meltpath();
    qDebug() << "Checking path restored from WunjoSettings::meltpath():" << meltPath;
...
void MltConnection::locateMeltAndProfilesPath(const QString &mltPath)
{
    ...
    WunjoSettings::setMltpath(profilePath);

    QString meltPath = locateMeltExe(profilePath);
    WunjoSettings::setMeltpath(meltPath);
...
{
    // Check for Wunjo installed luma files, add empty string at start for no luma
    if (qEnvironmentVariableIsSet("MLT_TESTS")) {
