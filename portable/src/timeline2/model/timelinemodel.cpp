    ...
    , m_softDelete(false)
{
    ...
    m_blackClip->set("wunjo:playlistid", "black_track");
...
void TimelineModel::setMarkerModel(std::shared_ptr<MarkerListModel> markerModel)
{
    ...
    m_guidesModel->loadCategories(WunjoSettings::guidesCategories(), false);
}
    ...
    if (isClip(itemId)) {
        ...
        auto spacerOperation = [this, itemId, affectAllTracks, &local_undo, &local_redo, delta, right, timeline](int position) {
            ...
            TimelineFunctions::requestSpacerEndOperation(timeline, cid, getItemPosition(cid), endPos, affectAllTracks ? -1 : trackId,
                                                         WunjoSettings::lockedGuides() ? -1 : position, local_undo, local_redo, false);
            return true;
        };
        if (delta > 0) {
            if (right) {
...
bool TimelineModel::requestTrackInsertion(int position, int &id, const QString &trackName, bool audioTrack, Fun &undo, Fun &redo, bool addCompositing)
{
    ...
    Fun local_name_update = [position, audioTrack, this]() {
        if (WunjoSettings::audiotracksbelow() == 0) {
            _resetView();
        } else {
            if (audioTrack) {
...
bool TimelineModel::requestTrackDeletion(int trackId, Fun &undo, Fun &redo)
{
    ...
    pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::TimelineTrack, trackId, m_uuid));
...
int TimelineModel::getNextSnapPos(int pos, std::vector<int> &snaps, std::vector<int> &ignored)
{
    ...
    if (hasSubtitles) {
        ...
        if (m_subtitleModel->isLocked() || !WunjoSettings::showSubtitles()) {
...
int TimelineModel::getOptimalTransitionDuration(int trackId, int position)
{
        ...
        if (lowerVideoTrackId > 0) {
            ...
            if (lowerCid > 0) {
                ...
                if (outPos - position > 2) {
                    return qMin(outPos - position, 2 * pCore->getDurationFromString(WunjoSettings::transition_duration()));
                }
            }
        }
    } else {
    ...
    return pCore->getDurationFromString(WunjoSettings::transition_duration());
}
...
const QString TimelineModel::sceneList(const QString &root, const QString &fullPath, const QString &filterData)
{
    ...
    QString playlist;
    Mlt::Consumer xmlConsumer(pCore->getProjectProfile(), "xml", fullPath.isEmpty() ? "wunjo_playlist" : fullPath.toUtf8().constData());
    if (!root.isEmpty()) {
    ...
    playlist = fullPath.isEmpty() ? QString::fromUtf8(xmlConsumer.get("wunjo_playlist")) : fullPath;
    return playlist;
}
...
std::shared_ptr<EffectStackModel> TimelineModel::getMasterEffectStackModel()
{
    ...
    if (m_masterStack == nullptr) {
        ...
        m_masterStack = EffectStackModel::construct(m_masterService, ObjectId(WunjoObjectType::Master, 0, m_uuid), m_undoStack);
