...
ClipModel::ClipModel(const std::shared_ptr<TimelineModel> &parent, std::shared_ptr<Mlt::Producer> prod, const QString &binClipId, int id,
    ...
    , m_effectStack(EffectStackModel::construct(m_producer, ObjectId(WunjoObjectType::TimelineClip, m_id, parent->uuid()), parent->m_undoStack))
...
{
    m_producer->set("wunjo:id", binClipId.toUtf8().constData());
    m_producer->set("_wunjo_cid", m_id);
                                       ...
                                       bool timeremap)
{
    ...
    m_producer->set("wunjo:id", binClip->clipId().toUtf8().constData());
    m_producer->set("_wunjo_cid", m_id);
    if (activeEffect > 0) {
        m_producer->set("wunjo:activeeffect", activeEffect);
    }
    m_endlessResize = !binClip->hasLimitedDuration();
}
...
void ClipModel::refreshProducerFromBin(int trackId)
{
    ...
    if (m_producer->property_exists("wunjo:audio_index")) {
        // The clip was disabled, restore original audio stream
        stream = m_producer->get_int("wunjo:audio_index");
    }
...
int ClipModel::audioStream() const
{
    if (m_currentState == PlaylistState::Disabled) {
        return m_producer->get_int("wunjo:audio_index");
    }
...
{
    READ_LOCK();
    return !service()->get_int("wunjo:hide_keyframes");
}

void ClipModel::setShowKeyframes(bool show)
{
    QWriteLocker locker(&m_lock);
    service()->set("wunjo:hide_keyframes", !show);
}
...
void ClipModel::passTimelineProperties(const std::shared_ptr<ClipModel> &other)
{
    ...
    dest.pass_list(source, "wunjo:hide_keyframes,wunjo:activeeffect");
}
...
int ClipModel::getMaxDuration() const
{
    ...
    if (m_clipType == ClipType::Timeline) {
        ...
        int maxDuration = m_producer->parent().get_int("wunjo:maxduration");
        if (maxDuration > 0) {
            return maxDuration;
        }
    }
...
{
    if (WunjoSettings::tagsintimeline()) {
