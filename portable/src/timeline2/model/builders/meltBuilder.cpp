                                  ...
                                  Mlt::Tractor tractor, const QString &originalDecimalPoint, const QString &chunks, const QString &dirty, bool enablePreview)
{
    ...
    for (int i = 0; i < tractor.count() && ok; i++) {
        ...
        if (track->property_exists("wunjo:playlistid")) {
            playlist_name = track->get("wunjo:playlistid");
        } else {
        ...
        case mlt_service_tractor_type: {
            ...
            if (track->get_int("wunjo:audio_track") == 1) {
                aTracksCount++;
            }
            break;
        }
        case mlt_service_playlist_type: {
            ...
            if (local_playlist.get_int("wunjo:audio_track") == 1) {
                aTracksCount++;
            }
            break;
        }
    ...
    for (int i = 0; i < tractor.count() && ok; i++) {
        ...
        case mlt_service_tractor_type: {
            ...
            bool audioTrack = track->get_int("wunjo:audio_track") == 1;
            if (!audioTrack) {
            ...
            if (track->get_int("wunjo:locked_track") > 0) {
            ...
            timeline->setTrackProperty(tid, QStringLiteral("wunjo:thumbs_format"), track->get("wunjo:thumbs_format"));
            timeline->setTrackProperty(tid, QStringLiteral("wunjo:audio_rec"), track->get("wunjo:audio_rec"));
            timeline->setTrackProperty(tid, QStringLiteral("wunjo:timeline_active"), track->get("wunjo:timeline_active"));
            break;
        }
        case mlt_service_playlist_type: {
            // that is a single track
            ...
            Mlt::Playlist local_playlist(*track);
            const QString trackName = local_playlist.get("wunjo:track_name");
            bool audioTrack = local_playlist.get_int("wunjo:audio_track") == 1;
            if (!audioTrack) {
                videoTracksIndexes << i;
            }
            ...
            if (local_playlist.get_int("wunjo:locked_track") > 0) {
                lockedTracksIndexes << tid;
            }
            timeline->setTrackProperty(tid, QStringLiteral("wunjo:thumbs_format"), local_playlist.get("wunjo:thumbs_format"));
            timeline->setTrackProperty(tid, QStringLiteral("wunjo:audio_rec"), track->get("wunjo:audio_rec"));
            timeline->setTrackProperty(tid, QStringLiteral("wunjo:timeline_active"), track->get("wunjo:timeline_active"));
            break;
        }
    ...
    if (prod && prod->is_valid()) {
        ...
        while ((service != nullptr) && service->is_valid()) {
            if (service->type() == mlt_service_transition_type) {
                ...
                QString isMix(t.get("wunjo:mixcut"));
                               ...
                               const QString &chunks, const QString &dirty, bool enablePreview, bool *projectErrors)
{
    ...
    while ((service != nullptr) && service->is_valid()) {
        if (service->type() == mlt_service_transition_type) {
            ...
            QString id(t.get("wunjo_id"));
            if (t.property_exists("internal_added") == false && t.property_exists("wunjo:mixcut") == false) {
                compositions << new Mlt::Transition(t);
                if (id.isEmpty()) {
                    qWarning() << "transition without id" << t.get("id") << t.get("mlt_service") << "on track" << t.get_b_track();
                    t.set("wunjo_id", t.get("mlt_service"));
                            ...
                            Fun &undo, Fun &redo, bool audioTrack, const QString &originalDecimalPoint)
{
    ...
    for (int i = 0; i < track.count(); i++) {
        ...
        if (i == 0) {
            ...
            int height = track.get_int("wunjo:trackheight");
            timeline->setTrackProperty(tid, "wunjo:trackheight", height == 0 ? "100" : QString::number(height));
            timeline->setTrackProperty(tid, "wunjo:collapsed", QString::number(track.get_int("wunjo:collapsed")));
            QString trackName = track.get("wunjo:track_name");
            if (!trackName.isEmpty()) {
                timeline->setTrackProperty(tid, QStringLiteral("wunjo:track_name"), trackName.toUtf8().constData());
            }
            if (audioTrack) {
                // This is an audio track
                timeline->setTrackProperty(tid, QStringLiteral("wunjo:audio_track"), QStringLiteral("1"));
                            ...
                            const QList<Mlt::Transition *> &compositions)
{
    ...
    for (int i = 0; i < max; i++) {
        ...
        case mlt_service_producer_type: {
            QString binId;
            if (clip->parent().get_int("_wunjo_processed") == 1) {
                // This is a bin clip, already processed no need to change id
                binId = QString(clip->parent().get("wunjo:id"));
            } else {
                const QString clipId = clip->parent().get("wunjo:control_uuid");
                if (QUuid(clipId).isNull()) {
                    qDebug() << "===== ERROR IN CLIP: " << clip->parent().get("wunjo:id") << ", UUID: " << clipId;
                ...
                int cType = clip->parent().get_int("wunjo:producer_type");
                ...
                if (!useMappedIds || binIdCorresp.size() == 0 || cType == ClipType::Timeline) {
                    ...
                    if (binClip == nullptr || (binClip->statusReady() &&
                                               binClip->getProducerIntProperty("wunjo:producer_type") != clip->parent().get_int("wunjo:producer_type"))) {
                    ...
                    if (clip->property_exists("wunjo:remove")) {
                ...
                clip->parent().set("wunjo:id", binId.toUtf8().constData());
                clip->parent().set("_wunjo_processed", 1);
            }
            ...
            if (binClip) {
                ...
                if (!binClip->isCompatible(st)) {
                    ...
                    binClip->resetProducerProperty(QStringLiteral("wunjo:clip_type"));
