                                              ...
                                              const QDir &targetDir, int duration)
{
    ...
    while (i.hasNext()) {
        ...
        if (i.value() == -1) {
            ...
            newTrackPlaylist->set("wunjo:playlistid", "black_track");
                ...
                if (timeline->isClip(itemId)) {
                    ...
                    if (internal.isEmpty()) {
                        ...
                        if (id.isEmpty()) {
                            ...
                            t->set("wunjo_id", t->get("mlt_service"));
                        }
                    }
                }
            }
    ...
    newTractor.set("wunjo:projectroot", pCore->currentDoc()->documentRoot().toUtf8().constData());
...
QString TimelineFunctions::copyClips(const std::shared_ptr<TimelineItemModel> &timeline, const std::unordered_set<int> &itemIds, int mainClip)
{
    ...
    int offset = -1;
    int lastFrame = -1;
    QDomElement container = copiedItems.createElement(QStringLiteral("wunjo-scene"));
                                   ...
                                   Fun &redo, int inPos, int duration)
{
    ...
    if (docId == pCore->currentDoc()->getDocumentProperty(QStringLiteral("documentid"))) {
        ...
        for (int i = 0; i < binClips.count(); ++i) {
            ...
            QString clipId = Xml::getXmlProperty(currentProd, QStringLiteral("wunjo:id"));
            if (clipId.isEmpty()) {
                // Invalid clip, maybe black track from a sequence, ignore
                continue;
            }
            const QString clipHash = Xml::getXmlProperty(currentProd, QStringLiteral("wunjo:file_hash"));
            if (!pCore->projectItemModel()->validateClip(clipId, clipHash)) {
                ...
                Xml::setXmlProperty(currentProd, QStringLiteral("wunjo:id"), updatedId);
                if (folderId.isEmpty()) {
    ...
    if (!docId.isEmpty() && docId != pCore->currentDoc()->getDocumentProperty(QStringLiteral("documentid"))) {
        ...
        auto disableProxy = [](QDomElement &producer) {
            const QString proxy = Xml::getXmlProperty(producer, QStringLiteral("wunjo:proxy"));
            if (proxy.length() < 4) {
                return;
            }
            const QString resource = Xml::getXmlProperty(producer, QStringLiteral("wunjo:originalurl"));
            if (!resource.isEmpty()) {
        ...
        auto useFreeBinId = [](QDomElement &producer, const QString &clipId, QMap<QString, QString> &mappedIds) {
            if (!pCore->projectItemModel()->isIdFree(clipId)) {
                QString updatedId = QString::number(pCore->projectItemModel()->getFreeClipId());
                Xml::setXmlProperty(producer, QStringLiteral("wunjo:id"), updatedId);
                mappedIds.insert(clipId, updatedId);
                return updatedId;
            }
            return clipId;
        ...
        auto pasteClip = [disableProxy, callBack, useFreeBinId, sourceFps, ratio](const QDomNodeList &clips, const QString &folderId, bool &clipsImported,
            ...
            for (int i = 0; i < clips.count(); ++i) {
                ...
                if (currentProd.tagName() != QLatin1String("producer") && currentProd.tagName() != QLatin1String("chain")) {
                    ...
                    if (currentProd.hasAttribute(QStringLiteral("wunjo:producer_type"))) {
                ...
                if (mappedIds.contains(clipId)) {
                    Xml::setXmlProperty(currentProd, QStringLiteral("wunjo:id"), mappedIds.value(clipId));
                }
                ...
                const QString ctrlUuid = Xml::getXmlProperty(currentProd, QStringLiteral("wunjo:control_uuid"));
                ...
                if (Xml::hasXmlProperty(currentProd, QStringLiteral("wunjo:clip_type"))) {
                    int clipType = Xml::getXmlProperty(currentProd, QStringLiteral("wunjo:clip_type")).toInt();
        ...
        auto remapClipIds = [](QDomNodeList &elements, const QMap<QString, QString> &map) {
            ...
            for (int i = 0; i < max; i++) {
                ...
                const QString currentId = Xml::getXmlProperty(e, QStringLiteral("wunjo:id"));
                if (map.contains(currentId)) {
                    Xml::setXmlProperty(e, QStringLiteral("wunjo:id"), map.value(currentId));
                }
            }
        };
        ...
        for (int i = 0; i < sequenceClips.count(); ++i) {
            ...
            QString clipId = currentProd.attribute(QStringLiteral("wunjo:id"));
            const QString uuid = currentProd.attribute(QStringLiteral("wunjo:uuid"));
            const QString controlUuid = currentProd.attribute(QStringLiteral("wunjo:control_uuid"));
            int duration = currentProd.attribute(QStringLiteral("wunjo:duration")).toInt();
            const QString clipname = currentProd.attribute(QStringLiteral("wunjo:clipname"));
            ...
            for (int j = 0; j < tractors.count(); ++j) {
                ...
                const QString subUuid = Xml::getXmlProperty(tr, QStringLiteral("wunjo:uuid"));
                if (subUuid.isEmpty()) {
                    // Not a sequence
                    continue;
                }
                if (subUuid != uuid) {
                    ...
                    QString subClipId = Xml::getXmlProperty(tr, QStringLiteral("wunjo:id"));
                    const QString subControlId = Xml::getXmlProperty(tr, QStringLiteral("wunjo:control_uuid"));
            ...
            for (int j = 0; j < tractors.count(); ++j) {
                ...
                if (subUuid != uuid) {
                    ...
                    QString subClipId = Xml::getXmlProperty(tr, QStringLiteral("wunjo:id"));
                    int subDuration = Xml::getXmlProperty(tr, QStringLiteral("wunjo:duration")).toInt();
                    const QString subClipname = Xml::getXmlProperty(tr, QStringLiteral("wunjo:clipname"));
                    QDomDocument subDoc;
                    ...
                    xmlProd->set("wunjo:id", subClipId.toUtf8().constData());
                    xmlProd->set("wunjo:producer_type", ClipType::Timeline);
                    xmlProd->set("wunjo:uuid", subUuid.toUtf8().constData());
                    xmlProd->set("wunjo:control_uuid", subControlId.toUtf8().constData());
                    xmlProd->set("wunjo:duration", xmlProd->frames_to_time(subDuration));
                    xmlProd->set("wunjo:clipname", subClipname.toUtf8().constData());
                    xmlProd->set("_wunjo_processed", 1);
            ...
            xmlProd->set("wunjo:id", clipId.toUtf8().constData());
            xmlProd->set("wunjo:producer_type", ClipType::Timeline);
            xmlProd->set("wunjo:uuid", uuid.toUtf8().constData());
            xmlProd->set("wunjo:duration", xmlProd->frames_to_time(duration));
            xmlProd->set("wunjo:clipname", clipname.toUtf8().constData());
            xmlProd->set("_wunjo_processed", 1);
    ...
    if (!clipsImported) {
        ...
        if (result && updatedPosition > 0) {
            pCore->seekMonitor(Wunjo::ProjectMonitor, updatedPosition);
        }
        return result;
    }
...
bool TimelineFunctions::requestDeleteBlankAt(const std::shared_ptr<TimelineItemModel> &timeline, int trackId, int position, bool affectAllTracks)
{
    ...
    std::function<bool(void)> undo = []() { return true; };
    std::function<bool(void)> redo = []() { return true; };
    requestSpacerEndOperation(timeline, cid, start, spaceStart, affectAllTracks ? -1 : trackId, WunjoSettings::lockedGuides() ? -1 : position, undo, redo);
    return true;
}
...
bool TimelineFunctions::requestDeleteAllBlanksFrom(const std::shared_ptr<TimelineItemModel> &timeline, int trackId, int position)
{
    ...
    if (timeline->isSubtitleTrack(trackId)) {
        ...
        while (blankStart != -1) {
            ...
            if (blankStart < start) {
                if (!requestSpacerEndOperation(timeline, cid, start, blankStart, trackId, WunjoSettings::lockedGuides() ? -1 : position, local_undo,
                                               local_redo, false)) {
...
bool TimelineFunctions::addMarkersAtGaps(const std::shared_ptr<TimelineItemModel> &timeline)
{
    ...
    for (auto &gap : gaps) {
        ...
        guideModel->addRangeMarker(gapStart, gapDuration, i18n("Gap"), WunjoSettings::default_marker_type());
    }
    return true;
}
...
QDomDocument TimelineFunctions::extractClip(const std::shared_ptr<TimelineItemModel> &timeline, int cid, const QString &binId)
{
    ...
    QDomElement container = destDoc.createElement(QStringLiteral("wunjo-scene"));
    ...
    for (int i = 0; i < producers.count(); ++i) {
        ...
        bool ok;
        int clipId = Xml::getXmlProperty(currentProd, QLatin1String("wunjo:id")).toInt(&ok);
        if (!ok) {
            // Check if this is a black bg track
            if (Xml::hasXmlProperty(currentProd, QLatin1String("wunjo:playlistid"))) {
            ...
            Xml::setXmlProperty(currentProd, QStringLiteral("wunjo:id"), QString::number(clipId));
        ...
        QString prodResource = Xml::getXmlProperty(binProducer, QStringLiteral("wunjo:originalurl"));
        if (!prodResource.isEmpty()) {
            ...
            if (info.isRelative()) {
                ...
                Xml::setXmlProperty(binProducer, QStringLiteral("wunjo:originalurl"), prodResource);
            }
        }
        ...
        for (int j = 0; j < elements.count(); ++j) {
            ...
            if (currentElement.tagName() == QLatin1String("entry")) {
                ...
                for (int k = 0; k < effects.count(); ++k) {
                    ...
                    QString filterId = Xml::getXmlProperty(effect, QLatin1String("wunjo_id"));
                    ...
                    for (int l = 0; l < properties.count(); ++l) {
                        ...
                        if (propName == QLatin1String("mlt_service") || propName == QLatin1String("wunjo_id")) {
                            continue;
                        }
    ...
    if (!isAudio) {
        ...
        for (int i = 0; i < compositions.count(); ++i) {
            ...
            const QString compoId = Xml::getXmlProperty(currentCompo, QLatin1String("wunjo_id"));
