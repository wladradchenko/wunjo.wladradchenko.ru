    ...
    , m_softDelete(false)
{
    if (auto ptr = parent.lock()) {
        ...
        if (!trackName.isEmpty()) {
            m_track->set("wunjo:track_name", trackName.toUtf8().constData());
        }
        if (audioTrack) {
            m_track->set("wunjo:audio_track", 1);
        ...
        m_track->set("wunjo:trackheight", WunjoSettings::trackheight());
        m_track->set("wunjo:timeline_active", 1);
        m_effectStack = EffectStackModel::construct(m_track, ObjectId(WunjoObjectType::TimelineTrack, m_id, ptr->uuid()), ptr->m_undoStack);
        // TODO
                                     ...
                                     const QList<int> &allowedClipMixes, bool bypassLock)
{
    ...
    if (finalDeletion) {
        pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::TimelineClip, clipId, timelineUuid));
    }
...
int TrackModel::getClipByPosition(int position, int playlist)
{
    ...
    int cid = prod->get_int("_wunjo_cid");
    if (playlist == -1) {
        if (hasStartMix(cid)) {
...
void TrackModel::setProperty(const QString &name, const QString &value)
{
    ...
    if (name == QLatin1String("wunjo:audio_track") || name == QLatin1String("hide")) {
        for (auto &m_playlist : m_playlists) {
...
{
    READ_LOCK();
    return m_track->get_int("wunjo:locked_track");
}

bool TrackModel::isTimelineActive() const
{
    READ_LOCK();
    return m_track->get_int("wunjo:timeline_active");
}
...
{
    return m_track->get_int("wunjo:audio_track") == 1;
}
...
{
    return (m_track->get_int("wunjo:audio_track") == 1 ? PlaylistState::AudioOnly : PlaylistState::VideoOnly);
}
...
{
    setProperty(QStringLiteral("wunjo:locked_track"), QStringLiteral("1"));
    if (auto ptr = m_parent.lock()) {
...
{
    setProperty(QStringLiteral("wunjo:locked_track"), nullptr);
    if (auto ptr = m_parent.lock()) {
        QModelIndex ix = ptr->makeTrackIndexFromID(m_id);
        Q_EMIT ptr->dataChanged(ix, ix, {TimelineModel::IsLockedRole});
    }
...
bool TrackModel::isAvailableWithExceptions(int position, int duration, const QVector<int> &exceptions)
{
    ...
    for (auto &playlist : m_playlists) {
        ...
        for (int ix = start_clip; ix <= end_clip; ix++) {
            ...
            if (prod) {
                if (!exceptions.contains(prod->get_int("_wunjo_cid"))) {
                    return false;
                }
            }
        }
    ...
    if (result) {
        ...
        Fun reverse = [this, clipIds, assetId, params, tracks, mixDuration, mixPosition, mixCutPos, firstInPos, secondInPos, switchFirstTrack,
            ...
            if (auto ptr = m_parent.lock()) {
                ...
                t->set("wunjo:mixcut", mixCutPos);
                t->set("wunjo_id", assetId.toUtf8().constData());
                ...
                std::shared_ptr<AssetParameterModel> asset(
                    new AssetParameterModel(std::move(t), xml, assetId, ObjectId(WunjoObjectType::TimelineMix, clipIds.second, ptr->uuid()), QString()));
    ...
    Fun build_mix = [clipIds, mixPosition, mixDurations, dest_track, secondClipCut, mixId, this]() {
        if (auto ptr = m_parent.lock()) {
            ...
            if (isAudioTrack()) {
                ...
                t->set("wunjo:mixcut", secondClipCut);
                t->set("start", -1);
                ...
                t->set("wunjo:mixcut", secondClipCut);
                t->set("wunjo_id", assetName.toUtf8().constData());
                if (isLuma) {
                    QString res = mixId;
            ...
            std::shared_ptr<AssetParameterModel> asset(
                new AssetParameterModel(std::move(t), xml, assetName, ObjectId(WunjoObjectType::TimelineMix, clipIds.second, ptr->uuid()), QString()));
            m_sameCompositions[clipIds.second] = asset;
            m_mixList.insert(clipIds.first, clipIds.second);
        }
        return true;
...
bool TrackModel::createMix(MixInfo info, std::pair<QString, QVector<QPair<QString, QVariant>>> params, std::pair<int, int> tracks, bool finalMove)
{
    ...
    if (auto ptr = m_parent.lock()) {
        ...
        t->set("wunjo:mixcut", info.mixOffset);
        t->set("wunjo_id", assetId.toUtf8().constData());
        t->set_tracks(tracks.first, tracks.second);
        m_track->plant_transition(*t.get(), tracks.first, tracks.second);
        QDomElement xml = TransitionsRepository::get()->getXml(assetId);
        ...
        std::shared_ptr<AssetParameterModel> asset(
            new AssetParameterModel(std::move(t), xml, assetId, ObjectId(WunjoObjectType::TimelineMix, info.secondClipId, ptr->uuid()), QString()));
...
bool TrackModel::createMix(MixInfo info, bool isAudio)
{
    ...
    if (auto ptr = m_parent.lock()) {
        ...
        QDomElement xml = TransitionsRepository::get()->getXml(assetName);
        std::shared_ptr<AssetParameterModel> asset(
            new AssetParameterModel(std::move(t), xml, assetName, ObjectId(WunjoObjectType::TimelineMix, info.secondClipId, ptr->uuid()), QString()));
        m_sameCompositions[info.secondClipId] = asset;
        m_mixList.insert(info.firstClipId, info.secondClipId);
        return true;
    }
...
void TrackModel::setMixDuration(int cid, int mixDuration, int mixCut)
{
    ...
    m_sameCompositions[cid]->getAsset()->set("wunjo:mixcut", mixCut);
...
bool TrackModel::loadMix(Mlt::Transition *t)
{
    ...
    QString assetId(t->get("wunjo_id"));
    if (assetId.isEmpty()) {
    ...
    std::shared_ptr<AssetParameterModel> asset(
        new AssetParameterModel(std::move(tr), xml, assetId, ObjectId(WunjoObjectType::TimelineMix, cid2, timelineUuid), QString()));
    m_sameCompositions[cid2] = asset;
    m_mixList.insert(cid1, cid2);
    int mixDuration = t->get_length() - 1;
    int mixCutPos = qMin(t->get_int("wunjo:mixcut"), mixDuration);
...
void TrackModel::switchMix(int cid, const QString &composition, Fun &undo, Fun &redo)
{
    ...
    Fun local_redo = [this, cid, composition, reverse]() {
        ...
        int mixCutPos = transition.get_int("wunjo:mixcut");
        QScopedPointer<Mlt::Field> field(m_track->field());
        field->lock();
        field->disconnect_service(transition);
        field->unlock();
        ...
        if (auto ptr = m_parent.lock()) {
            ...
            std::shared_ptr<AssetParameterModel> asset(
                new AssetParameterModel(std::move(t), xml, composition, ObjectId(WunjoObjectType::TimelineMix, cid, ptr->uuid()), QString()));
    ...
    Fun local_undo = [this, cid, currentAsset, allParams, originTracks]() {
        ...
        if (auto ptr = m_parent.lock()) {
            ...
            std::shared_ptr<AssetParameterModel> asset(
                new AssetParameterModel(std::move(t), xml, currentAsset, ObjectId(WunjoObjectType::TimelineMix, cid, ptr->uuid()), QString()));
            m_sameCompositions[cid] = asset;
        }
        m_playlists[0].unlock();
        m_playlists[1].unlock();
