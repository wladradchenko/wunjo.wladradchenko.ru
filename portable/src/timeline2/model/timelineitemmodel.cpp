...
QVariant TimelineItemModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == SortRole) {
        if (isTrack(id)) {
            return getTrackSortValue(id, WunjoSettings::audiotracksbelow());
        }
        return QVariant();
    }
    if (isClip(id)) {
        ...
        case NameRole:
        case Qt::DisplayRole: {
            return getTrackById_const(id)->getProperty("wunjo:track_name").toString();
        }
        case TypeRole:
        ...
        case HeightRole: {
            int collapsed = getTrackById_const(id)->getProperty("wunjo:collapsed").toInt();
            if (collapsed > 0) {
                return collapsed;
            }
            int height = getTrackById_const(id)->getProperty("wunjo:trackheight").toInt();
            return (height > 0 ? height : WunjoSettings::trackheight());
        }
        case ThumbsFormatRole:
            return getTrackById_const(id)->getProperty("wunjo:thumbs_format").toInt();
        case IsCompositeRole: {
        case AudioRecordRole:
            return getTrackById_const(id)->getProperty("wunjo:audio_rec").toInt();
        }
...
void TimelineItemModel::setTrackName(int trackId, const QString &text)
{
    ...
    const QString &currentName = getTrackProperty(trackId, QStringLiteral("wunjo:track_name")).toString();
    if (text == currentName) {
        return;
    }
    Fun undo_lambda = [this, trackId, currentName]() {
        setTrackProperty(trackId, QStringLiteral("wunjo:track_name"), currentName);
        return true;
    };
    Fun redo_lambda = [this, trackId, text]() {
        setTrackProperty(trackId, QStringLiteral("wunjo:track_name"), text);
        return true;
    };
    redo_lambda();
...
void TimelineItemModel::setTrackProperty(int trackId, const QString &name, const QString &value)
{
    ...
    if (name == QLatin1String("wunjo:track_name")) {
        roles.push_back(NameRole);
        if (!track->isAudioTrack()) {
            updateMultiTrack = true;
        }
    } else if (name == QLatin1String("wunjo:locked_track")) {
        roles.push_back(IsLockedRole);
    } else if (name == QLatin1String("hide")) {
        roles.push_back(IsDisabledRole);
        if (!isLoading) {
            if (!track->isAudioTrack()) {
                pCore->invalidateItem(ObjectId(WunjoObjectType::TimelineTrack, trackId, m_uuid));
                pCore->refreshProjectMonitorOnce();
                updateMultiTrack = true;
            } else {
                pCore->invalidateAudio(ObjectId(WunjoObjectType::TimelineTrack, trackId, m_uuid));
            }
        }
    } else if (name == QLatin1String("wunjo:timeline_active")) {
        roles.push_back(TrackActiveRole);
    } else if (name == QLatin1String("wunjo:thumbs_format")) {
        roles.push_back(ThumbsFormatRole);
    } else if (name == QLatin1String("wunjo:collapsed")) {
        roles.push_back(HeightRole);
    } else if (name == QLatin1String("wunjo:audio_rec")) {
...
const QString TimelineItemModel::getTrackFullName(int tid) const
{
    ...
    QString trackName = getTrackById_const(tid)->getProperty(QStringLiteral("wunjo:track_name")).toString();
    ...
    while (it != m_allTracks.cend()) {
        if ((*it)->isAudioTrack()) {
            pCore->mixer()->registerTrack((*it)->getId(), (*it)->getTrackService(), getTrackTagById((*it)->getId()),
                                          (*it)->getProperty(QStringLiteral("wunjo:track_name")).toString());
            if ((*it)->getProperty("wunjo:audio_rec").toInt() == 1) {
                recTrack = (*it)->getId();
            }
        }
        ++it;
...
void TimelineItemModel::buildTrackCompositing(bool rebuild)
{
    ...
    int currentClipType = m_tractor->get_int("wunjo:clip_type");
    int newClipType = audioTracks > 0 ? (videoTracks > 0 ? 0 : 1) : 2;
    if (currentClipType != newClipType) {
        m_tractor->set("wunjo:sequenceproperties.hasAudio", audioTracks > 0 ? 1 : 0);
        m_tractor->set("wunjo:sequenceproperties.hasVideo", videoTracks > 0 ? 1 : 0);
        m_tractor->set("wunjo:clip_type", newClipType);
    }
...
void TimelineItemModel::passSequenceProperties(const QMap<QString, QString> baseProperties)
{
    ...
    while (i.hasNext()) {
        i.next();
        tractor()->set(QStringLiteral("wunjo:sequenceproperties.%1").arg(i.key()).toUtf8().constData(), i.value().toUtf8().constData());
    }
    // Store groups data
    tractor()->set("wunjo:sequenceproperties.groups", groupsData().toUtf8().constData());
    tractor()->set("wunjo:sequenceproperties.documentuuid", pCore->currentDoc()->uuid().toString().toUtf8().constData());
    // Save timeline guides
    const QString guidesData = getGuideModel()->toJson();
    tractor()->set("wunjo:sequenceproperties.guides", guidesData.toUtf8().constData());
    QPair<int, int> tracks = getAVtracksCount();
    tractor()->set("wunjo:sequenceproperties.hasAudio", tracks.first > 0 ? 1 : 0);
    tractor()->set("wunjo:sequenceproperties.hasVideo", tracks.second > 0 ? 1 : 0);
    tractor()->set("wunjo:sequenceproperties.tracksCount", tracks.first + tracks.second);

    if (hasTimelinePreview()) {
        QPair<QStringList, QStringList> chunks = previewManager()->previewChunks();
        tractor()->set("wunjo:sequenceproperties.previewchunks", chunks.first.join(QLatin1Char(',')).toUtf8().constData());
        tractor()->set("wunjo:sequenceproperties.dirtypreviewchunks", chunks.second.join(QLatin1Char(',')).toUtf8().constData());
    }
}
