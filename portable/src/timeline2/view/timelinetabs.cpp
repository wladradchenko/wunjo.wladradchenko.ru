...
void TimelineTabs::renameTab(const QUuid &uuid, const QString &name)
{
    ...
    for (int i = 0; i < count(); i++) {
        if (static_cast<TimelineWidget *>(widget(i))->getUuid() == uuid) {
            ...
            pCore->projectManager()->setTimelineProperty(uuid, QStringLiteral("wunjo:clipname"), name);
            updateWindowTitle();
            break;
        }
    }
...
void TimelineTabs::buildClipMenu()
{
    ...
    // [Wunjo] AI tools sit at the top of the clip menu, spelled out in full
    // so they catch the eye; every AI plugin gathers here. Video tools are
    // hidden on audio clips and vice versa (see TimelineWidget::showClipMenu).
    QMenu *aiMenu = new QMenu(i18n("Artificial Intelligence"), this);
    aiMenu->setIcon(QIcon::fromTheme(QStringLiteral("tools-wizard")));
    aiMenu->menuAction()->setObjectName(QStringLiteral("ai_menu"));
    if (QAction *maskAction = coll->action(QStringLiteral("ai_object_mask"))) {
        aiMenu->addAction(maskAction);
    }
    if (QAction *facesAction = coll->action(QStringLiteral("ai_detect_faces"))) {
        aiMenu->addAction(facesAction);
    }
    if (QAction *speechAction = coll->action(QStringLiteral("audio_recognition"))) {
        // proxy action: its visibility is tuned per clip, which must not
        // affect the global action used in the Sequence > Subtitles menu
        QAction *speechProxy = new QAction(speechAction->icon(), speechAction->text(), this);
        speechProxy->setObjectName(QStringLiteral("ai_speech_proxy"));
        connect(speechProxy, &QAction::triggered, speechAction, &QAction::trigger);
        aiMenu->addAction(speechProxy);
    }
    m_timelineClipMenu->addMenu(aiMenu);
