...
TimelineController::TimelineController(QObject *parent)
    ...
    , m_snapStackIndex(-1)
    , m_effectZone({0, 0})
    , m_autotrackHeight(WunjoSettings::autotrackheight())
{
    ...
    connect(WunjoSettings::self(), &WunjoSettings::waveformScalerChanged, this, &TimelineController::audioZoomTextChanged);
}
...
{
    int currentIx = pCore->currentDoc()->getSequenceProperty(m_model->uuid(), QStringLiteral("wunjo:activeSubtitleIndex"), QStringLiteral("0")).toInt();
...
void TimelineController::setTargetTracks(bool hasVideo, const QMap<int, QString> &audioTargets)
{
    ...
    if (m_hasAudioTarget > 0) {
        if (m_lastAudioTarget.count() == audioTargets.count()) {
            ...
            if (WunjoSettings::multistream_checktrack() && audioTargets.count() > tracks.count()) {
...
bool TimelineController::selectCurrentItem(WunjoObjectType type, bool select, bool addToCurrent, bool showErrorMsg)
{
    int currentClip = -1;
    if (m_activeTrack == -1 || (m_model->isSubtitleTrack(m_activeTrack) && type != WunjoObjectType::TimelineClip)) {
        // Cannot select item
    } else if (type == WunjoObjectType::TimelineClip) {
        currentClip = m_model->getClipByPosition(m_activeTrack, pCore->getMonitorPosition(), activeSubLayer());
    } else if (type == WunjoObjectType::TimelineComposition) {
        currentClip = m_model->getCompositionByPosition(m_activeTrack, pCore->getMonitorPosition());
    } else if (type == WunjoObjectType::TimelineMix) {
        if (m_activeTrack >= 0) {
            currentClip = m_model->getClipByPosition(m_activeTrack, pCore->getMonitorPosition(), activeSubLayer());
        }
        if (currentClip > -1) {
...
int TimelineController::insertNewComposition(int tid, int clipId, int offset, QString transitionId, bool logUndo)
{
    ...
    int clip_duration = clipId > -1 ? m_model->getClipPlaytime(clipId) : pCore->getDurationFromString(WunjoSettings::transition_duration());
    int endPos = minimumPos + clip_duration;
    int position = minimumPos;
    int duration = qMin(clip_duration, pCore->getDurationFromString(WunjoSettings::transition_duration()));
    int lowerVideoTrackId = m_model->getPreviousVideoTrackIndex(tid);
    ...
    int defaultLength = pCore->getDurationFromString(WunjoSettings::transition_duration());
    ...
    if (duration < 0 || (isShortComposition && duration > 1.5 * defaultLength)) {
        ...
        duration = pCore->getDurationFromString(WunjoSettings::transition_duration());
...
{
    if (WunjoSettings::draginprogress()) {
...
{
    return WunjoSettings::frametimecode() ? QString::number(frames) : m_model->tractor()->frames_to_time(frames, mlt_time_smpte_df);
}
...
void TimelineController::switchTrackRecord(int tid, bool monitor)
{
    ...
    int recDisplayed = m_model->getTrackProperty(tid, QStringLiteral("wunjo:audio_rec")).toInt();
    if (monitor == false) {
        ...
        m_model->setTrackProperty(tid, QStringLiteral("wunjo:audio_rec"), QStringLiteral("0"));
    } else {
        // Enable rec controls
        if (recDisplayed == 1) {
            return;
        }
        m_model->setTrackProperty(tid, QStringLiteral("wunjo:audio_rec"), QStringLiteral("1"));
    }
...
{
    Q_EMIT pCore->showConfigDialog((Wunjo::ConfigPage)page, tab);
}
...
void TimelineController::setInPoint(bool ripple)
{
    ...
    auto requestResize = [this, ripple](int id, int size) {
        if (ripple) {
            m_model->requestItemRippleResize(m_model, id, size, false, true, !WunjoSettings::lockedGuides(), 0, false);
...
void TimelineController::setOutPoint(bool ripple)
{
    ...
    auto requestResize = [this, ripple](int id, int size) {
        if (ripple) {
            m_model->requestItemRippleResize(m_model, id, size, true, true, !WunjoSettings::lockedGuides(), 0, false);
        } else {
...
void TimelineController::addQuickMarker(int cid, int position)
{
    ...
    std::shared_ptr<ProjectClip> clip = pCore->bin()->getBinClip(getClipBinId(cid));
    GenTime pos(position, pCore->getCurrentFps());
    clip->getMarkerModel()->addMarker(pos, i18n("Marker"), WunjoSettings::default_marker_type());
}
    ...
    if (!markerFound) {
        ...
        if (showGui) {
            ...
            if (WunjoSettings::guidesShowThumbs()) {
                ...
                std::set<int> frames;
                frames.insert(frame);
                CacheTask::start(ObjectId(WunjoObjectType::BinClip, binId.toInt(), QUuid()), frames, pCore->guidesList());
            }
        }
    } else {
...
void TimelineController::addAsset(const QVariantMap &data)
{
    const auto selection = m_model->getCurrentSelection();
    if (!selection.empty()) {
        const QString effect = data.value(QStringLiteral("wunjo/effect")).toString();
        QVariantList effectSelection = m_model->addClipEffect(*selection.begin(), effect, false);
        if (WunjoSettings::seekonaddeffect() && effectSelection.count() == 1) {
...
{
    if (trackId > -1) {
        m_model->getTrackById(trackId)->setProperty(QStringLiteral("wunjo:trackheight"), QString::number(height));
        m_model->setTrackProperty(trackId, "wunjo:collapsed", QStringLiteral("0"));
...
void TimelineController::adjustAllTrackHeight(int trackId, int height)
{
    ...
    while (it != m_model->m_allTracks.cend()) {
        ...
        if (target_track != trackId && m_model->getTrackById_const(target_track)->isAudioTrack() == isAudio) {
            m_model->getTrackById(target_track)->setProperty(QStringLiteral("wunjo:trackheight"), QString::number(height));
        }
        ++it;
    }
...
void TimelineController::collapseAllTracks()
{
    ...
    bool collapsed = m_model->getTrackById_const(tid)->getProperty("wunjo:collapsed").toInt() > 0;
...
void TimelineController::collapseAllTrackHeight(int trackId, bool collapse, int collapsedHeight)
{
    ...
    while (it != m_model->m_allTracks.cend()) {
        ...
        if (m_model->getTrackById_const(target_track)->isAudioTrack() == isAudio) {
            if (collapse) {
                m_model->setTrackProperty(target_track, "wunjo:collapsed", QString::number(collapsedHeight));
            } else {
                m_model->setTrackProperty(target_track, "wunjo:collapsed", QStringLiteral("0"));
            }
        }
        ++it;
    }
...
{
    if (trackId > -1) {
        m_model->getTrackById(trackId)->setProperty(QStringLiteral("wunjo:trackheight"), QString::number(WunjoSettings::trackheight()));
        QModelIndex modelStart = m_model->makeTrackIndexFromID(trackId);
        Q_EMIT m_model->dataChanged(modelStart, modelStart, {TimelineModel::HeightRole});
        return;
    }
    ...
    while (it != m_model->m_allTracks.cend()) {
        int target_track = (*it)->getId();
        m_model->getTrackById(target_track)->setProperty(QStringLiteral("wunjo:trackheight"), QString::number(WunjoSettings::trackheight()));
        ++it;
    }
    int tracksCount = m_model->getTracksCount();
    QModelIndex modelStart = m_model->makeTrackIndexFromID(m_model->getTrackIndexFromPosition(0));
...
void TimelineController::adjustFade(int cid, const QString &effectId, int duration, int initialDuration)
{
    if (initialDuration == -2) {
        // Add default fade
        duration = pCore->getDurationFromString(WunjoSettings::fade_duration());
        initialDuration = 0;
    }
    if (duration <= 0) {
        // remove fade
...
{
    return qMax(10, WunjoSettings::headerwidth());
}

void TimelineController::setHeaderWidth(int width)
{
    WunjoSettings::setHeaderwidth(width);
}
...
{
    return m_model->requestItemRippleResize(m_model, itemId, size, right, logUndo, !WunjoSettings::lockedGuides(), snapDistance, allowSingleResize);
}
...
void TimelineController::insertSpace(int trackId, int frame)
{
    ...
    requestSpacerEndOperation(cid, start, start + spaceDuration, affectAllTracks ? -1 : trackId, {},
                              WunjoSettings::lockedGuides() || !affectAllTracks ? -1 : start);
}
...
void TimelineController::switchCompositing(bool enable)
{
    ...
    if (enable) {
        ...
        for (int track = 0; track < m_model->getTracksCount(); track++) {
            if (m_model->getTrackById(m_model->getTrackIndexFromPosition(track))->getProperty("wunjo:audio_track").toInt() == 0) {
...
void TimelineController::extractZone(QPoint zone, bool liftOnly)
{
    ...
    bool moveGuides = WunjoSettings::lockedGuides() == false && (uint)tracks.count() == m_model->m_allTracks.size();
    if (m_zone.isNull()) {
...
bool TimelineController::insertClipZone(const QString &binId, int tid, int position)
{
    ...
    if (res) {
        ...
        Fun redoPos = [this, newPos]() {
            Wunjo::MonitorId activeMonitor = pCore->monitorManager()->activeMonitor()->id();
...
int TimelineController::insertZone(const QString &binId, QPoint zone, bool overwrite, Fun &undo, Fun &redo)
{
    ...
    if (res) {
        ...
        Fun redoPos = [this, newPos]() {
            setPosition(newPos);
            pCore->getMonitor(Wunjo::ProjectMonitor)->refreshMonitorIfActive();
            return true;
        };
        Fun undoPos = [this, currentPos]() {
            setPosition(currentPos);
...
int TimelineController::addCompositionToClip(const QString &assetId, int clipId, int offset)
{
    ...
    int compoId = -1;
    if (assetId.isEmpty()) {
        QStringList compositions = WunjoSettings::favorite_transitions();
...
void TimelineController::addEffectToClip(const QString &assetId, int clipId)
{
    ...
    if (m_model->addClipEffect(clipId, assetId).size() > 0 && WunjoSettings::seekonaddeffect()) {
        // Move timeline cursor inside clip if it is not
        int in = m_model->getClipPosition(clipId);
        int out = in + m_model->getClipPlaytime(clipId);
        int position = pCore->getMonitorPosition();
...
void TimelineController::switchTrackActive(int trackId)
{
    ...
    m_model->setTrackProperty(trackId, QStringLiteral("wunjo:timeline_active"), active ? QStringLiteral("0") : QStringLiteral("1"));
    m_activeSnaps.clear();
}
...
void TimelineController::switchAllTrackActive()
{
    ...
    while (it != m_model->m_allTracks.cend()) {
        ...
        int target_track = (*it)->getId();
        m_model->setTrackProperty(target_track, QStringLiteral("wunjo:timeline_active"), active ? QStringLiteral("0") : QStringLiteral("1"));
        ++it;
    }
    m_activeSnaps.clear();
}
...
void TimelineController::makeAllTrackActive()
{
    ...
    while (it != m_model->m_allTracks.cend()) {
        int target_track = (*it)->getId();
        m_model->setTrackProperty(target_track, QStringLiteral("wunjo:timeline_active"), makeActive ? QStringLiteral("1") : QStringLiteral("0"));
        ++it;
    }
    m_activeSnaps.clear();
}
...
{
    return WunjoSettings::autoscroll() || !pCore->monitorManager()->projectMonitor()->isPlaying();
}
...
void TimelineController::resetTrackHeight()
{
    ...
    for (int track = tracksCount - 1; track >= 0; track--) {
        int trackIx = m_model->getTrackIndexFromPosition(track);
        m_model->getTrackById(trackIx)->setProperty(QStringLiteral("wunjo:trackheight"), QString::number(WunjoSettings::trackheight()));
    }
    QModelIndex modelStart = m_model->makeTrackIndexFromID(m_model->getTrackIndexFromPosition(0));
    QModelIndex modelEnd = m_model->makeTrackIndexFromID(m_model->getTrackIndexFromPosition(tracksCount - 1));
    Q_EMIT m_model->dataChanged(modelStart, modelEnd, {TimelineModel::HeightRole});
...
void TimelineController::pasteEffects(int targetId)
{
    ...
    if (copiedItems.documentElement().tagName() != QLatin1String("wunjo-scene")) {
        pCore->displayMessage(i18n("No information in clipboard"), ErrorMessage, 500);
        return;
    }
...
void TimelineController::activateTrackAndSelect(int trackPosition, bool notesMode)
{
    ...
    if (tid > -1) {
        ...
        if (!notesMode && pCore->activeTool() != ToolType::MulticamTool) {
            selectCurrentItem(WunjoObjectType::TimelineClip, true);
        }
    }
}
...
{
    if (WunjoSettings::videoColor().alpha() > 0) {
        return WunjoSettings::videoColor();
    }
...
{
    // The badge on an active track is an accent, not a verdict. It used to be
    // drawn from the scheme's "positive" colour — green whatever the user had
    // chosen — which left V1/A1 mint on a blue theme.
    return WunjoTheme::instance()->accent();
}

QColor TimelineController::targetTextColor() const
{
    // Text sitting on that badge: dark on a light accent, light on a dark one.
    const QColor accent = WunjoTheme::instance()->accent();
    return accent.lightness() > 140 ? QColor(13, 13, 13) : QColor(255, 255, 255);
}

QColor TimelineController::audioColor() const
{
    if (WunjoSettings::audioColor().alpha() > 0) {
        return WunjoSettings::audioColor();
    }
    return WunjoSettings::thumbColor1().darker(150);
}

QColor TimelineController::titleColor() const
{
    if (WunjoSettings::titleColor().alpha() > 0) {
        return WunjoSettings::titleColor();
    }
    KColorScheme scheme(QApplication::palette().currentColorGroup());
...
{
    if (WunjoSettings::imageColor().alpha() > 0) {
        return WunjoSettings::imageColor();
    }
    KColorScheme scheme(QApplication::palette().currentColorGroup());
...
{
    if (WunjoSettings::slideshowColor().alpha() > 0) {
        return WunjoSettings::slideshowColor();
    }
    KColorScheme scheme(QApplication::palette().currentColorGroup());
    QColor base = scheme.foreground(KColorScheme::LinkText).color();
...
void TimelineController::switchRecording(int trackId, bool record)
{
    ...
    if (record) {
        ...
        pCore->getMonitor(Wunjo::ProjectMonitor)->stopCountDown();
...
{
    return QStringLiteral("X%1").arg(WunjoSettings::waveformScaler());
}
...
void TimelineController::collapseActiveTrack()
{
    ...
    int collapsed = m_model->getTrackProperty(m_activeTrack, QStringLiteral("wunjo:collapsed")).toInt();
    ...
    m_model->setTrackProperty(m_activeTrack, QStringLiteral("wunjo:collapsed"), collapsed > 0 ? QStringLiteral("0") : QString::number(baseUnit));
}
...
void TimelineController::autofitTrackHeight(int timelineHeight, int collapsedHeight)
{
    ...
    auto it = m_model->m_allTracks.cbegin();
    while (it != m_model->m_allTracks.cend()) {
        if ((*it)->getProperty(QStringLiteral("wunjo:collapsed")).toInt() > 0) {
            collapsed++;
        }
        ++it;
    }
    ...
    while (it != m_model->m_allTracks.cend()) {
        if ((*it)->getProperty(QStringLiteral("wunjo:collapsed")).toInt() == 0) {
            (*it)->setProperty(QStringLiteral("wunjo:trackheight"), QString::number(trackHeight));
        }
        ++it;
    }
    // m_model->setTrackProperty(trackId, "wunjo:collapsed", QStringLiteral("0"));
        ...
        if (ix != -1 && ix < currentSubs.size()) {
            ...
            while (i.hasNext()) {
                ...
                if (ix < 0) {
                    ...
                    m_activeSubPosition = counter;
                    int currentIx =
                        pCore->currentDoc()->getSequenceProperty(m_model->uuid(), QStringLiteral("wunjo:activeSubtitleIndex"), QStringLiteral("0")).toInt();
...
void TimelineController::switchFocusClip()
{
    ...
    if (owner.type == WunjoObjectType::TimelineClip) {
