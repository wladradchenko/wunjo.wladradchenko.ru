    ...
    if (WunjoSettings::wunjorendererpath().isEmpty() || !QFileInfo::exists(WunjoSettings::wunjorendererpath())) {
        ...
        if (WunjoSettings::wunjorendererpath().isEmpty()) {
            KMessageBox::error(QApplication::activeWindow(),
                               i18n("Could not find the wunjo_render application, something is wrong with your installation. Rendering will not work"));
        }
    }
...
bool PreviewManager::initialize()
{
    ...
    connect(doc, &WunjoDoc::removeInvalidUndo, this, &PreviewManager::slotRemoveInvalidUndo, Qt::DirectConnection);
...
bool PreviewManager::buildPreviewTrack()
{
    ...
    m_previewTrack->set("wunjo:playlistid", "timeline_preview");
    m_tractor->lock();
    reconnectTrack();
    m_tractor->unlock();
    return true;
...
void PreviewManager::reconnectTrack()
{
    ...
    if (m_previewTrack) {
        ...
        tk->set("hide", 2);
        // tk->set("wunjo:playlistid", "timeline_preview");
        increment++;
    }
    if (m_overlayTrack) {
        ...
        tk->set("hide", 2);
        // tk->set("wunjo:playlistid", "timeline_overlay");
    }
}
...
void PreviewManager::disconnectTrack()
{
    if (m_previewTrackIndex > -1) {
        ...
        if (strcmp(prod->get("wunjo:playlistid"), "timeline_preview") == 0 || strcmp(prod->get("wunjo:playlistid"), "timeline_overlay") == 0) {
        ...
        if (m_tractor->count() == m_previewTrackIndex + 1) {
            ...
            if (strcmp(trkprod->get("wunjo:playlistid"), "timeline_overlay") == 0) {
                m_tractor->remove_track(m_previewTrackIndex);
            }
            delete trkprod;
        }
...
void PreviewManager::invalidatePreviews()
{
    ...
    bool timer = WunjoSettings::autopreview();
...
{
    int chunkSize = WunjoSettings::timelinechunks();
    ...
    if (add) {
        Q_EMIT dirtyChunksChanged();
        if (m_previewProcess.state() == QProcess::NotRunning && WunjoSettings::autopreview()) {
            m_previewTimer.start();
        }
    } else {
        ...
        Fun redo = [this, dirty = toRemove, isRendering]() {
            ...
            m_tractor->unlock();
            m_previewGatherTimer.start();
            if (isRendering || WunjoSettings::autopreview()) {
                m_previewTimer.start();
            }
            return true;
        };
...
void PreviewManager::startPreviewRender()
{
    ...
    if (!m_dirtyChunks.isEmpty()) {
        ...
        if (!WunjoSettings::proxypreview() && pCore->currentDoc()->useProxy()) {
...
void PreviewManager::doPreviewRender(const QString &scene)
{
    ...
    m_previewProcess.start(WunjoSettings::wunjorendererpath(), args);
...
void PreviewManager::gotPreviewRender(int frame, const QString &file, int progress)
{
    ...
    if (m_previewTrack->is_blank_at(frame)) {
        ...
        if (prod.is_valid() && prod.get_length() == WunjoSettings::timelinechunks()) {
            m_dirtyMutex.lock();
            ...
            qCDebug(WUNJO_LOG) << "* * * INVALID PROD: " << file;
            corruptedChunk(frame, file);
        }
    } else {
        qCDebug(WUNJO_LOG) << "* * * NON EMPTY PROD: " << frame;
    }
}
...
int PreviewManager::setOverlayTrack(Mlt::Playlist *overlay)
{
    ...
    m_overlayTrack->set("wunjo:playlistid", "timeline_overlay");
    reconnectTrack();
...
const QStringList PreviewManager::getCompressedList(const QVariantList items) const
{
    ...
    for (const QVariant &frame : items) {
        ...
        if (current - WunjoSettings::timelinechunks() == lastFrame) {
            lastFrame = current;
            if (frame == items.last()) {
