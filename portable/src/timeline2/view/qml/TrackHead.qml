...
Rectangle {
    ...
    // Head is a transparent container; the visible rounded "card" (with a bottom gap
    // matching the timeline lanes) is drawn by headCard below. cardColor/cardSelectedColor
    // are supplied by Timeline.qml.
    property color cardColor: activePalette.base
    property color cardSelectedColor: activePalette.highlight
    color: "transparent"
    border.width: 0
    ...
    Rectangle {
        // Visible rounded track-head card; the bottom gap shows the page colour and
        // matches the timeline lane separation.
        id: headCard
        anchors.fill: parent
        ...
        radius: 8
        color: trackHeadRoot.showAudioRecord
               ? Qt.tint(trackHeadRoot.cardColor, Qt.rgba(1, 0, 0, 0.16))
               : (trackHeadRoot.current ? Qt.tint(trackHeadRoot.cardColor, trackHeadRoot.cardSelectedColor) : trackHeadRoot.cardColor)
    ...
    Item {
            ...
            onClicked: {
                if (modifier & Qt.ShiftModifier) {
                    ...
                    if (trackHeadRoot.collapsed) {
                        var newHeight = Math.max(trackHeadRoot.collapsedHeight * 1.5, trackHeadRoot.controller.getTrackProperty(trackHeadRoot.trackId, "wunjo:trackheight"))
                        trackHeadRoot.controller.setTrackProperty(trackHeadRoot.trackId, "wunjo:trackheight", newHeight)
                        trackHeadRoot.controller.setTrackProperty(trackHeadRoot.trackId, "wunjo:collapsed", "0")
                    } else {
                        trackHeadRoot.controller.setTrackProperty(trackHeadRoot.trackId, "wunjo:collapsed", trackHeadRoot.collapsedHeight)
                    }
                }
        ...
        Label {
            id: trackLed
            property color bgColor: Qt.darker(trackHeadRoot.cardColor, 0.55)
        ...
        state:  'normalled'
            states: [
                ...
                State {
                    ...
                    PropertyChanges {
                        trackLed.bgColor: Qt.darker(trackHeadRoot.cardColor, 0.55)
                    }
                }
            ]
            transitions: [
        ...
        Label {
            ...
            verticalAlignment: Text.AlignVCenter
            horizontalAlignment: Text.AlignHCenter
            visible: K.WunjoSettings.uiDebugMode
        }
        Label {
            id: miniTrackLabel
            anchors.left: trackLed.right
        ...
        Row {
            ...
            ToolButton {
                ...
                icon.name: trackHeadRoot.isAudio ? (trackHeadRoot.isDisabled ? "audio-off" : "audio-volume-high")
                                   : (trackHeadRoot.isDisabled ? "wunjo-hide-video" : "wunjo-show-video")
                width: trackHeadRoot.collapsedHeight
                height: trackHeadRoot.collapsedHeight
