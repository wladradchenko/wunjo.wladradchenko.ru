...
Rectangle {
    ...
    readonly property bool hideDecorations: !K.WunjoSettings.showClipOverlays || !visible || trimInMouseArea.drag.active || trimOutMouseArea.drag.active || fadeInMouseArea.drag.active || fadeOutMouseArea.drag.active
    width : Math.round(clipDuration * timeScale)
    // Rounded "card" corners — modern clip look (border + thumbs follow this radius)
    radius: 8
    ...
    onFakeTidChanged: {
        if (clipRoot.fakeTid > -1 && parentTrack) {
            ...
            clipRoot.height = Qt.binding(function () {
                // -4 keeps the bottom gap that separates clips from the track below
                return parentTrack.height - 4
            })
        }
    }
    ...
    DropArea { //Drop area for clips
        ...
        onDropped: drag => {
            ...
            if (K.WunjoSettings.seekonaddeffect && (proxy.position < clipRoot.modelStart || proxy.position > clipRoot.modelStart + clipRoot.clipDuration)) {
    ...
    MouseArea {
        ...
        Rectangle {
            // Subtle card depth: soft top highlight + bottom shade over the base
            // fill. Declared before the thumbs so it stays behind them.
            id: clipSheen
            anchors.fill: parent
        ...
        Loader {
            ...
            active: clipRoot.visible
                    && !(clipRoot.hideClipViews
                         ...
                         || clipRoot.parentTrack.isAudio && !K.WunjoSettings.audiothumbnails
                         // not if it is a video clip, but video thumbs are disabled
                         || !clipRoot.parentTrack.isAudio && !K.WunjoSettings.videothumbnails
                         )
        ...
        Item {
            ...
            Component {
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        radius: 2
                        opacity: 0.7
                        visible: K.WunjoSettings.showmarkers && root.maxLabelWidth > K.UiUtils.baseSizeMedium && height < container.height && (markerBase.x > textMetrics.width || container.height > 2 * height)
            ...
            Item {
                ...
                Rectangle {
                    ...
                    width: label.width + (2 * itemBorder.border.width) + 4
                    height: label.height
                    radius: 3
