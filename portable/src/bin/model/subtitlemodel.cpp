    ...
    , m_subtitleFilter(new Mlt::Filter(pCore->getProjectProfile(), "avfilter.subtitles"))
{
    ...
    int id = pCore->currentDoc()->getSequenceProperty(timelineUuid, QStringLiteral("wunjo:activeSubtitleIndex"), QStringLiteral("0")).toInt();
            ...
            if (!line.isEmpty()) {
                if (!turn) {
                    ...
                    if (linespace.replace(" ", "").contains("ScriptInfo")) {
                        ...
                        turn++;
                        continue;
                    } else if (linespace.contains("WunjoExtradata")) {
                        section = "Wunjo Extradata";
                        turn++;
                        continue;
                ...
                if (section.contains("Wunjo Extradata")) {
...
int SubtitleModel::cutSubtitle(int layer, int position, Fun &undo, Fun &redo)
{
    ...
    if (start >= GenTime()) {
        ...
        if (WunjoSettings::subtitle_razor_mode() == RAZOR_MODE_DUPLICATE) {
            leftText = originalText;
            rightText = originalText;
        } else if (WunjoSettings::subtitle_razor_mode() == RAZOR_MODE_AFTER_FIRST_LINE) {
...
{
    int ix = pCore->currentDoc()->getSequenceProperty(m_timeline->uuid(), QStringLiteral("wunjo:activeSubtitleIndex"), QStringLiteral("0")).toInt();
    QString outFile = pCore->currentDoc()->subTitlePath(m_timeline->uuid(), ix, false);
...
{
    bool isLocked = m_subtitleFilter->get_int("wunjo:locked") == 1;
    m_subtitleFilter->set("wunjo:locked", isLocked ? 0 : 1);
...
{
    return m_subtitleFilter->get_int("wunjo:locked") == 1;
}
...
void SubtitleModel::addSubtitle(int startframe, int layer, QString text)
{
    ...
    int endframe = startframe + pCore->getDurationFromString(WunjoSettings::subtitle_duration());
    int id = TimelineModel::getNextId();
    if (text.isEmpty()) {
        text = i18n("Add Text");
    }
...
{
    int currentIx = pCore->currentDoc()->getSequenceProperty(m_timeline->uuid(), QStringLiteral("wunjo:activeSubtitleIndex"), QStringLiteral("0")).toInt();
...
{
    // int currentIx = pCore->currentDoc()->getSequenceProperty(m_timeline->uuid(), QStringLiteral("wunjo:activeSubtitleIndex"),
    ...
    endRemoveRows();
    pCore->currentDoc()->setSequenceProperty(m_timeline->uuid(), QStringLiteral("wunjo:activeSubtitleIndex"), ix);
    parseSubtitle(workPath);
}
