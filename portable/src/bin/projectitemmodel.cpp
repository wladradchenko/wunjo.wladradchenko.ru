...
void ProjectItemModel::buildPlaylist(const QUuid uuid)
{
    ...
    m_projectTractor->set("wunjo:projectTractor", 1);
...
bool ProjectItemModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
    ...
    if (data->hasFormat(QStringLiteral("wunjo/effect"))) {
        // Dropping effect on a Bin item
        QStringList effectData;
        effectData << QString::fromUtf8(data->data(QStringLiteral("wunjo/effect")));
        QStringList source = QString::fromUtf8(data->data(QStringLiteral("wunjo/effectsource"))).split(QLatin1Char(','));
        effectData << source;
        Q_EMIT effectDropped(effectData, parent);
        return true;
    }

    if (data->hasFormat(QStringLiteral("wunjo/clip"))) {
        const QStringList list = QString(data->data(QStringLiteral("wunjo/clip"))).split(QLatin1Char(';'));
        QString id;
        return requestAddBinSubClip(id, list.at(1).toInt(), list.at(2).toInt(), {}, list.at(0));
    }

    if (data->hasFormat(QStringLiteral("wunjo/tag"))) {
        // Dropping effect on a Bin item
        QString tag = QString::fromUtf8(data->data(QStringLiteral("wunjo/tag")));
...
{
    QStringList types{QStringLiteral("text/producerslist"), QStringLiteral("text/uri-list"), QStringLiteral("wunjo/clip"), QStringLiteral("wunjo/effect"),
                      QStringLiteral("wunjo/tag")};
    return types;
}
...
int16_t ProjectItemModel::getAudioMaxLevel(const QString &binId, int stream)
{
    ...
    auto search = m_allClipItems.find(binId.toInt());
    if (search != m_allClipItems.end()) {
        return search->second->getAudioMax(stream) / WunjoSettings::waveformScaler();
    }
...
bool ProjectItemModel::addItem(const std::shared_ptr<AbstractProjectItem> &item, const QString &parentId, Fun &undo, Fun &redo)
{
    ...
    if (!parentItem) {
        qCDebug(WUNJO_LOG) << "  / / ERROR IN PARENT FOLDER";
        return false;
    }
    if (item->itemType() == AbstractProjectItem::ClipItem && parentItem->itemType() != AbstractProjectItem::FolderItem) {
        qCDebug(WUNJO_LOG) << "  / / ERROR when inserting clip: a clip should be inserted in a folder";
        return false;
    }
    if (item->itemType() == AbstractProjectItem::SubClipItem && parentItem->itemType() != AbstractProjectItem::ClipItem) {
        qCDebug(WUNJO_LOG) << "  / / ERROR when inserting subclip: a subclip should be inserted in a clip";
        return false;
    }
...
{
    if (id.isEmpty()) {
        id = Xml::getXmlProperty(description, QStringLiteral("wunjo:id"), QStringLiteral("-1"));
    ...
    if (description.hasAttribute(QStringLiteral("type"))) {
        ...
        if (resource.endsWith(QLatin1String(".wmproj")) || resource.endsWith(QLatin1String(".mlt"))) {
    ...
    if (res) {
        ClipLoadTask::start(ObjectId(WunjoObjectType::BinClip, id.toInt(), QUuid()), description, false, -1, -1, this, false, std::bind(readyCallBack, id));
    }
    return res;
}
                                         ...
                                         const std::function<void(const QString &)> &readyCallBack)
{
    QWriteLocker locker(&m_lock);
    if (id.isEmpty()) {
        if (producer->property_exists("wunjo:id")) {
            id = QString::number(producer->get_int("wunjo:id"));
        }
        if (!isIdFree(id)) {
            id = QString::number(getFreeClipId());
        }
    ...
    if (producerService == QLatin1String("tractor") || producerService == QLatin1String("xml-string")) {
        ...
        if (props.property_exists("wunjo:producer_type")) {
            type = (ClipType::ProducerType)props.get_int("wunjo:producer_type");
        } else {
                                            ...
                                            Fun &redo)
{
    ...
    const QString tc = pCore->currentDoc()->timecode().getDisplayTimecodeFromFrames(in, WunjoSettings::frametimecode());
                                                    ...
                                                    Fun &redo)
{
    ...
    auto clip = getClipByBinID(subId);
    Q_ASSERT(clip->itemType() == AbstractProjectItem::ClipItem);
    const QString tc = pCore->currentDoc()->timecode().getDisplayTimecodeFromFrames(duration, WunjoSettings::frametimecode());
                                                       ...
                                                       QStringList &expandedFolders, QStringList &extraBins, const QUuid &activeUuid, int &zoomLevel)
{
    ...
    if (retainList.is_valid()) {
        ...
        if (playlist.is_valid() && playlist.type() == mlt_service_playlist_type) {
            ...
            expandedFolders = QString(playlistProps.get("wunjo:expandedFolders")).split(QLatin1Char(';'));
            extraBins = QString(playlistProps.get("wunjo:extraBins")).split(QLatin1Char(';'));
            folderProperties.pass_values(playlistProps, "wunjo:folder.");
            loadFolders(folderProperties, binIdCorresp);
            m_sequenceFolderId = -1;
            if (playlistProps.property_exists("wunjo:sequenceFolder")) {
                int sequenceFolder = playlistProps.get_int("wunjo:sequenceFolder");
            ...
            if (playlistProps.property_exists("wunjo:audioCaptureFolder")) {
                int audioCaptureFolder = playlistProps.get_int("wunjo:audioCaptureFolder");
            ...
            // Load Zoom level
            if (playlistProps.property_exists("wunjo:binZoom")) {
                zoomLevel = playlistProps.get_int("wunjo:binZoom");
            }

            // Read notes
            QString notes = playlistProps.get("wunjo:documentnotes");
            int version = playlistProps.get_int("wunjo:documentnotesversion");
            ...
            for (int i = 0; i < max; i++) {
                ...
                if (prod->is_blank() || !prod->is_valid() || prod->parent().property_exists("wunjo:remove")) {
                    qDebug() << "==== IGNORING BIN PRODUCER: " << prod->parent().get("wunjo:id");
                    continue;
                }
                std::shared_ptr<Mlt::Producer> producer;
                if (prod->parent().property_exists("wunjo:uuid")) {
                    const QUuid uuid(prod->parent().get("wunjo:uuid"));
                    ...
                    if (prod->parent().type() == mlt_service_tractor_type) {
                        ...
                        sequenceProps.pass_values(prod->parent(), "wunjo:sequenceproperties.");
                        if (pCore->closing) {
                            return {};
                        }
                        ...
                        int id(prod->parent().get_int("wunjo:id"));
                        trac->set("wunjo:id", id);
                        trac->set("wunjo:uuid", uuid.toString().toUtf8().constData());
                        trac->set("wunjo:producer_type", ClipType::Timeline);
                        trac->set("_wunjo_processed", 1);

                        std::shared_ptr<Mlt::Producer> prod2(trac->cut());
                        prod2->set("wunjo:id", id);
                        prod2->set("wunjo:uuid", uuid.toString().toUtf8().constData());
                        prod2->set("wunjo:producer_type", ClipType::Timeline);
                        prod2->set("_wunjo_processed", 1);
                        prod2->pass_list(prod->parent(), "length,out,wunjo:control_uuid,wunjo:clipname,wunjo:description,wunjo:folderid,wunjo:"
                                                         "duration,wunjo:maxduration,wunjo:zone_in,wunjo:zone_out,wunjo:clipzones");

                        if (foundIds.contains(id)) {
                            qWarning() << "ERROR, several Sequence Clips using the same id: " << id << ", UUID: " << uuid.toString()
                                       << "\n____________________";
                            brokenSequences.insert(uuid, QString(prod->parent().get("wunjo:control_uuid")));
                        }
                        foundIds << id;
                        ...
                        if (resource.endsWith(QLatin1String("<tractor>"))) {
                            ...
                            brokenSequences.insert(QUuid(uuid), QString(prod->parent().get("wunjo:control_uuid")));
                        }
                    }
                    continue;
                }
                producer.reset(new Mlt::Producer(prod->parent()));
                int id = producer->get_int("wunjo:id");
                if (!id) {
            ...
            if (!foundSequences.contains(activeUuid)) {
                ...
                const QUuid uuid(producer->get("wunjo:uuid"));
            ...
            while (!binProducers.isEmpty()) {
                ...
                QString parentId = qstrdup(prod->get("wunjo:folderid"));
                if (parentId.isEmpty()) {
                ...
                prod->set("_wunjo_processed", 1);
                const QString uuid(prod->get("wunjo:control_uuid"));
    ...
    for (int i = 1; i < documentTractor.count(); i++) {
        ...
        for (int j = 0; j < tractor.count(); j++) {
            ...
            for (int k = 0; k < trackPlaylist.count(); k++) {
                ...
                if (clip->parent().property_exists("wunjo:id")) {
                    int cid = clip->parent().get_int("wunjo:id");
                    ...
                    const QString hash = clip->parent().get("wunjo:file_hash");
                    ClipType::ProducerType matchType;
                    if (service.startsWith(QLatin1String("avformat"))) {
                        int cType = clip->parent().get_int("wunjo:clip_type");
                        switch (cType) {
                        case 1:
                    ...
                    bool found = false;
                    const QString uuid = clip->parent().get("wunjo:control_uuid");
                        ...
                        for (const auto &clipLoop : m_allItems) {
                            ...
                            if (c->itemType() == AbstractProjectItem::ClipItem) {
                                if (c->clipType() == matchType || matchType == ClipType::Unknown) {
                                    ...
                                    if (pClip->getProducerProperty(QStringLiteral("wunjo:file_hash")) == hash) {
                                        // Found a match
                                        // binIdCorresp[QString::number(cid)] = pClip->clipId();
                                        clip->parent().set("wunjo:uuid", pClip->getProducerProperty(QStringLiteral("wunjo:uuid")).toUtf8().constData());
                                        found = true;
                                        break;
                                    }
                                                              ...
                                                              bool timelineProducerOnly, const QString &aspectRatio)
{
    ...
    if (!aspectRatio.isEmpty()) {
        tempFile.setFileTemplate(QDir::temp().absoluteFilePath(QStringLiteral("wunjo-XXXXXX.wmproj")));
        if (!tempFile.open()) {
    ...
    Mlt::Consumer xmlConsumer(pCore->getProjectProfile(), "xml", aspectRatio.isEmpty() ? "wunjo_playlist" : tempFile.fileName().toUtf8().constData());
    if (!root.isEmpty()) {
    ...
    // Add active timeline as playlist of the main tractor so that when played through melt, the .wmproj file reads the playlist
    ...
    if (aspectRatio.isEmpty()) {
        playlist = QString::fromUtf8(xmlConsumer.get("wunjo_playlist"));
    ...
    Mlt::Consumer xmlConsumer2(newProfile, "xml", "wunjo_playlist");
    ...
    playlist = QString::fromUtf8(xmlConsumer2.get("wunjo_playlist"));
...
void ProjectItemModel::setSequencesFolder(int id)
{
    ...
    saveProperty(QStringLiteral("wunjo:sequenceFolder"), QString::number(id));
    if (id > -1) {
...
void ProjectItemModel::setAudioCaptureFolder(int id)
{
    ...
    saveProperty(QStringLiteral("wunjo:audioCaptureFolder"), QString::number(id));
}
