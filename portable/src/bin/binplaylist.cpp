...
void BinPlaylist::manageBinItemInsertion(const std::shared_ptr<AbstractProjectItem> &binElem)
{
    ...
    case AbstractProjectItem::FolderItem: {
        ...
        if (binElem->parent()) {
            QString propertyName = "wunjo:folder." + binElem->parent()->clipId() + QLatin1Char('.') + id;
    ...
    case AbstractProjectItem::ClipItem: {
        ...
        if (clip->isValid()) {
            ...
            dummy.set("wunjo:id", id.toUtf8().constData());
...
void BinPlaylist::manageBinItemDeletion(AbstractProjectItem *binElem)
{
    ...
    case AbstractProjectItem::FolderItem: {
        ...
        if (!binElem->lastParentId().isEmpty()) {
            QString propertyName = "wunjo:folder." + binElem->lastParentId() + QLatin1Char('.') + binElem->clipId();
...
void BinPlaylist::removeBinClip(const QString &id)
{
    ...
    for (int i = 0; !ok && i < size; i++) {
        ...
        QString prodId(prod->parent().get("wunjo:id"));
        if (prodId == id) {
...
void BinPlaylist::saveDocumentProperties(const QMap<QString, QString> &props, const QMap<QString, QString> &metadata)
{
    ...
    docProperties.pass_values(playlistProps, "wunjo:docproperties.");
    for (int i = 0; i < docProperties.count(); i++) {
        QString propName = QStringLiteral("wunjo:docproperties.%1").arg(docProperties.get_name(i));
        playlistProps.set(propName.toUtf8().constData(), nullptr);
    }

    // Clear previous metadata
    Mlt::Properties docMetadata;
    docMetadata.pass_values(playlistProps, "wunjo:docmetadata.");
    for (int i = 0; i < docMetadata.count(); i++) {
        QString propName = QStringLiteral("wunjo:docmetadata.%1").arg(docMetadata.get_name(i));
        playlistProps.set(propName.toUtf8().constData(), nullptr);
    }
    QMapIterator<QString, QString> i(props);
    while (i.hasNext()) {
        i.next();
        playlistProps.set(("wunjo:docproperties." + i.key()).toUtf8().constData(), i.value().toUtf8().constData());
    }

    QMapIterator<QString, QString> j(metadata);
    while (j.hasNext()) {
        j.next();
        playlistProps.set(("wunjo:docmetadata." + j.key()).toUtf8().constData(), j.value().toUtf8().constData());
    }
}
...
QMap<QString, QString> BinPlaylist::getProxies(const QString &root)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        QString proxy = prod->parent().get("wunjo:proxy");
        if (proxy.length() > 2) {
            if (QFileInfo(proxy).isRelative()) {
                proxy.prepend(root);
            }
            QString sourceUrl(prod->parent().get("wunjo:originalurl"));
