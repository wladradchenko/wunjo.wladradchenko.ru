...
/**
 * @brief Draws the placeholder text when user has not yet imported any media
 */
static void drawDropFilesPlaceholder(QPainter &painter, const QRect &rect, const QPalette &palette)
{
    ...
    const QString text = i18n("Double click or drop files here to import media");
    const int margin = painter.fontMetrics().height();
    painter.drawText(rect.adjusted(margin, margin, -margin, -margin), Qt::AlignCenter | Qt::TextWordWrap, text);
}
            ...
            if (type == AbstractProjectItem::ClipItem || type == AbstractProjectItem::SubClipItem || type == AbstractProjectItem::SubSequenceItem) {
                ...
                if (opt.decorationSize.height() > 0) {
                    ...
                    if (!pix.isNull()) {
                        ...
                        // thumbs wider than the reserved (project DAR) width must
                        // push the text away instead of having it drawn on top
                        decoWidth = qMax(decoWidth, r.width() + textMargin);
                ...
                if (!subText.isEmpty()) {
                    ...
                    if (hasAudioAndVideo && (cType == ClipType::AV || cType == ClipType::Playlist || cType == ClipType::Timeline)) {
                        ...
                        if (opt.state & QStyle::State_MouseOver) {
                            ...
                            painter->drawRoundedRect(m_audioDragRect, 3, 3);
                            painter->drawRoundedRect(m_videoDragRect, 3, 3);
                            painter->restore();
...
Bin::Bin(std::shared_ptr<ProjectItemModel> model, QWidget *parent, bool isMainBin, BinViewType type)
    ...
    , m_listType(type == BinUnknownView ? BinViewType(WunjoSettings::binMode()) : type)
    ...
    , m_processedAudio(0)
{
    ...
    m_slider->setValue(WunjoSettings::bin_zoom());
    ...
    m_sortDescend->setChecked(WunjoSettings::binSorting() > 99);
    connect(m_sortDescend, &QAction::triggered, this, [&]() {
        if (m_sortGroup->checkedAction()) {
            ...
            WunjoSettings::setBinSorting(actionData + (m_sortDescend->isChecked() ? 100 : 0));
        }
    });

    auto *sortAction = new KActionMenu(i18n("Sort By"), this);
    int binSort = WunjoSettings::binSorting() % 100;
    ...
    hoverPreview->setChecked(WunjoSettings::hoverPreview());
    connect(hoverPreview, &QAction::triggered, [](bool checked) { WunjoSettings::setHoverPreview(checked); });
    ...
    if (!m_isMainBin) {
        ...
        connect(m_discardCurrentClipJobs, &QAction::triggered, this, [&]() {
            const QString currentId = pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId();
            if (!currentId.isEmpty()) {
                pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, currentId.toInt(), QUuid()), AbstractTask::NOJOBTYPE, true);
            }
        });
    ...
    // Quick type-filter row (segmented All / Video / Image / Audio). Unlike the
    // funnel "Filter by Type" submenu, folders stay visible so the tree is preserved.
    auto *quickFilter = new QWidget(this);
    quickFilter->setObjectName(QStringLiteral("binQuickFilter"));
    auto *qfLayout = new QHBoxLayout(quickFilter);
    qfLayout->setContentsMargins(4, 2, 4, 2);
    qfLayout->setSpacing(4);
    auto *qfGroup = new QButtonGroup(quickFilter);
    qfGroup->setExclusive(true);
    const QVector<QPair<QString, QList<int>>> quickSegments = {
        {i18n("All"), {}},
        {i18n("Video"), {ClipType::AV, ClipType::Video, ClipType::Playlist, ClipType::Timeline, ClipType::SlideShow, ClipType::Animation}},
        {i18n("Image"), {ClipType::Image}},
        {i18n("Audio"), {ClipType::Audio}},
    };
    bool firstSegment = true;
    for (const auto &seg : quickSegments) {
        auto *segBtn = new QToolButton(quickFilter);
        segBtn->setText(seg.first);
        segBtn->setToolButtonStyle(Qt::ToolButtonTextOnly);
        segBtn->setCheckable(true);
        segBtn->setFocusPolicy(Qt::NoFocus);
        segBtn->setCursor(Qt::PointingHandCursor);
        segBtn->setFont(QFontDatabase::systemFont(QFontDatabase::SmallestReadableFont));
        segBtn->setChecked(firstSegment);
        firstSegment = false;
        const QList<int> segTypes = seg.second;
        connect(segBtn, &QToolButton::clicked, this, [this, segTypes]() { m_proxyModel->slotSetTypeFilters(segTypes); });
        qfGroup->addButton(segBtn);
        qfLayout->addWidget(segBtn);
    }
    qfLayout->addStretch();
    m_layout->insertWidget(1, quickFilter);

    // connect(pCore->projectManager(), SIGNAL(projectOpened(Project*)), this, SLOT(setProject(Project*)));
    m_headerInfo = QByteArray::fromBase64(WunjoSettings::treeviewheaders().toLatin1());
    if (m_isMainBin) {
...
void Bin::slotUpdatePalette()
{
    if (m_isMainBin) {
        ...
        QIcon videoIcon = QIcon::fromTheme(QStringLiteral("wunjo-show-video"));
...
bool Bin::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (event->type() == QEvent::MouseButtonRelease) {
        Monitor *monitor = pCore->getMonitor(Wunjo::ClipMonitor);
        if (!monitor->isActive()) {
...
void Bin::slotSaveHeaders()
{
    if ((m_itemView != nullptr) && m_listType == BinTreeView) {
        ...
        WunjoSettings::setTreeviewheaders(m_headerInfo.toBase64());
    }
}

void Bin::updateSortingAction(int ix)
{
    if (ix == WunjoSettings::binSorting()) {
        return;
    }
    int index = ix % 100;
...
void Bin::replaceSingleClip(const QString clipId, const QString &newUrl)
{
    ...
    if (currentItem) {
        ...
        sourceProps.insert(QStringLiteral("wunjo:originalurl"), currentItem->url());
        sourceProps.insert(QStringLiteral("wunjo:clipname"), currentItem->clipName());
        sourceProps.insert(QStringLiteral("wunjo:proxy"), currentItem->getProducerProperty(QStringLiteral("wunjo:proxy")));
        sourceProps.insert(QStringLiteral("_fullreload"), QStringLiteral("1"));
        newProps.insert(QStringLiteral("resource"), newUrl);
        newProps.insert(QStringLiteral("wunjo:originalurl"), newUrl);
        newProps.insert(QStringLiteral("wunjo:clipname"), QFileInfo(newUrl).fileName());
    ...
    for (const QModelIndex &ix : indexes) {
        ...
        if (currentItem) {
            ...
            if (!fileName.isEmpty()) {
                ...
                newProps.insert(QStringLiteral("wunjo:originalurl"), fileName);
                newProps.insert(QStringLiteral("wunjo:clipname"), QFileInfo(fileName).fileName());
                newProps.insert(QStringLiteral("wunjo:proxy"), QStringLiteral("-"));
                newProps.insert(QStringLiteral("_fullreload"), QStringLiteral("1"));
                // Check if replacement clip is long enough
                if (currentItem->hasLimitedDuration() && currentItem->isIncludedInTimeline()) {
                    ...
                    if (replacementProd->is_valid()) {
                        ...
                        if (service == QLatin1String("xml")) {
                            // Playlist or .wmproj project file, get frame to check for AV
    ...
    for (const QModelIndex &ix : indexes) {
        ...
        if (currentItem) {
            ...
            if (currentItem->hasUrl() && exists) {
                ...
                qCDebug(WUNJO_LOG) << "  / / " + url.toString();
            } else {
                if (!exists) {
        ...
        if (item->itemType() == AbstractProjectItem::ClipItem) {
            ...
            if (currentItem) {
                if (currentItem->clipType() == ClipType::Timeline) {
                    ...
                    Fun undo = []() { return true; };
                    Fun redo = []() { return true; };
                    xmlProd->set("wunjo:clipname", i18n("%1 (copy)", currentItem->clipName()).toUtf8().constData());
                    xmlProd->set("wunjo:sequenceproperties.documentuuid", m_doc->uuid().toString().toUtf8().constData());
                    Mlt::Properties props(xmlProd->get_properties());
                    props.clear("wunjo:control_uuid");
                    ...
                    if (!xml.isNull()) {
                        QString currentName = Xml::getXmlProperty(xml, QStringLiteral("wunjo:clipname"));
                        ...
                        if (!currentName.isEmpty()) {
                            ...
                            Xml::setXmlProperty(xml, QStringLiteral("wunjo:clipname"), currentName);
                        }
                        if (currentItem->clipType() == ClipType::Text) {
                            // Remove unique id
                            Xml::removeXmlProperty(xml, QStringLiteral("wunjo:uniqueId"));
                        }
                        Xml::removeXmlProperty(xml, QStringLiteral("wunjo:control_uuid"));
...
const QString Bin::setDocument(WunjoDoc *project, const QString &id)
{
    if (m_doc) {
...
void Bin::rebuildFilters(int tagsCount)
{
    ...
    typeFilter->setCheckable(true);
    typeMenu->addAction(typeFilter);
    typeFilter = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-add-slide-clip")), i18n("Slideshow"), &m_filterTypeGroup);
    ...
    typeFilter->setCheckable(true);
    typeMenu->addAction(typeFilter);
    typeFilter = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-add-color-clip")), i18n("Color"), &m_filterTypeGroup);
...
void Bin::createClip(const QDomElement &xml)
{
    ...
    QString groupId = ProjectClip::getXmlProperty(xml, QStringLiteral("wunjo:folderid"));
    ...
    if (path.endsWith(QStringLiteral(".mlt")) || path.endsWith(QStringLiteral(".wmproj"))) {
        QDomDocument doc;
        ...
        if (!d.hasErrorInProject() && doc.documentElement().hasAttribute(QStringLiteral("modified"))) {
            ...
            if (copyjob->exec()) {
                ...
                if (!f.open(QIODevice::WriteOnly | QIODevice::Text)) {
                    ...
                    f.close();
                    KMessageBox::information(
                        this, i18n("Your project file was modified by Wunjo.\nTo make sure you do not lose data, a backup copy called %1 was created.",
                                   backupFile));
                }
            }
        }
    }
    QString id = Xml::getXmlProperty(xml, QStringLiteral("wunjo:id"));
    if (id.isEmpty()) {
...
{
    if (pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId() != clipId) {
...
void Bin::selectProxyModel(const QModelIndex &id)
{
    ...
    if (id.isValid()) {
        ...
        if (currentItem) {
            if (pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId(true) == currentItem->clipId(true)) {
...
void Bin::slotInitView(QAction *action)
{
    ...
    if (action) {
        ...
        if (m_isMainBin) {
            WunjoSettings::setBinMode(viewType);
        }
    ...
    connect(m_itemView, &QAbstractItemView::clicked, this, [this](const QModelIndex &ix) {
        // Clicking media brings the preview tab forward (the effect stack may share its tab group)
        std::shared_ptr<AbstractProjectItem> item = m_itemModel->getBinItemByIndex(m_proxyModel->mapToSource(ix));
        if (item && item->itemType() != AbstractProjectItem::FolderItem) {
            pCore->window()->raiseMonitor(true);
        }
    });
    ...
    if (m_listType == BinTreeView) {
        ...
        connect(view->header(), &QHeaderView::sortIndicatorChanged, this, [this](int ix, Qt::SortOrder order) {
            ...
            WunjoSettings::setBinSorting(ix + (order == Qt::DescendingOrder ? 100 : 0));
        });
...
void Bin::slotSetIconSize(int size)
{
    ...
    WunjoSettings::setBin_zoom(size);
...
void Bin::setupAddClipAction(QMenu *addClipMenu, ClipType::ProducerType type, const QString &name, const QString &text, const QIcon &icon)
{
    ...
    if (name == QLatin1String("add_animation_clip") && !WunjoSettings::producerslist().contains(QLatin1String("glaxnimate"))) {
        action->setEnabled(false);
    } else if (name == QLatin1String("add_text_clip") && !WunjoSettings::producerslist().contains(QLatin1String("kdenlivetitle"))) {
        action->setEnabled(false);
    }
}
...
void Bin::showClipProperties(const std::shared_ptr<ProjectClip> &clip, bool forceRefresh)
{
    ...
    connect(panel, &ClipPropertiesController::seekToFrame, pCore->getMonitor(Wunjo::ClipMonitor), static_cast<void (Monitor::*)(int)>(&Monitor::slotSeek));
...
{
    if (pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId() == id) {
        pCore->getMonitor(Wunjo::ClipMonitor)->reloadActiveStream();
    }
}
...
void Bin::updateTargets(QString id)
{
    if (id == QLatin1String("-1")) {
        id = pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId();
    } else if (pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId() != id) {
        qDebug() << "ABOIRT A";
        return;
    }
    std::shared_ptr<ProjectClip> clip = m_itemModel->getClipByBinID(id);
...
{
    if (!m_menu) {
        qCDebug(WUNJO_LOG) << "Warning, menu was not created, something is wrong";
        return;
    }
...
{
    if (pCore->getMonitor(Wunjo::ClipMonitor)->slotOpenClip(clip, in, out, uuid) && clip) {
...
void Bin::setupMenu()
{
    ...
    m_addClip =
        addBinAction(QStringLiteral("add_clip"), i18n("Add Clip or Folder…"), QIcon::fromTheme(QStringLiteral("wunjo-add-clip")), QStringLiteral("addclip"));
...
void Bin::refreshClip(const QString &id)
{
    if (pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId() == id) {
        if (pCore->monitorManager()->clipMonitorVisible()) {
            pCore->getMonitor(Wunjo::ClipMonitor)->refreshMonitor(true);
        }
    }
}
...
void Bin::slotCreateProjectClip()
{
    ...
    if (act == nullptr) {
        ...
        qCDebug(WUNJO_LOG) << "// Error in clip creation action";
        return;
    }
...
void Bin::buildSequenceClip(int aTracks, int vTracks)
{
    ...
    dia_ui.video_tracks->setValue(vTracks == -1 ? WunjoSettings::videotracks() : vTracks);
    dia_ui.audio_tracks->setValue(aTracks == -1 ? WunjoSettings::audiotracks() : aTracks);
...
void Bin::slotTagDropped(const QString &tag, const QModelIndex &parent)
{
    if (parent.isValid()) {
        ...
        if (parentItem->itemType() == AbstractProjectItem::ClipItem || parentItem->itemType() == AbstractProjectItem::SubClipItem) {
            ...
            oldProps.insert(QStringLiteral("wunjo:tags"), currentTag);
            QMap<QString, QString> newProps;
            ...
            newProps.insert(QStringLiteral("wunjo:tags"), currentTag);
            ...
            Fun update_tags_redo = [this, clips, t]() {
                for (auto &cid : clips) {
                    ...
                    if (clip) {
                        ...
                        props.insert(QStringLiteral("wunjo:tags"), tagsList.join(QLatin1Char(';')));
    ...
    for (const QString &id : allClips) {
        ...
        if (clip) {
            ...
            if (add) {
                ...
                tags.removeAll(tag);
                newProps.insert(QStringLiteral("wunjo:tags"), tags.join(QLatin1Char(';')));
            }
...
{
    Q_ASSERT(owner.type == WunjoObjectType::BinClip);
...
void Bin::editMasterEffect(const std::shared_ptr<AbstractProjectItem> &clip)
{
    ...
    if (clip) {
        if (pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId() != clip->clipId()) {
            setCurrent(clip);
        }
...
{
    const QString clipId = pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId();
...
void Bin::slotExpandUrl(/*const ItemInfo &info,*/ const QString &url, QUndoCommand *command)
{
    ...
    for (int i = 0; i < producers.count(); i++) {
        ...
        // Be careful as to the wunjo:file_hash! It is not unique for
        ...
        if (mltService == QLatin1String("pixbuf") || mltService == QLatin1String("qimage") || mltService == QLatin1String("kdenlivetitle") ||
            ...
            hash = mltService + QLatin1Char(':') + Xml::getXmlProperty(prod, QStringLiteral("wunjo:clipname")) + QLatin1Char(':') +
                   Xml::getXmlProperty(prod, QStringLiteral("wunjo:folderid")) + QLatin1Char(':');
            ...
            if (singletonId.length() != 0) {
                ...
                qCDebug(WUNJO_LOG) << "found duplicate producer:" << hash << ", reusing newID:" << singletonId;
        ...
        qCDebug(WUNJO_LOG) << "originalId: " << originalId << ", newId: " << newId;
        ...
        EffectsList::setProperty(clone, QStringLiteral("wunjo:folderid"), folderId);
        ...
        if (mltService == QLatin1String("avformat-novalidate") // av clip
            ...
            if (QFileInfo(resource).isRelative()) {
                ...
                qCDebug(WUNJO_LOG) << "fixed resource path for producer, newId:" << newId << "resource:" << rootedResource.absoluteFilePath();
...
void Bin::slotOpenClipExtern()
{
    ...
    case ClipType::TextTemplate:
        ...
        break;
    case ClipType::Image: {
        if (pCore->packageType() != LinuxPackageType::Flatpak && WunjoSettings::defaultimageapp().isEmpty()) {
            QUrl url = KUrlRequesterDialog::getUrl(QUrl(), this, i18n("Enter path for your image editing application"));
            if (!url.isEmpty()) {
                WunjoSettings::setDefaultimageapp(url.toLocalFile());
        ...
        if (pCore->packageType() == LinuxPackageType::Flatpak || !WunjoSettings::defaultimageapp().isEmpty()) {
            errorString = pCore->openExternalApp(WunjoSettings::defaultimageapp(), {clip->url()}, ClipType::Image);
        } else {
            KMessageBox::error(QApplication::activeWindow(), i18n("Please set a default application to open image files"));
        }
    } break;
    case ClipType::Audio: {
        if (pCore->packageType() != LinuxPackageType::Flatpak && WunjoSettings::defaultaudioapp().isEmpty()) {
            QUrl url = KUrlRequesterDialog::getUrl(QUrl(), this, i18n("Enter path for your audio editing application"));
            if (!url.isEmpty()) {
                WunjoSettings::setDefaultaudioapp(url.toLocalFile());
                WunjoSettingsDialog *d = static_cast<WunjoSettingsDialog *>(KConfigDialog::exists(QStringLiteral("settings")));
                if (d) {
                    d->updateExternalApps();
                }
        ...
        if (pCore->packageType() == LinuxPackageType::Flatpak || !WunjoSettings::defaultaudioapp().isEmpty()) {
            errorString = pCore->openExternalApp(WunjoSettings::defaultaudioapp(), {clip->url()}, ClipType::Audio);
        } else {
    ...
    case ClipType::Video: {
        ...
        if (!WunjoSettings::defaultvideoapp().isEmpty()) {
            errorString = pCore->openExternalApp(WunjoSettings::defaultvideoapp(), {clip->url()}, ClipType::Video);
        } else {
    ...
    if (res == QDialog::Accepted) {
        ...
        if (dia_ui.duration() != clip->duration().frames(pCore->getCurrentFps())) {
            ...
            if (currentLength != dia_ui.duration()) {
                newprops.insert(QStringLiteral("wunjo:duration"), clip->framesToTime(dia_ui.duration()));
            }
        }
        if (clip->clipName().contains(i18n("(copy)"))) {
            // We edited a duplicated title clip, update name from new content text
            newprops.insert(QStringLiteral("wunjo:clipname"), dia_ui.titleSuggest());
            if (!path.isEmpty()) {
...
void Bin::showSlideshowWidget(const std::shared_ptr<ProjectClip> &clip)
{
    ...
    qCDebug(WUNJO_LOG) << " ** * CLIP ABS PATH: " << clip->url() << " = " << folder;
    ...
    if (dia->exec() == QDialog::Accepted) {
        ...
        properties.insert(QStringLiteral("wunjo:duration"), clip->framesToTime(m_doc->getFramePos(dia->clipDuration()) * dia->imageCount()));
        properties.insert(QStringLiteral("wunjo:clipname"), dia->clipName());
        ...
        oldProperties.insert(QStringLiteral("wunjo:duration"), clip->getProducerProperty(QStringLiteral("wunjo:duration")));
        oldProperties.insert(QStringLiteral("wunjo:clipname"), clip->name());
...
void Bin::reloadAllProducers(bool reloadThumbs)
{
    ...
    for (const std::shared_ptr<ProjectClip> &clip : std::as_const(clipList)) {
        ...
        if (type == ClipType::AV || type == ClipType::Video || type == ClipType::Audio || type == ClipType::Playlist) {
            ...
            Xml::removeXmlProperty(xml, QStringLiteral("wunjo:duration"));
        }
        if (clip->isValid()) {
            clip->resetProducerProperty(QStringLiteral("wunjo:duration"));
            if (clip->hasLimitedDuration()) {
        ...
        if (!xml.isNull()) {
            ...
            clip->setClipStatus(FileStatus::StatusWaiting);
            ObjectId oid(WunjoObjectType::BinClip, clip->clipId().toInt(), QUuid());
    ...
    for (auto &u : allSequences) {
        ...
        ClipLoadTask::start(ObjectId(WunjoObjectType::BinClip, seq->clipId().toInt(), QUuid()), QDomElement(), true, -1, -1, this);
    }
}
...
void Bin::checkAudioThumbs()
{
    ...
    if (!WunjoSettings::audiothumbnails()) {
    ...
    for (const auto &clip : std::as_const(clipList)) {
        ...
        if (type == ClipType::AV || type == ClipType::Audio || type == ClipType::Playlist || type == ClipType::Unknown) {
            AudioLevelsTask::start(ObjectId(WunjoObjectType::BinClip, clip->clipId().toInt(), QUuid()), clip.get(), false);
        }
    }
}
...
void Bin::getBinStats(uint *used, uint *unused, qint64 *usedSize, qint64 *unusedSize)
{
    ...
    for (const std::shared_ptr<ProjectClip> &clip : std::as_const(clipList)) {
        ...
        if (clip->refCount() == 0) {
            *unused += 1;
            *unusedSize += clip->getProducerInt64Property(QStringLiteral("wunjo:file_size"));
        } else {
            *used += 1;
            *usedSize += clip->getProducerInt64Property(QStringLiteral("wunjo:file_size"));
        }
    }
}
...
void Bin::rebuildProxies()
{
    ...
    for (const std::shared_ptr<ProjectClip> &clp : std::as_const(clipList)) {
        if (clp->hasProxy()) {
            ...
            pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, clp->clipId().toInt(), QUuid()), AbstractTask::PROXYJOB);
...
void Bin::checkProfile(const std::shared_ptr<Mlt::Producer> &producer)
{
    ...
    // A still is not what a project is shaped by. Dropping a portrait photo into
    // a project used to turn the whole project portrait — the monitor, the
    // render format, every clip that came after — because the first clip
    // decides the format and nothing said that a photograph is not that kind of
    // first clip. It has no frame rate to give either. Footage still sets the
    // format; a picture is fitted into whatever the project already is.
    ...
    if (service.contains(QStringLiteral("avformat"))) {
        ...
        if (*clipProfile.get() == *projectProfile.get()) {
            if (WunjoSettings::default_profile().isEmpty()) {
                ...
                WunjoSettings::setDefault_profile(pCore->getCurrentProfile()->path());
            }
        } else {
    ...
    if (clip && clip->clipType() != ClipType::Audio) {
        ...
        while (i.hasNext()) {
            ...
            for (int j : std::as_const(values)) {
                pCore->invalidateItem(ObjectId(WunjoObjectType::TimelineClip, j, i.key()));
            }
        }
    }
}
...
void Bin::invalidateClipAudio(const QString &binId)
{
    ...
    if (clip->hasAudio()) {
        ...
        while (i.hasNext()) {
            ...
            QList<int> values = i.value();
            for (int j : std::as_const(values)) {
                pCore->invalidateAudio(ObjectId(WunjoObjectType::TimelineClip, j, i.key()));
            }
        }
    }
}
...
void Bin::rebuildAudioThumb(const QString &binId)
{
    ...
    if (!clip->audioSynced()) {
        ...
        AudioLevelsTask::start(ObjectId(WunjoObjectType::BinClip, binId.toInt(), QUuid()), clip.get(), false);
    }
}
...
void Bin::checkProjectAudioTracks(QString clipId, int minimumTracksCount)
{
    ...
    if (requestedTracks > 0) {
        if (clipId.isEmpty()) {
            clipId = pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId();
        }
        QList<QAction *> list;
        ...
        connect(ac3, &QAction::triggered, [&]() { WunjoSettings::setMultistream_checktrack(false); });
...
void Bin::addClipMarker(const QString &binId, const QMap<int, QString> &markersData)
{
    ...
    clip->getMarkerModel()->addMarkers(markers, WunjoSettings::default_marker_type());
    if (WunjoSettings::guidesShowThumbs()) {
        CacheTask::start(ObjectId(WunjoObjectType::BinClip, binId.toInt(), QUuid()), missingFrames, pCore->guidesList());
    }
}
...
void Bin::saveFolderState()
{
    ...
    m_itemModel->saveProperty(QStringLiteral("wunjo:expandedFolders"), expandedFolders.join(QLatin1Char(';')));
    m_itemModel->saveProperty(QStringLiteral("wunjo:binZoom"), QString::number(WunjoSettings::bin_zoom()));
    m_itemModel->saveProperty(QStringLiteral("wunjo:extraBins"), pCore->window()->extraBinIds().join(QLatin1Char(';')));
}
...
void Bin::savePlaylist(const QString &binId, const QString &savePath, const QVector<QPoint> &zones, const QMap<QString, QString> &properties, bool createNew)
{
    ...
    // Here we could store some wunjo settings in the main playlist
...
void Bin::requestSelectionTranscoding(bool forceReplace)
{
    if (m_transcodingDialog == nullptr) {
        ...
        connect(m_transcodingDialog, &QDialog::accepted, this, [&]() {
            ...
            if (!forceReplace) {
                WunjoSettings::setTranscodingReplace(replace);
            }
            ...
            while (i.hasNext()) {
                ...
                ObjectId oid(WunjoObjectType::BinClip, i.key().toInt(), QUuid());
                if (clip->clipType() == ClipType::Timeline) {
                    ...
                    if (replace) {
                        ...
                        pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, i.key().toInt(), QUuid()), AbstractTask::AUDIOTHUMBJOB, true);
                        pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, i.key().toInt(), QUuid()), AbstractTask::PROXYJOB, true);
                    }
                    TranscodeTask::start(oid, i.value().first(), m_transcodingDialog->preParams(), m_transcodingDialog->params(i.key()),
        ...
        connect(m_transcodingDialog, &QDialog::accepted, this, [&, checkProfile]() {
            ...
            if (!ids.isEmpty()) {
                ...
                while (i.hasNext()) {
                    ...
                    std::shared_ptr<ProjectClip> clip = m_itemModel->getClipByBinID(i.key());
                    if (clip == nullptr) {
                        TranscodeTask::start(ObjectId(WunjoObjectType::NoItem, i.key().toInt(), QUuid()), i.value().first(),
                    ...
                    TranscodeTask::start(ObjectId(WunjoObjectType::BinClip, i.key().toInt(), QUuid()), i.value().first(), m_transcodingDialog->preParams(),
        ...
        connect(m_transcodingDialog, &QDialog::rejected, this, [&, checkProfile]() {
            ...
            if (!ids.isEmpty()) {
                ...
                while (i.hasNext()) {
                    ...
                    if (WunjoSettings::audiothumbnails() && (cType == ClipType::AV || cType == ClipType::Audio || clip->hasAudio())) {
                        AudioLevelsTask::start(ObjectId(WunjoObjectType::BinClip, i.key().toInt(), QUuid()), clip.get(), false);
                    }
...
void Bin::processMultiStream(const QString &clipId, QList<int> videoStreams, QList<int> audioStreams)
{
    ...
    if (WunjoSettings::automultistreams()) {
...
void Bin::updateSequenceClip(const QUuid &uuid, std::pair<int, int> durations, int pos, bool forceUpdate)
{
    ...
    if (!binId.isEmpty() && m_doc->isModified()) {
        std::shared_ptr<ProjectClip> clip = m_itemModel->getClipByBinID(binId);
        Q_ASSERT(clip != nullptr);
        clip->setProducerProperty(QStringLiteral("wunjo:maxduration"), durations.first);
        if (m_doc->sequenceThumbRequiresRefresh(uuid) || forceUpdate) {
            ...
            properties.insert(QStringLiteral("wunjo:duration"), clip->framesToTime(duration));
            clip->setProperties(properties);
...
void Bin::loadSequenceAudioThumb()
{
    ...
    for (const auto &s : sequences.values()) {
        ...
        if (binClip->frameDuration() > 1) {
            ObjectId oid(WunjoObjectType::BinClip, binClip->clipId().toInt(), QUuid());
...
const QString Bin::loadInfo(const QStringList binInfo, const QStringList existingNames)
{
    ...
    if (binInfo.count() != 3) {
        ...
        m_listType = BinViewType(WunjoSettings::binMode());
    } else {
        rootId = binInfo.at(1);
