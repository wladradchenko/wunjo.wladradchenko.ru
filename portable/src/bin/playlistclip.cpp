...
void PlaylistClip::parsePlaylistProps()
{
    ...
    if (retainList.is_valid()) {
        ...
        // Get active timeline
        m_activeTimeline = QUuid(playlist.get("wunjo:docproperties.activetimeline"));
        // Read project file properties
        QMap<QString, QString> parsableProperties;
        parsableProperties.insert(i18n("version"), QStringLiteral("wunjo:docproperties.wunjoversion"));
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (prod->parent().type() == mlt_service_tractor_type) {
                if (prod->parent().property_exists("wunjo:uuid")) {
                    ...
                    const QUuid sequenceUuid(props.get("wunjo:uuid"));
                    info.sequenceName = qstrdup(props.get("wunjo:clipname"));
                    info.sequenceId = qstrdup(props.get("id"));
                    info.sequenceDuration = qstrdup(props.get("wunjo:duration"));
        ...
        qDebug() << ":::: READING XMLROOT:" << s.get("wunjo:projectroot");
        m_playlistRoot = QString(s.get("wunjo:projectroot"));
    }
}
...
void PlaylistClip::generateTmpPlaylists()
{
    ...
    for (int i = 0; i < tractors.count(); ++i) {
        ...
        if (Xml::hasXmlProperty(trac, QStringLiteral("wunjo:projectTractor"))) {
...
bool PlaylistClip::hasAlpha()
{
    ...
    if (inputFile.open(QIODevice::ReadOnly)) {
        ...
        const QString wunjoAlpha = QStringLiteral("wunjo:has_alpha");
        const QString codecInfo = QStringLiteral(".codec.pix_fmt\">");
        QTextStream in(&inputFile);
        while (!in.atEnd()) {
            const QString line = in.readLine();
            if (line.contains(png) || line.contains(title) || line.contains(wunjoAlpha)) {
                hasAlpha = true;
                break;
            }
