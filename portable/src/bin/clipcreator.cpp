...
QDomElement createProducer(QDomDocument &xml, ClipType::ProducerType type, const QString &resource, const QString &name, int duration, const QString &service)
{
    ...
    if (type == ClipType::Timeline) {
        ...
        if (!service.isEmpty()) {
            prod.setAttribute(QStringLiteral("wunjo:uuid"), service);
        } else {
            const QUuid uuid = QUuid::createUuid();
            prod.setAttribute(QStringLiteral("wunjo:uuid"), uuid.toString());
        }
    }
    ...
    if (!name.isEmpty()) {
        properties[QStringLiteral("wunjo:clipname")] = name;
    }
    if (!service.isEmpty()) {
                                        ...
                                        const std::shared_ptr<ProjectItemModel> &model, const std::function<void(const QString &)> &readyCallBack)
{
    ...
    bk.set("wunjo:playlistid", "black_track");
    ...
    timeline->unlock();
    timeline->set("wunjo:uuid", uuid.toString().toUtf8().constData());
    timeline->set("wunjo:clipname", name.toUtf8().constData());
    timeline->set("wunjo:duration", 1);
    timeline->set("wunjo:producer_type", ClipType::Timeline);
    ...
    prod->set("wunjo:clipname", name.toUtf8().constData());
    prod->set("wunjo:duration", 1);
    ...
    bool res = false;
    if (tracks.first > 0) {
        timeline->set("wunjo:sequenceproperties.hasAudio", 1);
        prod->set("wunjo:sequenceproperties.hasAudio", 1);
    }
    if (tracks.second > 0) {
        timeline->set("wunjo:sequenceproperties.hasVideo", 1);
        prod->set("wunjo:sequenceproperties.hasVideo", 1);
    }
    timeline->set("wunjo:sequenceproperties.tracksCount", tracks.first + tracks.second);
...
QDomDocument ClipCreator::getXmlFromUrl(const QString &path)
{
    ...
    if (type.name().startsWith(QLatin1String("image/")) && !type.name().contains(QLatin1String("image/gif"))) {
        int duration = pCore->getDurationFromString(WunjoSettings::image_duration());
                                        ...
                                        const std::function<void(const QString &)> &readyCallBack, const bool &updateShouldCheckProfile)
{
    ...
    if (updateShouldCheckProfile) {
        pCore->bin()->shouldCheckProfile =
            (WunjoSettings::default_profile().isEmpty() || WunjoSettings::checkfirstprojectclip()) && !pCore->bin()->hasUserClip();
    }
    ...
    for (const QUrl &file : std::as_const(cleanList)) {
        ...
        if (info.isDir()) {
            ...
            if (folderFiles.isEmpty()) {
                ...
                if (!sublist.isEmpty()) {
                    if (!WunjoSettings::ignoresubdirstructure() || topLevel) {
                        // Create main folder
