    ...
    , m_masterClip(parent)
{
    ...
    ClipLoadTask::start(ObjectId(WunjoObjectType::BinClip, m_parentClipId.toInt(), QUuid()), QDomElement(), true, in, out, this);
}
...
{
    ClipLoadTask::start(ObjectId(WunjoObjectType::BinClip, m_parentClipId.toInt(), QUuid()), QDomElement(), true, m_inPoint, m_outPoint, this);
}
...
void ProjectSubClip::getThumbFromPercent(int percent)
{
    ...
    if (ThumbnailCache::get()->hasThumbnail(m_parentClipId, m_inPoint + framePos)) {
        ...
        CacheTask::start(ObjectId(WunjoObjectType::BinClip, m_parentClipId.toInt(), QUuid()), 30, m_inPoint, m_outPoint, this);
    }
}
...
void ProjectSubClip::setProperties(const QMap<QString, QString> &properties)
{
    ...
    if (properties.contains(QStringLiteral("wunjo:tags"))) {
        propertyFound = true;
        m_tags = properties.value(QStringLiteral("wunjo:tags"));
    }
    if (properties.contains(QStringLiteral("wunjo:rating"))) {
        propertyFound = true;
        m_rating = properties.value(QStringLiteral("wunjo:rating")).toUInt();
    }
    if (!propertyFound) {
        return;
    }
