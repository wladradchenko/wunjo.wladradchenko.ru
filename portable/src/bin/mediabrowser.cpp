    ...
    : QWidget(parent)
{
    ...
    importOnDoubleClick->setChecked(WunjoSettings::mediaDoubleClickImport());
    connect(importOnDoubleClick, &QAction::triggered, this, [](bool enabled) { WunjoSettings::setMediaDoubleClickImport(enabled); });
    ...
    autoPlay->setChecked(WunjoSettings::mediaBrowserAutoPlay());
    connect(autoPlay, &QAction::triggered, this, [](bool enable) { WunjoSettings::setMediaBrowserAutoPlay(enable); });
    ...
    m_inlinePreview->setChecked(WunjoSettings::mediaBrowserInlinePreview());
    connect(m_inlinePreview, &QAction::triggered, this, [this](bool enable) { WunjoSettings::setMediaBrowserInlinePreview(enable); });
    ...
    m_op->setIconSize(WunjoSettings::mediaIconSize());
    ...
    connect(m_op, &KDirOperator::currentIconSizeChanged, this, [](int size) { WunjoSettings::setMediaIconSize(size); });
    ...
    b->setChecked(WunjoSettings::ignoresubdirstructure());
    ...
    connect(b, &QCheckBox::toggled, this, [](bool enabled) { WunjoSettings::setIgnoresubdirstructure(enabled); });

    QCheckBox *b2 = new QCheckBox(i18n("Import image sequence"));
    b2->setChecked(WunjoSettings::autoimagesequence());
    connect(b2, &QCheckBox::toggled, this, [](bool enabled) { WunjoSettings::setAutoimagesequence(enabled); });
    ...
    connect(preview, &QAction::triggered, this, [this](bool) {
        ...
        if (sizes.count() > 1) {
            ...
            if (sizes.at(1) == 0 && WunjoSettings::mediaManagerPanelWidth() > 0) {
                m_splitter->setSizes({WunjoSettings::mediaManagerPanelWidth(), totalWidth - WunjoSettings::mediaManagerPanelWidth()});
            } else {
        ...
        WunjoSettings::setMediaManagerPanelLayout(splitterState);
    });

    connect(m_splitter, &QSplitter::splitterMoved, this, [this, preview](int pos, int index) {
        if (index == 1 && pos != WunjoSettings::mediaManagerPanelWidth()) {
            int min = 0;
            int max = 0;
            m_splitter->getRange(1, &min, &max);
            if (pos < max) {
                preview->setChecked(true);
                WunjoSettings::setMediaManagerPanelWidth(pos);
            } else {
                m_previewPanel->resetPlayer();
    ...
    const QByteArray state = QByteArray::fromBase64(WunjoSettings::mediaManagerPanelLayout().toUtf8());
...
void MediaBrowser::slotViewDoubleClicked()
{
    ...
    if (WunjoSettings::mediaDoubleClickImport()) {
        importSelection();
        return;
    }
...
{
    KFileItemList files = m_op->selectedItems();
    if (WunjoSettings::autoimagesequence() && files.size() == 1) {
        QMimeDatabase db;
...
bool MediaBrowser::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (event->type() == QEvent::FocusIn) {
        ...
        if (WunjoSettings::mediaBrowserInlinePreview()) {
