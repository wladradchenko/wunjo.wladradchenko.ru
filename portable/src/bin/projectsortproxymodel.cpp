...
bool ProjectSortProxyModel::filterAcceptsRowItself(int sourceRow, const QModelIndex &sourceParent) const
{
    ...
    if (!m_searchType.isEmpty()) {
        // Keep folders visible so the folder tree is preserved while filtering by type
        QModelIndex typeIx = sourceModel()->index(sourceRow, 0, sourceParent);
        if (sourceModel()->data(typeIx, AbstractProjectItem::ItemTypeRole).toInt() == AbstractProjectItem::FolderItem) {
            result = true;
        } else {
...
void ProjectSortProxyModel::slotSetTypeFilters(const QList<int> &typeFilters)
{
    beginFilterChange();
    m_searchType = typeFilters;
    endFilterChange(QSortFilterProxyModel::Direction::Rows);
