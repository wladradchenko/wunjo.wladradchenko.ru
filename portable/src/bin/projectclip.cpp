    ...
    , m_uuid(QUuid::createUuid())
{
    ...
    connect(m_markerModel.get(), &MarkerListModel::modelChanged, this,
            [&]() { setProducerProperty(QStringLiteral("wunjo:markers"), m_markerModel->toJson()); });
    const QString markers = getProducerProperty(QStringLiteral("wunjo:markers"));
    if (!markers.isEmpty()) {
    ...
    setTags(getProducerProperty(QStringLiteral("wunjo:tags")));
    loadMasks(getProducerProperty(QStringLiteral("wunjo:masks")));
    AbstractProjectItem::setRating(uint(getProducerIntProperty(QStringLiteral("wunjo:rating"))));
    connectEffectStack();
    if (m_clipType != ClipType::Timeline &&
        ...
        ObjectId oid(WunjoObjectType::BinClip, m_binId.toInt(), QUuid());
        ClipLoadTask::start(oid, QDomElement(), true, -1, -1, this);
        // Generate audio thumbnail
        if (WunjoSettings::audiothumbnails() && (m_clipType == ClipType::AV || m_clipType == ClipType::Audio || m_hasAudio)) {
                                                    ...
                                                    std::shared_ptr<Mlt::Producer> &producer)
{
    ...
    QMetaObject::invokeMethod(model.get(), "loadSubClips", Qt::QueuedConnection, Q_ARG(QString, id),
                              Q_ARG(QString, self->getProducerProperty(QStringLiteral("wunjo:clipzones"))), Q_ARG(bool, false));
    return self;
}
    ...
    , m_uuid(QUuid::createUuid())
{
    ...
    const QString proxy = getXmlProperty(description, QStringLiteral("wunjo:proxy"));
    if (proxy.length() > 3) {
        m_temporaryUrl = getXmlProperty(description, QStringLiteral("wunjo:originalurl"));
    }
    if (m_temporaryUrl.isEmpty()) {
        m_temporaryUrl = getXmlProperty(description, QStringLiteral("resource"));
    }
    if (m_name.isEmpty()) {
        QString clipName = getXmlProperty(description, QStringLiteral("wunjo:clipname"));
        if (!clipName.isEmpty()) {
            m_name = clipName;
...
void ProjectClip::updateAudioThumbnail(bool cachedThumb)
{
    ...
    if (m_clipType == ClipType::Audio) {
        ...
        if (thumb.isNull() && !pCore->taskManager.hasPendingJob(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), AbstractTask::AUDIOTHUMBJOB)) {
...
void ProjectClip::resetSequenceThumbnails()
{
    ...
    pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), AbstractTask::LOADJOB, true);
    m_thumbXml.clear();
    ...
    if (refreshOnly) {
        ...
        if (!xml.isNull()) {
            ...
            if (type == ClipType::Audio || type == ClipType::AV) {
                ...
                QString clipHash = getProducerProperty(QStringLiteral("wunjo:file_hash"));
                if (!clipHash.isEmpty()) {
...
bool ProjectClip::setProducer(std::shared_ptr<Mlt::Producer> producer, bool generateThumb, bool clearTrackProducers)
{
    ...
    if (replacingProducer) {
        ...
        pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), AbstractTask::THUMBJOB);
        m_thumbMutex.lock();
        m_thumbXml.clear();
        m_thumbMutex.unlock();
    }
    ...
    // Make sure we have a hash for this clip
    getFileHash();
    // Only now is the clip analysable: face detection switched on while it was
    // still loading found no producer and did nothing, so this is where that
    // analysis starts. The hash exists by now, so a clip already analysed under
    // another name is recognised and nothing runs twice.
    if (FaceDataStore::instance().isEnabled(m_binId) && !FaceDataStore::instance().isComplete(m_binId)) {
        FaceDetectTask::start(m_binId);
    }
    ...
    if (generateThumb && m_clipType != ClipType::Audio) {
        ...
        ClipLoadTask::start(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), QDomElement(), true, -1, -1, this);
    }
    if (!waitForTranscode && WunjoSettings::audiothumbnails() &&
        (m_clipType == ClipType::AV || m_clipType == ClipType::Audio || (m_hasAudio && m_clipType != ClipType::Timeline))) {
        AudioLevelsTask::start(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), this, false);
    }
    if (WunjoSettings::keep_original_frame_size() && !m_usesProxy &&
        ...
        if (producerSize != refSize) {
            ...
            WunjoSettings::setEnableBuiltInEffects(true);
...
void ProjectClip::checkProxy(bool rebuildProxy)
{
    ...
    if (rebuildProxy ||
        ...
        if (m_clipType == ClipType::Image) {
            if (pCore->currentDoc()->autoGenerateImageProxy(getProducerIntProperty(QStringLiteral("meta.media.width"))) &&
                getProducerProperty(QStringLiteral("wunjo:proxy")) == QLatin1String()) {
                clipToProxy = std::static_pointer_cast<ProjectClip>(shared_from_this());
            }
        } else if ((m_clipType == ClipType::AV || m_clipType == ClipType::Video) &&
                   (rebuildProxy || getProducerProperty(QStringLiteral("wunjo:proxy")) == QLatin1String())) {
    ...
    if (!generateProxy && WunjoSettings::hoverPreview() &&
        (m_clipType == ClipType::AV || m_clipType == ClipType::Video || m_clipType == ClipType::Playlist)) {
        QTimer::singleShot(1000, this, [this]() { CacheTask::start(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), 30, 0, 0, this); });
    }
    if (generateProxy) {
...
int ProjectClip::getStartTimecode(int streamIndex)
{
    ...
    QString cacheKey = streamIndex > 0 ? QStringLiteral("wunjo:record_start_frame_%1").arg(streamIndex) : QStringLiteral("wunjo:record_start_frame");
    ...
    // cache the value in a wunjo property
                                                                ...
                                                                bool secondPlaylist, const TimeWarpInfo timeremapInfo)
{
    ...
    if (qFuzzyCompare(speed, 1.0) && !timeremapInfo.enableRemap) {
        ...
        int maxDuration = 0;
        if (m_clipType == ClipType::Timeline && m_masterProducer->parent().property_exists("wunjo:maxduration")) {
            int duration = m_masterProducer->parent().get_int("wunjo:maxduration");
            if (duration > 0) {
                maxDuration = duration;
            } else {
                maxDuration = m_masterProducer->time_to_frames(m_masterProducer->get("wunjo:duration"));
            }
        }
        ...
        if (byPassTrackProducer ||
            ...
            if (m_clipType == ClipType::Timeline) {
                ...
                duration = m_masterProducer->time_to_frames(m_masterProducer->get("wunjo:duration"));
            }
            std::shared_ptr<Mlt::Producer> prod(m_masterProducer->cut(-1, duration > 0 ? duration - 1 : -1));
            if (m_clipType == ClipType::Timeline && m_masterProducer->parent().property_exists("wunjo:maxduration")) {
                prod->set("wunjo:maxduration", m_masterProducer->parent().get_int("wunjo:maxduration"));
            }
            return prod;
        }
        ...
        if (state == PlaylistState::AudioOnly) {
            ...
            if (m_clipType == ClipType::Timeline && m_audioProducers[audioTrackId]->parent().property_exists("wunjo:maxduration")) {
                int max = m_audioProducers[audioTrackId]->parent().get_int("wunjo:maxduration");
                prod->set("wunjo:maxduration", max);
                prod->set("length", max);
            }
            return prod;
        }
        if (state == PlaylistState::VideoOnly) {
            ...
            if (m_videoProducers.count(trackId) == 0) {
                ...
                if (m_masterProducer->property_exists("wunjo:maxduration")) {
                    finalProducer->set("wunjo:maxduration", m_masterProducer->get_int("wunjo:maxduration"));
                }
            ...
            int duration = m_masterProducer->time_to_frames(m_masterProducer->get("wunjo:duration"));
        ...
        int duration = m_masterProducer->time_to_frames(m_masterProducer->get("wunjo:duration")) - 1;
        std::shared_ptr<Mlt::Producer> prod(m_disabledProducer->cut(-1, duration > 0 ? duration : -1));
        // Ensure we don't lose audio index
        prod->set("wunjo:audio_index", audioStream);
        if (m_clipType == ClipType::Timeline) {
    ...
    warpProducer->set("wunjo:id", binId().toUtf8().constData());
    warpProducer->set("wunjo:control_uuid", m_controlUuid.toString().toUtf8().constData());
    ...
    if (master->parent().is_valid()) {
        ...
        if (master->parent().get_int("_loaded") == 1) {
            ...
            if (state == PlaylistState::Disabled) {
                ...
                if (master->property_exists("wunjo:audio_index")) {
                    disabledProducer->set("wunjo:audio_index", master->get_int("wunjo:audio_index"));
                }
...
void ProjectClip::cloneProducerToFile(const QString &path, bool thumbsProducer, bool audioOnly)
{
    ...
    if (audioOnly) {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            QString content = in.readAll();
            file.close();
            content.replace(getProducerProperty(QStringLiteral("resource")), getProducerProperty(QStringLiteral("wunjo:originalurl")));
            if (file.open(QIODevice::WriteOnly)) {
                QTextStream out(&file);
                out << content;
                file.close();
...
std::shared_ptr<Mlt::Producer> ProjectClip::cloneProducer(bool removeEffects, bool timelineProducer)
{
    ...
    /*if (timelineProducer) {
        // Strip the wunjo: properties, not useful in timeline
        const char *prefix = "wunjo:";
        ...
        propertiesToRemove.removeAll(QLatin1String("wunjo:id"));
...
{
    int in = getProducerIntProperty(QStringLiteral("wunjo:zone_in"));
    int max = getFramePlaytime();
    int out = qMin(getProducerIntProperty(QStringLiteral("wunjo:zone_out")), max);
    if (out <= in) {
        out = max;
    }
...
const QString ProjectClip::getFileHash()
{
    ...
    case ClipType::Text: {
        ...
        QByteArray uniqueId = getProducerProperty(QStringLiteral("wunjo:uniqueId")).toUtf8();
        if (uniqueId.isEmpty()) {
            const QUuid uuid = QUuid::createUuid();
            setProducerProperty(QStringLiteral("wunjo:uniqueId"), uuid.toString());
    ...
    default:
        ...
        ClipController::setProducerProperty(QStringLiteral("wunjo:file_size"), QString::number(hashData.second));
        break;
    }
    if (fileHash.isEmpty()) {
    ...
    ClipController::setProducerProperty(QStringLiteral("wunjo:file_hash"), result);
    return result;
}
...
void ProjectClip::setProperties(const QMap<QString, QString> &properties, bool refreshPanel)
{
    ...
    while (i.hasNext()) {
        ...
        if (i.key().startsWith(QLatin1String("wunjo:clipanalysis"))) {
    ...
    if (properties.contains(QStringLiteral("resource"))) {
        ...
        reload = true;
        ThumbnailCache::get()->invalidateThumbsForClip(m_binId);
        resetProducerProperty(QStringLiteral("wunjo:file_hash"));
    ...
    if (properties.contains(QStringLiteral("wunjo:proxy")) && !properties.contains("_fullreload")) {
        QString value = properties.value(QStringLiteral("wunjo:proxy"));
        ...
        if (value.isEmpty() || value == QLatin1String("-")) {
            ...
            if (pCore->taskManager.hasPendingJob(oid, AbstractTask::PROXYJOB)) {
                ...
                QString resource = getProducerProperty(QStringLiteral("wunjo:originalurl"));
                if (!resource.isEmpty()) {
            ...
            setProducerProperty(QStringLiteral("wunjo:originalurl"), url());
    ...
    if (properties.contains(QStringLiteral("length")) || properties.contains(QStringLiteral("wunjo:duration"))) {
        // Make sure length is >= wunjo:duration
        int producerLength = getProducerIntProperty(QStringLiteral("length"));
        int wunjoLength = getFramePlaytime();
        if (producerLength < wunjoLength) {
            setProducerProperty(QStringLiteral("length"), wunjoLength);
        }
        m_duration = getStringDuration();
        if (auto ptr = m_model.lock())
            std::static_pointer_cast<ProjectItemModel>(ptr)->onItemUpdated(std::static_pointer_cast<ProjectClip>(shared_from_this()),
    ...
    if (properties.contains(QStringLiteral("wunjo:tags"))) {
        setTags(properties.value(QStringLiteral("wunjo:tags")));
        if (auto ptr = m_model.lock()) {
            std::static_pointer_cast<ProjectItemModel>(ptr)->onItemUpdated(std::static_pointer_cast<ProjectClip>(shared_from_this()),
    ...
    if (properties.contains(QStringLiteral("wunjo:clipname"))) {
        const QString updatedName = properties.value(QStringLiteral("wunjo:clipname"));
    ...
    if (properties.contains(QStringLiteral("wunjo:description"))) {
        m_description = properties.value(QStringLiteral("wunjo:description"));
        refreshPanel = true;
        if (auto ptr = m_model.lock()) {
            std::static_pointer_cast<ProjectItemModel>(ptr)->onItemUpdated(std::static_pointer_cast<ProjectClip>(shared_from_this()),
                                                                           {AbstractProjectItem::DataDescription});
    ...
    if (reload) {
        ...
        if (hasProxy()) {
            ...
            reloadProducer(refreshOnly, properties.contains(QStringLiteral("wunjo:proxy")));
            if (!refreshOnly) {
                return;
            }
        }
        ...
        if (properties.contains(QStringLiteral("wunjo:active_streams")) && m_audioInfo) {
            // Clip is a multi audio stream and currently in clip monitor, update target tracks
            m_audioInfo->updateActiveStreams(properties.value(QStringLiteral("wunjo:active_streams")));
...
{
    if (tracksCount > -1) {
        setProducerProperty(QStringLiteral("wunjo:sequenceproperties.tracksCount"), tracksCount);
    }
...
{
    // Disable proxy file
    QString proxy = getProducerProperty(QStringLiteral("wunjo:proxy"));
    QList<std::shared_ptr<ProjectClip>> clipList{std::static_pointer_cast<ProjectClip>(shared_from_this())};
    if (reloadClip) {
...
void ProjectClip::updateParent(std::shared_ptr<TreeItem> parent)
{
    if (parent) {
        ...
        ClipController::setProducerProperty(QStringLiteral("wunjo:folderid"), item->clipId());
    }
...
{
    if (m_name.isEmpty()) {
        m_name = getProducerProperty(QStringLiteral("wunjo:clipname"));
        if (m_name.isEmpty()) {
...
bool ProjectClip::rename(const QString &name, int column)
{
    ...
    case 0:
        ...
        // Rename clip
        oldProperties.insert(QStringLiteral("wunjo:clipname"), m_name);
        newProperties.insert(QStringLiteral("wunjo:clipname"), name);
        edited = true;
        break;
    case 2:
        ...
        if (m_clipType == ClipType::TextTemplate) {
            ...
            oldProperties.insert(QStringLiteral("wunjo:description"), m_description);
            newProperties.insert(QStringLiteral("wunjo:description"), name);
        }
        edited = true;
        break;
    }
...
void ProjectClip::discardAudioThumb(bool recreate)
{
    ...
    pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), AbstractTask::AUDIOTHUMBJOB);
    ...
    resetProducerProperty(QStringLiteral("wunjo:audio_max"));
    m_audioThumbCreated = false;
    refreshAudioInfo();
    if (recreate) {
        // Generate thumb
...
QStringList ProjectClip::updatedAnalysisData(const QString &name, const QString &data, int offset)
{
    if (data.isEmpty()) {
        // Remove data
        return QStringList() << QStringLiteral("wunjo:clipanalysis.%1").arg(name) << QString();
        // m_controller->resetProperty("wunjo:clipanalysis." + name);
    }
    QString current = getProducerProperty("wunjo:clipanalysis." + name);
    if (!current.isEmpty()) {
        // TODO
        /*if (KMessageBox::questionTwoActions(QApplication::activeWindow(), i18n("Clip already contains analysis data %1", name), QString(),
            ...
            return QStringList() << QStringLiteral("wunjo:clipanalysis.%1").arg(name) << geometry.serialise();
            // m_controller->setProperty("wunjo:clipanalysis." + name, geometry.serialise());
        }*/
        // Add data with another name
        int i = 1;
        QString previous = getProducerProperty("wunjo:clipanalysis." + name + QString::number(i));
        while (!previous.isEmpty()) {
            ++i;
            previous = getProducerProperty("wunjo:clipanalysis." + name + QString::number(i));
        }
        return QStringList() << QStringLiteral("wunjo:clipanalysis.%1%2").arg(name).arg(i) << geometryWithOffset(data, offset);
        // m_controller->setProperty("wunjo:clipanalysis." + name + QLatin1Char(' ') + QString::number(i), geometryWithOffset(data, offset));
    }
    return QStringList() << QStringLiteral("wunjo:clipanalysis.%1").arg(name) << geometryWithOffset(data, offset);
    // m_controller->setProperty("wunjo:clipanalysis." + name, geometryWithOffset(data, offset));
}

QMap<QString, QString> ProjectClip::analysisData(bool withPrefix)
{
    return getPropertiesFromPrefix(QStringLiteral("wunjo:clipanalysis."), withPrefix);
}
...
bool ProjectClip::selfSoftDelete(Fun &undo, Fun &redo)
{
    Fun operation = [this]() {
        ...
        pCore->taskManager.discardJobs(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()));
...
void ProjectClip::updateZones()
{
    int zonesCount = childCount();
    if (zonesCount == 0) {
        resetProducerProperty(QStringLiteral("wunjo:clipzones"));
        return;
    }
    QJsonArray list;
    for (int i = 0; i < zonesCount; ++i) {
    ...
    setProducerProperty(QStringLiteral("wunjo:clipzones"), QString(json.toJson()));
}

int ProjectClip::getThumbFrame() const
{
    return qMax(0, getProducerIntProperty(QStringLiteral("wunjo:thumbnailFrame")));
}

void ProjectClip::setThumbFrame(int frame)
{
    setProducerProperty(QStringLiteral("wunjo:thumbnailFrame"), frame);
    ClipLoadTask::start(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), QDomElement(), true, -1, -1, this);
}
...
int ProjectClip::getThumbFromPercent(int percent, bool storeFrame)
{
    ...
    if (storeFrame) {
        setProducerProperty(QStringLiteral("wunjo:thumbnailFrame"), framePos);
    }
    return framePos;
}
...
void ProjectClip::setRating(uint rating)
{
    ...
    setProducerProperty(QStringLiteral("wunjo:rating"), int(rating));
    pCore->currentDoc()->setModified(true);
}

int16_t ProjectClip::getAudioMax(const int streamIdx) const
{
    const QString key = QStringLiteral("_wunjo:audio_max%1").arg(streamIdx);
...
{
    const QString key = QStringLiteral("_wunjo:audio%1").arg(streamIdx);
...
void ProjectClip::setClipStatus(FileStatus::ClipStatus status)
{
    if (status == FileStatus::StatusMissing && hasProxy()) {
        ...
        setProducerProperty(QStringLiteral("wunjo:proxy"), QStringLiteral("-"));
        setProducerProperty(QStringLiteral("resource"), getProducerProperty("wunjo:originalurl"));
...
void ProjectClip::renameAudioStream(int id, const QString &name)
{
    if (m_audioInfo) {
        ...
        QString prop = QStringLiteral("wunjo:streamname.%1").arg(id);
...
void ProjectClip::addAudioStreamEffect(int streamIndex, const QString effectName)
{
    ...
    setProducerProperty(QStringLiteral("wunjo:stream:%1").arg(streamIndex), effects.join(QLatin1Char('#')));
    for (auto &p : m_audioProducers) {
...
void ProjectClip::removeAudioStreamEffect(int streamIndex, QString effectName)
{
    ...
    if (m_streamEffects.contains(streamIndex)) {
        ...
        if (effects.isEmpty()) {
            ...
            resetProducerProperty(QStringLiteral("wunjo:stream:%1").arg(streamIndex));
        } else {
            m_streamEffects.insert(streamIndex, effects);
            setProducerProperty(QStringLiteral("wunjo:stream:%1").arg(streamIndex), effects.join(QLatin1Char('#')));
        }
...
{
    if (ownerId.type == WunjoObjectType::NoItem) {
        ownerId = ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid());
    } else {
        if (jobProgress > -1) {
...
void ProjectClip::updateDescription()
{
    if (m_clipType == ClipType::TextTemplate) {
        ...
        m_description = getProducerProperty(QStringLiteral("wunjo:description"));
...
std::pair<int, int> ProjectClip::fpsInfo(std::shared_ptr<Mlt::Producer> producer)
{
    if (producer) {
        ...
        if (std::find(allowedfps.begin(), allowedfps.end(), fps_den) == allowedfps.end()) {
            ...
            bool adjusted = false;
            std::pair<int, int> fpsInfo = WunjoDoc::getFpsFraction(target_fps, &adjusted);
...
void ProjectClip::addMask(const ObjectId &filterOwner, MaskInfo mask, bool autoAdd)
{
    ...
    QJsonDocument json(list);
    setProducerProperty(QStringLiteral("wunjo:masks"), QString::fromUtf8(json.toJson()));
    pCore->currentDoc()->setModified(true);
}
