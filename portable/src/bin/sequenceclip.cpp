    ...
    : ProjectClip(id, thumb, model, producer)
{
    ...
    m_sequenceUuid = QUuid(m_masterProducer->get("wunjo:uuid"));
    ...
    if (model->hasSequenceId(m_sequenceUuid)) {
        ...
        m_masterProducer->set("wunjo:uuid", m_sequenceUuid.toString().toUtf8().constData());
        m_masterProducer->parent().set("wunjo:uuid", m_sequenceUuid.toString().toUtf8().constData());
        const QString subValue(m_masterProducer->get("wunjo:sequenceproperties.subtitlesList"));
        if (!subValue.isEmpty()) {
            int ix = m_masterProducer->get_int("wunjo:sequenceproperties.wmproj:activeSubtitleIndex");
            pCore->currentDoc()->setSequenceProperty(m_sequenceUuid, QStringLiteral("wunjo:activeSubtitleIndex"), QString::number(ix));
...
{
    m_sequenceUuid = QUuid(getXmlProperty(description, QStringLiteral("wunjo:uuid")));
    connect(this, &ProjectClip::audioThumbReady, this, &SequenceClip::updateAudioSync);
}
...
QDomElement SequenceClip::toXml(QDomDocument &document, bool includeMeta, bool includeProfile)
{
    ...
    if (tag != QLatin1String("producer") && tag != QLatin1String("chain")) {
        ...
        prod.setAttribute(QStringLiteral("wunjo:id"), m_binId);
        prod.setAttribute(QStringLiteral("wunjo:producer_type"), ClipType::Timeline);
        prod.setAttribute(QStringLiteral("wunjo:uuid"), m_sequenceUuid.toString());
        prod.setAttribute(QStringLiteral("wunjo:duration"), QString::number(frameDuration()));
        prod.setAttribute(QStringLiteral("wunjo:clipname"), clipName());
    }
...
bool SequenceClip::setProducer(std::shared_ptr<Mlt::Producer> producer, bool generateThumb, bool clearTrackProducers)
{
    if (m_sequenceUuid.isNull()) {
        m_sequenceUuid = QUuid::createUuid();
        producer->parent().set("wunjo:uuid", m_sequenceUuid.toString().toUtf8().constData());
    }
    if (m_masterProducer) {
        // Pass sequence properties
        producer->parent().pass_list(m_masterProducer->parent(), "wunjo:clipzones,wunjo:zone_in,wunjo:zone_out");
    }
...
void SequenceClip::createDisabledMasterProducer()
{
    if (!m_disabledProducer) {
        ...
        target.pass_list(original, "wunjo:control_uuid,wunjo:id,wunjo:duration,wunjo:maxduration,length");
...
{
    QTemporaryFile *tmp = new QTemporaryFile(QDir::temp().absoluteFilePath(QStringLiteral("wunjo-XXXXXX.mlt")));
    if (tmp->open()) {
        tmp->close();
...
void SequenceClip::setProperties(const QMap<QString, QString> &properties, bool refreshPanel)
{
    for (auto i = properties.cbegin(), end = properties.cend(); i != end; ++i) {
        if (i.key().startsWith(QLatin1String("wunjo:sequenceproperties."))) {
            pCore->currentDoc()->setSequenceProperty(m_sequenceUuid, i.key(), i.value());
            if (i.key() == QLatin1String("wunjo:sequenceproperties.timecodeOffset")) {
    ...
    if (durationChanged) {
        ...
        if (pCore->taskManager.displayedClip == m_binId.toInt()) {
            ...
            pCore->getMonitor(Wunjo::ClipMonitor)->markAudioDirty(true);
            // Refresh monitor duration
            pCore->getMonitor(Wunjo::ClipMonitor)->adjustRulerSize(properties.value("length").toInt());
        }
    }
}
...
std::shared_ptr<Mlt::Producer> SequenceClip::sequenceProducer(const QUuid &)
{
    ...
    int maxDuration = m_masterProducer->parent().get_int("wunjo:maxduration");
    if (maxDuration > 0) {
...
{
    return pCore->currentDoc()->getSequenceProperty(m_sequenceUuid, "wunjo:sequenceproperties.timecodeOffset").toInt();
}

bool SequenceClip::hasTimecode()
{
    return pCore->currentDoc()->hasSequenceProperty(m_sequenceUuid, "wunjo:sequenceproperties.timecodeOffset");
}
...
void SequenceClip::updateAudioSync()
{
    ...
    if (pCore->taskManager.displayedClip == m_binId.toInt()) {
        // Mark audio thumb as dirty
        pCore->getMonitor(Wunjo::ClipMonitor)->markAudioDirty(false);
    }
}
