...
class OmniGenScheduler:
    ...
    def __call__(self, z, func, model_kwargs, use_kv_cache: bool=True, offload_kv_cache: bool=True, callback=None):
        ...
        for i in pbar:
            ...
            if callback:
                # Send progress data (percent and remaining time) to your callback function
                remaining = (pbar.format_dict.get('total', 0) - pbar.format_dict.get('n', 0)) / pbar.format_dict.get('rate', 0) if pbar.format_dict.get('rate', 0) and pbar.format_dict.get('total', 0) else 0
                minutes, seconds = divmod(int(remaining), 60)
                callback(round(i / self.num_steps * 100), f"Scheduler {minutes:02d}:{seconds:02d}.")
