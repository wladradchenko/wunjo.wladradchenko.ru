...
/**
 * @class Core
 * @brief Singleton that provides access to the different parts of Wunjo
 *
...
public:
    ...
    int getMonitorPosition(Wunjo::MonitorId id = Wunjo::ProjectMonitor) const;
...
Q_SIGNALS:
    ...
    void showConfigDialog(Wunjo::ConfigPage, int);
    ...
    /** Show / hide the panel that records the presets of @p pluginId. @p kind
     *  picks which sort of preset when the plugin keeps several. */
    void switchPluginSetPanel(const QString &pluginId, const QString &kind, bool show);
    /** Face detection produced new results for @p binId — redraw the boxes */
    void faceDataChanged(const QString &binId);
