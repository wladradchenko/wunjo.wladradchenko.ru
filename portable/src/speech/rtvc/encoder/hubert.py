...
class HubertEmbedder:
    ...
    def load_model(path, device="cpu"):
        model = hubert_soft(path)  # Assuming hubert_soft is a function you have defined elsewhere
        model.eval()
        if device != "cpu":
            model.half()
        model.to(device)
    ...
    def embed_audio(self, wavPath):
        audio = self.load_audio(wavPath)
        audln = audio.shape[0]
        vec_a = []
        idx_s = 0
        while (idx_s + 20 * 16000 < audln):
            ...
            if self.device != "cpu":
                feats = feats.half()
            with torch.no_grad():
                vec = self.model.units(feats).squeeze().data.cpu().float().numpy()
                vec_a.extend(vec)
