...
{
    return QStringList() << QStringLiteral("text/uri-list") << QStringLiteral("wunjo/clip") << QStringLiteral("text/producerslist");
}
...
void LibraryTree::dropEvent(QDropEvent *event)
{
    ...
    } else if (qMimeData->hasFormat(QStringLiteral("wunjo/clip"))) {
        Q_EMIT importSequence(QString(qMimeData->data(QStringLiteral("wunjo/clip"))).split(QLatin1Char(';')), dest);
    ...
    , m_selectNewFile(false)
{
    ...
    // Library path
    QString path;
    if (WunjoSettings::librarytodefaultfolder() || WunjoSettings::libraryfolder().isEmpty()) {
        path = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + QStringLiteral("/library");
    } else {
        path = WunjoSettings::libraryfolder();
    }
...
void LibraryWidget::setupActions()
{
    ...
    m_addAction = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-add-clip")), i18n("Add Clip to Project"), this);
...
void LibraryWidget::slotDeleteFromLibrary()
{
    QTreeWidgetItem *current = m_libraryTree->currentItem();
    if (!current) {
        qCDebug(WUNJO_LOG) << " * * *Deleting no item ";
        return;
    }
...
void LibraryWidget::slotUpdateLibraryPath()
{
    ...
    if (WunjoSettings::librarytodefaultfolder() || WunjoSettings::libraryfolder().isEmpty()) {
        ...
        m_directory.setPath(WunjoSettings::libraryfolder());
        if (!m_directory.exists()) {
            m_directory.mkpath(QStringLiteral("."));
        }
        showMessage(i18n("Library path set to custom: %1", WunjoSettings::libraryfolder()), KMessageWidget::Information);
    }
    QFileInfo fi(m_directory.absolutePath());
    if (!m_directory.exists() || !fi.isWritable()) {
        // Cannot write to new Library, try default one
        if (m_directory.absolutePath() != defaultPath) {
            showMessage(i18n("Cannot write to Library path: %1, using default", WunjoSettings::libraryfolder()), KMessageWidget::Warning);
            m_directory.setPath(defaultPath);
            if (!m_directory.exists()) {
                m_directory.mkpath(QStringLiteral("."));
            }
...
void LibraryWidget::slotItemsAdded(const QUrl &url, const KFileItemList &list)
{
    ...
    for (const KFileItem &fitem : list) {
        ...
        } else if (name.endsWith(QLatin1String(".mlt")) || name.endsWith(QLatin1String(".wmproj"))) {
