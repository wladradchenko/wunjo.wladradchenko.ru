...
{
    QStringList args = {QStringLiteral("delivery"), WunjoSettings::meltpath(), job.playlistPath};
    if (addPid) {
...
std::vector<RenderRequest::RenderJob> RenderRequest::process()
{
    ...
    WunjoDoc::setAutoclosePlaylists(doc, currentUuid.toString());
    ...
    if (m_presetParams.hasAlpha()) {
        WunjoDoc::makeBackgroundTrackTransparent(doc);
        modified = true;
    }
...
{
    QTemporaryFile tmp(QDir::temp().absoluteFilePath(QStringLiteral("wunjo-XXXXXX.%1").arg(extension)));
    if (!tmp.open()) {
...
QString RenderRequest::generatePlaylistFile()
{
    ...
    projectFolder.mkpath(QStringLiteral("wunjo-renderqueue"));
...
void RenderRequest::setDocGeneralParams(QDomDocument doc, int in, int out)
{
    ...
    consumer.setAttribute(QStringLiteral("rescale"), WunjoSettings::renderInterp().toUtf8().constData());
    consumer.setAttribute(QStringLiteral("deinterlacer"), WunjoSettings::renderDeinterlacer().toUtf8().constData());
                                           ...
                                           const QUuid &uuid)
{
    ...
    for (int i = orginalTractors.size(); i >= 0; i--) {
        ...
        const QUuid tractorUuid(Xml::getXmlProperty(originalTracktor, QStringLiteral("wunjo:uuid")));
        if (tractorUuid == uuid) {
    ...
    for (int i = orginalTractors.size(); i >= 0; i--) {
        ...
        QString trackName = Xml::getXmlProperty(originalTracktor, QStringLiteral("wunjo:track_name"));
        bool originalIsAudio = Xml::getXmlProperty(originalTracktor, QStringLiteral("wunjo:audio_track")).toInt() == 1;
        if (!originalIsAudio) {
        ...
        for (int j = 0; j < tracktors.size(); j++) {
            ...
            bool copyIsAudio = Xml::getXmlProperty(tractor, QStringLiteral("wunjo:audio_track")).toInt() == 1;
            if (!copyIsAudio) {
