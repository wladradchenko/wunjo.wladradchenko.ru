...
void ProxyTask::run()
{
    ...
    const QString dest = binClip->getProducerProperty(QStringLiteral("wunjo:proxy"));
    ...
    m_progress = 0;
    bool result = false;
    QString source = binClip->getProducerProperty(QStringLiteral("wunjo:originalurl"));
    ...
    if (type == ClipType::Playlist || type == ClipType::SlideShow) {
        ...
        if (binClip->hasAlpha()) {
            ...
            parameter = WunjoSettings::proxyalphaparams().simplified();
        ...
        if (source.startsWith(QLatin1String("consumer:"))) {
            display_ratio = WunjoDoc::getDisplayRatio(source.section(QLatin1Char(':'), 1));
        } else {
            display_ratio = WunjoDoc::getDisplayRatio(source);
        }
        ...
        jobProcess.start(WunjoSettings::meltpath(), mltParameters);
        ...
        if (WunjoSettings::proxyimagesize() < maxSize) {
            if (i.width() > i.height()) {
                proxy = i.scaledToWidth(WunjoSettings::proxyimagesize());
            } else {
                proxy = i.scaledToHeight(WunjoSettings::proxyimagesize());
            }
            proxy.save(dest);
            result = true;
        } else {
        ...
        m_isFfmpegJob = true;
        if (!QFileInfo(WunjoSettings::ffmpegpath()).isFile()) {
            // FFmpeg not detected, cannot process the Job
            QMetaObject::invokeMethod(pCore.get(), "displayBinMessage", Qt::QueuedConnection,
                                      Q_ARG(QString, i18n("FFmpeg not found, please set path in Wunjo's settings Environment")),
                                      Q_ARG(int, int(KMessageWidget::Warning)));
            result = true;
            m_progress = 100;
            QMetaObject::invokeMethod(m_object, "updateJobProgress");
        ...
        if (binClip->hasProducerProperty(QStringLiteral("wunjo:camcorderproxy"))) {
            ...
            const QString proxyPath = binClip->getProducerProperty(QStringLiteral("wunjo:camcorderproxy"));
            ...
            if (binClip->hasAlpha()) {
                // check if this is a VP8/VP9 clip and enforce libvpx codec
                proxyParams = WunjoSettings::proxyalphaparams().simplified();
            ...
            if (nvenc) {
                ...
                if (supported && proxyParams.contains(QStringLiteral("scale_npp")) && !WunjoSettings::nvScalingEnabled()) {
                    supported = false;
                }
            ...
            if (binClip->hasProducerProperty(QStringLiteral("wunjo:coverartstream"))) {
        ...
        QObject::connect(&jobProcess, &QProcess::readyReadStandardError, this, &ProxyTask::processLogInfo);
        QObject::connect(this, &ProxyTask::jobCanceled, &jobProcess, &QProcess::kill, Qt::DirectConnection);
        jobProcess.start(WunjoSettings::ffmpegpath(), parameters, QIODevice::ReadOnly);
        AbstractTask::setPreferredPriority(jobProcess.processId());
        jobProcess.waitForFinished(-1);
    ...
    if (result && !m_isCanceled) {
        if (QFileInfo(dest).size() == 0) {
            ...
            if (binClip) {
                stringMap proxyValue = {{QStringLiteral("wunjo:proxy"), QStringLiteral("-")}};
