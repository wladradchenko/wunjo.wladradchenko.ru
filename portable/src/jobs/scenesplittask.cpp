...
void SceneSplitTask::start(QObject *object, bool force)
{
    ...
    view.setupUi(d);
    view.threshold->setValue(WunjoSettings::scenesplitthreshold());
    view.add_markers->setChecked(WunjoSettings::scenesplitmarkers());
    view.range_markers->setChecked(WunjoSettings::scenesplitrangemarkers());
    view.range_markers->setEnabled(WunjoSettings::scenesplitmarkers());
    view.cut_scenes->setChecked(WunjoSettings::scenesplitsubclips());
    // Set  up categories
    ...
    WunjoSettings::setScenesplitthreshold(threshold);
    WunjoSettings::setScenesplitmarkers(addMarkers);
    WunjoSettings::setScenesplitrangemarkers(rangeMarkers);
    WunjoSettings::setScenesplitsubclips(addSubclips);
    ...
    for (auto &id : binIds) {
        ...
        if (id.contains(QLatin1Char('/'))) {
            ...
            owner = ObjectId(WunjoObjectType::BinClip, binData.first().toInt(), QUuid());
            ...
            owner = ObjectId(WunjoObjectType::BinClip, id.toInt(), QUuid());
...
void SceneSplitTask::run()
{
    ...
    if (WunjoSettings::ffmpegpath().isEmpty()) {
    ...
    m_jobProcess->start(WunjoSettings::ffmpegpath(), parameters);
