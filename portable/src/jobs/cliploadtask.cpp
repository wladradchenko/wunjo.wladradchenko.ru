...
ClipType::ProducerType ClipLoadTask::getTypeForService(const QString &id, const QString &path)
{
    if (id.isEmpty()) {
        ...
        if (ext == QLatin1String("mlt") || ext == QLatin1String("wmproj")) {
...
void ClipLoadTask::processProducerProperties(const std::shared_ptr<Mlt::Producer> &prod, const QDomElement &xml)
{
    ...
    for (int i = 0; i < props.count(); ++i) {
        ...
        if (!internalProperties.contains(propertyName) && !propertyName.startsWith(QLatin1Char('_'))) {
            ...
            if (propertyName.startsWith(QLatin1String("wunjo-force."))) {
...
void ClipLoadTask::run()
{
    ...
    if (type == ClipType::Playlist && Xml::getXmlProperty(m_xml, QStringLiteral("wunjo:proxy")).length() > 2) {
    ...
    case ClipType::TextTemplate: {
        ...
        if (!resource.isEmpty()) {
            ...
            QString xmlDuration = Xml::getXmlProperty(m_xml, QStringLiteral("wunjo:duration"));
        ...
        if (duration <= 0) {
            if (producerLength > 0) {
                ...
                duration = pCore->getDurationFromString(WunjoSettings::title_duration());
            }
        }
        if (producerLength <= 0) {
        ...
        producer->set("wunjo:duration", producer->frames_to_time(duration));
    ...
    case ClipType::Qml: {
        ...
        if (producerLength <= 0) {
            producerLength = pCore->getDurationFromString(WunjoSettings::title_duration());
        }
        producer = loadResource(resource, QStringLiteral("qml:"));
        producer->set("length", producerLength);
        producer->set("wunjo:duration", producer->frames_to_time(producerLength));
        producer->set("out", producerLength - 1);
        break;
    }
    case ClipType::Playlist: {
        ...
        if (producer) {
            ...
            if (f.open(QIODevice::ReadOnly)) {
                doc.setContent(&f);
                f.close();
                if (resource.endsWith(QLatin1String(".wmproj"))) {
                    QDomElement pl = doc.documentElement().firstChildElement(QStringLiteral("playlist"));
                    if (!pl.isNull()) {
                        QString offsetData = Xml::getXmlProperty(pl, QStringLiteral("wunjo:docproperties.seekOffset"));
                        if (offsetData.isEmpty() && Xml::getXmlProperty(pl, QStringLiteral("wunjo:docproperties.version")) == QLatin1String("0.98")) {
                        ...
                        if (!offsetData.isEmpty()) {
                            ...
                            if (ok) {
                                ...
                                producer->set("wunjo:duration", producer->frames_to_time(offset));
                            }
                        } else {
                    ...
                    QString duration = Xml::getXmlProperty(tractor, QStringLiteral("wunjo:duration"));
                    if (!duration.isEmpty()) {
                        int frames = producer->time_to_frames(duration.toUtf8().constData());
                        producer->set("out", frames - 1);
                        producer->set("length", frames);
                        producer->set("wunjo:duration", duration.toUtf8().constData());
                    }
                }
            } else {
    ...
    if (!producer || producer->is_blank() || !producer->is_valid()) {
        qCDebug(WUNJO_LOG) << " / / / / / / / / ERROR / / / / // CANNOT LOAD PRODUCER: " << resource;
        if (producer) {
            producer.reset();
        }
        auto binClip = pCore->projectItemModel()->getClipByBinID(QString::number(m_owner.itemId));
    ...
    if (pCore->currentDoc()->useExternalProxy() && mltService.contains(QLatin1String("avformat")) && !producer->property_exists("wunjo:proxy")) {
        ...
        if (!original.isEmpty()) {
            ...
            if (proxyAudioStreams != sourceStreams) {
                ...
                producer->set("wunjo:originalurl", original.toUtf8().constData());
                producer->set("wunjo:camcorderproxy", path.toUtf8().constData());
                const QString clipName = QFileInfo(original).fileName();
                producer->set("wunjo:clipname", clipName.toUtf8().constData());
                // Trigger proxy rebuild
                producer->set("_replaceproxy", 1);
                producer->set("_reloadName", 1);
            } else {
                // Match, we opened a proxy clip
                producer->set("wunjo:proxy", path.toUtf8().constData());
                producer->set("wunjo:originalurl", original.toUtf8().constData());
                const QString clipName = QFileInfo(original).fileName();
                producer->set("wunjo:clipname", clipName.toUtf8().constData());
            }
    ...
    QString clipName = Xml::getXmlProperty(m_xml, QStringLiteral("wunjo:clipname"));
    if (clipName.isEmpty()) {
        clipName = QFileInfo(Xml::getXmlProperty(m_xml, QStringLiteral("wunjo:originalurl"))).fileName();
    }
    if (!clipName.isEmpty()) {
        producer->set("wunjo:clipname", clipName.toUtf8().constData());
    }
    QString groupId = Xml::getXmlProperty(m_xml, QStringLiteral("wunjo:folderid"));
    if (!groupId.isEmpty()) {
        producer->set("wunjo:folderid", groupId.toUtf8().constData());
    }
    int clipOut = 0;
    ...
    if ((duration == 0 && (type == ClipType::Text || type == ClipType::TextTemplate || type == ClipType::QText || type == ClipType::Color ||
        ...
        if (m_xml.hasAttribute(QStringLiteral("length"))) {
            ...
            if (isGif && mltService == QLatin1String("qimage")) {
                length = pCore->getDurationFromString(WunjoSettings::image_duration());
                clipOut = qMax(1, length - 1);
            } else {
        ...
        int wunjo_duration = producer->time_to_frames(Xml::getXmlProperty(m_xml, QStringLiteral("wunjo:duration")).toUtf8().constData());
        if (wunjo_duration > 0) {
            producer->set("wunjo:duration", producer->frames_to_time(wunjo_duration, mlt_time_clock));
        } else {
            producer->set("wunjo:duration", producer->frames_to_time(producer->get_int("length")));
        }
    }
    if (clipOut > 0) {
    ...
    if (mltService == QLatin1String("xml") || mltService == QLatin1String("consumer")) {
        ...
        if (hasAudio) {
            if (hasVideo) {
                producer->set("wunjo:clip_type", 0);
            } else {
                producer->set("wunjo:clip_type", 1);
            }
        } else if (hasVideo) {
            producer->set("wunjo:clip_type", 2);
        }
