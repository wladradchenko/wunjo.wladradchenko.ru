...
void CutTask::start(const ObjectId &owner, int in, int out, QObject *object, bool force)
{
    ...
    checkProcess.start(WunjoSettings::ffmpegpath(), params);
    ...
    ui.audio_codec->setText(audioCodec);
    ui.add_clip->setChecked(WunjoSettings::add_new_clip());
    ...
    WunjoSettings::setAdd_new_clip(ui.add_clip->isChecked());
    delete d;
    ...
    CutTask *task = new CutTask(owner, path, encodingParams, in, out, WunjoSettings::add_new_clip(), object);
...
void CutTask::run()
{
    ...
    if (binClip) {
        ...
        m_jobProcess->start(WunjoSettings::ffmpegpath(), params, QIODevice::ReadOnly);
