    ...
    : QDialog(parent)
{
    ...
    int current = m_view.template_list->comboBox()->findText(WunjoSettings::selected_template());
    if (current > -1) {
...
void TitleTemplateDialog::updatePreview()
{
    ...
    WunjoSettings::setSelected_template(m_view.template_list->comboBox()->currentText());
