...

{
    ...
    maxChars->setValue(WunjoSettings::whisperMaxChars());
    check_maxchars->setChecked(WunjoSettings::cutWhisperMaxChars());
    ...
    connect(m_buttonGroup, QOverload<QAbstractButton *>::of(&QButtonGroup::buttonClicked), this,
            [this, selectedTrack = tid, sourceZone = zone, timeline](QAbstractButton *button) {
                ...
                WunjoSettings::setSubtitleMode(m_buttonGroup->checkedId());
    ...
    QAbstractButton *button = m_buttonGroup->button(WunjoSettings::subtitleMode());
    if (button) {
    ...
    connect(speech_model, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this, [this]() {
        if (WunjoSettings::speechEngine() == QLatin1String("whisper")) {
            ...
            WunjoSettings::setVosk_srt_model(speech_model->currentText());
        }
    });
...
bool SpeechDialog::startHeadless(const QString &modelName, const QString &language, int maxLineWidth)
{
    // status() is only refreshed by the async checkSetup() the dialog runs
    // when shown; headless we just verify the venv python binary exists.
    if (!WunjoSettings::speech_system_python() && m_stt->status() != AbstractPythonInterface::Installed &&
        !QFile::exists(m_stt->venvPythonExecs().python)) {
        qWarning() << "startHeadless: speech-to-text environment is not installed";
        return false;
    }
    if (!modelName.isEmpty()) {
        int ix = speech_model->findData(modelName);
        if (ix == -1) {
            ix = speech_model->findText(modelName, Qt::MatchFixedString);
        }
        if (ix > -1) {
            speech_model->setCurrentIndex(ix);
        }
    }
    if (!language.isEmpty() && speech_language->isEnabled()) {
        const int ix = speech_language->findData(language);
        if (ix > -1) {
            speech_language->setCurrentIndex(ix);
        }
    }
    translate_seamless->setChecked(false);
    translate_box->setChecked(false);
    check_maxchars->setChecked(maxLineWidth > 0);
    if (maxLineWidth > 0) {
        maxChars->setValue(maxLineWidth);
    }
    slotProcessSpeech();
    if (!m_speechJob || m_speechJob->state() == QProcess::NotRunning) {
        return false; // audio export or process start failed
    }
    // Never shown; free the dialog once the transcription job is done
    connect(m_speechJob.get(), static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(&QProcess::finished), this, &QObject::deleteLater);
    return true;
}
...
void SpeechDialog::buildSpeechModelsList(SpeechToTextEngine::EngineType engine, const QStringList models)
{
    ...
    if (engine == SpeechToTextEngine::EngineWhisper) {
        ...
        int ix = speech_model->findData(WunjoSettings::whisperModel());
        if (ix > -1) {
            speech_model->setCurrentIndex(ix);
        }
        translate_box->setEnabled(WunjoSettings::whisperModel() != QLatin1String("turbo"));
        translate_seamless->setEnabled(WunjoSettings::enableSeamless());
        translate_seamless->setChecked(WunjoSettings::srtSeamlessTranslate());
        ...
        if (WunjoSettings::enableSeamless()) {
        ...
        if (speech_language->count() == 0) {
            ...
            int ix = speech_language->findData(WunjoSettings::whisperLanguage());
            if (ix > -1) {
                speech_language->setCurrentIndex(ix);
            }
        }
        speech_language->setEnabled(!WunjoSettings::whisperModel().endsWith(QLatin1String(".en")));
        translate_box->setChecked(WunjoSettings::whisperTranslate());
    } else {
        // Vosk
        speech_model->addItems(models);
        if (!WunjoSettings::vosk_srt_model().isEmpty() && models.contains(WunjoSettings::vosk_srt_model())) {
            int ix = speech_model->findText(WunjoSettings::vosk_srt_model());
            if (ix > -1) {
                speech_model->setCurrentIndex(ix);
            }
        }
...
void SpeechDialog::slotProcessSpeech()
{
    if (translate_seamless->isChecked()) {
        WunjoSettings::setSrtSeamlessTranslate(true);
        WunjoSettings::setSeamless_input(seamless_in->currentData().toString());
        WunjoSettings::setSeamless_output(seamless_out->currentData().toString());
    } else {
        WunjoSettings::setSrtSeamlessTranslate(false);
    }
    buttonBox->button(QDialogButtonBox::Apply)->setEnabled(false);
    ...
    if (WunjoSettings::speechEngine() == QLatin1String("whisper")) {
        ...
        if (check_maxchars->isChecked() && check_maxchars->isEnabled()) {
            ...
            WunjoSettings::setWhisperMaxChars(maxCount);
        }
        WunjoSettings::setCutWhisperMaxChars(check_maxchars->isChecked());
        QStringList arguments = {m_stt->subtitleScript(), audio, modelName, QStringLiteral("ffmpeg_path=%1").arg(WunjoSettings::ffmpegpath())};
        if (!WunjoSettings::whisperDevice().isEmpty()) {
            arguments << QStringLiteral("device=%1").arg(WunjoSettings::whisperDevice());
        }
        if (translate_seamless->isChecked()) {
...
void SpeechDialog::fillSeamlessLanguages()
{
    ...
    if (!WunjoSettings::seamless_input().isEmpty()) {
        int ix = seamless_in->findData(WunjoSettings::seamless_input());
        int outIx = seamless_out->findData(WunjoSettings::seamless_output());
        if (ix > -1) {
