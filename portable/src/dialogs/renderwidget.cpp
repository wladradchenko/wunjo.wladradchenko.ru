    ...
    , m_blockProcessing(false)
{
    ...
    int ix = m_view.interp_type->findData(WunjoSettings::renderInterp());
    if (ix > -1) {
    ...
    connect(m_view.interp_type, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
            [&]() { WunjoSettings::setRenderInterp(m_view.interp_type->currentData().toString()); });
    ...
    ix = m_view.deinterlacer_type->findData(WunjoSettings::renderDeinterlacer());
    if (ix > -1) {
    ...
    connect(m_view.deinterlacer_type, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
            [&]() { WunjoSettings::setRenderDeinterlacer(m_view.deinterlacer_type->currentData().toString()); });
    ...
    m_view.encoder_threads->setValue(WunjoSettings::encodethreads());
    connect(m_view.encoder_threads, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this, &WunjoSettings::setEncodethreads);
    ...
    m_view.processing_threads->setValue(WunjoSettings::processingthreads());
    connect(m_view.processing_threads, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this, &WunjoSettings::setProcessingthreads);
    connect(m_view.processing_threads, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this, &RenderWidget::refreshParams);
    if (!WunjoSettings::parallelrender()) {
        m_view.processing_warning->hide();
    }
    m_view.processing_box->setChecked(WunjoSettings::parallelrender());
    ...
    connect(m_view.processing_box, &QGroupBox::toggled, this, [&](bool checked) {
        WunjoSettings::setParallelrender(checked);
        if (checked) {
    ...
    if (WunjoSettings::wunjorendererpath().isEmpty() || !QFileInfo::exists(WunjoSettings::wunjorendererpath())) {
        WunjoSettings::setWunjorendererpath(QString());
        Wizard::fixWunjoRenderPath();
    } else {
        // Ensure the Wunjo renderer is in the same folder as the running app
        qDebug() << ":::::::::: COMPARING RENDERER PATH: " << QCoreApplication::applicationDirPath()
                 << " != " << QFileInfo(WunjoSettings::wunjorendererpath()).absolutePath();
        if (QCoreApplication::applicationDirPath() != QFileInfo(WunjoSettings::wunjorendererpath()).absolutePath()) {
            WunjoSettings::setWunjorendererpath(QString());
            Wizard::fixWunjoRenderPath();
        }
    }
    if (WunjoSettings::wunjorendererpath().isEmpty()) {
        KMessageBox::error(this, i18n("Could not find the wunjo_render application, something is wrong with your installation. Rendering will not work"));
    }
    ...
    m_view.keep_log_files->setChecked(WunjoSettings::keepRenderLogFiles());
    connect(m_view.keep_log_files, &QCheckBox::toggled, this, [](bool enabled) { WunjoSettings::setKeepRenderLogFiles(enabled); });
...
void RenderWidget::slotUpdateRescaleWidth(int val)
{
    ...
    WunjoSettings::setDefaultrescalewidth(val);
    ...
    WunjoSettings::setDefaultrescaleheight(m_view.rescale_height->value());
    refreshParams();
}
...
void RenderWidget::slotUpdateRescaleHeight(int val)
{
    ...
    WunjoSettings::setDefaultrescaleheight(val);
    std::unique_ptr<ProfileModel> &profile = pCore->getCurrentProfile();
    ...
    WunjoSettings::setDefaultrescaleheight(m_view.rescale_width->value());
    refreshParams();
}
...
void RenderWidget::focusItem(const QString &profile)
{
    ...
    if (!index.isValid() && RenderPresetRepository::get()->presetExists(WunjoSettings::renderProfile())) {
        index = m_treeModel->findPreset(WunjoSettings::renderProfile());
    }
    if (index.isValid()) {
...
void RenderWidget::slotPrepareExport(bool delayedRendering)
{
    ...
    if (!QFile::exists(WunjoSettings::meltpath())) {
        m_view.infoMessage->setMessageType(KMessageWidget::Warning);
...
void RenderWidget::slotPrepareExport2(bool delayedRendering)
{
    ...
    if (m_view.render_zone->isChecked()) {
        Monitor *pMon = pCore->getMonitor(Wunjo::ProjectMonitor);
...
void RenderWidget::startRendering(RenderJobItem *item)
{
    ...
    if (!WunjoSettings::hwDecoding().isEmpty()) {
        env.insert(QLatin1String("MLT_AVFORMAT_HWACCEL"), WunjoSettings::hwDecoding());
    }
    proc.setProgram(WunjoSettings::wunjorendererpath());
    proc.setProcessEnvironment(env);
    if (WunjoSettings::keepRenderLogFiles()) {
...
{
    m_view.rescale_width->setValue(WunjoSettings::defaultrescalewidth() + WunjoSettings::defaultrescalewidth() % 2);
    m_view.rescale_height->setValue(WunjoSettings::defaultrescaleheight() + WunjoSettings::defaultrescaleheight() % 2);
    refreshView();
}
...
void RenderWidget::slotChangeSelection(const QModelIndex &current, const QModelIndex &previous)
{
    ...
    WunjoSettings::setRenderProfile(m_currentProfile);
    loadProfile();
}
...
void RenderWidget::refreshParams()
{
    ...
    // Parallel Processing
    int threadCount = WunjoSettings::processingthreads();
    ...
    if (!m_params.contains(QStringLiteral("threads"))) {
        int ffmpegThreads = WunjoSettings::encodethreads();
...
void RenderWidget::parseScriptFiles(const QString lastScript)
{
    ...
    if (!projectFolder.exists(QStringLiteral("wunjo-renderqueue"))) {
        return;
    }
    projectFolder.cd(QStringLiteral("wunjo-renderqueue"));
    QStringList scriptFiles = projectFolder.entryList(scriptsFilter, QDir::Files);
    if (scriptFiles.isEmpty()) {
        // No scripts, delete directory
        if (projectFolder.dirName() == QStringLiteral("wunjo-renderqueue") &&
...
void RenderWidget::slotStartScript()
{
    ...
    if (item) {
        ...
        renderItem->setData(1, StartTimeRole, t);
        renderItem->setData(1, LastTimeRole, t);
        QStringList argsJob = {QStringLiteral("delivery"), WunjoSettings::meltpath(), path, QStringLiteral("--pid"),
...
void RenderWidget::slotDeleteScript()
{
    ...
    if (item) {
        ...
        if (!success) {
            qCWarning(WUNJO_LOG) << "// Error removing script or playlist: " << path << ", " << path << ".mlt";
        }
        parseScriptFiles();
    }
}
...
bool RenderWidget::startWaitingRenderJobs()
{
...
#endif
    QTemporaryFile tmp(QDir::temp().absoluteFilePath(QStringLiteral("wunjo-XXXXXX") + ScriptFormat));
    if (!tmp.open()) {
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qCWarning(WUNJO_LOG) << "//////  ERROR writing to file: " << autoscriptFile;
    ...
    while (item != nullptr) {
        if (item->status() == WAITINGJOB) {
            ...
            outStream << '\"' << WunjoSettings::wunjorendererpath() << "\" " << params << '\n';
        }
        item = static_cast<RenderJobItem *>(m_view.running_jobs->itemBelow(item));
    }
