...
class WunjoSettingsDialog : public KConfigDialog
{
    Q_OBJECT

public:
    WunjoSettingsDialog(QMap<QString, QString> mappable_actions, bool gpuAllowed, QWidget *parent = nullptr);
    ~WunjoSettingsDialog() override;
    void showPage(Wunjo::ConfigPage page, int option);
    /** @brief Raise the settings tab of one installed plugin. */
    void showPluginTab(const QString &pluginId);
    void checkProfile();
    /** @brief update wunjo settings for external app path if they were changed outside config dialog */
...
private:
    ...
    KPageWidgetItem *m_pageAppearance;
    KPageWidgetItem *m_pageSpeech;

    QComboBox *m_appThemeCombo{nullptr};
    KColorButton *m_accentButton{nullptr};
    ...
    void initColorsPage();
    /** @brief Init the Appearance page (theme selector + primary accent color). */
...
Q_SIGNALS:
    ...
    /** @brief A settings changed that requires a Wunjo restart, trigger it */
    void restartWunjo(bool resetConfig = false);
