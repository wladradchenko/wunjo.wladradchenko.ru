...
public:
    ...
    /** @brief Run the recognition pipeline without showing the dialog (the
     *  scripting API path). Configures the fields programmatically, starts
     *  processing and self-deletes when the transcription job finishes.
     *  @return false when speech deps are not installed or export failed. */
    bool startHeadless(const QString &modelName, const QString &language, int maxLineWidth);
