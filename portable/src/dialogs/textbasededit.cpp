...
void VideoTextEdit::blockClicked(Qt::KeyboardModifiers modifiers, bool play)
{
    if (m_hoveredBlock > -1 && m_hoveredBlock < speechZones.count()) {
        ...
        pCore->getMonitor(Wunjo::ClipMonitor)->requestSeek(GenTime(startMs).frames(pCore->getCurrentFps()));
        pCore->getMonitor(Wunjo::ClipMonitor)
        ...
        if (play) {
            pCore->getMonitor(Wunjo::ClipMonitor)->slotPlayZone();
        }
    }
}
    ...
    , m_stt(nullptr)
{
    ...
    m_translateAction->setEnabled(WunjoSettings::whisperModel() != QLatin1String("turbo"));
    ...
    connect(m_translateAction, &QAction::triggered, [](bool enabled) { WunjoSettings::setWhisperTranslate(enabled); });
    connect(configAction, &QAction::triggered, []() { pCore->window()->slotShowPreferencePage(Wunjo::PageSpeech); });
    connect(menu, &QMenu::aboutToShow, this, [this]() {
        m_translateAction->setChecked(WunjoSettings::whisperTranslate());
        // Whisper turbo model does not handle translation
        m_translateAction->setEnabled(WunjoSettings::speechEngine() == QLatin1String("whisper") &&
                                      WunjoSettings::whisperModel() != QLatin1String("turbo"));
    });
    ...
    connect(m_speechConfig, &QAction::triggered, this, [this]() {
        ...
        pCore->window()->slotShowPreferencePage(Wunjo::PageSpeech);
    });
    ...
    connect(pCore.get(), &Core::speechModelUpdate, this, [&](SpeechToTextEngine::EngineType engine, const QStringList &models) {
        ...
        if (engine == SpeechToTextEngine::EngineVosk && WunjoSettings::speechEngine() == QLatin1String("vosk")) {
    ...
    connect(m_modelsMenu, &QMenu::triggered, this, [this](QAction *a) {
        if (WunjoSettings::speechEngine() == QLatin1String("whisper")) {
            ...
            WunjoSettings::setWhisperModel(modelName);
            m_translateAction->setEnabled(modelName != QLatin1String("turbo"));
        } else {
            WunjoSettings::setVosk_text_model(a->data().toString());
        }
    });
    info_message->hide();
    updateEngine();
    ...
    speech_zone->setChecked(WunjoSettings::speech_zone());
    connect(speech_zone, &QCheckBox::checkStateChanged, [](Qt::CheckState state) { WunjoSettings::setSpeech_zone(state == Qt::Checked); });
...
void TextBasedEdit::slotZoomIn()
{
    ...
    WunjoSettings::setSubtitleEditFontSize(fontSize);
    m_visualEditor->setFontPointSize(WunjoSettings::subtitleEditFontSize());
...
{
    if (WunjoSettings::subtitleEditFontSize() > 0) {
        QTextCursor cursor = m_visualEditor->textCursor();
        m_visualEditor->selectAll();
        m_visualEditor->setFontPointSize(WunjoSettings::subtitleEditFontSize());
        m_visualEditor->setTextCursor(cursor);
...
void TextBasedEdit::updateEngine()
{
    ...
    if (WunjoSettings::speechEngine() == QLatin1String("whisper")) {
        ...
        int ix = language_box->findData(WunjoSettings::whisperLanguage());
        if (ix > -1) {
            language_box->setCurrentIndex(ix);
        }
        language_box->setEnabled(!WunjoSettings::whisperModel().endsWith(QLatin1String(".en")));
...
void TextBasedEdit::buildVoskModelsList(const QStringList models)
{
    ...
    if (!WunjoSettings::vosk_text_model().isEmpty() && models.contains(WunjoSettings::vosk_text_model())) {
        for (auto &a : acts) {
            if (a->data().toString() == WunjoSettings::vosk_text_model()) {
                a->setChecked(true);
                found = true;
                break;
            }
...
void TextBasedEdit::buildWhisperModelsList(const QStringList whisperModels)
{
    ...
    for (auto &w : whisperModels) {
        ...
        a->setData(w);
        m_modelsGroup->addAction(a);
        if (w == WunjoSettings::whisperModel()) {
            a->setChecked(true);
            found = true;
        }
    }
...
void TextBasedEdit::startRecognition()
{
    ...
    if (WunjoSettings::speechEngine() == QLatin1String("whisper")) {
        // Whisper engine
        if (!WunjoSettings::speech_system_python() && (!m_stt->checkSetup() || !m_stt->missingDependencies({QStringLiteral("openai-whisper")}).isEmpty())) {
        ...
        if (WunjoSettings::whisperDisableFP16()) {
            language.append(QStringLiteral(" fp16=False"));
        }
    } else {
        // VOSK engine
        if (!WunjoSettings::speech_system_python() && (!m_stt->checkSetup() || !m_stt->missingDependencies({QStringLiteral("vosk")}).isEmpty())) {
            showMessage(i18n("Please configure speech to text."), KMessageWidget::Warning, m_speechConfig);
            return;
        }
        // Start python script
    ...
    m_binId = pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId();
    ...
    if (clip->clipType() == ClipType::Playlist) {
        ...
        m_playlistWav.setFileTemplate(QDir::temp().absoluteFilePath(QStringLiteral("wunjo-XXXXXX.wav")));
                    ...
                    if (WunjoSettings::speechEngine() == QLatin1String("whisper")) {
                        ...
                        args = {m_stt->speechScript(),
                                ...
                                QStringLiteral("--task=%1")
                                    .arg(WunjoSettings::whisperTranslate() && m_translateAction->isEnabled() ? QStringLiteral("translate")
                        ...
                        if (speech_zone->isChecked()) {
                            m_tmpCutWav.setFileTemplate(QDir::temp().absoluteFilePath(QStringLiteral("wunjo-XXXXXX.wav")));
                            if (!m_tmpCutWav.open()) {
                                showMessage(i18n("Cannot create temporary file."), KMessageWidget::Warning);
                                return;
                            }
        ...
        m_tCodeJob->start(WunjoSettings::meltpath(),
        ...
        if (WunjoSettings::speechEngine() == QLatin1String("whisper")) {
            ...
            args = {
                ...
                QStringLiteral("--model=%1").arg(modelName),
                QStringLiteral("--task=%1")
                    .arg(WunjoSettings::whisperTranslate() && m_translateAction->isEnabled() ? QStringLiteral("translate") : QStringLiteral("transcribe")),
                QStringLiteral("--language=%1").arg(language),
                QStringLiteral("--ffmpeg_path=%1").arg(WunjoSettings::ffmpegpath())};
            if (!WunjoSettings::whisperDevice().isEmpty()) {
                args << QStringLiteral("--device=%1").arg(WunjoSettings::whisperDevice());
            }
            connect(m_speechJob.get(), &QProcess::readyReadStandardOutput, this, &TextBasedEdit::slotProcessWhisperSpeech);
...
void TextBasedEdit::slotProcessSpeechStatus(int, QProcess::ExitStatus status)
{
    ...
    if (status == QProcess::CrashExit) {
        ...
        if (clip) {
            ...
            QString oldSpeech;
            if (clipItem) {
                oldSpeech = clipItem->getProducerProperty(QStringLiteral("wunjo:speech"));
            }
            QMap<QString, QString> oldProperties;
            oldProperties.insert(QStringLiteral("wunjo:speech"), oldSpeech);
            QMap<QString, QString> properties;
            properties.insert(QStringLiteral("wunjo:speech"), m_visualEditor->toHtml());
...
void TextBasedEdit::previewPlaylist(bool createNew)
{
    ...
    QMap<QString, QString> properties;
    properties.insert(QStringLiteral("wunjo:baseid"), m_binId);
    QStringList playZones;
    for (const auto &p : std::as_const(zones)) {
        playZones << QStringLiteral("%1:%2").arg(p.x()).arg(p.y());
    }
    properties.insert(QStringLiteral("wunjo:cutzones"), playZones.join(QLatin1Char(';')));
    if (createNew) {
        int ix = 1;
        m_playlist = QStringLiteral("%1-cut%2.wmproj").arg(sourcePath).arg(ix);
...
void TextBasedEdit::openClip(std::shared_ptr<ProjectClip> clip)
{
    ...
    if (clip && clip->isValid() && clip->hasAudio()) {
        ...
        QString refId = clip->getProducerProperty(QStringLiteral("wunjo:baseid"));
        if (!refId.isEmpty() && refId == m_refId) {
            // We opened a resulting playlist, do not clear text edit
            // TODO: this is broken. We should try reading the wunjo:speech data from the sequence xml
            return;
        }
        ...
        if (!refId.isEmpty()) {
            ...
            if (refClip) {
                speech = refClip->getProducerProperty(QStringLiteral("wunjo:speech"));
                clipNameLabel->setText(refClip->clipName());
            }
            QStringList zones = clip->getProducerProperty("wunjo:cutzones").split(QLatin1Char(';'));
            for (const QString &z : std::as_const(zones)) {
                cutZones << QPoint(z.section(QLatin1Char(':'), 0, 0).toInt(), z.section(QLatin1Char(':'), 1, 1).toInt());
            }
        } else {
            m_refId.clear();
            speech = clip->getProducerProperty(QStringLiteral("wunjo:speech"));
...
void TextBasedEdit::addBookmark()
{
    ...
    if (zones.count() == 1) {
        int monitorPos = pCore->getMonitor(Wunjo::ClipMonitor)->position();
