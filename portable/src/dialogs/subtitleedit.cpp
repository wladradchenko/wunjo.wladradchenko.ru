    ...
    , m_model(nullptr)
{
    ...
    buttonAdd->setWhatsThis(xi18nc("@info:whatsthis", "Creates a new subtitle with the default length (set in <interface>Settings->Configure "
                                                      "Wunjo…->Misc</interface>) at the current playhead position/frame."));
...
void SubtitleEdit::applyFontSize()
{
    ...
    if (WunjoSettings::subtitleEditFontSize() > 0 && fontSize != WunjoSettings::subtitleEditFontSize()) {
        if (m_isSimpleEdit) {
            QTextCursor cursor = simpleSubText->textCursor();
            simpleSubText->selectAll();
            simpleSubText->setFontPointSize(WunjoSettings::subtitleEditFontSize());
            simpleSubText->setTextCursor(cursor);
        } else {
            subText->selectAll();
            subText->setFontPointSize(WunjoSettings::subtitleEditFontSize());
            QTextCursor cursor = subText->textCursor();
...
void SubtitleEdit::goToPrevious()
{
    if (m_model) {
        ...
        if (id > -1) {
            ...
            pCore->getMonitor(Wunjo::ProjectMonitor)->requestSeek(prev.frames(pCore->getCurrentFps()));
