    ...
    , m_clip(clip)
{
    ...
    interval->setValue(GenTime(WunjoSettings::multipleguidesinterval()));
    ...
    connect(&m_watcher, &QFutureWatcherBase::finished, this, [this] {
        ...
        if (!thumb.isNull()) {
            ...
            qCDebug(WUNJO_LOG) << "!!!!!!!!!!!  ERROR CREATING THUMB";
        }
    });
...
{
    WunjoSettings::setDefault_marker_type(marker_category->currentCategory());
