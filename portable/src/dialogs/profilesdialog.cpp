...
void ProfilesDialog::fillList(const QString &selectedProfile)
{
    ...
    if (!WunjoSettings::default_profile().isEmpty()) {
        int ix = m_view.profiles_list->findData(WunjoSettings::default_profile());
        if (ix > -1) {
            m_view.profiles_list->setCurrentIndex(ix);
        } else {
            // Error, profile not found
            qCWarning(WUNJO_LOG) << "Project profile not found, disable  editing";
        }
    }
...
void ProfilesDialog::slotSetDefaultProfile()
{
    ...
    if (!path.isEmpty()) {
        WunjoSettings::setDefault_profile(path);
    }
}
