...
void TimeRemap::checkClipUpdate(const QModelIndex &topLeft, const QModelIndex &, const QVector<int> &roles)
{
    ...
    if (!m_view->movingKeyframe()) {
        ObjectId oid(WunjoObjectType::TimelineClip, m_cid, m_uuid);
...
void TimeRemap::selectedClip(int cid, const QUuid uuid)
{
    ...
    disconnect(pCore->getMonitor(Wunjo::ClipMonitor), &Monitor::seekRemap, m_view, &RemapView::slotSetPosition);
    disconnect(pCore->getMonitor(Wunjo::ProjectMonitor), &Monitor::seekPosition, this, &TimeRemap::monitorSeek);
    if (!m_uuid.isNull()) {
    ...
    connect(pCore->getMonitor(Wunjo::ClipMonitor), &Monitor::seekRemap, m_view, &RemapView::slotSetPosition, Qt::UniqueConnection);
    ...
    ObjectId oid(WunjoObjectType::TimelineClip, cid, m_uuid);
    ...
    if (!m_binId.isEmpty() && pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId() == m_binId) {
        connect(pCore->getMonitor(Wunjo::ClipMonitor), &Monitor::seekPosition, pCore->getMonitor(Wunjo::ClipMonitor), &Monitor::seekRemap,
                Qt::UniqueConnection);
    }
    m_seekConnection1 = connect(m_view, &RemapView::seekToPos, this, [this](int topPos, int bottomPos) {
        if (topPos > -1) {
            if (pCore->getMonitor(Wunjo::ClipMonitor)->activeClipId() != m_binId) {
                int min = pCore->getItemIn(ObjectId(WunjoObjectType::TimelineClip, m_cid, m_uuid));
                int lastLength = pCore->getItemDuration(ObjectId(WunjoObjectType::TimelineClip, m_cid, m_uuid));
                int max = min + lastLength;
                pCore->selectBinClip(m_binId, true, min, {min, max});
            }
            pCore->getMonitor(Wunjo::ClipMonitor)->requestSeekIfVisible(topPos);
        }
        if (bottomPos > -1) {
            pCore->getMonitor(Wunjo::ProjectMonitor)->requestSeek(bottomPos + m_view->m_startPos);
        }
    });
    connect(pCore->getMonitor(Wunjo::ProjectMonitor), &Monitor::seekPosition, this, &TimeRemap::monitorSeek, Qt::UniqueConnection);
}
...
void TimeRemap::setClip(std::shared_ptr<ProjectClip> clip, int in, int out)
{
    ...
    m_seekConnection1 = connect(m_view, &RemapView::seekToPos, pCore->getMonitor(Wunjo::ClipMonitor), &Monitor::requestSeek, Qt::UniqueConnection);
    m_seekConnection2 = connect(pCore->getMonitor(Wunjo::ClipMonitor), &Monitor::seekPosition, this, [&](int pos) { m_view->slotSetPosition(pos); });
