...
// static
void ClipCreationDialog::createColorClip(WunjoDoc *doc, const QString &parentFolder, std::shared_ptr<ProjectItemModel> model,
                                         const std::function<void(const QString &)> &readyCallBack, int suggestedDuration)
{
    ...
    int defaultDuration = pCore->getDurationFromString(WunjoSettings::color_duration());
    ...
    dia_ui.clip_color->setColor(WunjoSettings::colorclipcolor());
    ...
    if (dia->exec() == QDialog::Accepted) {
        ...
        WunjoSettings::setColorclipcolor(color);
...
void ClipCreationDialog::createAnimationClip(WunjoDoc *doc, const QString &parentId, const std::function<void(const QString &)> &readyCallBack,
                                             int suggestedDuration)
{
    ...
    QDir dir;
    QString path = KRecentDirs::dir(QStringLiteral(":WunjoAnimationFolder"));
    if (path.isEmpty()) {
    ...
    KRecentDirs::add(QStringLiteral(":WunjoAnimationFolder"), QFileInfo(fileName).absolutePath());
    ...
    if (!parentId.isEmpty()) {
        properties.insert(QStringLiteral("wunjo:folderid"), parentId);
    }
                                         ...
                                         int suggestedDuration)
{
    ...
    if (dia->exec() == QDialog::Accepted) {
        // WunjoSettings::setColorclipcolor(color);
        QMap<QString, QString> properties;
        properties.insert(QStringLiteral("wunjo:clipname"), dia_ui.name->text());
        if (!parentId.isEmpty()) {
            properties.insert(QStringLiteral("wunjo:folderid"), parentId);
        }
        ...
        if (clip && (newDuration != clip->getFramePlaytime())) {
            ...
            if (currentLength != newDuration) {
                properties.insert(QStringLiteral("wunjo:duration"), clip->framesToTime(newDuration));
...
// static
void ClipCreationDialog::createSlideshowClip(WunjoDoc *doc, const QString &parentId, std::shared_ptr<ProjectItemModel> model,
                                             const std::function<void(const QString &)> &readyCallBack, int suggestedDuration)
{
    QScopedPointer<SlideshowClip> dia(
        new SlideshowClip(doc->timecode(), KRecentDirs::dir(QStringLiteral(":WunjoSlideShowFolder")), nullptr, QApplication::activeWindow()));

    if (dia->exec() == QDialog::Accepted) {
        // Ready, create xml
        KRecentDirs::add(QStringLiteral(":WunjoSlideShowFolder"), QUrl::fromLocalFile(dia->selectedPath()).adjusted(QUrl::RemoveFilename).toLocalFile());
        WunjoSettings::setSlideshowmimeextension(dia->extension());
...
void ClipCreationDialog::createTitleClip(WunjoDoc *doc, const QString &parentFolder, const QString &templatePath, std::shared_ptr<ProjectItemModel> model,
                                         const std::function<void(const QString &)> &readyCallBack, int suggestedDuration)
{
    ...
    QPointer<TitleWidget> dia_ui =
        new TitleWidget(QUrl::fromLocalFile(templatePath), dir.absolutePath(), pCore->getMonitor(Wunjo::ProjectMonitor), pCore->bin());
    if (suggestedDuration > 0) {
        int duration = qMin(suggestedDuration, doc->getFramePos(WunjoSettings::title_duration()));
...
void ClipCreationDialog::createTitleTemplateClip(WunjoDoc *doc, const QString &parentFolder, std::shared_ptr<ProjectItemModel> model,
                                                 const std::function<void(const QString &)> &readyCallBack, int suggestedDuration)
{
...
// void ClipCreationDialog::createClipsCommand(WunjoDoc *doc, const QList<QUrl> &urls, const QStringList &groupInfo, Bin *bin,
...
void ClipCreationDialog::createClipsCommand(WunjoDoc *doc, const QString &parentFolder, const std::shared_ptr<ProjectItemModel> &model,
                                            const std::function<void(const QString &)> &readyCallBack, int suggestedDuration)
{
    ...
    b->setWhatsThis(
        xi18nc("@info:whatsthis", "When enabled, Wunjo will look for other images with the same name pattern and import them as an image sequence."));
    b->setChecked(WunjoSettings::autoimagesequence());
    QCheckBox *bf = new QCheckBox(i18n("Ignore subfolder structure"));
    bf->setChecked(WunjoSettings::ignoresubdirstructure());
    bf->setToolTip(i18n("Do not create subfolders in Project Bin"));
    bf->setWhatsThis(
        xi18nc("@info:whatsthis",
               "When enabled, Wunjo will import all clips contained in the folder and its subfolders without creating the subfolders in Project Bin."));
    QFrame *f = new QFrame();
    ...
    if (result == QDialog::Accepted) {
        WunjoSettings::setAutoimagesequence(b->isChecked());
        WunjoSettings::setIgnoresubdirstructure(bf->isChecked());
        list = fileWidget->selectedUrls();
        if (!list.isEmpty()) {
            KRecentDirs::add(QStringLiteral(":WunjoClipFolder"), list.constFirst().adjusted(QUrl::RemoveFilename).toLocalFile());
        }
        if (WunjoSettings::autoimagesequence() && list.count() >= 1) {
            ...
            if (!fileName.isEmpty() && fileName.at(fileName.size() - 1).isDigit()) {
                ...
                if (item.mimetype().startsWith(QLatin1String("image"))) {
                    ...
                    qCDebug(WUNJO_LOG) << " / // IMPORT PATTERN: " << pattern << " COUNT: " << patternlist.count();
                    ...
                    if (count > 1) {
                        ...
                        QString duration = doc->timecode().reformatSeparators(WunjoSettings::sequence_duration());
                        std::unordered_map<QString, QString> properties;
