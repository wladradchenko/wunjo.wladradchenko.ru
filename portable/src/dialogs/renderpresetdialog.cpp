    ...
    , m_manualPreset(false)
{
    ...
    helpButton->setChecked(WunjoSettings::showRenderTextParameters());
    parameters->setVisible(WunjoSettings::showRenderTextParameters());
    connect(helpButton, &QPushButton::toggled, this, [this](bool checked) {
        parameters->setVisible(checked);
        WunjoSettings::setShowRenderTextParameters(checked);
    });
    ...
    if (false && m_monitor == nullptr) {
        ...
        /*m_monitor = new Monitor(Wunjo::RenderMonitor, pCore->monitorManager(), this);
        ...
        pCore->monitorManager()->activateMonitor(Wunjo::RenderMonitor);
        m_monitor->start();*/
    }
}
