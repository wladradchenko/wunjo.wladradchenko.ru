...
{
    setWindowTitle(i18nc("@title:window", "Welcome to Wunjo"));
    ...
    m_page->setTitle(i18n("Welcome to Wunjo %1", QString(WUNJO_VERSION)));
    m_page->setSubTitle(i18n("Using MLT %1", mlt_version_get_string()));
    setPixmap(QWizard::LogoPixmap, QIcon::fromTheme(QStringLiteral(":/pics/wunjo.png")).pixmap(logoHeight, logoHeight));
    ...
    if (m_errors.isEmpty()) {
        ...
        const QStringList gpu = pCore->getMonitor(Wunjo::ClipMonitor)->getGPUInfo();
        ...
        m_startLayout->addWidget(lab);
        // HW accel
        const QString detectedCodecs = WunjoSettings::supportedHWCodecs().join(QLatin1Char(' '));
        ...
        if (!WunjoSettings::hwDecoding().isEmpty()) {
            cb_decoding->setChecked(true);
            cb_decoding->setText(i18n("Enable hardware decoding: %1", WunjoSettings::hwDecoding()));
        }
        connect(cb_decoding, &QCheckBox::toggled, this, [this, cb_vaapi, cb_nvidia, cb_amf, cb_qsv, cb_videotoolbox, cb_decoding](bool enabled) {
            if (!enabled) {
                WunjoSettings::setHwDecoding(QString());
                Q_EMIT pCore->updateHwDecoding();
            } else if (cb_nvidia->isChecked()) {
                WunjoSettings::setHwDecoding(QStringLiteral("nvdec"));
            } else if (cb_vaapi->isChecked()) {
                WunjoSettings::setHwDecoding(QStringLiteral("vaapi"));
            } else if (cb_videotoolbox->isChecked()) {
                WunjoSettings::setHwDecoding(QStringLiteral("videotoolbox"));
            } else if (cb_qsv->isChecked() || cb_amf->isChecked()) {
                WunjoSettings::setHwDecoding(QStringLiteral("d3d11va"));
            }
            if (WunjoSettings::hwDecoding().isEmpty()) {
        ...
        connect(pb, &QPushButton::clicked, this, [&, cb_vaapi, cb_nvidia, cb_amf, cb_qsv, cb_videotoolbox, cb_decoding, pb]() {
            ...
            if (cb_decoding->isChecked()) {
                const QString lastDecoder = WunjoSettings::hwDecoding();
                ...
                if (lastDecoder != WunjoSettings::hwDecoding()) {
                    Q_EMIT pCore->updateHwDecoding();
                }
                if (WunjoSettings::hwDecoding().isEmpty()) {
                    cb_decoding->setChecked(false);
...
void Wizard::checkMltComponents()
{
    ...
    if (!pCore->getMltRepository()) {
        ...
        if (!hasBreeze) {
            ...
            m_warnings.append(
                i18n("<li>Missing package: <b>Breeze</b> icons (breeze-icon-theme)<br/>provides many icons used in Wunjo. Install recommended</li>"));
        }
#endif
        ...
        if (consumersItemList.contains(QStringLiteral("sdl2_audio"))) {
            ...
            WunjoSettings::setSdlAudioBackend(QStringLiteral("sdl2_audio"));
            WunjoSettings::setAudiobackend(QStringLiteral("sdl2_audio"));
#if defined(Q_OS_WIN)
            // Use wasapi by default on Windows
            WunjoSettings::setAudiodrivername(QStringLiteral("wasapi"));
#endif
        } else if (consumersItemList.contains(QStringLiteral("sdl_audio"))) {
            // MLT < 6.6.0
            WunjoSettings::setSdlAudioBackend(QStringLiteral("sdl_audio"));
            WunjoSettings::setAudiobackend(QStringLiteral("sdl_audio"));
        } else if (consumersItemList.contains(QStringLiteral("rtaudio"))) {
            WunjoSettings::setSdlAudioBackend(QStringLiteral("sdl2_audio"));
            WunjoSettings::setAudiobackend(QStringLiteral("rtaudio"));
        } else {
            // SDL module
...
// Static
const QString Wizard::fixWunjoRenderPath()
{
    QString wunjorenderpath;
    QString errorMessage;
    // Find path for Wunjo renderer
#ifdef Q_OS_WIN
    wunjorenderpath = QCoreApplication::applicationDirPath() + QStringLiteral("/wunjo_render.exe");
#else
    wunjorenderpath = QCoreApplication::applicationDirPath() + QStringLiteral("/wunjo_render");
#endif
    if (!QFile::exists(wunjorenderpath)) {
        const QStringList mltpath({QFileInfo(WunjoSettings::meltpath()).canonicalPath(), qApp->applicationDirPath()});
        wunjorenderpath = QStandardPaths::findExecutable(QStringLiteral("wunjo_render"), mltpath);
        if (wunjorenderpath.isEmpty()) {
            wunjorenderpath = QStandardPaths::findExecutable(QStringLiteral("wunjo_render"));
        }
        if (wunjorenderpath.isEmpty()) {
            errorMessage = i18n("<li>Missing app: <b>wunjo_render</b><br/>needed for rendering.</li>");
        }
    }

    if (!wunjorenderpath.isEmpty()) {
        WunjoSettings::setWunjorendererpath(wunjorenderpath);
    }
    return errorMessage;
}
...
void Wizard::slotCheckPrograms(QString &infos, QString &warnings, QString &errors)
{
    ...
    const QStringList mltpath({QFileInfo(WunjoSettings::meltpath()).canonicalPath(), qApp->applicationDirPath()});
    QString exepath;
    if (WunjoSettings::ffmpegpath().isEmpty() || !QFileInfo::exists(WunjoSettings::ffmpegpath())) {
    ...
    QString playpath;
    if (WunjoSettings::ffplaypath().isEmpty() || !QFileInfo::exists(WunjoSettings::ffplaypath())) {
    ...
    QString probepath;
    if (WunjoSettings::ffprobepath().isEmpty() || !QFileInfo::exists(WunjoSettings::ffprobepath())) {
    ...
    if (WunjoSettings::wunjorendererpath().isEmpty() || !QFileInfo::exists(WunjoSettings::wunjorendererpath())) {
        const QString renderError = fixWunjoRenderPath();
    ...
    if (!exepath.isEmpty()) {
        WunjoSettings::setFfmpegpath(exepath);
    }
    if (!playpath.isEmpty()) {
        WunjoSettings::setFfplaypath(playpath);
    }
    if (!probepath.isEmpty()) {
        WunjoSettings::setFfprobepath(probepath);
    }

    // set up some default applications
    QString program;
    if (WunjoSettings::defaultimageapp().isEmpty()) {
...
#endif
        if (!imageBinary.isEmpty()) {
            WunjoSettings::setDefaultimageapp(imageBinary);
        }
    }
    if (WunjoSettings::defaultaudioapp().isEmpty()) {
        ...
        if (!audioBinary.isEmpty()) {
            WunjoSettings::setDefaultaudioapp(audioBinary);
        }
    }
    if (WunjoSettings::glaxnimatePath().isEmpty()) {
...
#endif
        if (!animBinary.isEmpty()) {
            WunjoSettings::setGlaxnimatePath(animBinary);
        }
    }
}
...
void Wizard::installExtraMimes(const QString &baseName, const QStringList &globs)
{
    ...
    if (!mime.isValid() || mime.isDefault()) {
        qCDebug(WUNJO_LOG) << "MIME type " << baseName << " not found";
    } else {
        ...
        if (!packageFile.open(QIODevice::WriteOnly)) {
            qCCritical(WUNJO_LOG) << "Couldn't open" << packageFileName << "for writing";
            return;
        }
...
void Wizard::runUpdateMimeDatabase()
{
    ...
    if (exitCode != 0) {
        qCWarning(WUNJO_LOG) << proc.program() << "exited with error code" << exitCode;
    }
}
...
{
    QString errorMessage;
    if (WunjoSettings::meltpath().isEmpty()) {
        errorMessage.append(i18n("Your MLT installation cannot be found. Install MLT and restart Wunjo.\n"));
    }
...
bool Wizard::checkHwEncoder(const QString &name, const QStringList &args, const QTemporaryFile &file)
{
    ...
    hwEncoders.start(WunjoSettings::ffmpegpath(), args);
...
void Wizard::testHwEncoders()
{
    ...
    for (auto &codec : possibleCodecs) {
        ...
        proc.start(WunjoSettings::ffmpegpath(), args, QIODevice::ReadOnly);
    ...
    WunjoSettings::setSupportedHWCodecs(workingCodecs);
    qDebug() << "==========\nFOUND SUPPORTED CODECS: " << WunjoSettings::supportedHWCodecs();

    // Testing NVIDIA SCALER
    QStringList args3{"-hide_banner", "-filters"};
    qDebug() << "// FFMPEG ARGS: " << args3;
    hwEncoders.start(WunjoSettings::ffmpegpath(), args3);
    ...
    WunjoSettings::setNvScalingEnabled(nvScalingSupported);
}

const QString Wizard::getHWCodecFriendlyName()
{
    const QString hwCodecs = WunjoSettings::supportedHWCodecs().join(QLatin1Char(' '));
...
void Wizard::updateHwStatus()
{
    ...
    bool hwEnabled = !WunjoSettings::supportedHWCodecs().isEmpty();
