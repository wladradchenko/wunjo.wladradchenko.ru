    ...
    , m_wasUpgraded(wasUpgraded)
{
    ...
    m_engine->load(showWelcome ? QUrl(QStringLiteral("qrc:/qt/qml/online/wunjo/make/Splash.qml"))
                               : QUrl(QStringLiteral("qrc:/qt/qml/online/wunjo/make/Simplesplash.qml")));
...
{
    WunjoSettings::setShowWelcome(show);
}
...
{
    // Honor the requested palette: dark = "Wunjo", light = "WunjoLight".
    // WunjoTheme persists the choice and re-applies the stylesheet + accent.
    WunjoTheme::instance()->setDark(dark);
}
