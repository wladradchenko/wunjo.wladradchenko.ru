...

{
    ...
    if (!m_cannotGrab) {
        ...
        // Reading the screen is what this widget does, and this compositor will
        // not allow it. Saying so on the button is the honest outcome; a button
        // that is enabled and silently does nothing is the one to avoid.
        button->setEnabled(false);
        button->setToolTip(i18n("Picking a color from the screen is not available on this display server."));
    }
