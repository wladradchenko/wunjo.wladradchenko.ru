    ...
    , m_max(max)
{
    ...
    // icon-only look; the action names live in the drop-down menu
...
void ProgressButton::applyDefaultAction(QAction *action)
{
    setDefaultAction(action);
    setText(QString());
}

void ProgressButton::actionEvent(QActionEvent *event)
{
...
void ProgressButton::setProgress(int progress)
{
    ...
    if (progress < 0) {
        applyDefaultAction(m_defaultAction);
    ...
    if (!m_timer.isValid() || progress == 0) {
        applyDefaultAction(m_dummyAction);
        m_timer.start();
    }
    if (progress == m_max) {
        applyDefaultAction(m_defaultAction);
...
{
    Q_UNUSED(event)
    // Paint the base button ourselves with the text stripped from the style
    // option: the button is icon-only, action names live in the menu (the
    // style re-copies action text onto the button, so clearing text()
    // is not enough).
    {
        ...
        stylePainter.drawComplexControl(QStyle::CC_ToolButton, opt);
    }
    if (m_progress < m_iconSize) {
        ...
        painter.drawRoundedRect(rect.x(), rect.bottom() - 7, rect.width(), 6, 3, 3);
    }
}
