    ...
    , m_labelText(label)
{
    ...
    m_scale->setCurrentItem(WunjoSettings::dragvalue_mode());
    m_menu->addAction(m_scale);

    m_directUpdate = new QAction(i18n("Direct update"), this);
    m_directUpdate->setCheckable(true);
    m_directUpdate->setChecked(WunjoSettings::dragvalue_directupdate());
...
void DragValue::slotEditingFinished()
{
    if (m_intEdit) {
        ...
        m_intEdit->blockSignals(false);
        if (!WunjoSettings::dragvalue_directupdate()) {
...
{
    WunjoSettings::setDragvalue_mode(mode);
}

void DragValue::slotSetDirectUpdate(bool directUpdate)
{
    WunjoSettings::setDragvalue_directupdate(directUpdate);
}
...
void CustomLabel::mouseMoveEvent(QMouseEvent *e)
{
    if ((e->buttons() & Qt::LeftButton) && m_dragStartPosition != QPoint(-1, -1)) {
        ...
        if (m_dragMode) {
            if (WunjoSettings::dragvalue_mode() > 0 || !m_showSlider) {
                ...
                if (WunjoSettings::dragvalue_mode() == 2) {
                    diff = (diff > 0 ? 1 : -1) * pow(diff, 2);
                }
                double nv = m_value + diff * m_step;
                if (!qFuzzyCompare(nv, m_value)) {
                    setNewValue(nv, WunjoSettings::dragvalue_directupdate(), false);
                }
            } else {
                double nv;
