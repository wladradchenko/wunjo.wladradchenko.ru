...
private:
    ...
    * @param destroyImage (optional) Whether or not to keep the XImage in m_image
    ...
    /** @brief True when the compositor refuses to hand over screen pixels —
     *  Wayland without the screenshot portal. The button is then disabled and
     *  says why, rather than looking clickable and doing nothing. */
    bool m_cannotGrab{true};
    QRect m_grabRect;
    QPoint m_clickPoint;
