...
static void initFilterDb() {
    ...
    // Lottie animations
    bool allowLottie = WunjoSettings::producerslist().contains("glaxnimate");
    if (allowLottie) {
...
static QStringList query(Category cat)
{
    ...
    case User: {
        const QStringList customs = WunjoSettings::addedExtensions().split(' ', Qt::SkipEmptyParts);
        QStringList user;
        ...
        for (const auto& ext : customs) {
            if (ext.startsWith("*.")) {
                ...
                qCDebug(WUNJO_LOG) << "Unrecognized custom format: " << ext;
            }
        }
    ...
    default:
        qCDebug(WUNJO_LOG) << "Unrecognized category type: " << (int)cat;
        // fallthrough
    case All:
        return {"*"};
    }
