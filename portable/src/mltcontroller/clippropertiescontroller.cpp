...
QMimeData *AnalysisTree::mimeData(const QList<QTreeWidgetItem *> &list) const
{
    ...
    mime->setData(QStringLiteral("wunjo/geometry"), mimeData.toUtf8());
    return mime;
}
    ...
    , m_activeAudioStreams(-1)
{
    ...
    m_tabWidget->setCurrentIndex(WunjoSettings::properties_panel_page());
...
QHBoxLayout *ClipPropertiesController::durationProperty(const QString &label, const QString &propertyName)
{
    ...
    int wunjo_length = m_properties->time_to_frames(m_properties->get("wunjo:duration"));
    if (wunjo_length > 0) {
        m_originalProperties.insert(QStringLiteral("wunjo:duration"), m_properties->get("wunjo:duration"));
    }
    m_originalProperties.insert(QStringLiteral("length"), m_properties->get("length"));
    timePos->setValue(wunjo_length > 0 ? wunjo_length : m_properties->get_int("length"));

    int original_length = m_properties->get_int("wunjo:original_length");
...
QHBoxLayout *ClipPropertiesController::timecodeProperty(const QString &label, const QString &propertyName)
{
    ...
    int propertyValue = 0;
    if (propertyName.startsWith(QLatin1String("wunjo:sequenceproperties."))) {
        const QUuid uuid(m_properties->get("wunjo:uuid"));
...
QWidget *ClipPropertiesController::constructPropertiesPage()
{
    ...
    if (m_type == ClipType::Timeline) {
        ...
        auto hlay = timecodeProperty(i18n("Timecode Offset:"), QStringLiteral("wunjo:sequenceproperties.timecodeOffset"));
    ...
    if (m_controller->supportsProxy()) {
        // Proxy
        auto hlay = proxyProperty(i18n("Proxy clip"), QStringLiteral("wunjo:proxy"));
        fpBox->addLayout(hlay);
    }
...
QWidget *ClipPropertiesController::constructAudioPropertiesPage()
{
    ...
    if (!audioStreamsInfo.isEmpty()) {
        ...
        m_originalProperties.insert(QStringLiteral("wunjo:active_streams"), streamString.join(QLatin1Char(';')));
        auto hlay = new QHBoxLayout;
        ...
        connect(m_audioStreamsView, &QListWidget::itemChanged, this, [this](QListWidgetItem *item) {
            ...
            QString currentStreams = m_originalProperties.value(QStringLiteral("wunjo:active_streams"));
            ...
            if (streamModified) {
                ...
                QMap<QString, QString> properties;
                properties.insert(QStringLiteral("wunjo:active_streams"), activeStreams.join(QLatin1Char(';')));
                Q_EMIT updateClipProperties(m_id, m_originalProperties, properties);
                m_originalProperties = properties;
        ...
        connect(ac, &KDualAction::activeChanged, this, [this, audioStreamsInfo](bool activated) {
            ...
            int vindx = -1;
            if (activated) {
                properties.insert(QStringLiteral("wunjo:active_streams"), QStringLiteral("-1"));
            } else {
                properties.insert(QStringLiteral("wunjo:active_streams"), QString());
...
{
    WunjoSettings::setProperties_panel_page(ix);
}
...
void ClipPropertiesController::slotReloadProperties()
{
    ...
    m_controller->mirrorOriginalProperties(m_sourceProperties);
    m_clipLabel->setText(m_properties->get("wunjo:clipname"));
    switch (m_type) {
    case ClipType::Color: {
        mlt_color color;
        m_originalProperties.insert(QStringLiteral("resource"), m_properties->get("resource"));
    ...
    case ClipType::Video: {
        ...
        if (proxy != m_originalProperties.value(QStringLiteral("wunjo:proxy"))) {
            m_originalProperties.insert(QStringLiteral("wunjo:proxy"), proxy);
            Q_EMIT proxyModified(proxy);
        }
        if (m_audioStreamsView && m_audioStreamsView->count() > 0) {
            int audio_ix = m_properties->get_int("audio_index");
            m_originalProperties.insert(QStringLiteral("wunjo:active_streams"), m_properties->get("wunjo:active_streams"));
    ...
    case ClipType::Timeline: {
        int tracks = m_properties->get_int("wunjo:sequenceproperties.tracksCount");
        QList<QStringList> propertyMap;
        propertyMap.append({i18n("Tracks:"), QString::number(tracks)});
        fillProperties();
        const QUuid uuid(m_properties->get("wunjo:uuid"));
        int timecodeOffset = pCore->currentDoc()->getSequenceProperty(uuid, "wunjo:sequenceproperties.timecodeOffset").toInt();
        m_originalProperties.insert(QStringLiteral("wunjo:sequenceproperties.timecodeOffset"), QString::number(timecodeOffset));
...
{
    QMap<QString, QString> properties;
    // wunjo_length is the default duration for image / title clips
    int wunjo_length = m_properties->time_to_frames(m_properties->get("wunjo:duration"));
    int current_length = m_properties->get_int("length");
    if (wunjo_length > 0) {
        // special case, image/title clips store default duration in wunjo:duration property
        properties.insert(QStringLiteral("wunjo:duration"), m_properties->frames_to_time(duration));
...
void ClipPropertiesController::slotEnableForce(int state)
{
    ...
    if (state == Qt::Unchecked) {
        ...
        if (param == QLatin1String("force_duration")) {
            ...
            timePos->setValue(m_properties->get_int("wunjo:original_length"));
            int original = m_properties->get_int("wunjo:original_length");
            m_properties->set("wunjo:original_length", nullptr);
            slotDurationChanged(original);
            return;
        }
        if (param == QLatin1String("wunjo:transparency")) {
        ...
        if (param == QLatin1String("force_duration")) {
            ...
            if (original_length == 0) {
                int wunjo_length = m_properties->time_to_frames(m_properties->get("wunjo:duration"));
                m_properties->set("wunjo:original_length", wunjo_length > 0 ? m_properties->get("wunjo:duration") : m_properties->get("length"));
            }
...
void ClipPropertiesController::fillProperties()
{
    ...
    qint64 filesize = m_sourceProperties->get_int64("wunjo:file_size");
    if (filesize > 0) {
    ...
    if (m_type == ClipType::Image || m_type == ClipType::AV || m_type == ClipType::Audio || m_type == ClipType::Video) {
        ...
        if (!m_controller->hasProducerProperty(QStringLiteral("wunjo:kextractor"))) {
            ...
            subProperties.pass_values(*m_properties, "wunjo:meta.extractor.");
    ...
    if (m_type == ClipType::Image) {
        ...
        int tracks = m_sourceProperties->get_int("wunjo:sequenceproperties.tracksCount");
        propertyMap.append({i18n("Tracks:"), QString::number(tracks)});
...
{
    m_controller->setProducerProperty(QStringLiteral("wunjo:kextractor"), 1);
    if (!meta.isEmpty()) {
        new QTreeWidgetItem(m_propertiesTree, QStringList{QString(), QString()});
    }
    for (auto i = meta.cbegin(), end = meta.cend(); i != end; ++i) {
        new QTreeWidgetItem(m_propertiesTree, QStringList{i.key(), i.value()});
        m_controller->setProducerProperty(QStringLiteral("wunjo:meta.extractor.%1").arg(i.key()), i.value());
    }
}
...
QMap<QString, QString> ClipPropertiesController::getMetadateMagicLantern()
{
    ...
    m_controller->setProducerProperty(QStringLiteral("wunjo:magiclantern"), 1);
    ...
    while (!file.atEnd()) {
        ...
        m_controller->setProducerProperty("wunjo:meta.magiclantern." + key, value);
...
QMap<QString, QString> ClipPropertiesController::getMetadataExif()
{
    ...
    if (QFile::exists(url)) {
        ...
        m_controller->setProducerProperty(QStringLiteral("wunjo:exiftool"), 1);
        ...
        for (const QString &tagline : std::as_const(list)) {
            ...
            if (!key.isEmpty() && !value.isEmpty()) {
                m_controller->setProducerProperty("wunjo:meta.exiftool." + key, value);
                metadata.insert(key, value);
            }
        }
...
void ClipPropertiesController::slotFillMeta(QTreeWidget *tree)
{
    ...
    int exifUsed = m_controller->getProducerIntProperty(QStringLiteral("wunjo:exiftool"));
    if (exifUsed == 1) {
        // we have cached metadata
        Mlt::Properties subProperties;
        subProperties.pass_values(*m_properties, "wunjo:meta.exiftool.");
        if (subProperties.count() > 0) {
            for (int i = 0; i < subProperties.count(); i++) {
                exifMetadata.insert(subProperties.get_name(i), subProperties.get(i));
            }
        }
    } else if (WunjoSettings::use_exiftool()) {
    ...
    int magic = m_controller->getProducerIntProperty(QStringLiteral("wunjo:magiclantern"));
    if (magic == 1) {
        // We have cached metadata
        Mlt::Properties subProperties;
        subProperties.pass_values(*m_properties, "wunjo:meta.magiclantern.");
        for (int i = 0; i < subProperties.count(); i++) {
            magicLanternMetadata.insert(subProperties.get_name(i), subProperties.get(i));
        }
    } else if (m_type != ClipType::Image && WunjoSettings::use_magicLantern()) {
...
void ClipPropertiesController::slotFillAnalysisData()
{
    ...
    Mlt::Properties subProperties;
    subProperties.pass_values(*m_properties, "wunjo:clipanalysis.");
    if (subProperties.count() > 0) {
        for (int i = 0; i < subProperties.count(); i++) {
...
void ClipPropertiesController::slotDeleteAnalysis()
{
    ...
    Q_EMIT editAnalysis(m_id, "wunjo:clipanalysis." + current->text(0), QString());
}
...
void ClipPropertiesController::slotLoadAnalysis()
{
    ...
    while (i.hasNext()) {
        i.next();
        Q_EMIT editAnalysis(m_id, "wunjo:clipanalysis." + i.key(), i.value());
    }
}
