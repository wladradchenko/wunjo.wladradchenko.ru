...
ClipController::ClipController(const QString &clipId, const std::shared_ptr<Mlt::Producer> &producer, const QDomElement &description)
    ...
    , m_effectStack(m_masterProducer
                        ? EffectStackModel::construct(m_masterProducer, ObjectId(WunjoObjectType::BinClip, clipId.toInt(), QUuid()), pCore->undoStack())
                        : nullptr)
    , m_hasAudio(false)
    , m_hasVideo(false)
    , m_controllerBinId(clipId)
{
    if (m_masterProducer && !m_masterProducer->is_valid()) {
        qCDebug(WUNJO_LOG) << "// WARNING, USING INVALID PRODUCER";
        return;
    }
    if (m_properties) {
        m_hasMultipleVideoStreams = m_properties->property_exists("wunjo:multistreams");
        setProducerProperty(QStringLiteral("wunjo:id"), m_controllerBinId);
        if (m_masterProducer->parent().property_exists("wunjo:control_uuid")) {
            const QString uuid = m_masterProducer->parent().get("wunjo:control_uuid");
            m_controlUuid = QUuid(uuid);
        } else {
            m_controlUuid = QUuid::createUuid();
            m_properties->set("wunjo:control_uuid", m_controlUuid.toString().toUtf8().constData());
        }
        getInfoForProducer();
        checkAudioVideo();
    } else {
        m_producerLock.lockForWrite();
        if (Xml::hasXmlProperty(description, QStringLiteral("wunjo:control_uuid"))) {
            const QString uuid = Xml::getXmlProperty(description, QStringLiteral("wunjo:control_uuid"));
            m_controlUuid = QUuid(uuid);
        } else {
            m_controlUuid = QUuid::createUuid();
        }
...
void ClipController::addMasterProducer(const std::shared_ptr<Mlt::Producer> &producer)
{
    ...
    m_effectStack = EffectStackModel::construct(m_masterProducer, ObjectId(WunjoObjectType::BinClip, id, QUuid()), pCore->undoStack());
    ...
    setProducerProperty(QStringLiteral("wunjo:id"), m_controllerBinId);
    if (!m_properties->property_exists("wunjo:control_uuid")) {
        m_properties->set("wunjo:control_uuid", m_controlUuid.toString().toUtf8().constData());
    }
    getInfoForProducer();
    checkAudioVideo();
    ...
    if (!m_hasMultipleVideoStreams && m_service.startsWith(QLatin1String("avformat")) && (m_clipType == ClipType::AV || m_clipType == ClipType::Video)) {
        ...
        for (int ix = 0; ix < aStreams; ++ix) {
            ...
            if (type == QLatin1String("video")) {
                ...
                if (codec_name == QLatin1String("png")) {
                    ...
                    setProducerProperty(QStringLiteral("wunjo:coverartstream"), ix);
                    continue;
                }
        ...
        if (subtitleStreams.count() > 0) {
            setProducerProperty(QStringLiteral("wunjo:subtitle_streams"), (int)subtitleStreams.count());
        }
        if (videoStreams.count() > 1) {
            setProducerProperty(QStringLiteral("wunjo:multistreams"), 1);
...
void ClipController::getProducerXML(QDomDocument &document, bool includeMeta, bool includeProfile)
{
    ...
    if (m_masterProducer) {
        ...
        qCDebug(WUNJO_LOG) << " + + ++ NO MASTER PROD";
    }
}
...
void ClipController::getInfoForProducer()
{
    ...
    if (m_service == QLatin1String("qtext")) {
        ...
        QString originalService = m_properties->get("wunjo:orig_service");
        if (!originalService.isEmpty()) {
            m_service = originalService;
        }
    }
    QString proxy = m_properties->get("wunjo:proxy");
    ...
    if (!m_service.isEmpty() && proxy.length() > 2) {
        ...
        if (QFileInfo(proxy).isRelative()) {
            ...
            m_properties->set("wunjo:proxy", proxy.toUtf8().constData());
        }
        if (proxy == path) {
            // This is a proxy producer, read original url from wunjo property
            path = m_properties->get("wunjo:originalurl");
    ...
    QString origurl = m_properties->get("wunjo:originalurl");
    if (!origurl.isEmpty()) {
        m_properties->set("wunjo:originalurl", m_path.toUtf8().constData());
    }
    ...
    if (m_usesProxy && m_path.endsWith(QStringLiteral(".mlt"))) {
        ...
        } else if (m_properties->property_exists("wunjo:producer_type")) {
            m_clipType = (ClipType::ProducerType)m_properties->get_int("wunjo:producer_type");
        } else {
            m_clipType = ClipType::Playlist;
        }
    } else if (m_service == QLatin1String("tractor") || m_service == QLatin1String("xml-string")) {
        if (m_properties->property_exists("wunjo:producer_type")) {
            m_clipType = (ClipType::ProducerType)m_properties->get_int("wunjo:producer_type");
        } else {
            m_clipType = ClipType::Timeline;
        }
    ...
    if (!hasLimitedDuration()) {
        int playtime = m_masterProducer->time_to_frames(m_masterProducer->parent().get("wunjo:duration"));
        if (playtime <= 0) {
            // Fix clips having missing wunjo:duration
            m_masterProducer->parent().set("wunjo:duration", m_masterProducer->frames_to_time(m_masterProducer->get_playtime(), mlt_time_clock));
...
void ClipController::buildAudioInfo(int audioIndex)
{
    ...
    for (auto stream = streams.cbegin(), end = streams.cend(); stream != end; ++stream) {
        QString streamEffect = m_properties->get(QStringLiteral("wunjo:stream:%1").arg(stream.key()).toUtf8().constData());
...
{
    std::string props = "wunjo:control_uuid,wunjo:proxy,wunjo:originalurl,wunjo:multistreams,rotate,force_aspect_ratio,"
                        "force_fps,force_"
                        ...
                        "test_audio,ttl,wunjo:tags,wunjo:markers,wunjo:masks,wunjo:rating";
    if (passLength) {
        props.append(",length");
    }
...
void ClipController::updateProducer(const std::shared_ptr<Mlt::Producer> &producer)
{
    ...
    const QString proxy(producer->get("wunjo:proxy"));
...
int ClipController::getFramePlaytime() const
{
    ...
    if (!hasLimitedDuration() || m_clipType == ClipType::Playlist || m_clipType == ClipType::Timeline) {
        if (m_masterProducer->parent().property_exists("wunjo:maxduration")) {
            int playtime = m_masterProducer->parent().get_int("wunjo:maxduration");
            if (playtime > 0) {
                return playtime;
            }
        }
        if (!m_masterProducer->parent().property_exists("wunjo:duration")) {
            return m_masterProducer->get_length();
        }
        int playtime = m_masterProducer->time_to_frames(m_masterProducer->parent().get("wunjo:duration"));
...
QString ClipController::getProducerProperty(const QString &name) const
{
    ...
    if (m_usesProxy && name.startsWith(QLatin1String("meta."))) {
        QString correctedName = QStringLiteral("wunjo:") + name;
...
const QSize ClipController::getFrameSize() const
{
    ...
    if (m_usesProxy) {
        int width = m_properties->get_int("wunjo:original.meta.media.width");
        int height = m_properties->get_int("wunjo:original.meta.media.height");
        if (width == 0) {
            width = m_properties->get_int("wunjo:original.width");
        }
        if (height == 0) {
            height = m_properties->get_int("wunjo:original.height");
        }
        if (width > 0 && height > 0) {
...
void ClipController::checkAudioVideo()
{
    ...
    if (m_masterProducer->get_int("_placeholder") == 1 || m_masterProducer->get_int("_missingsource") == 1) {
        ...
        QString orig_service = m_masterProducer->get("wunjo:orig_service");
    ...
    if (m_masterProducer->property_exists("wunjo:clip_type")) {
        int clipType = m_masterProducer->get_int("wunjo:clip_type");
        ...
        QString service = m_masterProducer->get("wunjo:orig_service");
        if (service.isEmpty()) {
        ...
        if (m_clipType == ClipType::Timeline) {
            ...
            if (m_masterProducer->parent().get_int("wunjo:sequenceproperties.hasAudio") == 1) {
                m_hasAudio = true;
            }
            if (m_masterProducer->parent().get_int("wunjo:sequenceproperties.hasVideo") == 1) {
                m_hasVideo = true;
            }
            if (m_hasAudio) {
                if (m_hasVideo) {
                    m_masterProducer->parent().set("wunjo:clip_type", 0);
                } else {
                    m_masterProducer->parent().set("wunjo:clip_type", 1);
                }
            } else if (!m_hasVideo) {
                ...
                m_masterProducer->parent().set("wunjo:clip_type", 2);
            }
            if (m_hasAudio) {
                buildAudioInfo(-1);
            }
        ...
        if (avTypes.contains(m_clipType)) {
            ...
            if (frame->is_valid()) {
                ...
                if (m_hasAudio) {
                    if (m_hasVideo) {
                        m_masterProducer->set("wunjo:clip_type", 0);
                    } else {
                        m_masterProducer->set("wunjo:clip_type", 1);
                    }
                } else if (m_hasVideo) {
                    m_masterProducer->set("wunjo:clip_type", 2);
                }
...
{
    setProducerProperty(QStringLiteral("wunjo:zone_in"), zone.x());
    setProducerProperty(QStringLiteral("wunjo:zone_out"), zone.y());
}

const QString ClipController::getClipHash() const
{
    return getProducerProperty(QStringLiteral("wunjo:file_hash"));
}
...
{
    QReadLocker lock(&m_producerLock);
    if (m_properties->get_int("wunjo:original.backup") == 1) {
        return;
    }
    int propsCount = m_properties->count();
    // store original props
    QStringList doNotPass{QStringLiteral("wunjo:proxy"), QStringLiteral("wunjo:originalurl"), QStringLiteral("wunjo:clipname")};
    for (int j = 0; j < propsCount; j++) {
        ...
        if (!propName.startsWith(QLatin1Char('_'))) {
            propName.prepend(QStringLiteral("wunjo:original."));
            m_properties->set(propName.toUtf8().constData(), m_properties->get(j));
        }
    }
    m_properties->set("wunjo:original.backup", 1);
}

void ClipController::clearBackupProperties()
{
    QReadLocker lock(&m_producerLock);
    if (m_properties->get_int("wunjo:original.backup") == 0) {
        return;
    }
    int propsCount = m_properties->count();
    ...
    for (int j = 0; j < propsCount; j++) {
        QString propName = m_properties->get_name(j);
        if (propName.startsWith(QLatin1String("wunjo:original."))) {
            passProps << propName;
        }
    }
    ...
    m_properties->clear("wunjo:original.backup");
}

void ClipController::mirrorOriginalProperties(std::shared_ptr<Mlt::Properties> props)
{
    QReadLocker lock(&m_producerLock);
    if (m_usesProxy && QFileInfo(m_properties->get("resource")).fileName() == QFileInfo(m_properties->get("wunjo:proxy")).fileName()) {
        ...
        sourceProps.pass_values(*m_properties, "wunjo:original.");
        props->inherit(sourceProps);
    } else {
...
int ClipController::effectsCount()
{
    ...
    for (int ix = 0; ix < service.filter_count(); ++ix) {
        ...
        QString id = effect->get("wunjo_id");
        if (!id.isEmpty()) {
            count++;
        }
    }
...
{
    return m_properties->get_int("wunjo:subtitle_streams");
}

const QString ClipController::getOriginalUrl()
{
    QString path = m_properties->get("wunjo:originalurl");
    if (path.isEmpty()) {
        path = m_path;
    }
...
{
    const QString proxy = getProducerProperty(QStringLiteral("wunjo:proxy"));
