...
std::shared_ptr<TransitionTreeModel> TransitionTreeModel::construct(bool flat, QObject *parent)
{
    ...
    for (const auto &transition : std::as_const(allTransitions)) {
        ...
        bool isFav = WunjoSettings::favorite_transitions().contains(transition.first);
...
void TransitionTreeModel::reparseUpdatedAssets()
{
    ...
    for (auto &f : lumaFiles) {
        QFileInfo fName(f);
        bool isFav = WunjoSettings::favorite_transitions().contains(f);
...
void TransitionTreeModel::setFavorite(const QModelIndex &index, bool favorite, bool isEffect)
{
    ...
    QStringList favs = WunjoSettings::favorite_transitions();
    if (favorite) {
        favs << id;
    } else {
        favs.removeAll(id);
    }
    WunjoSettings::setFavorite_transitions(favs);
}
...
QMimeData *TransitionTreeModel::mimeData(const QModelIndexList &indexes) const
{
    ...
    if (item) {
        ...
        mimeData->setData(QStringLiteral("wunjo/composition"), assetId.toUtf8());
