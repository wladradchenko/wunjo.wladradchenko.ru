    ...
    : AssetListWidget(false, includeList, tenBit, parent)
{
    ...
    if (!WunjoSettings::showTransitionsInfo()) {
        m_viewSplitter->setSizes({50, 0});
    } else {
        const QByteArray restoreData = WunjoSettings::transitionsInfoHeight().toLatin1();
        if (restoreData.isEmpty()) {
            m_viewSplitter->setSizes({50, 20});
            const QByteArray splitterData = m_viewSplitter->saveState();
            WunjoSettings::setTransitionsInfoHeight(QString::fromLatin1(splitterData));
        } else {
...
{
    WunjoSettings::setShowTransitionsInfo(enable);
    if (enable) {
        const QByteArray restoreData = WunjoSettings::transitionsInfoHeight().toLatin1();
        if (restoreData.isEmpty()) {
            m_viewSplitter->setSizes({50, 20});
            const QByteArray saveData = m_viewSplitter->saveState();
            WunjoSettings::setTransitionsInfoHeight(QString::fromLatin1(saveData));
        } else {
...
{
    Q_UNUSED(assetId);
    return QStringLiteral("wunjo/composition");
}
...
void TransitionListWidget::generatePreviews()
{
    ...
    if (scriptPath.isEmpty()) {
        ...
        QString devPath = QString::fromLocal8Bit(qgetenv("WUNJO_SOURCE_DIR"));
        if (!devPath.isEmpty()) {
        ...
        if (scriptPath.isEmpty()) {
            scriptPath = QCoreApplication::applicationDirPath() + QStringLiteral("/../share/wunjo/scripts/generate_transition_previews.py");
            if (!QFile::exists(scriptPath)) {
...
{
    WunjoSettings::setTransitionsFilter(m_filterButton->isChecked());
