...
enum class WunjoObjectType { TimelineClip, TimelineComposition, TimelineTrack, TimelineMix, TimelineSubtitle, BinClip, Master, NoItem };

struct ObjectId
{
    WunjoObjectType type;
    int itemId;
    QUuid uuid;
    explicit constexpr ObjectId(const WunjoObjectType tp = WunjoObjectType::NoItem, int id = -1, const QUuid uid = QUuid())
        : type(tp)
        , itemId(id)
        , uuid(uid)
    {
