    ...
    , m_bitRate(0)
{
    ...
    for (int ix = 0; ix < streams; ix++) {
        ...
        if (type == QLatin1String("audio")) {
            ...
            memset(property, 0, 200);
            snprintf(property, sizeof(property), "wunjo:streamname.%d", ix);
    ...
    QString active = producer->get("wunjo:active_streams");
    ...
    if (m_audioStreams.count() > 1 && active.isEmpty()) {
        ...
        producer->set("wunjo:active_streams", streamString.join(QLatin1Char(';')).toUtf8().constData());
    }
...
{
    qCDebug(WUNJO_LOG) << "Info for audio stream " << m_audioStreamIndex << "\n\tChannels: " << m_channels << "\n\tSampling rate: " << m_samplingRate
                          << "\n\tBit rate: " << m_bitRate;
}
...
void AudioStreamInfo::updateActiveStreams(const QString &activeStreams)
{
    ...
    if (activeStreams.isEmpty()) {
        switch (WunjoSettings::multistream()) {
        case 1:
