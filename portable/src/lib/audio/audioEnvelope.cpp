    ...
    , m_startpos(startPos)
{
    ...
    if (!m_producer || !m_producer->is_valid()) {
        qCDebug(WUNJO_LOG) << "// Cannot create envelope for producer: " << binId;
    } else {
...
{
    qCDebug(WUNJO_LOG) << "Loading envelope …";
    ...
    qCDebug(WUNJO_LOG) << "Calculating the envelope (" << m_envelopeSize << " frames) took " << t.elapsed() << " ms.";
    qCDebug(WUNJO_LOG) << "Normalizing envelope …";
...
void AudioEnvelope::dumpInfo()
{
    if (!m_audioSummary.isFinished()) {
        qCDebug(WUNJO_LOG) << "Envelope not yet generated, no information available.";
    } else {
        const AudioSummary &summary = audioSummary();
        qCDebug(WUNJO_LOG) << "Envelope info"
