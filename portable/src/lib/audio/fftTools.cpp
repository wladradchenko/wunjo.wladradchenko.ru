...
#ifdef DEBUG_FFTTOOLS
        qCDebug(WUNJO_LOG) << "Triangle window (factor " << window[size] << "):";
        for (int i = 0; i < size; ++i) {
            qCDebug(WUNJO_LOG) << window[i];
        }
        qCDebug(WUNJO_LOG) << "Triangle window end.";
#endif
...
#ifdef DEBUG_FFTTOOLS
        qCDebug(WUNJO_LOG) << "Hanning window (factor " << window[size] << "):";
        for (int i = 0; i < size; ++i) {
            qCDebug(WUNJO_LOG) << window[i];
        }
        qCDebug(WUNJO_LOG) << "Hanning window end.";
#endif
                             ...
                             const uint windowSize, const float param)
{
...
#ifdef DEBUG_FFTTOOLS
        qCDebug(WUNJO_LOG) << "Reusing FFT configuration with size " << windowSize;
#endif
        myCfg = m_fftCfgs.value(cfgSig);
    } else {
#ifdef DEBUG_FFTTOOLS
        qCDebug(WUNJO_LOG) << "Creating FFT configuration with size " << windowSize;
#endif
...
#ifdef DEBUG_FFTTOOLS
            qCDebug(WUNJO_LOG) << "Reusing window function with signature " << winSig;
#endif
            window = m_windowFunctions.value(winSig);
        } else {
#ifdef DEBUG_FFTTOOLS
            qCDebug(WUNJO_LOG) << "Building new window function with signature " << winSig;
#endif
    ...
    if (!mFile) {
        qCDebug(WUNJO_LOG) << "Opening file failed.";
    } else {
        mFile << "val = [ ";
        ...
        mFile.close();
        qCDebug(WUNJO_LOG) << "File written.";
    }
#endif

#ifdef DEBUG_FFTTOOLS
    qCDebug(WUNJO_LOG) << "Calculated FFT in " << start.elapsed() << " ms.";
#endif
    delete[] freqData;
    delete[] data;
}
...
#ifdef DEBUG_FFTTOOLS
    qCDebug(WUNJO_LOG) << "Interpolated " << targetSize << " nodes from " << in.size() << " input points in " << start.elapsed() << " ms";
#endif
