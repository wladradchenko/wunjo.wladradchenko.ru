...
void Core::updateHideBarsTimer(bool inhibit)
{
    if (inhibit) {
        ...
        if (m_inhibitHideBarTimer && !WunjoSettings::showtitlebars()) {
            m_hideTimer.start();
        }
...
bool Core::build(LinuxPackageType packageType, bool testMode, bool debugMode, bool showWelcome)
{
    ...
    const QStringList currentVersion = QString(WUNJO_VERSION).split(QLatin1Char('.'));
    if (currentVersion.length() == 3 && currentVersion.last().toInt() < 10) {
        ...
        if (QT_VERSION_CHECK(currentVersion.at(0).toInt(), currentVersion.at(1).toInt(), currentVersion.at(2).toInt()) <
            QT_VERSION_CHECK(WunjoSettings::lastSeenVersionMajor(), WunjoSettings::lastSeenVersionMinor(), WunjoSettings::lastSeenVersionMicro())) {
            wasUpgraded = true;
        }
    }
    ...
    if (!testMode) {
        ...
        if (lockFile.exists()) {
            ...
            if (WunjoSettings::gpu_accel()) {
                ...
                if (firstRun) {
                    ...
                    // Check if a previous ui.rc file exists, so propose to erase
        ...
        if (currentVersion.length() == 3 && (firstRun || WunjoSettings::lastSeenVersionMajor() == 0)) {
            // Store current Wunjo version
            WunjoSettings::setLastSeenVersionMajor(currentVersion.at(0).toInt());
            WunjoSettings::setLastSeenVersionMinor(currentVersion.at(1).toInt());
            WunjoSettings::setLastSeenVersionMicro(currentVersion.at(2).toInt());
        }
        m_self->buildSplash(firstRun, showWelcome && WunjoSettings::showWelcome() && !WunjoSettings::openlastproject(), showRecovery, wasUpgraded);
    }
    if (m_self->closing) {
        return true;
    }
...
void Core::buildSplash(bool firstRun, bool showWelcome, bool showCrashRecovery, bool wasUpgraded)
{
    ...
    if (showWelcome) {
        ...
        QStringList profileIds = WunjoSettings::recentProfiles();
        QStringList profileNames = WunjoSettings::recentProfileNames();
        if (profileIds.isEmpty()) {
            // Insert default profile
            if (!WunjoSettings::default_profile().isEmpty()) {
                profileIds = {WunjoSettings::default_profile()};
                profileNames = {WunjoSettings::default_profile()};
            } else {
                profileIds = {QStringLiteral("atsc_1080p_25")};
                profileNames = {QStringLiteral("HD 1080p 25 fps")};
            }
        }
        m_splash = new Splash(QString(WUNJO_VERSION), urls.values(), profileIds, profileNames, true, firstRun, showCrashRecovery, wasUpgraded);
    } else {
        m_splash = new Splash(QString(WUNJO_VERSION), {}, {}, {}, false, firstRun, showCrashRecovery, wasUpgraded);
    }
...
void Core::initGUI(const QString &MltPath, const QUrl &Url, const QStringList &clipsToLoad)
{
    ...
    if (inSandbox) {
        ...
        WunjoSettings::setFfmpegpath(QDir::cleanPath(appPath + QStringLiteral("/ffmpeg")));
        WunjoSettings::setFfplaypath(QDir::cleanPath(appPath + QStringLiteral("/ffplay")));
        WunjoSettings::setFfprobepath(QDir::cleanPath(appPath + QStringLiteral("/ffprobe")));
        WunjoSettings::setMeltpath(QDir::cleanPath(appPath + QStringLiteral("/melt")));
    ...
    // The effects installed plugins bring are written to the effects folder before
    // the repository scans it, so a plugin added or removed since the last session
    // is already reflected in the effect list on start.
    PluginManager::instance().syncEffects();
    m_mainWindow->init();
    ...
    m_profile = WunjoSettings::default_profile();
    // load default profile and ask user to select one if not found.
    if (m_profile.isEmpty()) {
        m_profile = ProjectManager::getDefaultProjectFormat();
        WunjoSettings::setDefault_profile(m_profile);
    }

    if (!ProfileRepository::get()->profileExists(m_profile)) {
        KMessageBox::error(m_mainWindow, i18n("The default profile of Wunjo is not set or invalid, press OK to set it to a correct value."));
        ...
        if (!ok) {
            KMessageBox::error(
                m_mainWindow,
                i18n("The given profile is invalid. We default to the profile \"dv_pal\", but you can change this from Wunjo's settings panel"));
    ...
    if (qApp->isSessionRestored()) {
        // NOTE: we are restoring only one window, because Wunjo only uses one MainWindow
...
void Core::startFromGuessedProfile(QString descriptiveString, QString fps, bool interlaced, int vTracks, int aTracks)
{
    ...
    if (!matchingProfile.isEmpty()) {
        ...
        WunjoSettings::setDefault_profile(matchingProfile);
    } else {
        qDebug() << "=================\n\nCOULD NOT FIND PROFILE BASED ON STRING: " << descriptiveString << ", FRAME RATE: " << fps
                 << ", INTERLACED: " << interlaced;
    }

    WunjoSettings::setVideotracks(vTracks);
    WunjoSettings::setAudiotracks(aTracks);
    m_mainWindow->show();
    QMetaObject::invokeMethod(pCore->projectManager(), "slotLoadOnOpen", Qt::QueuedConnection);
}
...
void Core::restoreLayout()
{
    ...
    // Always start in the active workspace's canonical layout. Restoring the
    // raw dock state of the previous session kept stale panel sets alive
    // whenever the shipped layouts changed — updates only showed up after
    // re-clicking a workspace pill.
    QString layoutId = WunjoSettings::activeLayout();
    if (layoutId.isEmpty()) {
        layoutId = QStringLiteral("editing");
    }
    Q_EMIT loadLayoutById(layoutId, true);
    m_mainWindow->show();
    if (!WunjoSettings::showtitlebars()) {
...
QString Core::openExternalApp(QString appPath, QStringList args, ClipType::ProducerType clipType)
{
    ...
    if (pCore->packageType() == LinuxPackageType::Flatpak) {
        ...
        qCInfo(WUNJO_LOG) << "Starting external Flatpak" << args;
    ...
    qCInfo(WUNJO_LOG) << "Starting external app" << appPath << "with arguments" << args;
    if (!process.startDetached()) {
        return process.errorString();
    }
    return QString();
...
QQmlEngine *Core::sharedQmlEngine()
{
    ...
    if (!s_engine) {
        ...
        // Expose the active accent color to all QML views (timeline, monitors) so
        // brand-accent bindings follow the user's primary color live.
        s_engine->rootContext()->setContextProperty(QStringLiteral("wunjoTheme"), WunjoTheme::instance());
    }
    return s_engine;
}
...
bool Core::setCurrentProfile(const QString profilePath)
{
    ...
    if (ProfileRepository::get()->profileExists(profilePath)) {
        ...
        QStringList profiles = WunjoSettings::recentProfiles();
        QStringList profileNames = WunjoSettings::recentProfileNames();
        ...
        if (!profileInserted) {
            ...
            WunjoSettings::setRecentProfiles(profiles);
            ...
            WunjoSettings::setRecentProfileNames(profileNames);
        }
...
void Core::refreshProjectItem(const ObjectId &id)
{
    ...
    case WunjoObjectType::BinClip:
        if (m_monitorManager->clipMonitorVisible()) {
            m_monitorManager->activateMonitor(Wunjo::ClipMonitor);
        ...
        break;
    case WunjoObjectType::Master:
        if (m_monitorManager->isActive(Wunjo::ProjectMonitor)) {
            if (m_monitorManager->projectMonitorVisible()) {
                m_monitorManager->activateMonitor(Wunjo::ProjectMonitor, false);
                m_monitorManager->refreshProjectMonitor(true);
            }
            m_monitorManager->markMonitorDirty(Wunjo::ClipMonitor, id.uuid);
        } else {
            m_monitorManager->markMonitorDirty(Wunjo::ProjectMonitor, id.uuid);
            if (m_monitorManager->clipMonitorVisible()) {
                m_monitorManager->activateMonitor(Wunjo::ClipMonitor, false);
                m_monitorManager->refreshClipMonitor(true);
            }
        }
        break;
...
std::shared_ptr<EffectStackModel> Core::getItemEffectStack(const QUuid &uuid, int itemType, int itemId)
{
    ...
    switch (itemType) {
    case int(WunjoObjectType::TimelineClip): {
    ...
    case int(WunjoObjectType::TimelineTrack): {
        auto tl = currentDoc()->getTimeline(uuid, true);
        if (tl) {
    ...
    case int(WunjoObjectType::BinClip):
        return m_projectItemModel->getClipEffectStack(itemId);
    case int(WunjoObjectType::Master): {
        auto tl = currentDoc()->getTimeline(uuid, true);
        if (tl) {
...
{
    if (id.type == WunjoObjectType::TimelineClip && m_guiConstructed) {
        auto tl = m_mainWindow->getTimeline(id.uuid);
        if (tl) {
...
void Core::updateItemModel(ObjectId id, const QString &service, const QString &updatedParam)
{
    ...
    if (id.type != WunjoObjectType::TimelineClip || m_mainWindow->getCurrentTimeline()->loading) {
        return;
    }
...
void Core::showClipKeyframes(ObjectId id, bool enable)
{
    ...
    if (id.type == WunjoObjectType::TimelineClip) {
        tl->controller()->showClipKeyframes(id.itemId, enable);
    } else if (id.type == WunjoObjectType::TimelineComposition) {
...
int Core::getMonitorPosition(Wunjo::MonitorId id) const
{
    if (m_guiConstructed) {
        switch (id) {
        case Wunjo::ClipMonitor:
...
{
    if (m_guiConstructed && id.type != WunjoObjectType::BinClip) {
        auto tl = m_mainWindow->getTimeline(id.uuid);
        if (tl) {
            if (id.type == WunjoObjectType::TimelineClip) {
...
{
    if (WunjoSettings::applyEffectParamsToGroup()) {
        switch (id.type) {
        case WunjoObjectType::TimelineClip:
...
void Core::openDocumentationLink(const QUrl &link)
{
    ...
    if (KMessageBox::questionTwoActions(
            ...
            i18n("This will open a browser to display Wunjo's online documentation at the following url:\n %1", link.toDisplayString()), {},
...
{
    // Our own addresses open without asking; anything else is somebody's link
    // and gets the confirmation.
    if (link.host() != QLatin1String("wunjo.online") && link.host() != QLatin1String("wunjo.online") && link.host() != QLatin1String("docs.wunjo.online") &&
        link.host() != QLatin1String("github.com") &&
...
std::pair<QString, int> Core::getSelectedClipAndOffset()
{
    ...
    ObjectId id(WunjoObjectType::TimelineClip, cid, m_mainWindow->getCurrentTimeline()->getUuid());
...
{
    return currentDoc()->getSequenceProperty(currentDoc()->activeUuid, QStringLiteral("wunjo:sequenceproperties.timecodeOffset")).toInt();
}

void Core::updateHwDecoding()
{
    qputenv("MLT_AVFORMAT_HWACCEL", WunjoSettings::hwDecoding().toUtf8());
}
