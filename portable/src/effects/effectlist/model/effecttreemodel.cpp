...
std::shared_ptr<EffectTreeModel> EffectTreeModel::construct(const QString &categoryFile, QObject *parent)
{
    ...
    if (!categoryFile.isEmpty() && Xml::docContentFromFile(doc, categoryFile, false)) {
        ...
        for (int i = 0; i < groups.count(); i++) {
            ...
            if (!WunjoSettings::gpu_accel() && groupName == i18n("GPU effects")) {
                continue;
            }
        ...
        // We also create "Misc", "Audio", "Plugins" and "Custom" categories
        ...
        self->m_pluginCategory = self->rootItem->appendChild(QList<QVariant>{i18n("Plugins"), QStringLiteral("root")});
        ...
        self->m_pluginCategory = self->rootItem;
    ...
    QString favCategory = QStringLiteral("wunjo:favorites");
    for (const auto &effect : std::as_const(allEffects)) {
        ...
        // An effect a plugin brought with it lands in its own group, whatever
        // type it declares — it is neither built in nor made by the user.
        if (PluginManager::instance().ownsEffect(effect.first)) {
            targetCategory = self->m_pluginCategory;
        }

        // we create the data list corresponding to this profile
        bool isFav = WunjoSettings::favorite_effects().contains(effect.first);
        ...
        if (WunjoSettings::favorite_effects().contains(effect.first) && effectCategory.contains(favCategory)) {
...
void EffectTreeModel::reloadEffect(const QString &path)
{
    ...
    if (PluginManager::instance().ownsEffect(asset.first)) {
        targetCategory = m_pluginCategory;
    } else if (type == AssetListType::AssetType::Custom || type == AssetListType::AssetType::CustomAudio) {
    ...
    bool isFav = WunjoSettings::favorite_effects().contains(asset.first);
    QString effectName = EffectsRepository::get()->getName(asset.first);
    // Same columns as construct() fills, or the effect would be filtered out of
    // the list until the next start ("Only show reviewed items" reads column 6).
    bool supportsTenBit = false;
    bool isPreferred = false;
    bool includeListed = false;
    EffectsRepository::get()->getAttributes(asset.first, &isPreferred, &includeListed, &supportsTenBit);
    QList<QVariant> data{effectName, asset.first,  QVariant::fromValue(type), isFav, targetCategory->row(),
                         isPreferred, includeListed, supportsTenBit};
    targetCategory->appendChild(data);
}

void EffectTreeModel::removeEffect(const QString &id)
{
    if (!m_pluginCategory) {
        return;
    }
    for (int i = 0; i < m_pluginCategory->childCount(); i++) {
        std::shared_ptr<TreeItem> item = m_pluginCategory->child(i);
        if (item->dataColumn(IdCol).toString() == id) {
            m_pluginCategory->removeChild(item);
            break;
        }
    }
    // the file was already deleted with the plugin, only the asset is left
    EffectsRepository::get()->unregisterEffect(id);
}
...
void EffectTreeModel::setFavorite(const QModelIndex &index, bool favorite, bool isEffect)
{
    ...
    QStringList favs = WunjoSettings::favorite_effects();
    if (!favorite) {
        favs.removeAll(id);
    } else {
        favs << id;
    }
    WunjoSettings::setFavorite_effects(favs);
}
...
QMimeData *EffectTreeModel::mimeData(const QModelIndexList &indexes) const
{
    ...
    if (item) {
        ...
        mimeData->setData(QStringLiteral("wunjo/effect"), assetId.toUtf8());
