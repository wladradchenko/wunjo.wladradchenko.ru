...
{
    if (WunjoSettings::enableBuiltInEffects()) {
...
void EffectStackModel::removeAllEffects(Fun &undo, Fun &redo)
{
    ...
    Fun undo_current = [this, current, fadeIns, fadeOuts]() {
        if (auto srv = m_masterService.lock()) {
            srv->set("wunjo:activeeffect", current);
        }
        m_fadeIns = fadeIns;
        m_fadeOuts = fadeOuts;
...
QDomElement EffectStackModel::toXml(QDomDocument &document)
{
    ...
    for (int i = 0; i < rootItem->childCount(); ++i) {
        ...
        QStringList passProps{QStringLiteral("disable"), QStringLiteral("wunjo:collapsed"), QStringLiteral("wunjo:builtin"),
                              QStringLiteral("wunjo:hiddenbuiltin"), QStringLiteral("wunjo:kfrhidden")};
...
bool EffectStackModel::fromMltXml(const QDomElement &effectsXml)
{
    ...
    for (int i = 0; i < nodeList.count(); ++i) {
        ...
        if (!Xml::hasXmlProperty(node, QStringLiteral("wunjo_id")) || Xml::hasXmlProperty(node, QStringLiteral("internal_added"))) {
            // Internal effect, ignore
            continue;
        }
        if (Xml::hasXmlProperty(node, QStringLiteral("wunjo:builtin")) && Xml::getXmlProperty(node, QStringLiteral("disable")) == QLatin1String("1")) {
            // Disabled built-in effect, ignore
            continue;
        }
        const QString effectId = Xml::getXmlProperty(node, QStringLiteral("wunjo_id"));
        Fun undo = []() { return true; };
        Fun redo = []() { return true; };
...
bool EffectStackModel::fromXml(const QDomElement &effectsXml, Fun &undo, Fun &redo)
{
    ...
    for (int i = 0; i < nodeList.count(); ++i) {
        ...
        if (m_ownerId.type == WunjoObjectType::TimelineClip && EffectsRepository::get()->isUnique(effectId) && hasFilter(effectId)) {
        ...
        if (Xml::getXmlProperty(node, QLatin1String("wunjo:builtin")).toInt() == 1) {
            if (!effectEnabled && !WunjoSettings::enableBuiltInEffects()) {
                continue;
            }
        ...
        if (Xml::hasXmlProperty(node, QLatin1String("wunjo:builtin"))) {
            effect->setBuiltIn();
            if (Xml::hasXmlProperty(node, QLatin1String("wunjo:hiddenbuiltin"))) {
                effect->filter().set("wunjo:hiddenbuiltin", Xml::getXmlProperty(node, QLatin1String("wunjo:hiddenbuiltin")).toInt());
            }
        }
...
bool EffectStackModel::copyEffectWithUndo(const std::shared_ptr<AbstractEffectItem> &sourceItem, PlaylistState::ClipState state, Fun &undo, Fun &redo)
{
    ...
    if (sourceEffect->filter().property_exists("wunjo:kfrhidden")) {
        effect->filter().set("wunjo:kfrhidden", sourceEffect->filter().get_int("wunjo:kfrhidden"));
    }
    if (sourceEffect->filter().property_exists("wunjo:collapsed")) {
        effect->filter().set("wunjo:collapsed", sourceEffect->filter().get_int("wunjo:collapsed"));
    }
    if (sourceEffect->filter().property_exists("wunjo:builtin")) {
        int builtin = sourceEffect->filter().get_int("wunjo:builtin");
        if (builtin == 1 && !enabled && !WunjoSettings::enableBuiltInEffects()) {
            // Ignore disabled builtin effects
            return false;
        }
        effect->setBuiltIn();
        if (sourceEffect->filter().property_exists("wunjo:hiddenbuiltin")) {
            effect->filter().set("wunjo:hiddenbuiltin", sourceEffect->filter().get_int("wunjo:hiddenbuiltin"));
        }
    }
    if (!enabled) {
        effect->filter().set("disable", 1);
    }
    if (m_ownerId.type == WunjoObjectType::TimelineTrack || m_ownerId.type == WunjoObjectType::Master) {
...
{
    QWriteLocker locker(&m_lock);
    if ((m_ownerId.type == WunjoObjectType::TimelineClip || m_ownerId.type == WunjoObjectType::TimelineTrack) && pCore->window() &&
    ...
    if (params.contains(QLatin1String("wunjo:builtin"))) {
        effect->setBuiltIn();
    }
    Fun local_undo = removeItem_lambda(effect->getId());
    // TODO the parent should probably not always be the root
    ...
    if (res) {
        ...
        if (effectId.startsWith(QLatin1String("fadein")) || effectId.startsWith(QLatin1String("fade_from_"))) {
            ...
            effect->filter().set("out", out);*/
            outFades++;
        } else if (m_ownerId.type == WunjoObjectType::TimelineTrack) {
                                         ...
                                         bool logUndo)
{
    ...
    for (const auto &leaf : rootItem->getLeaves()) {
        ...
        if (fadeInDuration > 0 && m_fadeIns.count(leaf->getId()) > 0) {
            ...
            bool hasZone = effect->filter().get_int("wunjo:force_in_out") == 1 && effect->filter().get_int("out");
            ...
            if (m_ownerId.type == WunjoObjectType::TimelineTrack && !hasZone) {
                int oldEffectOut = effect->filter().get_out();
            ...
            } else if (m_ownerId.type == WunjoObjectType::TimelineClip && effect->data(QModelIndex(), AssetParameterModel::RequiresInOut).toBool() == true) {
                int oldEffectIn = qMax(0, effect->filter().get_in());
                int oldEffectOut = effect->filter().get_out();
...
void EffectStackModel::moveEffect(int destRow, const std::shared_ptr<AbstractEffectItem> &item)
{
    QWriteLocker locker(&m_lock);
    int oldRow = item->row();
    if (WunjoSettings::enableBuiltInEffects() && destRow < oldRow) {
...
void EffectStackModel::registerItem(const std::shared_ptr<TreeItem> &item)
{
    ...
    if (!item->isRoot()) {
        ...
        if (!m_loadingExisting) {
            ...
            if (m_ownerId.type == WunjoObjectType::Master || m_ownerId.type == WunjoObjectType::TimelineTrack) {
                                     ...
                                     const QString &originalDecimalPoint, const QUuid &uuid)
{
    ...
    if (auto ptr = service.lock()) {
        for (int i = 0; i < ptr->filter_count(); i++) {
            ...
            if (filter->get_int("internal_added") > 0 && m_ownerId.type != WunjoObjectType::TimelineTrack) {
                // Required to load master audio effects
                if (m_ownerId.type == WunjoObjectType::Master && filter->get("mlt_service") == QLatin1String("avfilter.subtitles")) {
                    ...
                    subProperties.insert(QStringLiteral("wunjo:locked"), filter->get("wunjo:locked"));
            ...
            if (!filter->property_exists("wunjo_id")) {
                // don't consider internal MLT stuff
                continue;
            }
            effectEnabled = filter->get_int("disable") == 0;
            bool forcedInOut = filter->get_int("wunjo:force_in_out") == 1 && filter->get_int("out") > 0;
            if (disabledStack && filter->get_int("wunjo:bin-disabled") == 0) {
                disabledStack = false;
            }
            if (!effectEnabled && filter->get_int("wunjo::builtin") == 1) {
                if (!WunjoSettings::enableBuiltInEffects()) {
            ...
            const QString effectId = qstrdup(filter->get("wunjo_id"));
            if (m_ownerId.type == WunjoObjectType::TimelineClip && EffectsRepository::get()->isUnique(effectId) && hasFilter(effectId)) {
                pCore->displayMessage(i18n("Effect %1 cannot be added twice.", EffectsRepository::get()->getName(effectId)), ErrorMessage);
                continue;
            }
...
void EffectStackModel::setActiveEffect(int ix)
{
    ...
    if (auto ptr = m_masterService.lock()) {
        current = ptr->get_int("wunjo:activeeffect");
        ptr->set("wunjo:activeeffect", ix);
    }
...
int EffectStackModel::getActiveEffect() const
{
    QWriteLocker locker(&m_lock);
    if (auto ptr = m_masterService.lock()) {
        if (ptr->property_exists("wunjo:activeeffect")) {
            return ptr->get_int("wunjo:activeeffect");
        }
...
bool EffectStackModel::checkConsistency()
{
    ...
    for (const auto &service : m_childServices) {
        ...
        // MLT inserts some default normalizer filters that are not managed by Wunjo, which explains  why the filter count is not equal
        int wunjoFilterCount = 0;
        for (int i = 0; i < ptr->filter_count(); i++) {
        ...
        if (wunjoFilterCount != int(allFilters.size())) {
            qDebug() << "ERROR: Wrong filter count: " << wunjoFilterCount << " = " << allFilters.size();
            return false;
        }

        int ct = 0;
        for (uint i = 0; i < allFilters.size(); ++i) {
            while (ptr->filter(ct)->get("wunjo_id") == nullptr && ct < ptr->filter_count()) {
                ct++;
            }
...
bool EffectStackModel::addEffectKeyFrame(int frame, double normalisedVal)
{
    ...
    std::shared_ptr<EffectItemModel> sourceEffect = std::static_pointer_cast<EffectItemModel>(rootItem->child(ix));
    std::shared_ptr<KeyframeModelList> listModel = sourceEffect->getKeyframeModel();
    if (m_ownerId.type == WunjoObjectType::TimelineTrack) {
...
void EffectStackModel::updateEffectZones()
{
    ...
    if (m_ownerId.type == WunjoObjectType::Master) {
...
void EffectStackModel::passEffects(Mlt::Producer *producer, const QString &exception)
{
    auto ms = m_masterService.lock();
    for (int i = 0; i < ms->filter_count(); i++) {
        if (ms->filter(i)->get_int("internal_added") > 0 || !ms->filter(i)->property_exists("wunjo_id")) {
            continue;
        }
...
void EffectStackModel::applyAssetCommand(int row, const QModelIndex &index, const QString &previousValue, QString value, QUndoCommand *command)
{
    ...
    if (WunjoSettings::applyEffectParamsToGroupWithSameValue()) {
...
void EffectStackModel::appendAudioBuildInEffects()
{
    ...
    effect->filter().set("disable", 1);
    effect->filter().set("wunjo:kfrhidden", 1);
    effect->setBuiltIn();
    effect->prepareKeyframes();
    connect(effect.get(), &AssetParameterModel::modelChanged, this, &EffectStackModel::modelChanged);
    connect(effect.get(), &AssetParameterModel::replugEffect, this, &EffectStackModel::replugEffect, Qt::DirectConnection);
...
{
    if (m_ownerId.type == WunjoObjectType::TimelineClip || m_ownerId.type == WunjoObjectType::BinClip || m_ownerId.type == WunjoObjectType::Master) {
        if (rootItem->childCount() > 0) {
            for (int i = 0; i < rootItem->childCount(); i++) {
                std::shared_ptr<EffectItemModel> effect = std::static_pointer_cast<EffectItemModel>(rootItem->child(i));
        ...
        effect->filter().set("disable", 1);
        // effect->filter().set("wunjo:kfrhidden", 1);
        effect->filter().set("wunjo:collapsed", 1);
        effect->setBuiltIn();
        connect(effect.get(), &AssetParameterModel::modelChanged, this, &EffectStackModel::modelChanged);
        connect(effect.get(), &AssetParameterModel::replugEffect, this, &EffectStackModel::replugEffect, Qt::DirectConnection);
        connect(effect.get(), &AssetParameterModel::showEffectZone, this, &EffectStackModel::updateEffectZones);
