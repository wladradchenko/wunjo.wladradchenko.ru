    ...
    , m_blockWheel(false)
{
    ...
    if (!effectModel->isAssetEnabled()) {
        ...
        if (WunjoSettings::disable_effect_parameters()) {
    ...
    connect(setIn, &QToolButton::clicked, this, [this]() {
        if (m_model->getOwnerId().type == WunjoObjectType::BinClip) {
            m_outPos->setValue(pCore->getMonitor(Wunjo::ClipMonitor)->position());
        } else {
            int pos = pCore->getMonitorPosition();
            int trimIn = 0;
            if (m_model->getOwnerId().type == WunjoObjectType::TimelineClip) {
    ...
    bool displayBuiltInEffect = WunjoSettings::enableBuiltInEffects() && m_model->isBuiltIn();
    inOutButton->setVisible(m_model->getOwnerId().type != WunjoObjectType::TimelineClip && !displayBuiltInEffect);
...
void CollapsibleEffectView::slotCreateRegion()
{
    ...
    if (d->exec() == QDialog::Accepted && !d->selectedUrls().isEmpty()) {
        KRecentDirs::add(QStringLiteral(":WunjoClipFolder"), d->selectedUrls().first().adjusted(QUrl::RemoveFilename).toLocalFile());
...
QDomElement CollapsibleEffectView::effectForSave() const
{
    ...
    effect.removeAttribute(QStringLiteral("wunjo_ix"));
    /*
    if (m_paramWidget) {
...
void CollapsibleEffectView::slotSaveEffect(const QString title, const QString description)
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        // effect id as it is sometimes used in Wunjo to trigger special behaviors
...
void CollapsibleEffectView::setGroupIndex(int ix)
{
    ...
    m_effect.setAttribute(QStringLiteral("wunjo_info"), m_info.toString());*/
}
...
int CollapsibleEffectView::effectIndex() const
{
    ...
    return m_effect.attribute(QStringLiteral("wunjo_ix")).toInt();
}
...
void CollapsibleEffectView::dragEnterEvent(QDragEnterEvent *event)
{
    Q_UNUSED(event)
    /*
    if (event->mimeData()->hasFormat(QStringLiteral("wunjo/effectslist"))) {
        frame->setProperty("target", true);
        frame->setStyleSheet(frame->styleSheet());
        event->acceptProposedAction();
    } else if (m_paramWidget->doesAcceptDrops() && event->mimeData()->hasFormat(QStringLiteral("wunjo/geometry")) &&
...
void CollapsibleEffectView::switchInOut(bool checked)
{
    ...
    if (inOut.first == inOut.second || !checked) {
        ...
        case WunjoObjectType::BinClip: {
            int lastOut = m_model->filter().get_int("_wunjo_zone_out");
            if (lastOut > 0) {
                int in = m_model->filter().get_int("_wunjo_zone_in");
                inOut = {in, lastOut};
            } else {
        ...
        case WunjoObjectType::Master: {
            if (!checked) {
                ...
                if (lastOut > 0) {
                    ...
                    inOut = {in, in + pCore->getDurationFromString(WunjoSettings::transition_duration())};
                }
            }
            break;
        }
        case WunjoObjectType::TimelineClip: {
            if (!checked) {
                ...
                if (lastOut > 0) {
                    ...
                    int out = in + pCore->getDurationFromString(WunjoSettings::transition_duration());
