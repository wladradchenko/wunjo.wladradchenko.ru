    ...
    : QWidget(parent)
{
    ...
    maskColor->setColor(WunjoSettings::maskColor());
    borderColor->setColor(WunjoSettings::maskBorderColor());
    borderWidth->setValue(WunjoSettings::maskBorderWidth());
    ...
    connect(ac, &QAction::triggered, this, []() { pCore->window()->slotShowPreferencePage(Wunjo::PageSpeech, 1); });
    ...
    connect(m_maskHelper, &AutomaskHelper::processCrashed, this, [this](QString errorMessage) {
        Monitor *clipMon = pCore->getMonitor(Wunjo::ClipMonitor);
...
bool MaskManager::initMaskMode(bool autoAdd, bool editMode)
{
    ...
    if (!m_connected) {
        ...
        connect(pCore->getMonitor(Wunjo::ClipMonitor)->getControllerProxy(), &MonitorProxy::positionChanged, m_maskHelper, &AutomaskHelper::monitorSeek,
                Qt::UniqueConnection);
        m_connected = true;
    }
    clipMon->abortPreviewMask();
...
void MaskManager::exportFrames(bool autoAdd, bool editMode)
{
    ...
    std::function<void(const QString &)> callBack = [this, autoAdd, zone = m_zone, editMode, binId = clip->binId()](const QString &) {
        ...
        } else if (m_owner.type == WunjoObjectType::TimelineClip) {
...
void MaskManager::setOwner(ObjectId owner)
{
    ...
    if (pCore->getMonitor(Wunjo::ClipMonitor)->maskMode() != MaskModeType::MaskNone) {
        return;
    }
    // Disconnect previous clip
    disconnect(pCore->getMonitor(Wunjo::ClipMonitor)->getControllerProxy(), &MonitorProxy::positionChanged, m_maskHelper, &AutomaskHelper::monitorSeek);
    if (m_owner.type != WunjoObjectType::NoItem) {
        std::shared_ptr<ProjectClip> clip = getOwnerClip();
        if (clip) {
...
void MaskManager::previewMask(bool show)
{
    ...
    if (show && maskTree->currentItem()) {
        ...
        const QString maskFile = maskTree->currentItem()->data(0, MASKFILE).toString();
        pCore->getMonitor(Wunjo::ClipMonitor)->previewMask(binId, maskFile, in, out, MaskModeType::MaskPreview);
    } else {
        pCore->getMonitor(Wunjo::ClipMonitor)->abortPreviewMask();
    }
}
...
{
    if (WunjoSettings::samModelFile().isEmpty() || !QFile::exists(WunjoSettings::samModelFile())) {
        samStatus->show();
        buttonAdd->setEnabled(false);
    } else {
        samStatus->hide();
...
void MaskManager::applyMask()
{
    ...
    if (m_filterOwner == m_owner) {
        Monitor *clipMon = pCore->getMonitor(m_filterOwner.type == WunjoObjectType::BinClip ? Wunjo::ClipMonitor : Wunjo::ProjectMonitor);
        clipMon->slotActivateMonitor();
    }
...
void MaskManager::importMask()
{
    ...
    if (!xml.isNull()) {
        ...
        Xml::setXmlProperty(xml.documentElement(), QStringLiteral("wunjo:clipname"), maskName);
