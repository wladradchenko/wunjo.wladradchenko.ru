...
{
    if (event->mimeData()->hasFormat(QStringLiteral("wunjo/effect"))) {
...
void EffectStackView::dragMoveEvent(QDragMoveEvent *event)
{
    ...
    if (dragRow > -1 && event->source() == this) {
        const QString sourceData = event->mimeData()->data(QStringLiteral("wunjo/effectsource"));
...
void EffectStackView::dropEvent(QDropEvent *event)
{
    ...
    const QString effectId = event->mimeData()->data(QStringLiteral("wunjo/effect"));
    if (event->source() == this) {
        const QString sourceData = event->mimeData()->data(QStringLiteral("wunjo/effectsource"));
        int oldRow = sourceData.section(QLatin1Char(','), 2, 2).toInt();
...
{
    return pCore->getMonitor(Wunjo::ClipMonitor)->maskMode() == MaskModeType::MaskInput;
}
...
void EffectStackView::setModel(std::shared_ptr<EffectStackModel> model, const QSize frameSize)
{
    ...
    if (WunjoSettings::enableBuiltInEffects() && currentState != newState) {
...
void EffectStackView::loadEffects()
{
    ...
    if (max > 1) {
        ...
        pCore->getMonitor(item.type == WunjoObjectType::BinClip ? Wunjo::ClipMonitor : Wunjo::ProjectMonitor)
...
void EffectStackView::slotDeleteEffect(const std::shared_ptr<EffectItemModel> &effect)
{
    ...
    if ((type == WunjoObjectType::TimelineClip || type == WunjoObjectType::BinClip) && WunjoSettings::applyEffectParamsToGroup()) {
...
void EffectStackView::startDrag(const QPixmap pix, const QString assetId, ObjectId sourceObject, int row, bool singleTarget)
{
    ...
    mime->setData(QStringLiteral("wunjo/effectsource"), effectSource);
...
void EffectStackView::unsetModel(bool reset)
{
    ...
    Wunjo::MonitorId id = Wunjo::NoMonitor;
    if (m_model) {
        ObjectId item = m_model->getOwnerId();
        pCore->showEffectZone(item, {0, 0}, false);
        id = item.type == WunjoObjectType::BinClip ? Wunjo::ClipMonitor : Wunjo::ProjectMonitor;
...
void EffectStackView::updateSamProgress(int progress, bool exportStep)
{
    if (m_removeBg) {
        if (progress == 100) {
            if (pCore->getMonitor(Wunjo::ClipMonitor)->maskMode() == MaskModeType::MaskInput) {
...
void EffectStackView::constructBuildinWidget()
{
    ...
    auto flipToggled = [this](bool checked, const QString &effectName) {
        if (checked) {
            ...
            params.insert(QStringLiteral("wunjo:builtin"), QStringLiteral("1"));
            params.insert(QStringLiteral("wunjo:hiddenbuiltin"), QStringLiteral("1"));
