    ...
    : AbstractAssetsRepository<AssetListType::AssetType>()
{
    ...
    const QStringList effects = WunjoSettings::favorite_effects();
    ...
    if (!invalidEffect.isEmpty()) {
        ...
        QStringList newFavorites = WunjoSettings::favorite_effects();
        for (const QString &effect : std::as_const(invalidEffect)) {
            newFavorites.removeAll(effect);
        }
        WunjoSettings::setFavorite_effects(newFavorites);
    }
}
...
QPair<QString, QString> EffectsRepository::fixCustomAssetFile(const QString &path)
{
    ...
    for (int i = 0; i < nbr_effect; ++i) {
        ...
        if (currentEffect.hasAttribute(QLatin1String("wunjo_info"))) {
            ...
            currentEffect.removeAttribute(QLatin1String("wunjo_info"));
            effectAdjusted = true;
...
void EffectsRepository::unregisterEffect(const QString &id)
{
    m_assets.erase(id);
}
