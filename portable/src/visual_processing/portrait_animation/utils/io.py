...
def scale_coord(x1, y1, width, height, natural_width, natural_height, updated_width, updated_height):
    # Calculate the scaling factors
    scale_x = updated_width / natural_width
    scale_y = updated_height / natural_height

    return int(x1 * scale_x), int(y1 * scale_y), int(width * scale_x), int(height * scale_y)


def update_crop(img_rgb, crop):
    x1 = crop.get("x", 0)
    y1 = crop.get("y", 0)
    height = crop.get("height", 0)
    width = crop.get("width", 0)
    natural_height = crop.get("naturalHeight", 0)
    natural_width = crop.get("naturalWidth", 0)
    updated_height, updated_width, _ = img_rgb.shape
    crop["x"], crop["y"], crop["width"], crop["height"] = scale_coord(x1, y1, width, height, natural_width, natural_height, updated_width, updated_height)
    return crop
...
def load(fp):
    ...
    if suffix_ == "npy":
        ...
        with open(fp, "rb") as f:
            return pickle.load(f)  # Properly handle file with 'with open'
    else:
