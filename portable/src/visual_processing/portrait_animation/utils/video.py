...
def images2video(images, wfp, **kwargs):
    ...
    small_ram = kwargs.get('small_ram', False)
    ...
    n = len(images)
    for i in tqdm(range(n), desc="Writing...", total=n, unit='iB', unit_scale=True, file=sys.stdout):
        if small_ram:
...
def concat_frames(driving_image_lst, source_image_lst, I_p_lst):
    ...
    for idx, _ in tqdm(enumerate(I_p_lst), desc="Concatenating result...", total=len(I_p_lst), unit='iB', unit_scale=True, file=sys.stdout):
        I_p = I_p_lst[idx]
...
class VideoWriter:
    def __init__(self, **kwargs):
        ...
        self.small_ram = kwargs.get('macro_block_size', False)
