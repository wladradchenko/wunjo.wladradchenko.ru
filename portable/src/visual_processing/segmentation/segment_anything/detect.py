...
from . import sam_model_registry, SamPredictor
...
class SegmentAnything:
    ...
    @staticmethod
    def save_black_mask(save_name, mask, mask_save_path, width=None, height=None, kernel_size=10, blur_factor=None):
        # Reduce the dimensions if necessary
        if mask.ndim == 4:
            mask = mask[0, 0]
        ...
        # Blur mask
        if blur_factor:
            pil_image = pil_image.filter(ImageFilter.GaussianBlur(blur_factor))
