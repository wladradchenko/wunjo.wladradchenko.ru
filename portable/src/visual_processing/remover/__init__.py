...
class RemoveObjectProcessor:
    ...
    def read_retouch_mask(masks, width, height, flow_mask_dilates=8, mask_dilates=5, kernel_size=10):
        ...
        for mask_img in masks_img:
            ...
            resized_masks, _, _ = RemoveObjectProcessor.resize_frames([Image.fromarray(mask_img.astype(np.uint8))], size=(width, height))
            ...
            if flow_mask_dilates > 0:
                ...
                flow_mask_img = RemoveObjectProcessor.binary_mask(mask_img).astype(np.uint8)
            flow_masks.append(Image.fromarray(flow_mask_img * 255))
            # Dilate the mask for masks_dilated
            if mask_dilates > 0:
                mask_img = binary_dilation(mask_img, iterations=mask_dilates).astype(np.uint8)
            else:
                mask_img = RemoveObjectProcessor.binary_mask(mask_img).astype(np.uint8)
    ...
    def process_frames_with_retouch_mask(self, frames, masks_dilated, flow_masks, height, width):
        ...
        frames = RemoveObjectProcessor.to_tensors()(frames).unsqueeze(0) * 2 - 1
        flow_masks = RemoveObjectProcessor.to_tensors()(flow_masks).unsqueeze(0)
        masks_dilated = RemoveObjectProcessor.to_tensors()(masks_dilated).unsqueeze(0)
    ...
    def process_video_with_mask(self, frames, masks_dilated, flow_masks, frames_inp, width, height, raft_iter=20, subvideo_length=80, neighbor_length=10, ref_stride=10, use_half=False):
        ...
        for f in tqdm(range(0, video_length, neighbor_stride)):
            ...
            with torch.no_grad():
                ...
                pred_img = model(selected_imgs.half(), selected_pred_flows_bi, selected_masks, selected_update_masks, l_t)
                del selected_pred_flows_bi, selected_masks, selected_update_masks
                ...
                for i in range(len(neighbor_ids)):
                    ...
                    torch.cuda.empty_cache()
            del neighbor_ids, ref_ids, pred_img, binary_masks
            torch.cuda.empty_cache()

        return comp_frames


class RemoveObjectUtils:
    @staticmethod
    def process_remover(img, mask, model):
        img = img.convert("RGB")
    ...
    @staticmethod
    def upscale_remover_frame(mask, frame, original_frame, width, height):
        color = (0, 0, 0, 0)
