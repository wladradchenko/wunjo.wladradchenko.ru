...
class RivaEncryption(object):
    ...
    def loadModel(cls):
        ...
        if sys.platform == 'win32':
            ...
            encoder_cmd = f'icacls "{encoder_path}" /grant:r "Everyone:(R,W)" /T' if is_admin else f'icacls "{encoder_path}" /grant:r "{username}:(R,W)" /T'
            decoder_cmd = f'icacls "{decoder_path}" /grant:r "Everyone:(R,W)" /T' if is_admin else f'icacls "{decoder_path}" /grant:r "{username}:(R,W)" /T'
