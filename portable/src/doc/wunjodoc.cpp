...
// The document version is the Wunjo project file version. Only increment this on major releases if
// the file format changes and requires manual processing in the document validator.
// Increasing the document version means that older Wunjo versions won't be able to open the project files
...
int WunjoDoc::next_id = 0;

// create a new blank document
WunjoDoc::WunjoDoc(QString projectFolder, QUndoGroup *undoGroup, const QString &profileName, const QMap<QString, QString> &properties,
    ...
    , m_projectFolder(std::move(projectFolder))
{
    next_id = 0;
    if (parent) {
        connect(this, &WunjoDoc::updateCompositionMode, parent, &MainWindow::slotUpdateCompositeAction);
    }
    connect(m_commandStack.get(), &QUndoStack::indexChanged, this, &WunjoDoc::slotModified);
    connect(m_commandStack.get(), &DocUndoStack::invalidate, this, &WunjoDoc::checkPreviewStack, Qt::DirectConnection);
...
WunjoDoc::WunjoDoc(const QUrl &url, QDomDocument &newDom, QString projectFolder, QUndoGroup *undoGroup, MainWindow *parent)
    : QObject(parent)
    , m_autosave(nullptr)
    , m_uuid(QUuid::createUuid())
    , m_document(newDom)
...
WunjoDoc::WunjoDoc(std::shared_ptr<DocUndoStack> undoStack, std::pair<int, int> tracks, MainWindow *parent)
    : QObject(parent)
    , m_autosave(nullptr)
    , m_uuid(QUuid::createUuid())
    , m_clipsCount(0)
...
DocOpenResult WunjoDoc::Open(const QUrl &url, const QString &projectFolder, QUndoGroup *undoGroup,
    bool recoverCorruption, MainWindow *parent)
{
    ...
    qCDebug(WUNJO_LOG) << "// opening file " << url.toLocalFile();
    ...
    if (!parseResult) {
        if (recoverCorruption) {
            // Try to recover broken file produced by Wunjo 0.9.4
            int correction = 0;
            ...
            if (!parseResult) {
                ...
                qCDebug(WUNJO_LOG) << "Corrupted document read successfully.";
                result.setModified(true);
            }
        } else {
            result.setError(
    ...
    qCDebug(WUNJO_LOG) << "// validating project file";
    DocumentValidator validator(domDoc, url);
    bool success = validator.isProject();
    if (!success) {
        // It is not a project file
        result.setError(i18n("File %1 is not a Wunjo project file", url.toLocalFile()));
        return result;
    }
    ...
    if (!success) {
        result.setError(i18n("File %1 is not a valid Wunjo project file.", url.toLocalFile()));
        return result;
    }
    ...
    if (!success) {
        result.setError(i18n("GPU acceleration is turned off in Wunjo settings, but is required for this project's Movit filters."));
        return result;
    }
    ...
    // create WunjoDoc object
    auto doc = std::unique_ptr<WunjoDoc>(new WunjoDoc(url, domDoc, projectFolder, undoGroup, parent));
    if (!validationResult.second.isEmpty()) {
...
WunjoDoc::~WunjoDoc()
{
    ...
    if (pCore->window()) {
        disconnect(this, &WunjoDoc::docModified, pCore->window(), &MainWindow::slotUpdateDocumentState);
    }
...
void WunjoDoc::initializeProperties(bool newDocument, std::pair<int, int> tracks, int audioChannels)
{
    // init default document properties
    m_documentProperties[QStringLiteral("enableproxy")] = QString::number(int(WunjoSettings::enableproxy()));
    m_documentProperties[QStringLiteral("proxyparams")] = WunjoSettings::proxyparams();
    m_documentProperties[QStringLiteral("proxyextension")] = WunjoSettings::proxyextension();
    m_documentProperties[QStringLiteral("previewparameters")] = WunjoSettings::previewparams();
    m_documentProperties[QStringLiteral("previewextension")] = WunjoSettings::previewextension();
    m_documentProperties[QStringLiteral("externalproxyparams")] = WunjoSettings::externalProxyProfile();
    m_documentProperties[QStringLiteral("enableexternalproxy")] = QString::number(int(WunjoSettings::externalproxy()));
    m_documentProperties[QStringLiteral("generateproxy")] = QString::number(int(WunjoSettings::generateproxy()));
    m_documentProperties[QStringLiteral("proxyminsize")] = QString::number(WunjoSettings::proxyminsize());
    m_documentProperties[QStringLiteral("generateimageproxy")] = QString::number(int(WunjoSettings::generateimageproxy()));
    m_documentProperties[QStringLiteral("proxyimageminsize")] = QString::number(WunjoSettings::proxyimageminsize());
    m_documentProperties[QStringLiteral("proxyimagesize")] = QString::number(WunjoSettings::proxyimagesize());
    m_documentProperties[QStringLiteral("proxyresize")] = QString::number(WunjoSettings::proxyscale());
    ...
    if (newDocument) {
        ...
        m_documentProperties[QStringLiteral("guidesCategories")] = MarkerListModel::categoriesListToJSon(WunjoSettings::guidesCategories());
    }
    connect(pCore.get(), &Core::saveGuideCategories, this, &WunjoDoc::saveGuideCategories);
}

const QStringList WunjoDoc::guidesCategories()
{
...
void WunjoDoc::updateGuideCategories(const QStringList &categories, const QMap<int, int> remapCategories)
{
...
void WunjoDoc::saveGuideCategories()
{
...
int WunjoDoc::updateClipsCount()
{
    m_clipsCount = m_document.elementsByTagName(QLatin1String("entry")).size();
    return m_clipsCount;
}

int WunjoDoc::clipsCount() const
{
    return m_clipsCount;
}

const QByteArray WunjoDoc::getAndClearProjectXml()
{
...
QDomDocument WunjoDoc::createEmptyDocument(int videotracks, int audiotracks, bool disableProfile)
{
    QList<TrackInfo> tracks;
    // Tracks are added «backwards», so we need to reverse the track numbering
    // mbt 331: http://www.wunjo.online/mantis/view.php?id=331
...
QDomDocument WunjoDoc::createEmptyDocument(const QList<TrackInfo> &tracks, bool disableProfile)
{
    ...
    Mlt::Consumer xmlConsumer(*docProfile.get(), "xml:wunjo_playlist");
    if (disableProfile) {
        xmlConsumer.set("no_profile", 1);
    }
    xmlConsumer.set("terminate_on_pause", 1);
    xmlConsumer.set("store", "wunjo");
    ...
    for (int i = 0; i < tracks.count(); ++i) {
        ...
        track.set("wunjo:track_name", tracks.at(i).trackName.toUtf8().constData());
        track.set("wunjo:timeline_active", 1);
        track.set("wunjo:trackheight", WunjoSettings::trackheight());
        if (tracks.at(i).type == AudioTrack) {
            track.set("wunjo:audio_track", 1);
        }
        if (tracks.at(i).isLocked) {
            track.set("wunjo:locked_track", 1);
        }
    ...
    xmlConsumer.run();
    QString playlist = QString::fromUtf8(xmlConsumer.get("wunjo_playlist"));
    doc.setContent(playlist);
    return doc;
}

bool WunjoDoc::useProxy() const
{
    return m_documentProperties.value(QStringLiteral("enableproxy")).toInt() != 0;
}

bool WunjoDoc::useExternalProxy() const
{
    return m_documentProperties.value(QStringLiteral("enableexternalproxy")).toInt() != 0;
}

bool WunjoDoc::autoGenerateProxy(int width) const
{
...
bool WunjoDoc::autoGenerateImageProxy(int width) const
{
...
void WunjoDoc::setZoom(const QUuid &uuid, int horizontal, int vertical)
{
...
void WunjoDoc::importSequenceProperties(const QUuid uuid, const QStringList properties)
{
...
QPoint WunjoDoc::zoom(const QUuid &uuid) const
{
...
QDomDocument WunjoDoc::xmlSceneList(const QString &scene)
{
    ...
    for (int i = 0; i < maxEffects; ++i) {
        ...
        for (int j = 0; j < params.count(); ++j) {
            ...
            if (e.attribute(QStringLiteral("name")) == QLatin1String("wunjo_id")) {
    ...
    if (!customeffects.documentElement().childNodes().isEmpty()) {
        Xml::setXmlProperty(mainPlaylist, QStringLiteral("wunjo:customeffects"), customeffects.toString());
    }*/
...
bool WunjoDoc::saveSceneList(const QString &path, const QString &scene, bool saveOverExistingFile)
{
    ...
    if (m_documentOpenStatus != CleanProject && saveOverExistingFile) {
        ...
        QString baseFile = path.section(QStringLiteral(".wmproj"), 0, 0);
        int ct = 0;
        QString backupFile = baseFile + QStringLiteral("_backup") + QString::number(ct) + QStringLiteral(".wmproj");
        while (QFile::exists(backupFile)) {
            ct++;
            backupFile = baseFile + QStringLiteral("_backup") + QString::number(ct) + QStringLiteral(".wmproj");
        }
        QString message;
        if (m_documentOpenStatus == UpgradedProject) {
            message =
                i18n("Your project file was upgraded to the latest Wunjo document version.\nTo make sure you do not lose data, a backup copy called %1 "
                     "was created.",
                     backupFile);
        } else {
            message = i18n("Your project file was modified by Wunjo.\nTo make sure you do not lose data, a backup copy called %1 was created.", backupFile);
        }
        ...
        if (copyjob->exec()) {
            ...
            KMessageBox::information(
                ...
                i18n("Your project file was upgraded to the latest Wunjo document version, but it was not possible to create the backup copy %1.",
                     backupFile));
        }
    }
    QSaveFile file(path);
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qCWarning(WUNJO_LOG) << "//////  ERROR writing to file: " << path;
    ...
    (void)QtConcurrent::run(&WunjoDoc::cleanupBackupFiles, this);
    QFileInfo info(path);
    ...
    fileName.append(QStringLiteral("-%1.wmproj.jpg").arg(timeStamp));
...
QString WunjoDoc::projectTempFolder() const
{
...
QString WunjoDoc::projectRenderFolder(const QString &newPath) const
{
    // If the project is being saved to a new location, return the new path
    if (!newPath.isEmpty() && (WunjoSettings::videotodefaultfolder() == WunjoDoc::SaveToProjectFolder || m_sameProjectFolder)) {
    ...
    if (WunjoSettings::videotodefaultfolder() == WunjoDoc::SaveToCustomFolder && !WunjoSettings::videofolder().isEmpty()) {
        return WunjoSettings::videofolder();
    }
    // If we save to project folder, return it
    if (m_url.isValid() && (WunjoSettings::videotodefaultfolder() == WunjoDoc::SaveToProjectFolder || m_sameProjectFolder)) {
...
QString WunjoDoc::projectDataFolder(const QString &newPath) const
{
    if (WunjoSettings::videotodefaultfolder() == WunjoDoc::SaveToCustomFolder && !WunjoSettings::videofolder().isEmpty()) {
        return WunjoSettings::videofolder();
    }
    ...
    if (m_projectFolder.isEmpty()) {
        ...
        if (WunjoSettings::customprojectfolder()) {
            return WunjoSettings::defaultprojectfolder();
        }
        return QStandardPaths::writableLocation(QStandardPaths::MoviesLocation);
    }
    if (WunjoSettings::videotodefaultfolder() == WunjoDoc::SaveToProjectFolder || m_sameProjectFolder) {
        // Always render to project folder
        if (WunjoSettings::customprojectfolder() && !m_sameProjectFolder) {
            return WunjoSettings::defaultprojectfolder();
        }
        return QFileInfo(m_url.toLocalFile()).absolutePath();
    }
...
const QString WunjoDoc::extractFrameFolder(const QString &proposedPath) const
{
...
QString WunjoDoc::projectCaptureFolder() const
{
    if (WunjoSettings::capturetoprojectfolder() == WunjoDoc::SaveToCustomFolder && !WunjoSettings::capturefolder().isEmpty()) {
        return WunjoSettings::capturefolder();
    }
    if (WunjoSettings::capturetoprojectfolder() == WunjoDoc::SaveToProjectFolder || m_sameProjectFolder ||
        WunjoSettings::capturetoprojectfolder() == WunjoDoc::SaveToProjectSubFolder) {
        QString projectFolder = QStandardPaths::writableLocation(QStandardPaths::MoviesLocation);

        if (m_projectFolder.isEmpty()) {
            // Project has not been saved yet
            if (WunjoSettings::customprojectfolder()) {
                projectFolder = WunjoSettings::defaultprojectfolder();
            }
        } else if (WunjoSettings::customprojectfolder() && !m_sameProjectFolder) {
            projectFolder = WunjoSettings::defaultprojectfolder();
        } else {
            projectFolder = QFileInfo(m_url.toLocalFile()).absolutePath();
        }

        if (WunjoSettings::capturetoprojectfolder() == WunjoDoc::SaveToProjectSubFolder && !WunjoSettings::captureprojectsubfolder().isEmpty()) {
            // Wherever the project file is, we want a subfolder
            projectFolder += QDir::separator() + WunjoSettings::captureprojectsubfolder();
        }
...
void WunjoDoc::setProjectFolder(const QUrl &url)
{
...
const QList<QUrl> WunjoDoc::getProjectData(bool *ok)
{
    // Move proxies
    ...
    for (const auto &binClip : binClips) {
        ...
        QString proxy = projClip->getProducerProperty(QStringLiteral("wunjo:proxy"));
        QFileInfo p(proxy);
...
void WunjoDoc::slotMoveFinished(KJob *job)
{
    if (job->error() != 0) {
...
bool WunjoDoc::profileChanged(const QString &profile) const
{
    return !(*pCore->getCurrentProfile().get() == *ProfileRepository::get()->getProfile(profile).get());
}

Render *WunjoDoc::renderer()
{
    return nullptr;
}

std::shared_ptr<DocUndoStack> WunjoDoc::commandStack()
{
    return m_commandStack;
}

int WunjoDoc::getFramePos(const QString &duration)
{
    return m_timecode.getFrameCount(duration);
}

Timecode WunjoDoc::timecode() const
{
    return m_timecode;
}

int WunjoDoc::width() const
{
    return pCore->getCurrentProfile()->width();
}

int WunjoDoc::height() const
{
    return pCore->getCurrentProfile()->height();
}

QUrl WunjoDoc::url() const
{
    return m_url;
}

void WunjoDoc::setUrl(const QUrl &url)
{
    m_url = url;
    if (url.isEmpty()) {
        setModified(true);
...
QStringList WunjoDoc::getAllSubtitlesPath(bool final)
{
    QStringList result;
...
void WunjoDoc::prepareRenderAssets(const QDir &destFolder)
{
    // Copy current subtitles to assets render folder
    updateWorkFilesBeforeSave(destFolder.absoluteFilePath(m_url.fileName()), true);
}

void WunjoDoc::restoreRenderAssets()
{
    // Copy current subtitles to assets render folder
    updateWorkFilesAfterSave();
}

void WunjoDoc::updateWorkFilesBeforeSave(const QString &newUrl, bool onRender)
{
    QMapIterator<QUuid, std::shared_ptr<TimelineItemModel>> j(m_timelines);
...
void WunjoDoc::updateWorkFilesAfterSave()
{
    QMapIterator<QUuid, std::shared_ptr<TimelineItemModel>> j(m_timelines);
    while (j.hasNext()) {
        j.next();
        if (j.value()->hasSubtitleModel()) {
            int ix = getSequenceProperty(j.value()->uuid(), QStringLiteral("wunjo:activeSubtitleIndex"), QStringLiteral("0")).toInt();
...
bool WunjoDoc::sequenceThumbRequiresRefresh(const QUuid &uuid) const
{
    return m_sequenceThumbsNeedsRefresh.contains(uuid);
}

void WunjoDoc::setSequenceThumbRequiresUpdate(const QUuid &uuid)
{
    m_sequenceThumbsNeedsRefresh.insert(uuid);
}

void WunjoDoc::sequenceThumbUpdated(const QUuid &uuid)
{
    m_sequenceThumbsNeedsRefresh.remove(uuid);
}

bool WunjoDoc::isModified() const
{
    return m_modified;
}

void WunjoDoc::requestBackup()
{
    m_document.documentElement().setAttribute(QStringLiteral("modified"), 1);
}

const QString WunjoDoc::description(const QString suffix) const
{
...
QString WunjoDoc::searchFileRecursively(const QDir &dir, const QString &matchSize, const QString &matchHash) const
{
    ...
    for (int i = 0; i < filesAndDirs.size() && foundFileName.isEmpty(); ++i) {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            if (QString::number(file.size()) == matchSize) {
                ...
                qCDebug(WUNJO_LOG) << filesAndDirs.at(i) << "size match but not hash";
            }
        }
    }
...
QStringList WunjoDoc::getBinFolderClipIds(const QString &folderId) const
{
    return pCore->bin()->getBinFolderClipIds(folderId);
}

void WunjoDoc::slotCreateTextTemplateClip(const QString &group, const QString &groupId, QUrl path)
{
    Q_UNUSED(group)
...
void WunjoDoc::setDocumentProperty(const QString &name, const QString &value)
{
    if (value.isEmpty()) {
...
const QString WunjoDoc::getDocumentProperty(const QString &name, const QString &defaultValue) const
{
    return m_documentProperties.value(name, defaultValue);
}

bool WunjoDoc::hasDocumentProperty(const QString &name) const
{
    return m_documentProperties.contains(name);
}

void WunjoDoc::setSequenceProperty(const QUuid &uuid, const QString &name, const QString &value)
{
...
void WunjoDoc::setSequenceProperty(const QUuid &uuid, const QString &name, int value)
{
    setSequenceProperty(uuid, name, QString::number(value));
}

const QString WunjoDoc::getSequenceProperty(const QUuid &uuid, const QString &name, const QString defaultValue) const
{
    if (m_sequenceProperties.contains(uuid)) {
...
const QStringList WunjoDoc::getSequenceNames() const
{
    QMapIterator<QUuid, std::shared_ptr<TimelineItemModel>> m(m_timelines);
    QStringList sequenceNames;
    while (m.hasNext()) {
        m.next();
        sequenceNames << QString(m.value()->tractor()->get("wunjo:clipname"));
    }
    return sequenceNames;
}

bool WunjoDoc::hasSequenceProperty(const QUuid &uuid, const QString &name) const
{
    if (m_sequenceProperties.contains(uuid)) {
...
void WunjoDoc::clearSequenceProperty(const QUuid &uuid, const QString &name)
{
    if (m_sequenceProperties.contains(uuid)) {
        m_sequenceProperties[uuid].remove(name);
    }
...
const QMap<QString, QString> WunjoDoc::getSequenceProperties(const QUuid &uuid) const
{
    if (m_sequenceProperties.contains(uuid)) {
...
QMap<QString, QString> WunjoDoc::getRenderProperties() const
{
...
void WunjoDoc::saveCustomEffects(const QDomNodeList &customeffects)
{
    QDomElement e;
...
void WunjoDoc::updateProjectFolderPlacesEntry()
{
    /*
    ...
    QString wunjoName = QCoreApplication::applicationName();
    ...
    while (!bookmark.isNull()) {
        ...
        if (udi.isEmpty() && appName == wunjoName && bookmark.text() == i18n("Project Folder")) {
    ...
    if (!exists) {
        ...
        bookmark.setMetaDataItem(QStringLiteral("OnlyInApp"), wunjoName);
        bookmarkManager->emitChanged(root);
    }
}

// static
double WunjoDoc::getDisplayRatio(const QString &path)
{
    QDomDocument doc;
...
void WunjoDoc::backupLastSavedVersion(const QString &path)
{
    ...
    fileName.append(QLatin1Char('-') + m_documentProperties.value(QStringLiteral("documentid")));
    const QString timeStamp = info.lastModified().toString(QStringLiteral("yyyy-MM-dd-hh-mm"));
    fileName.append(QStringLiteral("-%1.wmproj").arg(timeStamp));
...
void WunjoDoc::cleanupBackupFiles()
{
    QDir backupFolder(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + QStringLiteral("/.backup"));
    ...
    projectFile.append(QStringLiteral("-??"));
    projectFile.append(QStringLiteral("-??"));
    projectFile.append(QStringLiteral("-??.wmproj"));
...
const QMap<QString, QString> WunjoDoc::metadata() const
{
    return m_documentMetadata;
}

void WunjoDoc::setMetadata(const QMap<QString, QString> &meta)
{
    setModified(true);
    m_documentMetadata = meta;
}

QMap<QString, QString> WunjoDoc::proxyClipsById(const QStringList &ids, bool proxy, const QMap<QString, QString> &proxyPath)
{
    ...
    for (auto &id : ids) {
        ...
        if (!proxy) {
            ...
            existingProxies.insert(id, clip->getProducerProperty(QStringLiteral("wunjo:proxy")));
        } else if (proxyPath.contains(id)) {
            newProps.insert(QStringLiteral("wunjo:proxy"), proxyPath.value(id));
        }
...
void WunjoDoc::slotProxyCurrentItem(bool doProxy, QList<std::shared_ptr<ProjectClip>> clipList, bool force, QUndoCommand *masterCommand)
{
    ...
    if (WunjoSettings::proxyalphaextension().isEmpty()) {
        initProxyAlphaSettings();
    }
    QString alphaExtension = WunjoSettings::proxyalphaextension();
        ...
        if (item->supportsProxy() && item->statusReady()) {
            ...
            if (doProxy) {
                ...
                if (useExternalProxy() && item->hasLimitedDuration()) {
                    if (item->hasProducerProperty(QStringLiteral("wunjo:camcorderproxy"))) {
                        const QString camProxy = item->getProducerProperty(QStringLiteral("wunjo:camcorderproxy"));
                        ...
                        if (!path.isEmpty()) {
                            ...
                            if (proxyAudioStreams != sourceAudioStreams) {
                                ...
                                newProps.insert(QStringLiteral("wunjo:camcorderproxy"), path);
                ...
                newProps.insert(QStringLiteral("wunjo:proxy"), path);
                // We need to insert empty proxy so that undo will work
                // TODO: how to handle clip properties
                // oldProps = clip->currentProperties(newProps);
                oldProps.insert(QStringLiteral("wunjo:proxy"), QStringLiteral("-"));
            } else {
...
double WunjoDoc::getDocumentVersion() const
{
    return DOCUMENTVERSION;
}

QMap<QString, QString> WunjoDoc::documentProperties(bool saveHash)
{
    ...
    m_documentProperties.insert(QStringLiteral("wunjoversion"), QStringLiteral(WUNJO_VERSION));
    ...
    if (m_documentProperties.contains(QStringLiteral("decimalPoint"))) {
        // "wunjo:docproperties.decimalPoint" was removed in document version 100
    ...
    m_documentProperties.insert(QStringLiteral("binsort"), QString::number(WunjoSettings::binSorting()));
    QMapIterator<QUuid, std::shared_ptr<TimelineItemModel>> j(m_timelines);
    while (j.hasNext()) {
        j.next();
        j.value()->passSequenceProperties(getSequenceProperties(j.key()));
        if (saveHash) {
            j.value()->tractor()->set("wunjo:sequenceproperties.timelineHash", j.value()->timelineHash().toHex().constData());
        }
    }
    return m_documentProperties;
}

void WunjoDoc::loadDocumentGuides(const QUuid &uuid, std::shared_ptr<TimelineItemModel> model)
{
...
bool WunjoDoc::loadDocumentProperties()
{
    ...
    QString name;
    QDomElement e;
    const QString bins = Xml::getXmlProperty(pl, QStringLiteral("wunjo:extraBins"));
    ...
    for (int i = 0; i < props.count(); i++) {
        ...
        if (name.startsWith(QLatin1String("wunjo:docproperties."))) {
        ...
        } else if (name.startsWith(QLatin1String("wunjo:docmetadata."))) {
            name = name.section(QLatin1Char('.'), 1);
        ...
        if (discard) {
            ...
            if (KMessageBox::warningContinueCancel(pCore->window(),
                                                   i18n("Project file <b>%1</b> contains suspicious proxy parameters. This command might be "
                                                        ...
                                                        m_url.fileName(), WunjoSettings::ffmpegpath(), proxyparams, proxyExtension),
...
void WunjoDoc::updateProjectProfile(bool reloadProducers, bool reloadThumbs)
{
...
void WunjoDoc::resetProfile(bool reloadThumbs)
{
    updateProjectProfile(true, reloadThumbs);
    Q_EMIT docModified(true);
}

void WunjoDoc::slotSwitchProfile(const QString &profile_path, bool reloadThumbs)
{
...
std::pair<int, int> WunjoDoc::getFpsFraction(double fps, bool *adjusted)
{
...
void WunjoDoc::switchProfile(ProfileParam *pf, const QString &clipName)
{
    ...
    if (!matchingProfile.isEmpty()) {
        ...
        if (WunjoSettings::default_profile().isEmpty()) {
            ...
            case KMessageBox::PrimaryAction:
                ...
                pCore->taskManager.slotCancelJobs(false, {AbstractTask::PROXYJOB, AbstractTask::AUDIOTHUMBJOB, AbstractTask::TRANSCODEJOB});
                WunjoSettings::setDefault_profile(profile->path());
...
void WunjoDoc::doAddAction(const QString &name, QAction *a, const QKeySequence &shortcut)
{
...
QAction *WunjoDoc::getAction(const QString &name)
{
    return pCore->window()->actionCollection()->action(name);
}

void WunjoDoc::previewProgress(int p)
{
    if (pCore->window()) {
...
void WunjoDoc::displayMessage(const QString &text, MessageType type, int timeOut)
{
    Q_EMIT pCore->window()->displayMessage(text, type, timeOut);
}

void WunjoDoc::selectPreviewProfile()
{
    // Read preview profiles and find the best match
    if (!WunjoSettings::previewparams().isEmpty()) {
        setDocumentProperty(QStringLiteral("previewparameters"), WunjoSettings::previewparams());
        setDocumentProperty(QStringLiteral("previewextension"), WunjoSettings::previewextension());
        return;
    }
    KConfig conf(QStringLiteral("encodingprofiles.rc"), KConfig::CascadeConfig, QStandardPaths::AppDataLocation);
    KConfigGroup group(&conf, "timelinepreview");
    QMap<QString, QString> values = group.entryMap();
    if (!WunjoSettings::supportedHWCodecs().isEmpty()) {
        QString codecFormat = QStringLiteral("x264-");
        codecFormat.append(WunjoSettings::supportedHWCodecs().constFirst().section(QLatin1Char('_'), 1));
...
QString WunjoDoc::getAutoProxyProfile()
{
...
QString WunjoDoc::getAutoProxyAlphaProfile()
{
    if (WunjoSettings::proxyalphaextension().isEmpty() || WunjoSettings::proxyalphaparams().isEmpty()) {
        initProxyAlphaSettings();
    }
    return WunjoSettings::proxyalphaparams();
}

void WunjoDoc::initProxySettings()
{
    // Read preview profiles and find the best match
    KConfig conf(QStringLiteral("encodingprofiles.rc"), KConfig::CascadeConfig, QStandardPaths::AppDataLocation);
...
void WunjoDoc::initProxyAlphaSettings()
{
    // Read preview profiles and find the best match
    KConfig conf(QStringLiteral("encodingprofiles.rc"), KConfig::CascadeConfig, QStandardPaths::AppDataLocation);
    ...
    if (!values.isEmpty()) {
        params = values.first();
        WunjoSettings::setProxyalphaparams(params.section(QLatin1Char(';'), 0, 0));
        WunjoSettings::setProxyalphaextension(params.section(QLatin1Char(';'), 1));
    }
}

void WunjoDoc::checkPreviewStack(int ix)
{
    // A command was pushed in the middle of the stack, remove all cached data from last undos
    Q_EMIT removeInvalidUndo(ix);
}

void WunjoDoc::initCacheDirs()
{
    bool ok = false;
    QString wunjoCacheDir;
    QString documentId = QDir::cleanPath(m_documentProperties.value(QStringLiteral("documentid")));
    documentId.toLongLong(&ok, 10);
    if (m_projectFolder.isEmpty()) {
        wunjoCacheDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation);
    } else {
        wunjoCacheDir = m_projectFolder;
    }
    if (!ok || documentId.isEmpty() || wunjoCacheDir.isEmpty()) {
        return;
    }
    QString basePath = wunjoCacheDir + QLatin1Char('/') + documentId;
    QDir dir(basePath);
    dir.mkpath(QStringLiteral("."));
    dir.mkdir(QStringLiteral("preview"));
    dir.mkdir(QStringLiteral("audiothumbs"));
    dir.mkdir(QStringLiteral("videothumbs"));
    QDir cacheDir(wunjoCacheDir);
    cacheDir.mkdir(QStringLiteral("proxy"));
}

const QDir WunjoDoc::getCacheDir(CacheType type, bool *ok, const QUuid uuid) const
{
    QString basePath;
    QString wunjoCacheDir;
    QString documentId = QDir::cleanPath(m_documentProperties.value(QStringLiteral("documentid")));
    ...
    if (m_projectFolder.isEmpty()) {
        ...
        if (!*ok || documentId.isEmpty() || wunjoCacheDir.isEmpty()) {
            *ok = false;
            return QDir(wunjoCacheDir);
        }
    } else {
        // Use specified folder to store all files
        wunjoCacheDir = m_projectFolder;
    }
    basePath = wunjoCacheDir + QLatin1Char('/') + documentId; // CacheBase
    switch (type) {
    case SystemCacheRoot:
        return QStandardPaths::writableLocation(QStandardPaths::CacheLocation);
    case CacheRoot:
        basePath = wunjoCacheDir;
        break;
    case CachePreview:
...
QStringList WunjoDoc::getProxyHashList()
{
    return pCore->bin()->getProxyHashList();
}

std::shared_ptr<TimelineItemModel> WunjoDoc::getTimeline(const QUuid &uuid, bool allowEmpty)
{
...
QList<QUuid> WunjoDoc::getTimelinesUuids() const
{
    return m_timelines.keys();
}

QStringList WunjoDoc::getTimelinesIds()
{
    QStringList ids;
    QMapIterator<QUuid, std::shared_ptr<TimelineItemModel>> j(m_timelines);
    while (j.hasNext()) {
...
void WunjoDoc::addTimeline(const QUuid &uuid, std::shared_ptr<TimelineItemModel> model, bool force)
{
...
bool WunjoDoc::checkConsistency()
{
    if (m_timelines.isEmpty()) {
...
void WunjoDoc::loadSequenceGroupsAndGuides(const QUuid &uuid)
{
...
void WunjoDoc::closeTimeline(const QUuid uuid, bool onDeletion)
{
    Q_ASSERT(m_timelines.find(uuid) != m_timelines.end());
...
void WunjoDoc::storeGroups(const QUuid &uuid)
{
    Q_ASSERT(m_timelines.find(uuid) != m_timelines.end());
...
void WunjoDoc::checkUsage(const QUuid &uuid)
{
    Q_ASSERT(m_timelines.find(uuid) != m_timelines.end());
    qDebug() << "===== CHECKING USAGE FOR: " << uuid << " = " << m_timelines.value(uuid).use_count();
}

std::shared_ptr<MarkerSortModel> WunjoDoc::getFilteredGuideModel(const QUuid uuid)
{
    Q_ASSERT(m_timelines.find(uuid) != m_timelines.end());
    return m_timelines.value(uuid)->getFilteredGuideModel();
}

std::shared_ptr<MarkerListModel> WunjoDoc::getGuideModel(const QUuid uuid) const
{
    Q_ASSERT(m_timelines.find(uuid) != m_timelines.end());
    return m_timelines.value(uuid)->getGuideModel();
}

int WunjoDoc::openedTimelineCount() const
{
    return m_timelines.size();
}

const QStringList WunjoDoc::getSecondaryTimelines() const
{
...
const QString WunjoDoc::projectName() const
{
    if (!m_url.isValid()) {
...
const QString WunjoDoc::documentRoot() const
{
    return m_documentRoot;
}

bool WunjoDoc::updatePreviewSettings(const QString &profile)
{
    if (profile.isEmpty()) {
        return false;
    }
...
QMap<int, QStringList> WunjoDoc::getProjectTags() const
{
...
int WunjoDoc::audioChannels() const
{
    return getDocumentProperty(QStringLiteral("audioChannels"), QStringLiteral("2")).toInt();
}

QString &WunjoDoc::modifiedDecimalPoint()
{
    return m_modifiedDecimalPoint;
}

const QString WunjoDoc::subTitlePath(const QUuid &uuid, int ix, bool final, bool restoreFromBackup)
{
    QString documentId = QDir::cleanPath(m_documentProperties.value(QStringLiteral("documentid")));
...
void WunjoDoc::duplicateSequenceProperty(const QUuid &destUuid, const QUuid &srcUuid, const QString &subsData)
{
    QJsonArray list;
...
QMap<std::pair<int, QString>, QString> WunjoDoc::multiSubtitlePath(const QUuid &uuid)
{
    const QString data = getSequenceProperty(uuid, QStringLiteral("subtitlesList"));
    return WunjoDoc::JSonToSubtitleList(data);
}

QMap<std::pair<int, QString>, QString> WunjoDoc::JSonToSubtitleList(const QString &data)
{
...
std::map<QString, SubtitleStyle> WunjoDoc::globalSubtitleStyles(const QUuid &uuid)
{
...
bool WunjoDoc::hasSubtitles() const
{
    QMapIterator<QUuid, std::shared_ptr<TimelineItemModel>> j(m_timelines);
    while (j.hasNext()) {
        j.next();
...
void WunjoDoc::generateRenderSubtitleFile(const QUuid &uuid, int in, int out, const QString &subtitleFile)
{
    if (m_timelines.contains(uuid)) {
...
// static
void WunjoDoc::useOriginals(QDomDocument &doc)
{
...
// static
void WunjoDoc::disableSubtitles(QDomDocument &doc)
{
...
void WunjoDoc::makeBackgroundTrackTransparent(QDomDocument &doc)
{
    ...
    for (int i = 0; i < prods.length(); ++i) {
        ...
        if (Xml::getXmlProperty(prod, QStringLiteral("wunjo:playlistid")) == QStringLiteral("black_track")) {
            Xml::setXmlProperty(prod, QStringLiteral("resource"), QStringLiteral("0"));
        }
    }
}

void WunjoDoc::setAutoclosePlaylists(QDomDocument &doc, const QString &mainSequenceUuid)
{
    ...
    for (int i = 0; i < tractors.length(); ++i) {
        const QString uuid = Xml::getXmlProperty(tractors.at(i).toElement(), QStringLiteral("wunjo:uuid"));
...
void WunjoDoc::processProxyNodes(QDomNodeList producers, const QString &root, const QMap<QString, QString> &proxies)
{
    ...
    for (int n = 0; n < producers.length(); ++n) {
        ...
        originalProducerService = Xml::getXmlProperty(e, QStringLiteral("wunjo:original.mlt_service"));
...
void WunjoDoc::cleanupTimelinePreview(const QDateTime &documentDate)
{
    if (m_url.isEmpty()) {
...
// static
const QStringList WunjoDoc::getDefaultGuideCategories()
{
    // Don't change this or it will break compatibility for projects created with Wunjo < 22.12
...
const QUuid WunjoDoc::uuid() const
{
    return m_uuid;
}

void WunjoDoc::loadSequenceProperties(const QUuid &uuid, Mlt::Properties sequenceProps)
{
...
bool WunjoDoc::isBusy() const
{
    return loading || closing;
}

const QStringList WunjoDoc::extractCompositionLumas()
{
...
const QStringList WunjoDoc::extractExternalEffectFiles()
{
