...
QPair<bool, QString> DocumentValidator::validate(const double currentVersion, const int currentPatchVersion)
{
    ...
    QDomElement wunjoDoc = mlt.firstChildElement(QStringLiteral("wunjodoc"));
    QString rootDir = mlt.attribute(QStringLiteral("root"));
    if (rootDir == QLatin1String("$CURRENTPATH")) {
        // The document was extracted from a Wunjo archived project, fix root directory
        ...
        wunjoDoc = mlt.firstChildElement(QStringLiteral("wunjodoc"));
    ...
    QLocale documentLocale = QLocale::c(); // Document locale for conversion. Previous MLT / Wunjo versions used C locale by default
    ...
    if (mlt.hasAttribute(QStringLiteral("LC_NUMERIC"))) { // Backwards compatibility
        // Check document numeric separator (added in Wunjo 16.12.1 and removed in Wunjo 20.08)
        QString sep = Xml::getXmlProperty(main_playlist, "wunjo:docproperties.decimalPoint", QStringLiteral("."));
        ...
        if (localeMatch.second == LocaleHandling::MatchType::NoMatch) {
            ...
            KMessageBox::error(QApplication::activeWindow(),
                               i18n("The document was created in \"%1\" locale, which is not installed on your system. Please install that language pack. "
                                    "Until then, Wunjo might not be able to correctly open the document.",
                                    mltLocale));
        }
    ...
    double version = -1;
    int patchVersion = 0;
    if (wunjoDoc.isNull() || !wunjoDoc.hasAttribute(QStringLiteral("version"))) {
        // Newer Wunjo document version
        version = Xml::getXmlProperty(main_playlist, QStringLiteral("wunjo:docproperties.version")).toDouble();
        patchVersion = Xml::getXmlProperty(main_playlist, QStringLiteral("wunjo:docproperties.patchversion")).toInt();
    } else {
        bool ok;
        version = documentLocale.toDouble(wunjoDoc.attribute(QStringLiteral("version")), &ok);
        if (!ok) {
            ...
            version = tempLocale.toDouble(wunjoDoc.attribute(QStringLiteral("version")), &ok);
            if (!ok) {
                version = wunjoDoc.attribute(QStringLiteral("version")).toDouble(&ok);
            }
            if (!ok) {
                // Last try: replace comma with a dot
                QString versionString = wunjoDoc.attribute(QStringLiteral("version"));
                if (versionString.contains(QLatin1Char(','))) {
                    versionString.replace(QLatin1Char(','), QLatin1Char('.'));
                }
                version = versionString.toDouble(&ok);
                if (!ok) {
                    qCDebug(WUNJO_LOG) << "// CANNOT PARSE VERSION NUMBER, ERROR!";
                }
            }
        }
    }
...
bool DocumentValidator::upgradePatchVersion(double version, int patchVersion)
{
    if (version == 1.1) {
        if (patchVersion < 1) {
            // Wunjo document 1.1 patch level 0
            ...
            for (int i = 0; i < filters.count(); ++i) {
                ...
                if (Xml::getXmlProperty(filter, QStringLiteral("wunjo_id")) == QLatin1String("qtblend")) {
...
{
    qCDebug(WUNJO_LOG) << "Opening a document with version " << version << " / " << currentVersion;
    ...
    if (version > currentVersion) {
        qCCritical(WUNJO_LOG) << "Unable to open document with version " << version << "because it is newer than what this Wunjo release supports ("
                                 << currentVersion << ")";
        KMessageBox::error(QApplication::activeWindow(),
                           i18n("This project is unsupported because its document version (%1) is new than what this Wunjo release supports (version %2 and "
                                "older).\nPlease consider upgrading your Wunjo version.",
                                version, currentVersion),
    ...
    if (qFuzzyCompare(version, 0.5) || qFuzzyCompare(version, 0.7)) {
        // 0.7 is unsupported
        qCCritical(WUNJO_LOG) << "Unable to open document with version " << version << "(0.5 and 0.7 are unsupported)";
    ...
    QDomElement infoXml;
    QDomNodeList docs = m_doc.elementsByTagName(QStringLiteral("wunjodoc"));
    if (!docs.isEmpty()) {
        infoXmlNode = m_doc.elementsByTagName(QStringLiteral("wunjodoc")).at(0);
    ...
    if (version <= 0.6) {
        ...
        if (tractor.isNull()) {
            ...
            QDomNodeList wunjotracks = m_doc.elementsByTagName(QStringLiteral("wunjotrack"));
            for (int i = 0; i < wunjotracks.count(); ++i) {
                ...
                blank_tractor.appendChild(blank_track);
                if (wunjotracks.at(i).toElement().attribute(QStringLiteral("cliptype")) == QLatin1String("Sound")) {
...
#define KEEP_TRACK_ORDER 1
...
#else
                // Insert the new track in an order that hopefully matches the 3 video, then 2 audio tracks of Wunjo 0.7.0
                ...
                if (!tractor_elem.isNull()) {
                    ...
                    qCWarning(WUNJO_LOG) << "tractor was not a QDomElement";
                    tractor.insertAfter(track, QDomNode());
                }
#endif
            }
        ...
        for (int i = 0; i < max; ++i) {
            ...
            while (!m.isNull()) {
                if (m.toElement().tagName() == QLatin1String("filter")) {
                    ...
                    QString current_id = filt.attribute(QStringLiteral("wunjo_id"));
                    if (current_id != last_id) {
                        effectix++;
                        last_id = current_id;
                    }
                    QDomElement e = m_doc.createElement(QStringLiteral("property"));
                    e.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo_ix"));
        ...
        /*
         * Convert as much of the wunjodoc as possible. Use the producer in
        ...
        QDomElement infoXml_new = m_doc.createElement(QStringLiteral("wunjodoc"));
        ...
        if (westley_element.isNull()) {
            qCWarning(WUNJO_LOG) << "westley0 element in document was not a QDomElement - unable to add producers to new wunjodoc";
        } else {
            ...
            for (int i = 0; i < kmax; ++i) {
                ...
                if (wproducer.isNull()) {
                    qCWarning(WUNJO_LOG) << "Found producer in westley0, that was not a QDomElement";
                    continue;
                }
                ...
                if (wproducer.attribute(QStringLiteral("type")).toInt() == int(ClipType::Text)) {
                    ...
                    kproducer.setTagName(QStringLiteral("wunjo_producer"));
                    ...
                    QDomElement kproducer = m_doc.createElement(QStringLiteral("wunjo_producer"));
...
#define LOOKUP_FOLDER 1
        ...
        if (!infoXml_old.isNull()) {
            ...
            int groupId = max_kproducer_id + 1; // Start at +1 of max id of the wunjo_producers
                ...
                if (!folder.isNull()) {
                    ...
                    for (int j = 0; j < psize; ++j) {
                        ...
                        if (!fproducer.isNull()) {
                            ...
                            QDomNodeList wunjo_producers = infoXml_new.elementsByTagName(QStringLiteral("wunjo_producer"));
                            int kpsize = wunjo_producers.size();
                            for (int k = 0; k < kpsize; ++k) {
                                QDomElement kproducer = wunjo_producers.at(k).toElement(); // Its an element for sure
        ...
        // adds <avfile /> information to <wunjo_producer />
        QDomNodeList kproducers = m_doc.elementsByTagName(QStringLiteral("wunjo_producer"));
        ...
        for (int i = 0; i < avfiles.count(); ++i) {
            ...
            if (avfile.isNull()) {
                qCWarning(WUNJO_LOG) << "found an <avfile /> that is not a QDomElement";
            } else {
                ...
                if (kproducer == QDomElement()) {
                    qCWarning(WUNJO_LOG) << "no match for <avfile /> with id =" << id;
                } else {
    ...
    if (version <= 0.83) {
        ...
        if (m_doc.toString().contains(QStringLiteral("font-size"))) {
            ...
            QDomNodeList kproducerNodes = m_doc.elementsByTagName(QStringLiteral("wunjo_producer"));
                        ...
                        if (items.at(j).attributes().namedItem(QStringLiteral("type")).nodeValue() == QLatin1String("QGraphicsTextItem")) {
                            ...
                            if (textProperties.namedItem(QStringLiteral("font-pixel-size")).isNull() &&
                                ...
                                if (convert == KMessageBox::PrimaryAction) {
                                     ...
                                     * http://www.wunjo.online/mantis/view.php?id=749)
                                     */
                                }
                            }
                        }
    ...
    if (version <= 0.85) {
        ...
        for (int i = 0; i < effectNodes.count(); ++i) {
            ...
            if (Xml::getXmlProperty(effect, QStringLiteral("mlt_service")) == QLatin1String("ladspa")) {
                ...
                QStringList info = getInfoFromEffectName(Xml::getXmlProperty(effect, QStringLiteral("wunjo_id")));
                if (info.isEmpty()) {
                    continue;
                }
                // info contains the correct ladspa.id from wunjo effect name, and a list of parameter's old and new names
                Xml::setXmlProperty(effect, QStringLiteral("wunjo_id"), info.at(0));
    ...
    if (version <= 0.88) {
        ...
        for (int i = 0; i < markers.count(); ++i) {
            ...
            QDomElement property = m_doc.createElement(QStringLiteral("property"));
            property.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo:marker.") + marker.attribute(QStringLiteral("id")) + QLatin1Char(':') +
        ...
        for (int i = 0; i < guides.count(); ++i) {
            ...
            QDomElement property = m_doc.createElement(QStringLiteral("property"));
            property.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo:guide.") + guide.attribute(QStringLiteral("time")));
        ...
        for (int i = 0; i < folders.count(); ++i) {
            QDomElement folder = folders.at(i).toElement();
            QDomElement property = m_doc.createElement(QStringLiteral("property"));
            property.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo:folder.-1.") + folder.attribute(QStringLiteral("id")));
        ...
        QDomNodeList wunjo_producers = m_doc.elementsByTagName(QStringLiteral("wunjo_producer"));
        ...
        for (int j = 0; j < wunjo_producers.count(); j++) {
            QDomElement prod = wunjo_producers.at(j).toElement();
            QString id = prod.attribute(QStringLiteral("id"));
            ...
            for (int k = 0; k < mlt_producers.count(); k++) {
                ...
                if (mltprod.attribute(QStringLiteral("id")) == id) {
                    if (!folder.isEmpty()) {
                        ...
                        QDomElement property = m_doc.createElement(QStringLiteral("property"));
                        property.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo:folderid"));
        ...
        for (int i = 0; i < slowmotionIds.count(); i++) {
            ...
            if (!ids.contains(slo)) {
                // rebuild producer from Wunjo's old xml format
                for (int j = 0; j < wunjo_producers.count(); j++) {
                    QDomElement prod = wunjo_producers.at(j).toElement();
                    QString id = prod.attribute(QStringLiteral("id"));
                    if (id == slo) {
                        // We found the wunjo_producer, build MLT producer
    ...
    if (version < 0.91) {
        ...
        for (int i = 0; i < old_tracks.count(); i++) {
            ...
            if (!trackPlaylist.isNull()) {
                QDomElement wunjoTrack = old_tracks.at(i).toElement();
                if (wunjoTrack.attribute(QStringLiteral("type")) == QLatin1String("audio")) {
                    Xml::setXmlProperty(trackPlaylist, QStringLiteral("wunjo:audio_track"), QStringLiteral("1"));
                }
                if (wunjoTrack.attribute(QStringLiteral("locked")) == QLatin1String("1")) {
                    Xml::setXmlProperty(trackPlaylist, QStringLiteral("wunjo:locked_track"), QStringLiteral("1"));
                }
                Xml::setXmlProperty(trackPlaylist, QStringLiteral("wunjo:track_name"), wunjoTrack.attribute(QStringLiteral("trackname")));
            }
        }
        // Find bin playlist
        ...
        if (!notesList.isEmpty()) {
            ...
            Xml::setXmlProperty(playlist, QStringLiteral("wunjo:documentnotes"), notes);
        }
        // Migrate clip groups
        ...
        if (!groupElement.isEmpty()) {
            ...
            Xml::setXmlProperty(playlist, QStringLiteral("wunjo:clipgroups"), d2.toString());
        }
        ...
        if (!effectsElement.isEmpty()) {
            ...
            Xml::setXmlProperty(playlist, QStringLiteral("wunjo:customeffects"), d2.toString());
        }
        Xml::setXmlProperty(playlist, QStringLiteral("wunjo:docproperties.version"), QString::number(currentVersion));
        if (!infoXml.isNull()) {
            Xml::setXmlProperty(playlist, QStringLiteral("wunjo:docproperties.projectfolder"), infoXml.attribute(QStringLiteral("projectfolder")));
        }

        // Remove deprecated Wunjo extra info from xml doc before sending it to MLT
        QDomElement docXml = mlt.firstChildElement(QStringLiteral("wunjodoc"));
        if (!docXml.isNull()) {
    ...
    if (version < 0.92) {
        ...
        for (int i = 0; i < transitionList.count(); i++) {
            ...
            QString id = Xml::getXmlProperty(trans, QStringLiteral("wunjo_id"));
            if (id == QLatin1String("luma")) {
                Xml::setXmlProperty(trans, QStringLiteral("wunjo_id"), QStringLiteral("wipe"));
    ...
    if (version < 0.93) {
        ...
        for (int i = 0; i < entries.count(); i++) {
            ...
            for (int j = 0; j < effects.count(); j++) {
                ...
                QString id = Xml::getXmlProperty(eff, QStringLiteral("wunjo_id"));
                if (keyframeFilterToConvert.contains(id) && !parsedIds.contains(id)) {
                    ...
                    for (int k = j + 1; k < effects.count(); k++) {
                        ...
                        QString subId = Xml::getXmlProperty(subEffect, QStringLiteral("wunjo_id"));
                        if (subId == id) {
    ...
    if (version < 0.97) {
        ...
        for (int i = 0; i < props.count(); ++i) {
            ...
            if (prop.startsWith(QLatin1String("wunjo:guide."))) {
                // Process guide
            ...
            } else if (prop.startsWith(QLatin1String("wunjo:marker."))) {
                // Process marker
        ...
        if (!guidesList.isEmpty()) {
            ...
            Xml::setXmlProperty(main_playlist, QStringLiteral("wunjo:docproperties.guides"), json.toJson());
        }
        ...
        for (int i = 0; i < max; ++i) {
            ...
            // Move to new wunjo:id format
            const QString id = prod.attribute(QStringLiteral("id")).section(QLatin1Char('_'), 0, 0);
            Xml::setXmlProperty(prod, QStringLiteral("wunjo:id"), id);
            if (markersList.contains(id)) {
                QJsonDocument json(markersList.value(id));
                Xml::setXmlProperty(prod, QStringLiteral("wunjo:markers"), json.toJson());
            }
    ...
    if (version < 0.98) {
        ...
        for (int i = 0; i < playlists.count(); i++) {
            if (playlists.at(i).toElement().attribute(QStringLiteral("id")) == QLatin1String("main bin") ||
                ...
                QString oldGroups = Xml::getXmlProperty(mainplaylist.toElement(), QStringLiteral("wunjo:clipgroups"));
                ...
                if (Xml::getXmlProperty(playlists.at(i).toElement(), QStringLiteral("wunjo:audio_track")) == QLatin1String("1")) {
                ...
                pltype.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo:audio_track"));
                ...
                plname.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo:track_name"));
        ...
        Xml::setXmlProperty(mainplaylist.toElement(), QStringLiteral("wunjo:docproperties.groups"), groupsData);
    }
    if (version < 0.99) {
        // rename main bin playlist, create extra tracks for old type AV clips, port groups to JSon
        QDomNodeList masterProducers = m_doc.elementsByTagName(QStringLiteral("producer"));
        for (int i = 0; i < masterProducers.count(); i++) {
            QMap<QString, QString> map = Xml::getXmlPropertyByWildcard(masterProducers.at(i).toElement(), QLatin1String("wunjo:clipzone."));
            if (map.isEmpty()) {
                continue;
            }
            QJsonArray list;
            ...
            Xml::setXmlProperty(masterProducers.at(i).toElement(), QStringLiteral("wunjo:clipzones"), QString(json.toJson()));
        }
    }

    // Doc 1.01: Wunjo 21.08.0
    if (version < 1.01) {
        ...
        for (int i = 0; i < max; ++i) {
            QDomElement t = transitions.at(i).toElement();
            if (Xml::getXmlProperty(t, QStringLiteral("wunjo_id")) == QLatin1String("wipe")) {
            ...
            } else if (Xml::getXmlProperty(t, QStringLiteral("wunjo_id")) == QLatin1String("affine")) {
        ...
        for (int i = 0; i < max; ++i) {
            ...
            if (Xml::getXmlProperty(t, QStringLiteral("wunjo_id")) == QLatin1String("pan_zoom")) {
                Xml::renameXmlProperty(t, QStringLiteral("transition.geometry"), QStringLiteral("transition.rect"));
            }
        }
    }

    // Doc 1.02: Wunjo 21.08.1
    if (version < 1.02) {
        ...
        for (int i = 0; i < max; ++i) {
            QDomElement t = effects.at(i).toElement();
            QString wunjoId = Xml::getXmlProperty(t, QStringLiteral("wunjo_id"));
            if (Xml::getXmlProperty(t, QStringLiteral("mlt_service")) == QLatin1String("affine") && wunjoId != QLatin1String("pan_zoom")) {
                QDomElement effect = EffectsRepository::get()->getXml(wunjoId);

                // check whether the effect already uses mlt rect
                if (!Xml::hasXmlProperty(t, QStringLiteral("transition.rect"))) {
                    QString newId = wunjoId.append(" mlt7");

                    Xml::renameXmlProperty(t, QStringLiteral("transition.geometry"), QStringLiteral("transition.rect"));
                    Xml::setXmlProperty(t, QStringLiteral("wunjo_id"), newId);
    ...
    if (version < 1.03) {
        ...
        for (int i = 0; i < max; ++i) {
            QDomElement t = effects.at(i).toElement();
            QString wunjoId = Xml::getXmlProperty(t, QStringLiteral("wunjo_id"));
            if (wunjoId.startsWith(QLatin1String("fade_"))) {
                bool fadeIn = wunjoId.startsWith(QLatin1String("fade_from_"));
    ...
    if (version < 1.04) {
        ...
        for (int i = 0; i < max; ++i) {
            QDomElement t = transitions.at(i).toElement();
            QString wunjoId = Xml::getXmlProperty(t, QStringLiteral("wunjo_id"));
            if (wunjoId == QLatin1String("slide")) {
    ...
    /*if (version < 1.1) {
        ...
        for (int i = 0; i < max; ++i) {
            QDomElement t = effects.at(i).toElement();
            QString wunjoId = Xml::getXmlProperty(t, QStringLiteral("wunjo_id"));
            if (wunjoId == QLatin1String("opencv.tracker") && t.hasAttribute(QLatin1String("in"))) {
...
void DocumentValidator::convertSubtitles()
{
    ...
    for (int i = 0; i < tractors.size(); i++) {
        oldSubData = Xml::getXmlProperty(tractors.at(i).toElement(), QStringLiteral("wunjo:sequenceproperties.subtitlesList"));
        ...
        if (json.isArray()) {
            ...
            Xml::setXmlProperty(tractor, QStringLiteral("wunjo:sequenceproperties.subtitlesList"), newSubData);
            const QString timelineUuid = Xml::getXmlProperty(tractor, QStringLiteral("wunjo:uuid"));
            const QString documentUuid = Xml::getXmlProperty(tractor, QStringLiteral("wunjo:sequenceproperties.documentuuid"));
...
auto DocumentValidator::upgradeTo100(const QLocale &documentLocale) -> QString
{
    ...
    if (decimalPoint != '.') {
        ...
        QList<QString> generalPropertiesToFix = {
            ...
            "length",
            "aspect_ratio",
            "wunjo:clipanalysis.motion_vector_list",
            "wunjo:original.meta.attr.com.apple.quicktime.rating.user.markup",
        };
...
void DocumentValidator::updateProducerInfo(const QDomElement &prod, const QDomElement &source)
{
    ...
    if (pxy.length() > 1) {
        Xml::setXmlProperty(prod, QStringLiteral("wunjo:proxy"), pxy);
        Xml::setXmlProperty(prod, QStringLiteral("wunjo:originalurl"), source.attribute(QStringLiteral("resource")));
    }
    if (source.hasAttribute(QStringLiteral("file_hash"))) {
        Xml::setXmlProperty(prod, QStringLiteral("wunjo:file_hash"), source.attribute(QStringLiteral("file_hash")));
    }
    if (source.hasAttribute(QStringLiteral("file_size"))) {
        Xml::setXmlProperty(prod, QStringLiteral("wunjo:file_size"), source.attribute(QStringLiteral("file_size")));
    }
    if (source.hasAttribute(QStringLiteral("name"))) {
        Xml::setXmlProperty(prod, QStringLiteral("wunjo:clipname"), source.attribute(QStringLiteral("name")));
    }
    if (source.hasAttribute(QStringLiteral("zone_out"))) {
        Xml::setXmlProperty(prod, QStringLiteral("wunjo:zone_out"), source.attribute(QStringLiteral("zone_out")));
    }
    if (source.hasAttribute(QStringLiteral("zone_in"))) {
        Xml::setXmlProperty(prod, QStringLiteral("wunjo:zone_in"), source.attribute(QStringLiteral("zone_in")));
    }
    if (source.hasAttribute(QStringLiteral("cutzones"))) {
        ...
        for (const QString &data : zoneList) {
            ...
            Xml::setXmlProperty(prod, QStringLiteral("wunjo:clipzone.") + zoneName,
...
{
    QStringList info;
    // Returns a list to convert old Wunjo ladspa effects
...
bool DocumentValidator::checkMovit()
{
    ...
    if (WunjoSettings::preferredcomposite() != i18n("auto") && TransitionsRepository::get()->exists(WunjoSettings::preferredcomposite())) {
        compositeTrans = WunjoSettings::preferredcomposite();
    ...
    for (int i = 0; i < max; ++i) {
        ...
        if (filterId == QLatin1String("movit.white_balance") && hasWB) {
            ...
            Xml::setXmlProperty(filt, QStringLiteral("wunjo_id"), QStringLiteral("frei0r.colgate"));
        ...
        if (filterId == QLatin1String("movit.blur") && hasBlur) {
            ...
            Xml::setXmlProperty(filt, QStringLiteral("wunjo_id"), QStringLiteral("frei0r.IIRblur"));
        ...
        if (filterId == QLatin1String("movit.mirror")) {
            ...
            Xml::setXmlProperty(filt, QStringLiteral("wunjo_id"), QStringLiteral("mirror"));
...
void DocumentValidator::checkOrphanedProducers()
{
    ...
    for (int i = 0; i < producers.count(); ++i) {
        ...
        if (!binProducers.contains(id)) {
            QString binId = Xml::getXmlProperty(prod, QStringLiteral("wunjo:binid"));
            Xml::setXmlProperty(prod, QStringLiteral("wunjo:id"), binId);
            if (!binId.isEmpty() && binProducers.contains(binId)) {
                continue;
            }
            qCWarning(WUNJO_LOG) << " ///////// WARNING, FOUND UNKNOWN PRODUDER: " << id << " ----------------";
            ...
            for (int j = 0; j < producers.count(); j++) {
                ...
                qCDebug(WUNJO_LOG) << " / /LKNG FOR: " << service << " / " << orphanValue << ", checking: " << binProd.attribute(QStringLiteral("id"));
