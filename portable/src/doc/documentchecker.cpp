...
bool DocumentChecker::resolveProblemsWithGUI()
{
    ...
    for (int i = 0; i < max; ++i) {
        ...
        if (Xml::hasXmlProperty(tractor, QStringLiteral("wunjo:projectTractor"))) {
...
bool DocumentChecker::hasErrorInProject()
{
    ...
    for (int i = 0; i < playlists.count(); ++i) {
        ...
        if (pl.attribute(QStringLiteral("id")) == BinPlaylist::binPlaylistId) {
            ...
            m_documentid = Xml::getXmlProperty(mainBinPlaylist, QStringLiteral("wunjo:docproperties.documentid"));
            if (m_documentid.isEmpty()) {
                // invalid document id, recreate one
                m_documentid = QString::number(QDateTime::currentMSecsSinceEpoch());
                Xml::setXmlProperty(mainBinPlaylist, QStringLiteral("wunjo:docproperties.documentid"), m_documentid);
                m_doc.documentElement().setAttribute(QStringLiteral("modified"), 1);
            ...
            storageFolder = Xml::getXmlProperty(mainBinPlaylist, QStringLiteral("wunjo:docproperties.storagefolder"));
            if (!storageFolder.isEmpty()) {
                ...
                if (!QFile::exists(storageFolder)) {
                    ...
                    if (projectDir.mkpath(m_documentid)) {
                        ...
                        Xml::setXmlProperty(mainBinPlaylist, QStringLiteral("wunjo:docproperties.storagefolder"), projectDir.absoluteFilePath(m_documentid));
                        m_doc.documentElement().setAttribute(QStringLiteral("modified"), 1);
                    } else {
                        // Cannot create storage folder, use default location
                        Xml::removeXmlProperty(mainBinPlaylist, QStringLiteral("wunjo:docproperties.storagefolder"));
                        m_doc.documentElement().setAttribute(QStringLiteral("modified"), 1);
                    }
                }
            }
        ...
        } else if (Xml::getXmlProperty(pl, QStringLiteral("wunjo:playlistid")) == QLatin1String("timeline_preview")) {
    ...
    // First ensure that each bin item has a wunjo:uuid value and build a map of wunjo:id, {resource, uuid}
    ...
    for (int i = 0; i < max; ++i) {
        ...
        if (Xml::hasXmlProperty(e, QStringLiteral("wunjo:playlistid")) || e.attribute(QStringLiteral("id")) == QLatin1String("black_track")) {
            // Black track producer, ignore
            continue;
        }
        const QString id = e.attribute(QLatin1String("id"));
        int kid = Xml::getXmlProperty(e, "wunjo:id").toInt();
        ...
        if (!Xml::hasXmlProperty(e, QStringLiteral("wunjo:control_uuid"))) {
            const QUuid uuid = QUuid::createUuid();
            Xml::setXmlProperty(e, QStringLiteral("wunjo:control_uuid"), uuid.toString());
            m_recoveryMap.insert(kid, {resource, uuid});
            m_hashMap.insert(kid, Xml::getXmlProperty(e, QStringLiteral("wunjo:file_hash")));
            binClipsMap.insert(kid, uuid);
            uuidUpgrade = true;
        } else {
            const QUuid uuid(Xml::getXmlProperty(e, QStringLiteral("wunjo:control_uuid")));
            binClipsMap.insert(kid, uuid);
        }
    }
    ...
    for (int i = 0; i < max; ++i) {
        ...
        int kid = Xml::getXmlProperty(e, QStringLiteral("wunjo:id")).toInt();
        const QString id = e.attribute(QLatin1String("id"));
    ...
    for (int i = 0; i < max; ++i) {
        ...
        const QString resource = Xml::getXmlProperty(e, QStringLiteral("wunjo:uuid"));
        if (Xml::hasXmlProperty(e, QStringLiteral("wunjo:projectTractor")) || resource.isEmpty()) {
    ...
    while (t.hasNext()) {
        ...
        if (!binClipsMap.contains(t.key())) {
            // Warning, wunjo:id not found in bin, add an entry for it
    ...
    for (int i = 0; i < max; ++i) {
        QDomElement e = documentTractors.item(i).toElement();
        if (Xml::hasXmlProperty(e, QStringLiteral("wunjo:projectTractor"))) {
        ...
        if (Xml::hasXmlProperty(e, QStringLiteral("wunjo:id"))) {
...
{
    if (!Xml::getXmlProperty(producer, QStringLiteral("wunjo:id")).isEmpty()) {
    ...
    for (int j = 0; j < max; j++) {
        ...
        if (e.attribute(QStringLiteral("producer")) == producerName) {
            // Match found
            QString entryName = Xml::getXmlProperty(e, QStringLiteral("wunjo:id"));
            if (!entryName.isEmpty()) {
                Xml::setXmlProperty(producer, QStringLiteral("wunjo:id"), entryName);
                return true;
            }
        }
    }
...
bool DocumentChecker::ensureProducerIsNotPlaceholder(QDomElement &producer)
{
    ...
    QString original_service = Xml::getXmlProperty(producer, QStringLiteral("wunjo:orig_service"));
    ...
    } else if (resource.endsWith(QLatin1String(".wmproj")) || resource.endsWith(QLatin1String(".mlt"))) {
...
{
    // Tell Wunjo to recreate proxy
    producer.setAttribute(QStringLiteral("_replaceproxy"), QStringLiteral("1"));
    // Remove reference to missing proxy
    Xml::setXmlProperty(producer, QStringLiteral("wunjo:proxy"), QStringLiteral("-"));

    // Replace proxy url with real clip in MLT producers
    QString prefix;
    QString originalService = Xml::getXmlProperty(producer, QStringLiteral("wunjo:original.mlt_service"));
    QString service = Xml::getXmlProperty(producer, QStringLiteral("mlt_service"));
...
void DocumentChecker::removeProxy(const QDomNodeList &items, const QString &clipId, bool recreate)
{
    ...
    for (int i = 0; i < items.count(); ++i) {
        ...
        QString parentId = getWunjoClipId(e);
        if (parentId != clipId) {
            continue;
        }
        // Tell Wunjo to recreate proxy
        ...
        // Remove reference to missing proxy
        Xml::setXmlProperty(e, QStringLiteral("wunjo:proxy"), QStringLiteral("-"));

        // Replace proxy url with real clip in MLT producers
        QString prefix;
        const QString originalService = Xml::getXmlProperty(e, QStringLiteral("wunjo:original.mlt_service"));
        const QString originalPath = Xml::getXmlProperty(e, QStringLiteral("wunjo:originalurl"));
...
{
    if (!isBinClip) {
        int currentId = Xml::getXmlProperty(e, QStringLiteral("wunjo:id")).toInt();

        if (!Xml::hasXmlProperty(e, QStringLiteral("wunjo:control_uuid"))) {
            if (Xml::hasXmlProperty(e, QStringLiteral("wunjo:playlistid"))) {
                // Black track producer, ignore
                return false;
            }
            ...
            if (resource.isEmpty()) {
                // Check for sequence
                if (Xml::getXmlProperty(e, QStringLiteral("wunjo:producer_type")).toInt() == ClipType::Timeline) {
                    resource = Xml::getXmlProperty(e, QStringLiteral("wunjo:uuid"));
                }
            }
            if (currentId > 0 && m_recoveryMap.contains(currentId) && m_recoveryMap.value(currentId).first == resource) {
                // Match
                Xml::setXmlProperty(e, "wunjo:control_uuid", m_recoveryMap.value(currentId).second.toString());
            } else {
                ...
                if (Xml::hasXmlProperty(e, QStringLiteral("warp_resource"))) {
                    ...
                    if (Xml::getXmlProperty(e, QStringLiteral("mlt_service")) == QLatin1String("xml") &&
                        ...
                        if (m_recoveryMap.contains(currentId)) {
                            Xml::setXmlProperty(e, QStringLiteral("wunjo:control_uuid"), m_recoveryMap.value(currentId).second.toString());
                            processed = true;
                        } else {
                ...
                if (!processed) {
                    ...
                    if (matchingIds.isEmpty()) {
                        ...
                        const QString hash = Xml::getXmlProperty(e, QStringLiteral("wunjo:file_hash"));
                        if (!hash.isEmpty()) {
                    ...
                    if (matchingIds.size() > 0) {
                        ...
                        Xml::setXmlProperty(e, QStringLiteral("wunjo:id"), QString::number(matchingIds.firstKey()));
                        Xml::setXmlProperty(e, QStringLiteral("wunjo:control_uuid"), matchingIds.value(matchingIds.firstKey()).toString());
                    } else {
                        Xml::setXmlProperty(e, QStringLiteral("wunjo:remove"), QStringLiteral("1"));
                    }
                }
            }
        }
...
QString DocumentChecker::getMissingProducers(QDomElement &e, const QDomNodeList &entries, const QString &storageFolder)
{
    ...
    if (service == QLatin1String("qtext")) {
        ...
        checkMissingImagesAndFonts(titlesList.first, titlesList.second, Xml::getXmlProperty(e, QStringLiteral("wunjo:id")));
        return QString();
    }

    QString clipId = getWunjoClipId(e);
    ...
    if (index > -1) {
        if (m_items[index].hash.isEmpty()) {
            m_items[index].hash = Xml::getXmlProperty(e, QStringLiteral("wunjo:file_hash"));
            m_items[index].fileSize = Xml::getXmlProperty(e, QStringLiteral("wunjo:file_size"));
        }
    }
    ...
    auto checkClip = [this, clipId, clipType, isBinClip](QDomElement &e, const QString &resource) {
        if (isSequenceWithSpeedEffect(e)) {
            ...
            item.originalFilePath = resource;
            item.type = MissingType::Clip;
            item.hash = Xml::getXmlProperty(e, QStringLiteral("wunjo:file_hash"));
            item.fileSize = Xml::getXmlProperty(e, QStringLiteral("wunjo:file_size"));
    ...
    QString proxy = Xml::getXmlProperty(e, QStringLiteral("wunjo:proxy"));
    if (isBinClip && !proxy.isEmpty() && proxy.length() > 1) {
        ...
        QString original = Xml::getXmlProperty(e, QStringLiteral("wunjo:originalurl"));
        ...
        if (!QFile::exists(original)) {
            ...
            if (!movedOriginal.isEmpty()) {
                ...
                Xml::setXmlProperty(e, QStringLiteral("wunjo:originalurl"), movedOriginal);
    ...
    if (!QFile::exists(resource)) {
        ...
        const QByteArray hash = Xml::getXmlProperty(e, "wunjo:file_hash").toLatin1();
        if (!hash.isEmpty()) {
            ...
            if (hash != fileData) {
                if (slideshow) {
                    ...
                    Xml::setXmlProperty(e, "wunjo:file_hash", fileData);
                } else {
                    // Clip was changed, notify and trigger clip reload
                    Xml::removeXmlProperty(e, "wunjo:file_hash");
                    DocumentResource item;
                    item.originalFilePath = resource;
                    item.clipId = clipId;
                    item.clipType = clipType;
...
QString DocumentChecker::fixLutFile(const QString &file)
{
...
#else
    searchPath.cd(QStringLiteral("../share/wunjo/luts/"));
#endif
    ...
    // Try in Wunjo's standard KDE path
...
{
    QDir searchPath(WunjoSettings::mltpath());
    QString fname = QFileInfo(file).fileName();
...
#else
    searchPath.cd(QStringLiteral("../share/wunjo/"));
#endif
    if (file.contains(QStringLiteral("/PAL"))) {
        searchPath.cd(QStringLiteral("lumas/PAL/"));
    } else {
...
QMap<QString, QString> DocumentChecker::getAssetsServiceIds(const QDomDocument &doc, const QString &tagName)
{
    ...
    for (int i = 0; i < max; ++i) {
        ...
        const QString service = Xml::getXmlProperty(filter, QStringLiteral("wunjo_id"));
...
void DocumentChecker::fixMissingSource(const QString &id, const QDomNodeList &producers, const QDomNodeList &chains)
{
    ...
    for (int i = 0; i < producers.count(); ++i) {
        ...
        QString parentId = Xml::getXmlProperty(e, QStringLiteral("wunjo:id"));
        if (parentId == id) {
            // Fix clip
...
QStringList DocumentChecker::fixSequences(QDomElement &e, const QDomNodeList &producers, const QStringList &tractorIds)
{
    ...
    if (!(isBinClip && service == QLatin1String("tractor") && resource.endsWith(QLatin1String("tractor>")))) {
        // This is not a broken sequence clip (bug in Wunjo 23.04.0)
        // nothing to fix, quit
        return {};
    }

    const QString brokenId = e.attribute(QStringLiteral("id"));
    const QString brokenUuid = Xml::getXmlProperty(e, QStringLiteral("wunjo:uuid"));
    ...
    if (brokenId != brokenUuid && tractorIds.contains(brokenUuid)) {
        ...
        for (int k = 0; k < producers.count(); ++k) {
            ...
            if (Xml::hasXmlProperty(prod, QStringLiteral("wunjo:playlistid"))) {
        ...
        for (int j = 0; j < tractors.count(); ++j) {
            ...
            if (!Xml::hasXmlProperty(current, QStringLiteral("wunjo:projectTractor")) && !insertedTractors.contains(current.attribute("id"))) {
                QDomElement tk = m_doc.createElement(QStringLiteral("track"));
...
void DocumentChecker::fixProxyClip(const QDomNodeList &items, const QString &id, const QString &oldUrl, const QString &newUrl)
{
    ...
    for (int i = 0; i < items.count(); ++i) {
        ...
        if (!Xml::getXmlProperty(e, QStringLiteral("wunjo:proxy")).isEmpty()) {
            // Only set originalurl on master producer
            Xml::setXmlProperty(e, QStringLiteral("wunjo:proxy"), newUrl);
        }
    }
}
...
void DocumentChecker::usePlaceholderForClip(const QDomNodeList &items, const QString &clipId)
{
    ...
    for (int i = items.count() - 1; i >= 0; --i) {
        ...
        e = items.item(i).toElement();
        if (Xml::getXmlProperty(e, QStringLiteral("wunjo:id")) == clipId) {
            // Fix clip
            Xml::setXmlProperty(e, QStringLiteral("_placeholder"), QStringLiteral("1"));
            QString service = Xml::getXmlProperty(e, QStringLiteral("mlt_service"));
            Xml::setXmlProperty(e, QStringLiteral("wunjo:orig_service"), service);
...
void DocumentChecker::removeAssetsById(QDomDocument &doc, const QString &tagName, const QStringList &idsToDelete)
{
    ...
    for (int i = 0; i < assets.count(); ++i) {
        ...
        QString service = Xml::getXmlProperty(asset, QStringLiteral("wunjo_id"));
        if (service.isEmpty()) {
...
void DocumentChecker::fixAssetsById(QDomDocument &doc, const QString &tagName, const QString &oldId, const QString &newId)
{
    ...
    for (int i = 0; i < assets.count(); ++i) {
        ...
        if (service == oldId) {
            // Rename asset
            Xml::setXmlProperty(asset, QStringLiteral("wunjo_id"), newId);
...
void DocumentChecker::fixClip(const QDomNodeList &items, const QString &clipId, const QString &newPath)
{
    ...
    for (int i = items.count() - 1; i >= 0; --i) {
        ...
        if (Xml::hasXmlProperty(e, QStringLiteral("wunjo:originalurl"))) {
            // Only set originalurl on master producer
            Xml::setXmlProperty(e, QStringLiteral("wunjo:originalurl"), newPath);
        }
        if (Xml::hasXmlProperty(e, QStringLiteral("wunjo:original.resource"))) {
            // Only set original.resource on master producer
            Xml::setXmlProperty(e, QStringLiteral("wunjo:original.resource"), newPath);
        }
        if (service == QLatin1String("timewarp")) {
...
QString DocumentChecker::getWunjoClipId(const QDomElement &producer)
{
    QString clipId = Xml::getXmlProperty(producer, QStringLiteral("wunjo:id"));
    if (clipId.isEmpty()) {
