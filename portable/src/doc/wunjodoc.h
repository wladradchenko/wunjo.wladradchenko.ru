...
/** @class WunjoDoc
 *  @brief Represents a wunjo project file
 *
 *  Instances of WunjoDoc classes are created by void MainWindow::newFile(bool showProjectSettings, bool force)
*/
...
/** Object returned by WunjoDoc::Open(), containing a pointer to a WunjoDoc
...
public:
    ...
    friend class WunjoTests;
    /** @brief Create a new empty Wunjo project with the specified profile and requested number of tracks.
     *
     * @param tracks The number of <video, audio> tracks to create in the project.
    */
    WunjoDoc(QString projectFolder, QUndoGroup *undoGroup, const QString &profileName, const QMap<QString, QString> &properties,
                const QMap<QString, QString> &metadata, const std::pair<int, int> &tracks, int audioChannels, MainWindow *parent = nullptr);
    /** @brief Open an existing Wunjo project, returning nothing if the project cannot be opened. */
    ...
    WunjoDoc(std::shared_ptr<DocUndoStack> undoStack, std::pair<int, int> tracks = {2, 2}, MainWindow *parent = nullptr);
    ~WunjoDoc() override;
    ...
    */
    ...
    /** @brief Move document properties into sequence properties (mostly useful to convert older WunjoDoc formats.*/
...
private:
    /** @brief Create a new WunjoDoc using the provided QDomDocument (an
     * existing project file), used by the Open() named constructor. */
    WunjoDoc(const QUrl &url, QDomDocument& newDom, QString projectFolder, QUndoGroup *undoGroup,
        MainWindow *parent = nullptr);
    /** @brief Set document default properties using hard-coded values and WunjoSettings.
    ...
    /** @brief Tells whether the current document was modified by Wunjo on opening, and a backup should be created on save. */
    ...
    /** @brief Updates the project folder location entry in the wunjo file dialogs to point to the current project folder. */
...
public Q_SLOTS:
     ...
     *
     * If crash recovery is turned on, a timer calls WunjoDoc::slotAutoSave() \n
     ...
     *
     * The autosave files are in ~/.kde/data/stalefiles/wunjo/ */
...
public:
    ...
    /** @returns a unique_ptr to the WunjoDoc, or an empty unique_ptr */
    std::unique_ptr<WunjoDoc> getDocument() { return std::move(m_doc); }
    ...
    void setDocument(std::unique_ptr<WunjoDoc> doc)
    {
        m_doc = std::move(doc);
        m_succeeded = true;
    }
