...
void MixerWidget::buildUI(Mlt::Tractor *service, const QString &trackName)
{
    ...
    // Wide enough for the actual button row (mute/solo/monitor/effects, plus
    // collapse on the master strip) so the icons don't get squeezed together
    const int buttonCount = 2 + (m_collapse ? 1 : 0) + (m_solo ? 1 : 0) + (m_monitor ? 1 : 0);
    setMinimumWidth(buttonCount * (m_muteButton->sizeHint().width() + 2));
...
void MixerWidget::buildControlButtons()
{
    ...
    if (m_tid > -1) {
        ...
        m_collapse->setIcon(WunjoSettings::mixerCollapse() ? QIcon::fromTheme("arrow-left") : QIcon::fromTheme("arrow-right"));
        ...
        m_collapse->setChecked(WunjoSettings::mixerCollapse());
    }
}
...
void MixerWidget::setupConnections()
{
        ...
        if (m_tid == -1) {
            ...
            if (m_levelFilter) {
                if (active) {
                    ...
                    m_levelFilter->set("wunjo:muted_level", m_lastVolume);
                    m_levelFilter->set("level", -1000);
                    m_levelFilter->set("disable", 0);
                } else {
                    m_lastVolume = m_levelFilter->get_double("wunjo:muted_level");
    ...
    if (m_collapse) {
        connect(m_collapse, &QToolButton::clicked, this, [&]() {
            WunjoSettings::setMixerCollapse(m_collapse->isChecked());
    ...
    connect(m_volumeSlider, &QAbstractSlider::valueChanged, this, [&](int value) {
        ...
        if (m_recording || (m_monitor && m_monitor->isChecked())) {
            ...
            WunjoSettings::setAudiocapturevolume(value / 100);
...
void MixerWidget::updateMonitorState()
{
    if (m_monitor && m_monitor->isChecked()) {
        ...
        m_volumeSlider->setTickLabelsVisible(true);
        m_volumeSlider->setNeutralPosition(WunjoSettings::audiocapturevolume() * 100);
        m_volumeSpin->setNeutralPosition(WunjoSettings::audiocapturevolume());
        m_volumeSpin->setValue(WunjoSettings::audiocapturevolume());
    } else {
