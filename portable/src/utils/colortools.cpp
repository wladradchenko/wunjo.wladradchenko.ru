...
QImage ColorTools::hsvHueShiftPlane(const QSize &size, int S, int V, int MIN, int MAX)
{
...
#ifdef DEBUG_CT
    qCDebug(WUNJO_LOG) << "Requested: Saturation " << S << ", Value " << V;
    QColor colTest(-1, 256, 257);
    qCDebug(WUNJO_LOG) << "-1 mapped to " << colTest.red() << ", 256 to " << colTest.green() << ", 257 to " << colTest.blue();
#endif
    ...
    for (int x = 0; x < size.width(); ++x) {
        ...
        for (int y = 0; y < size.height(); ++y) {
            ...
            //            qCDebug(WUNJO_LOG) << "hue: " << hue << ", huediff: " << huediff;
