...
{
    if (!WunjoSettings::glaxnimatePath().isEmpty() && QFile(WunjoSettings::glaxnimatePath()).exists()) {
        return true;
    }
    ...
    WunjoSettings::setGlaxnimatePath(url.toLocalFile());
...
{
    QString error = pCore->openExternalApp(WunjoSettings::glaxnimatePath(), {filename}, ClipType::Animation);
    if (!error.isEmpty()) {
...
void GlaxnimateLauncher::openClip(int clipId)
{
    ...
    QString name = QStringLiteral("wunjo-%1").arg(QCoreApplication::applicationPid());
    ...
    if (!m_server->listen(name)) {
        ...
        error = pCore->openExternalApp(WunjoSettings::glaxnimatePath(), args, ClipType::Animation);
        if (error.isEmpty()) {
            m_sharedMemory.reset(new QSharedMemory(name));
            return;
        }
    } else {
        if (pCore->openExternalApp(WunjoSettings::glaxnimatePath(), args, ClipType::Animation).isEmpty()) {
            m_sharedMemory.reset(new QSharedMemory(name));
            return;
        } else {
...
void GlaxnimateLauncher::onReadyRead()
{
    ...
    if (m_isProtocolValid) {
        ...
        if (frameNum != m_parent->m_frameNum) {
            qDebug() << "glaxnimate time =" << time << "=> Wunjo frameNum =" << frameNum;
