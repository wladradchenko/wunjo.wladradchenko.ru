...
from backend.folders import TALKER_MODEL_FOLDER
from backend.extension import download_model, check_download_size
...
def enhancer(images, method='gfpgan', bg_upsampler='realesrgan'):
    ...
    if os.path.isfile(os.path.join(TALKER_MODEL_FOLDER, 'talker.json')):
        with open(os.path.join(TALKER_MODEL_FOLDER, 'talker.json'), 'r', encoding="utf8") as file:
            config_talker = json.load(file)
    else:
        raise "[Error] Config file talker.json is not exist"

    local_model_path = os.path.join(TALKER_MODEL_FOLDER, './gfpgan/weights')
    ...
    if method == 'gfpgan':
        ...
        url = config_talker["gfpgan"]["GFPGANv1.4.pth"]
    elif method == 'RestoreFormer':
        arch = 'RestoreFormer'
        channel_multiplier = 2
        model_name = 'RestoreFormer'
        url = config_talker["gfpgan"]["RestoreFormer.pth"]
    elif method == 'codeformer':
        arch = 'CodeFormer'
        channel_multiplier = 2
        model_name = 'CodeFormer'
        url = config_talker["gfpgan"]["codeformer.pth"]
    else:
    ...
    if bg_upsampler == 'realesrgan':
        if not torch.cuda.is_available():  # CPU
            ...
            os.path.join(TALKER_MODEL_FOLDER, './gfpgan/weights')
            model_realesrgan_path = os.path.join(local_model_path, model_name + '.pth')
            url_realesrgan = config_talker["gfpgan"]["RealESRGAN_x2plus.pth"]
