...
window.onload = function() {
    ...
    trainButton.addEventListener('click', (event) => trainWindow(event.currentTarget));
    ...
    document.getElementById('a-change-processor').addEventListener('click', (event) => {
...
function sendDataToTrain(elem, trainType) {
    ...
    fetch('/synthesize_process/')
        ...
        .then(data => {
            ...
            processAsyncTrainParam(data, elem, trainType).then(() => {
...
async function processAsyncTrainParam(data, elem, trainType) {
    if (data.status_code === 200) {
        ...
        if (!trainAudioPath.value) {
            var messageSetP = await translateWithGoogle("Путь к аудио пуст.", 'auto', targetLang);
        ...
        if (!trainMarkPath.value) {
            var messageSetP = await translateWithGoogle("Путь к файлу разметки пуст.", 'auto', targetLang);
            messageTrainSpeech.innerHTML = `<p style='margin-top: 5pt;'>${messageSetP}</p>`;
            return;
        }

        const trainCheckpointPathField = elem.querySelector("#speech-train-checkpoint-info");
        let trainCheckpointPath = elem.querySelector("#speech-train-checkpoint-path");
        ...
        const selectedLang = trainSelectLang.options[trainSelectLang.selectedIndex].value;

        const trainSelectBatchSize = elem.querySelector("#speech-train-batch-size");
        // Get value from the current option
        const selectedBatchSize = trainSelectBatchSize.options[trainSelectBatchSize.selectedIndex].value;
