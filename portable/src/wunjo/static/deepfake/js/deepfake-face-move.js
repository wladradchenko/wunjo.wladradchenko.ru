// FACE MOVE //
function handleCheckboxFaceMoveClick(clickedId) {
...
function initiateFaceMovePop(button, audio_url = undefined, audio_name = undefined) {
  var introFaceMove = introJs();
  introFaceMove.setOptions({
    steps: [
      {
        title: "Face animation panel",
        position: "right",
        intro: `<div style="width: 80vw; max-width: 90vw; height: 83vh; max-height: 90vh;align-items: inherit;display: flex;flex-direction: column;justify-content: space-between">
                    <div>
                        ...
                        <legend>Settings</legend>
                            <div style="padding: 5pt;">
                              <input type="checkbox" id="multiface-deepfake" name="multiface" onclick="handleCheckboxFaceMoveClick('multiface-deepfake')">
                              <label for="multiface-deepfake">Replace all faces</label>
                            </div>
                            <div style="padding: 5pt;">
                              <input type="checkbox" id="similarface-deepfake" name="similarface" onclick="handleCheckboxFaceMoveClick('similarface-deepfake')">
                        ...
                        <div style="display: flex;justify-content: space-evenly;">
                            <div style="width: 35vw;">
                                <span class="dragBox" style="margin-bottom: 15px;width: 100%;display: flex;text-align: center;flex-direction: column;position: relative;justify-content: center;height: 45vh;">
                                      Load target image
                                    <input accept="image/*" type="file" onChange="handleFaceMoveImg(event, document.getElementById('preview-media-target'), this.parentElement, document.getElementById('message-about-status-target'))" ondragover="drag(this.parentElement)" ondrop="drop(this.parentElement)" />
                                </span>
                            ...
                            <div style="width: 35vw;">
                                <span class="dragBox" style="margin-bottom: 15px;width: 100%;display: flex;text-align: center;flex-direction: column;position: relative;justify-content: center;height: 45vh;">
                                      Load source video
                                    <input accept="video/*" type="file" onChange="handleFaceMoveVideo(event, document.getElementById('preview-media-source'), this.parentElement, document.getElementById('message-about-status-source'))" ondragover="drag(this.parentElement)" ondrop="drop(this.parentElement)" />
                                </span>
                    ...
                    <div>
                        <i id="inspect-models-face-move" style="font-size: 10pt;margin-top: 5px;"></i>
                        <button class="introjs-button" style="background: #f7db4d;margin-top: 10pt;text-align: center;width: 100%;padding-right: 0 !important;padding-left: 0 !important;padding-bottom: 0.5rem !important;padding-top: 0.5rem !important;" onclick="triggerFaceMoveSynthesis(this.parentElement.parentElement.parentElement);">Start processing</button>
                    </div>
                    </div>`,
      },
    ],
  ...
  introFaceMove.setOption('keyboardNavigation', false).start();

  // Msg about inspect models
  const inspectElem = document.getElementById("inspect-models-face-move")
  inspectElem.innerHTML = "";
  getInspectMessage(inspectElem, "/inspect_face_animation");
}

// HANDLE FACE MOVE //
async function handleFaceMoveImg(event, previewElement, parentElement, messageElement){
    ...
    if (file) {
        ...
        if (fileType === 'image') {
            ...
            var messageAboutStatusText = await translateWithGoogle("Choose a face to animate by tool","auto",targetLang);
            messageElement.innerHTML = `${messageAboutStatusText} <i class="fa-solid fa-draw-polygon" style="margin-left: 10px;"></i>`;
            var canvas = await setupImageCanvas(previewElement, fileUrl, "40vh", "35vw");
            canvas.addEventListener('click', setPointOnCanvas);
        }
    }
}

async function handleFaceMoveVideo(event, previewElement, parentElement, messageElement){
    const fileInput = event.target;
    const file = fileInput.files[0];
    if (file) {
        const fileUrl = window.URL.createObjectURL(file);
        ...
        if (fileType === 'video') {
            messageElement.style.display = "flex";
            messageElement.style.background = getRandomColor();
            var messageAboutStatusText = await translateWithGoogle("Video is loading...","auto",targetLang);
            messageElement.innerHTML = `${messageAboutStatusText}`;
            var canvas = await setupVideoTimeline(previewElement, fileUrl, "40vh", "35vw");
            messageElement.style.background = getRandomColor();
...
function triggerFaceMoveSynthesis(elem) {
    fetch("/synthesize_process/")
        .then(response => response.json())
        .then(data => processFaceMove(data, elem))
        .catch(error => {
            console.error("Error fetching the synthesis process status:", error);
        });
}

async function processFaceMove(data, element) {
    ...
    fetch("/synthesize_face_swap/", {
        ...
        body: JSON.stringify(faceMoveParameters)
    });
