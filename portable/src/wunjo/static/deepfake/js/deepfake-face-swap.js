...
function deepfakeFaceSwap(
  ...
  document
    .getElementById("videoStartTarget")
  ...
  document
    .getElementById("videoStartSource")
    .addEventListener("change", function () {
...
function handleVideoMetadataMedia(
  video,
  fieldsetControlId,
  videoLengthId,
  videoStartId
...
function handleImageMetadataMedia(
  fieldsetControlId,
  videoLengthId,
  videoStartId
) {
...
function sendDataToFaceSwap(elem) {
  ...
  fetch("/synthesize_process/")
    ...
    .then((data) => {
      ...
      processAsyncFaceSwap(data, elem)
        .then(() => {
...
async function processAsyncFaceSwap(data, elem) {
  if (data.status_code === 200) {
    ...
    var canvasRectanglesTarget =
      previewFaceSwapTarget.querySelector("#canvasTarget");
    var canvasRectanglesListTarget = [];
    if (canvasRectanglesTarget) {
      canvasRectanglesListTarget = JSON.parse(
        canvasRectanglesTarget.dataset.rectangles
      );
    }
    ...
    var canvasRectanglesSource =
      previewFaceSwapSource.querySelector("#canvasSource");
    var canvasRectanglesListSource = [];
    if (canvasRectanglesSource) {
      canvasRectanglesListSource = JSON.parse(
        canvasRectanglesSource.dataset.rectangles
      );
    }
    ...
    if (imgFaceSwapSource) {
      ...
      var messageSetP = await translateWithGoogle(
        "Вы не загрузили изображение для источника лица. Нажмите на окно загрузки изображения.",
        "auto",
        targetLang
      );
      messageFaceSwap.innerHTML = `<p style='margin-top: 5pt;'>${messageSetP}</p>`;
    ...
    if (canvasRectanglesListTarget.length === 0) {
      messageFaceSwap.innerHTML +=
        "<p style='margin-top: 5pt;'>Вы не выделили лицо. Нажмите на кнопку выделить лицо и выделите лицо на целевом контенте.</p>";
    }

    if (canvasRectanglesListSource.length === 0) {
      messageFaceSwap.innerHTML +=
        "<p style='margin-top: 5pt;'>Вы не выделили лицо. Нажмите на кнопку выделить лицо и выделите лицо на источнике.</p>";
    }

    // advanced settings
    var backgroundEnhancerFaceSwap = elem.querySelector(
    ...
    if (
      mediaNameTarget &&
      mediaNameSource &&
      canvasRectanglesListTarget.length > 0 &&
      canvasRectanglesListSource.length > 0
    ) {
