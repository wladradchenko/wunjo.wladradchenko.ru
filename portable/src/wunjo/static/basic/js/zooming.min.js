...
//				    this.el.dataset.eventClickX = 0;
//				    this.el.dataset.eventClickY = 0;
//                    this.el.addEventListener('click', (event) => {
//                        const rect = this.el.getBoundingClientRect();
//                        const clickXRelative = event.clientX - rect.left - rect.width / 2;
//                        const clickYRelative = event.clientY - rect.top - rect.height / 2;
//                        this.el.dataset.eventClickX = clickXRelative;
//                        this.el.dataset.eventClickY = clickYRelative;
//                    });
...
//                };
			...
			calculateTranslate: function() {
				...
				return {
					x: parseFloat(this.el.dataset.eventClickX) < 0 ? Math.max(-this.rect.width / 2, -1 * parseFloat(this.el.dataset.eventClickX)) : Math.min(this.rect.width / 2, -1 * parseFloat(this.el.dataset.eventClickX)),
					y: parseFloat(this.el.dataset.eventClickY) < 0 ? Math.max(-this.rect.height / 2, -1 * parseFloat(this.el.dataset.eventClickY)) : Math.min(this.rect.height / 2, -1 * parseFloat(this.el.dataset.eventClickY))
				}
			},
...
function handleZoomClick(img) {
    if (!img.dataset.hasListener) {  // Check if there is already a handler
        img.addEventListener('click', (event) => {
            const rect = img.getBoundingClientRect();
            const clickXRelative = event.clientX - rect.left - rect.width / 2;
            const clickYRelative = event.clientY - rect.top - rect.height / 2;
            img.dataset.eventClickX = clickXRelative;
            img.dataset.eventClickY = clickYRelative;
        });
        img.dataset.hasListener = 'true';  // Mark that the handler has been added
    }
}