...
QDomDocument TitleDocument::xmlItem(QGraphicsItem *item, int width, int height, bool embedImages, const QString &projectPath)
{
    ...
    case QGraphicsTextItem::Type: {
        ...
        if (t->data(OriginXLeft).toInt() == AxisInverted) {
            content.setAttribute(QStringLiteral("wunjo-axis-x-inverted"), t->data(OriginXLeft).toInt());
        }
        if (t->data(OriginYTop).toInt() == AxisInverted) {
            content.setAttribute(QStringLiteral("wunjo-axis-y-inverted"), t->data(OriginYTop).toInt());
        }
        if (t->textWidth() > 0) {
                               ...
                               GraphicsSceneRectMove *scene, QGraphicsRectItem *startv, QGraphicsRectItem *endv, int *duration, int &missingElements)
{
    ...
    if (doc.documentElement().hasAttribute(QStringLiteral("duration"))) {
        ...
        *duration = Timecode().getFrameCount(WunjoSettings::title_duration());
    }
...
QGraphicsItem *TitleDocument::loadItemFromXml(const QDomNode &itemNode, const QString &path, int width, int height, int &missingElements, int &maxZValue)
{
    ...
    if (zValue > -1000) {
        if (itemNode.attributes().namedItem(QStringLiteral("type")).nodeValue() == QLatin1String("QGraphicsTextItem")) {
            ...
            // Older Wunjo version did not store pixel size but point size
            ...
            if (!txtProperties.namedItem(QStringLiteral("wunjo-axis-x-inverted")).isNull()) {
                txt->setData(OriginXLeft, txtProperties.namedItem(QStringLiteral("wunjo-axis-x-inverted")).nodeValue().toInt());
            }
            if (!txtProperties.namedItem(QStringLiteral("wunjo-axis-y-inverted")).isNull()) {
                txt->setData(OriginYTop, txtProperties.namedItem(QStringLiteral("wunjo-axis-y-inverted")).nodeValue().toInt());
            }
