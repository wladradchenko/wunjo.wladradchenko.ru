    ...
    , m_guides(QList<QGraphicsLineItem *>())
{
    setupUi(this);
    if (TITLERVERSION == 0) {
        if (WunjoSettings::titlerVersion() < 600) {
            ...
            WunjoSettings::setTitlerVersion(int(ceil(100 * metadata->get_double("version"))));
        }
        TITLERVERSION = WunjoSettings::titlerVersion();
    }
    ...
    QAbstractButton *selectedButton = m_textAlignGroup->button(WunjoSettings::titlerAlign());
    if (selectedButton) {
    ...
    m_duration->setValue(WunjoSettings::title_duration());
    ...
    connect(m_textAlignGroup, QOverload<QAbstractButton *>::of(&QButtonGroup::buttonClicked), this, [this]() {
        WunjoSettings::setTitlerAlign(m_textAlignGroup->checkedId());
        slotUpdateText();
    });
    ...
    displayBg->setChecked(WunjoSettings::titlerShowbg());
    m_timePos->setEnabled(WunjoSettings::titlerShowbg());
    connect(displayBg, &QCheckBox::checkStateChanged, this, [&](Qt::CheckState state) {
        WunjoSettings::setTitlerShowbg(state == Qt::Checked);
        bgBox->setEnabled(!WunjoSettings::titlerShowbg());
        m_timePos->setEnabled(WunjoSettings::titlerShowbg());
        displayBackgroundFrame();
    });

    bgBox->setCurrentIndex(WunjoSettings::titlerbg());
    bgBox->setEnabled(!WunjoSettings::titlerShowbg());
    connect(bgBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, [&](int ix) {
        WunjoSettings::setTitlerbg(ix);
        displayBackgroundFrame();
    });
    ...
    m_unicodeAction = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-insert-unicode")), QString(), this);
    ...
    m_selectText = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-select-texts")), QString(), this);
    ...
    m_selectRects = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-select-rects")), QString(), this);
    ...
    m_selectImages = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-select-images")), QString(), this);
    ...
    m_buttonRect = m_toolbar->addAction(QIcon::fromTheme(QStringLiteral("wunjo-insert-rect")), i18n("Add Rectangle"));
    ...
    m_buttonDownload = new KNSWidgets::Action(i18n("Download New Title Templates…"), QStringLiteral(":data/wunjo_titles.knsrc"), this);
    ...
    show_guides->setChecked(WunjoSettings::titlerShowGuides());
    hguides->setValue(WunjoSettings::titlerHGuides());
    vguides->setValue(WunjoSettings::titlerVGuides());
    guideColor->setColor(WunjoSettings::titleGuideColor());
    ...
    createButton->setEnabled(WunjoSettings::producerslist().contains(QStringLiteral("kdenlivetitle")));
...
void TitleWidget::slotImageTool()
{
    ...
    QString clipFolder = KRecentDirs::dir(QStringLiteral(":WunjoImageFolder"));
    ...
    if (url.isValid()) {
        KRecentDirs::add(QStringLiteral(":WunjoImageFolder"), url.adjusted(QUrl::RemoveFilename).toLocalFile());
...
void TitleWidget::displayBackgroundFrame()
{
    if (!displayBg->isChecked()) {
        ...
        default: {
            QColor col = WunjoSettings::titlerbg() == 1 ? Qt::black : Qt::white;
            QPixmap bg(int(r.width()), int(r.height()));
            QPainter p2(&bg);
...
{
    if (!url.isValid()) {
        QString startFolder = KRecentDirs::dir(QStringLiteral(":WunjoProjectsTitles"));
        url = QFileDialog::getOpenFileUrl(this, i18n("Load Title"), QUrl::fromLocalFile(startFolder.isEmpty() ? m_projectTitlePath : startFolder),
                                          i18n("Wunjo title") + QStringLiteral(" (*.kdenlivetitle)"));
    }
    if (url.isValid()) {
        ...
        KRecentDirs::add(QStringLiteral(":WunjoProjectsTitles"), m_projectTitlePath);
    }
}
...
void TitleWidget::showGuides(int state)
{
    ...
    WunjoSettings::setTitlerShowGuides(state == Qt::Checked);
}

void TitleWidget::updateGuides(int)
{
    WunjoSettings::setTitlerHGuides(hguides->value());
    WunjoSettings::setTitlerVGuides(vguides->value());
    if (!m_guides.isEmpty()) {
        qDeleteAll(m_guides);
        m_guides.clear();
    }
    QPen framepen;
    QColor gColor(WunjoSettings::titleGuideColor());
...
{
    WunjoSettings::setTitleGuideColor(col);
    QColor guideCol(col);
    for (QGraphicsLineItem *it : std::as_const(m_guides)) {
