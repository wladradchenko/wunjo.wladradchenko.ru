...
{
    QDir root(WunjoSettings::mltpath());
    if (!root.cd(QStringLiteral("../presets/consumer/avformat"))) {
        // Cannot find MLT's presets directory
        qCWarning(WUNJO_LOG) << " / / / WARNING, cannot find MLT's preset folder";
        return;
    }
...
std::unique_ptr<RenderPresetModel> &RenderPresetRepository::getPreset(const QString &name)
{
    ...
    if (!presetExists(name)) {
        // TODO
        qCWarning(WUNJO_LOG) << "//// WARNING: profile not found: " << name << ". Returning default h264 profile instead.";
        QString default_profile = QStringLiteral("MP4-H264/AAC");
        if (m_profiles.count(default_profile) == 0) {
            qCWarning(WUNJO_LOG) << "//// WARNING: default profile not found: " << default_profile << ". Returning random profile instead.";
...
bool RenderPresetRepository::deletePreset(const QString &name, bool dontRefresh)
{
    ...
    if (!edit.endsWith(QLatin1String("customprofiles.xml"))) {
        ...
        KNS::Engine engine(0);
        if (engine.init("wunjo_render.knsrc")) {
