...
bool AbstractTreeModel::checkConsistency()
{
    ...
    if (!rootItem || !rootItem->m_isRoot || !rootItem->isInModel() || m_allItems.count(rootItem->getId()) == 0) {
        qCCritical(WUNJO_LOG) << "AbstractTreeModel is not valid because root is not properly constructed";
        return false;
    }
    ...
    while (!queue.empty()) {
        ...
        if (seenIDs.count(currentId) != 0) {
            qCCritical(WUNJO_LOG) << "Invalid tree: Id found twice."
                                     << "It either a cycle or a clash in id attribution";
            return false;
        }
        if (m_allItems.count(currentId) == 0) {
            qCCritical(WUNJO_LOG) << "Invalid tree: Id not found. Item is not registered";
            return false;
        }
        auto currentItem = m_allItems[currentId].lock();
        if (currentItem->depth() != currentDepth) {
            qCCritical(WUNJO_LOG) << "Invalid tree: invalid depth info found";
            return false;
        }
        if (!currentItem->isInModel()) {
            qCCritical(WUNJO_LOG) << "Invalid tree: item thinks it is not in a model";
            return false;
        }
        if (currentId != rootItem->getId()) {
            if ((currentDepth == 0 || currentItem->m_isRoot)) {
                qCCritical(WUNJO_LOG) << "Invalid tree: duplicate root";
                return false;
            }
            if (auto ptr = currentItem->parentItem().lock()) {
                if (ptr->getId() != parentId || ptr->child(currentItem->row())->getId() != currentItem->getId()) {
                    qCCritical(WUNJO_LOG) << "Invalid tree: invalid parent link";
                    return false;
                }
            } else {
                qCCritical(WUNJO_LOG) << "Invalid tree: invalid parent";
                return false;
            }
        }
        ...
        for (const auto &child : currentItem->m_childItems) {
            if (currentItem->child(i) != child) {
                qCCritical(WUNJO_LOG) << "Invalid tree: invalid child ordering";
                return false;
            }
