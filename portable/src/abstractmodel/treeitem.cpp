...
std::shared_ptr<TreeItem> TreeItem::appendChild(const QList<QVariant> &data)
{
    ...
    qCCritical(WUNJO_LOG) << "Something went wrong when appending child in TreeItem. Model is not available anymore";
    Q_ASSERT(false);
...
bool TreeItem::appendChild(const std::shared_ptr<TreeItem> &child)
{
    ...
    if (auto oldParent = child->parentItem().lock()) {
        ...
        qCCritical(WUNJO_LOG) << "Trying to append a child to TreeItem that alrealdy has a parent";
        return false;
    }
    if (auto ptr = m_model.lock()) {
...
void TreeItem::moveChild(int ix, const std::shared_ptr<TreeItem> &child)
{
    if (auto ptr = m_model.lock()) {
        ...
        qCCritical(WUNJO_LOG) << "Something went wrong when moving child in TreeItem. Model is not available anymore";
        Q_ASSERT(false);
    }
}
...
void TreeItem::removeChild(const std::shared_ptr<TreeItem> &child)
{
    if (auto ptr = m_model.lock()) {
        ...
        qCCritical(WUNJO_LOG) << "Something went wrong when removing child in TreeItem. Model is not available anymore";
        Q_ASSERT(false);
    }
}
...
void TreeItem::registerSelf(const std::shared_ptr<TreeItem> &self)
{
    ...
    if (auto ptr = self->m_model.lock()) {
        ...
        qCCritical(WUNJO_LOG) << "Construction of treeItem failed because parent model is not available anymore";
        Q_ASSERT(false);
    }
}
