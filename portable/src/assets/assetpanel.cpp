...
#include "plugins/pluginsetpanel.h"
...
AssetPanel::AssetPanel(QWidget *parent)
    ...
    , m_pluginSetPanel(new PluginSetPanel(this))
{
    ...
    connect(m_switchCompoButton, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this, [&]() {
        if (m_transitionWidget->stackOwner().type == WunjoObjectType::TimelineComposition) {
    ...
    applyToSameOnly->setChecked(WunjoSettings::applyEffectParamsToGroupWithSameValue());
    connect(applyToSameOnly, &QAction::toggled, this, [](bool enabled) { WunjoSettings::setApplyEffectParamsToGroupWithSameValue(enabled); });
    ...
    m_applyEffectGroups->menuAction()->setChecked(WunjoSettings::applyEffectParamsToGroup());
    connect(m_applyEffectGroups->menuAction(), &QAction::triggered, this, [this]() {
        WunjoSettings::setApplyEffectParamsToGroup(m_applyEffectGroups->menuAction()->isChecked());
    ...
    lay->addWidget(m_pluginSetPanel);
    ...
    m_pluginSetPanel->setVisible(false);
    // The pen next to a plugin effect's set list swaps the stack for the panel
    // that records those sets, and going back restores the stack.
    connect(pCore.get(), &Core::switchPluginSetPanel, this, [this](const QString &pluginId, const QString &kind, bool show) {
        if (show) {
            m_pluginSetPanel->setPlugin(pluginId, kind);
        }
        m_pluginSetPanel->setVisible(show);
        m_effectStackWidget->setVisible(!show);
        m_maskManager->setVisible(false);
    });
    connect(m_pluginSetPanel, &PluginSetPanel::closeRequested, this, [this]() { Q_EMIT pCore->switchPluginSetPanel(QString(), QString(), false); });
...
void AssetPanel::showMix(int cid, const std::shared_ptr<AssetParameterModel> &transitionModel, bool refreshOnly)
{
    ...
    ObjectId id(WunjoObjectType::TimelineMix, cid, transitionModel->getOwnerId().uuid);
    if (refreshOnly) {
...
{
    if ((m_effectStackWidget->isVisible() && m_effectStackWidget->isLocked()) || m_maskManager->isLocked() || m_pluginSetPanel->isLocked()) {
        return;
    }
    ...
    if (m_effectStackWidget->stackOwner() == id) {
        ...
        if (id.type == WunjoObjectType::TimelineClip && m_splitButton->isActive()) {
            m_splitButton->setActive(false);
    ...
    m_enableStackButton->setVisible(id.type != WunjoObjectType::TimelineComposition);
...
void AssetPanel::clearAssetPanel(int itemId)
{
    ...
    if (id.type == WunjoObjectType::TimelineClip && id.itemId == itemId) {
        clear();
        return;
    }
    id = m_transitionWidget->stackOwner();
    if (id.type == WunjoObjectType::TimelineComposition && id.itemId == itemId) {
        clear();
        return;
    }
    id = m_mixWidget->stackOwner();
    if (id.type == WunjoObjectType::TimelineMix && id.itemId == itemId) {
        clear();
    }
}
...
void AssetPanel::clear()
{
    ...
    m_maskManager->setOwner(ObjectId(WunjoObjectType::NoItem, {}));
    m_assetTitle->clear();
}

void AssetPanel::processSplitEffect(bool enable)
{
    WunjoObjectType id = m_effectStackWidget->stackOwner().type;
    if (id == WunjoObjectType::TimelineClip) {
        Q_EMIT doSplitEffect(enable);
    } else if (id == WunjoObjectType::BinClip) {
...
{
    if (m_effectStackWidget->isVisible()) {
        ObjectId id(WunjoObjectType::TimelineClip, itemId, uuid);
        if (m_effectStackWidget->stackOwner() == id) {
            Q_EMIT pCore->getMonitor(Wunjo::ProjectMonitor)->seekPosition(pCore->getMonitorPosition());
        }
    } else if (m_transitionWidget->isVisible()) {
        ObjectId id(WunjoObjectType::TimelineComposition, itemId, uuid);
        if (m_transitionWidget->stackOwner() == id) {
            Q_EMIT pCore->getMonitor(Wunjo::ProjectMonitor)->seekPosition(pCore->getMonitorPosition());
        }
    }
