...
template <> void CurveParamWidget<BezierSplineEditor>::slotShowAllHandles(bool show)
{
    ...
    WunjoSettings::setBezier_showallhandles(show);
}
...
CurveParamWidget<CurveWidget_t>::CurveParamWidget(std::shared_ptr<AssetParameterModel> model, QModelIndex index, QWidget *parent)
    ...
    , m_showPixmap(WunjoSettings::bezier_showpixmap())
{
    ...
    m_edit->setGridLines(WunjoSettings::bezier_gridlines());
    m_ui.buttonShowPixmap->setChecked(WunjoSettings::bezier_showpixmap());
    m_ui.buttonShowAllHandles->setChecked(WunjoSettings::bezier_showallhandles());
    slotShowAllHandles(WunjoSettings::bezier_showallhandles());
...
template <typename CurveWidget_t> void CurveParamWidget<CurveWidget_t>::slotGridChange()
{
    ...
    WunjoSettings::setBezier_gridlines(m_edit->gridLines());
}
...
{
    m_showPixmap = show;
    WunjoSettings::setBezier_showpixmap(show);
    if (show) {
        if (m_mode == CurveModes::Hue) {
            m_edit->setPixmap(
