    ...
    , m_listType(OTHERLIST)
{
    ...
    if (!configFile.isEmpty()) {
        ...
        connect(m_knsbutton, &KNSWidgets::Button::dialogFinished, this, [this, configFile](const QList<KNSCore::Entry> &changedEntries) {
            if (changedEntries.count() > 0) {
                if (configFile.contains(QStringLiteral("wunjo_wipes.knsrc"))) {
    ...
    if (!values.isEmpty()) {
        if (values.first() == QLatin1String("%lumaPaths")) {
            ...
            m_listType = MASKLIST;
        } else if (values.first().startsWith(QLatin1String("%pluginSets"))) {
            m_listType = PLUGINSETLIST;
            // a plugin that does several things keeps several sorts of preset;
            // the parameter says which of them it takes
            m_setKind = values.first().section(QLatin1Char(':'), 1);
        }
    }
    ...
    } else if (m_listType == PLUGINSETLIST) {
        // the pen leads to where sets are recorded, the list is only the choice
        m_maskButton->setIcon(QIcon::fromTheme(QStringLiteral("document-edit")));
        m_maskButton->setToolTip(i18n("Record a set from a video or a photo"));
        connect(m_maskButton, &QToolButton::clicked, this, [this]() {
            const QString plugin = PluginManager::instance().pluginForEffect(m_model->getAssetId());
            if (!plugin.isEmpty()) {
                Q_EMIT pCore->switchPluginSetPanel(plugin, m_setKind, true);
            }
        });
    } else {
        m_maskButton->hide();
    ...
    connect(this->m_list, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this, [this](int index) {
        if (m_list->currentData() == QStringLiteral("custom_file")) {
            ...
            if (m_listType == MASKLIST) {
                ...
                applyPluginSet(m_list->currentData().toString());
            } else {
...
void UrlListParamWidget::slotRefresh()
{
    ...
    if (!values.isEmpty() && values.first() == QLatin1String("%lumaPaths")) {
        ...
        names.clear();
        m_listType = LUTLIST;
        // check for Wunjo installed luts files
        ...
        const QString lastUsedPath = KRecentDirs::dir(QStringLiteral(":WunjoUrlLutParamFolder"));
        ...
        while (l.hasNext()) {
            l.next();
            if (l.value().endsWith(QLatin1String(".cube"), Qt::CaseInsensitive) && !WunjoSettings::validated_luts().contains(l.value())) {
                // Open LUT file and check validity
                if (isValidCubeFile(l.value())) {
                    QStringList validated = WunjoSettings::validated_luts();
                    validated << l.value();
                    WunjoSettings::setValidated_luts(validated);
                } else {
        ...
        } else if (owner.type == WunjoObjectType::BinClip) {
    ...
    } else if (!values.isEmpty() && values.first().startsWith(QLatin1String("%pluginSets"))) {
        // the sets the owning plugin recorded in this project
        values.clear();
        names.clear();
        m_listType = PLUGINSETLIST;
        const QString plugin = PluginManager::instance().pluginForEffect(m_model->getAssetId());
        ...
        if (!currentValue.isEmpty() && m_list->findData(currentValue) < 0) {
            ...
            // switching the effect to another one
            m_list->addItem(i18n("%1 (missing)", QFileInfo(currentValue).completeBaseName()), currentValue);
        }
    }
    ...
    if (m_listType == OTHERLIST) {
        ...
        // sets are recorded in their panel, never picked as a loose file
...
void UrlListParamWidget::showEvent(QShowEvent *event)
{
    QWidget::showEvent(event);
    if (m_listType == PLUGINSETLIST) {
        // coming back from the set panel, where sets were just added or deleted
        slotRefresh();
    }
}

void UrlListParamWidget::applyPluginSet(const QString &file)
{
    if (file.isEmpty()) {
        Q_EMIT valueChanged(m_index, QString(), true);
        m_currentIndex = m_list->currentIndex();
        return;
    }
    const PluginSets::Set set = PluginSets::read(file);
    if (!set.isValid()) {
        m_list->setCurrentIndex(m_currentIndex);
        return;
    }
    // The effect only points at the set: the recording plays at the density it
    // was made with, frame by frame, and the plugin reads it when it renders.
    // Squeezing it into the effect's keyframes would mean either a ruler with a
    // point on every frame — unusable, and slow enough to stall the interface —
    // or throwing away the fast moves; a blink lasts two frames. The keyframe
    // timeline below stays the user's own instead: what is drawn there is added
    // on top of the recording.
    Q_EMIT valueChanged(m_index, file, true);
    m_currentIndex = m_list->currentIndex();
    const int duration = pCore->getItemDuration(m_model->getOwnerId());
    if (duration > 0 && set.count > duration) {
        pCore->displayMessage(i18n("%1 holds %2 frames, this clip is %3 — the rest is cut when it renders", set.name, set.count, duration), InformationMessage);
    }
}
...
void UrlListParamWidget::openFile()
{
    ...
    switch (m_listType) {
    case LUTLIST:
        path = KRecentDirs::dir(QStringLiteral(":WunjoUrlLutParamFolder"));
        break;
    case MASKLIST: {
        bool ok;
    ...
    case LUMALIST:
    default:
        path = KRecentDirs::dir(QStringLiteral(":WunjoUrlListParamFolder"));
    }
    ...
    if (!urlString.isEmpty()) {
        if (m_listType == LUTLIST) {
            KRecentDirs::add(QStringLiteral(":WunjoUrlLutParamFolder"), QFileInfo(urlString).absolutePath());
        } else if (m_listType == LUMALIST || m_listType == OTHERLIST) {
            KRecentDirs::add(QStringLiteral(":WunjoUrlListParamFolder"), QFileInfo(urlString).absolutePath());
        }
