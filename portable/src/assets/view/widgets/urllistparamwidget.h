...
protected:
    /** @brief The recorded sets are project data: pick up what the set panel
     *  added or removed while the effect stack was hidden behind it. */
    void showEvent(QShowEvent *event) override;

private:
    enum ListType { LUMALIST, LUTLIST, MASKLIST, PLUGINSETLIST, OTHERLIST };
    /** @brief Put the keyframes of the chosen set on the effect's parameters,
     *  asking first when the recording outlasts the clip. */
    void applyPluginSet(const QString &file);
    QStringList m_fileExt;
    /** @brief Which sort of preset this parameter takes, when the plugin has
     *  more than one ("%pluginSets:face"). */
    QString m_setKind;
    int m_currentIndex;
    ListType m_listType;
