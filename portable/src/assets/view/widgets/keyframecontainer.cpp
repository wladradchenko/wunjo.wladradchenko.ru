    ...
    , m_layout(layout)
{
    ...
    seekKeyframe->setChecked(WunjoSettings::keyframeseek());
    connect(seekKeyframe, &QAction::triggered, [&](bool selected) { WunjoSettings::setKeyframeseek(selected); });
    ...
    switch (WunjoSettings::defaultkeyframeinterp()) {
    ...
    connect(kfType, &KSelectAction::actionTriggered, this, [&](QAction *ac) { WunjoSettings::setDefaultkeyframeinterp(ac->data().toInt()); });
    ...
    autoLimit->setChecked(WunjoSettings::limitAutoKeyframes() > 0);
    connect(autoLimit, &QAction::toggled, this, [this](bool toggled) {
        if (toggled) {
            WunjoSettings::setLimitAutoKeyframes(WunjoSettings::limitAutoKeyframesInterval());
        } else {
            WunjoSettings::setLimitAutoKeyframes(0);
        }
    });
...
void KeyframeContainer::monitorSeek(int pos)
{
    int in = 0;
    int out = 0;
    bool canHaveZone = m_model->getOwnerId().type == WunjoObjectType::Master || m_model->getOwnerId().type == WunjoObjectType::TimelineTrack;
    if (canHaveZone) {
        bool ok = false;
...
void KeyframeContainer::slotRefresh()
{
    ...
    m_time->setRange(0, duration - 1);
    if (m_model->monitorId == Wunjo::ProjectMonitor) {
...
void KeyframeContainer::slotUpdateKeyframesFromMonitor(const QPersistentModelIndex &index, const QVariant &res)
{
    ...
    if (m_keyframes->isEmpty()) {
        ...
        if (m_time->getValue() > 0) {
            ...
            if (type == ParamType::Roto_spline) {
                if (m_model->monitorId == Wunjo::ClipMonitor) {
    ...
    if (WunjoSettings::autoKeyframe() && m_neededScene != SceneType::MonitorSceneDefault) {
...
void KeyframeContainer::slotAddRemove(bool addOnly)
{
    ...
    if (m_keyframes->hasKeyframe(position)) {
        ...
        if (addOnPlay && WunjoSettings::limitAutoKeyframes() > 0) {
            if (m_lastKeyframePos < 0) {
                m_lastKeyframePos = position;
            } else if (position < m_lastKeyframePos) {
                m_lastKeyframePos = -1;
            } else if (position - m_lastKeyframePos < WunjoSettings::limitAutoKeyframes()) {
                // Abort keyframe
                return;
            } else {
                // Proceed
...
bool KeyframeContainer::slotAddKeyframe(int pos)
{
    ...
    return m_keyframes->addKeyframe(GenTime(pos, pCore->getCurrentFps()), KeyframeType::KeyframeEnum(WunjoSettings::defaultkeyframeinterp()));
}
