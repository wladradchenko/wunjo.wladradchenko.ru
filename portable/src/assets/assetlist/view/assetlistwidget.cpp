    ...
    , m_isEffect(isEffect)
{
    ...
    if (m_isEffect) {
        ...
        videoEffects->setIcon(QIcon::fromTheme(QStringLiteral("wunjo-show-video")));
        ...
        customEffects->setIcon(QIcon::fromTheme(QStringLiteral("wunjo-custom-effect")));
    ...
    if (m_isEffect) {
        m_filterButton->setChecked(WunjoSettings::effectsFilter());
    } else {
        m_filterButton->setChecked(WunjoSettings::transitionsFilter());
    }

    connect(includeList, &QAction::triggered, this, [this, tenBit](bool enable) {
        WunjoSettings::setEnableAssetsIncludeList(enable);
        if (enable) {
    ...
    connect(tenBit, &QAction::toggled, this, [this, includeList](bool enabled) {
        WunjoSettings::setTenbitpipeline(enabled);
        if (enabled) {
            if (!m_filterButton->isChecked()) {
                QSignalBlocker bk(m_filterButton);
                m_filterButton->setChecked(true);
    ...
    if (m_isEffect) {
        KNSWidgets::Action *downloadAction = new KNSWidgets::Action(i18n("Download New Effects..."), QStringLiteral(":data/wunjo_effects.knsrc"), this);
        ...
        KNSWidgets::Action *downloadAction = new KNSWidgets::Action(i18n("Download New Wipes..."), QStringLiteral(":data/wunjo_wipes.knsrc"), this);
        m_toolbar->addAction(downloadAction);
        connect(downloadAction, &KNSWidgets::Action::dialogFinished, this, [&](const QList<KNSCore::Entry> &changedEntries) {
            if (changedEntries.count() > 0) {
    ...
    showInfo->setChecked(m_isEffect ? WunjoSettings::showEffectsInfo() : WunjoSettings::showTransitionsInfo());
    ...
    if (m_isEffect) {
        // Icon view for effects still needs work
        /*if (WunjoSettings::effectViewAsIcon()) {
            toggleView->setChecked(true);
            toggleViewMode(true);
        }*/
    } else {
        if (WunjoSettings::transitionViewAsIcon()) {
            toggleView->setChecked(true);
            toggleViewMode(true);
        }
    }
...
{
    return WunjoSettings::showeffectinfo();
}

void AssetListWidget::setShowDescription(bool show)
{
    WunjoSettings::setShoweffectinfo(show);
...
void AssetListWidget::onCustomContextMenu(const QPoint &pos)
{
    ...
    if (index.isValid()) {
        ...
        if (assetId != QStringLiteral("root")) {
            ...
            // An effect that came with a plugin sits in the same folder as the
            // user's own ones, but belongs to its plugin: editing or deleting it
            // here would be undone the next time that plugin syncs its effects.
            bool isCustom = isCustomType(itemType) && !PluginManager::instance().ownsEffect(assetId);
            for (auto &ac : actions) {
...
void AssetListWidget::updateAssetInfo(const QModelIndex &current, const QModelIndex &)
{
    if (current.isValid()) {
        ...
        if (!isCustomType(type) && !PluginManager::instance().ownsEffect(id)) {
            // Add link to our documentation (a plugin's effect is not in it)
...
void AssetListWidget::toggleViewMode(bool checked)
{
    ...
    if (m_isEffect) {
        WunjoSettings::setEffectViewAsIcon(checked);
    } else {
        WunjoSettings::setTransitionViewAsIcon(checked);
        if (checked) {
