...
bool KeyframeModelList::addKeyframe(GenTime pos, KeyframeType::KeyframeEnum type)
{
    ...
    if (res && WunjoSettings::applyEffectParamsToGroup()) {
...
bool KeyframeModelList::addKeyframe(int frame, double val)
{
    ...
    auto op = [this, pos, val, pType](std::shared_ptr<KeyframeModel> param, bool, Fun &undo, Fun &redo) {
        ...
        return param->addKeyframe(pos, KeyframeType::KeyframeEnum(WunjoSettings::defaultkeyframeinterp()), value, true, undo, redo);
    };
    const QString opText = update ? i18n("Change keyframe type") : i18n("Add keyframe");
    Fun undo = []() { return true; };
    Fun redo = []() { return true; };
    ...
    if (res && WunjoSettings::applyEffectParamsToGroup()) {
        ...
        for (auto km : std::as_const(groupedKfrModels)) {
            ...
            auto op2 = [km, posWithOffset, val, pType](std::shared_ptr<KeyframeModel> param, bool, Fun &undo, Fun &redo) {
                ...
                return param->addKeyframe(posWithOffset, KeyframeType::KeyframeEnum(WunjoSettings::defaultkeyframeinterp()), value, true, undo, redo);
            };
            res = res && km->applyOperation(op2, undo, redo);
        }
    }
...
bool KeyframeModelList::removeKeyframeWithUndo(GenTime pos, Fun &undo, Fun &redo)
{
    ...
    if (result && WunjoSettings::applyEffectParamsToGroup()) {
        ObjectId id = getOwnerId();
        double fps = pCore->getCurrentFps();
        GenTime offset = pos - GenTime(pCore->getItemIn(id), fps);
...
void KeyframeModelList::setParametersFromTask(const paramVector &params)
{
    ...
    // refresh() rebuilds the keyframes with addKeyframe(notify = false), which
    // deliberately keeps the row signals quiet — so nothing would tell a view
    // that everything changed. This method exists for fills that come from
    // outside the interface (an analysis, a recorded set), so it has to say so.
...
void KeyframeModelList::checkConsistency()
{
    ...
    auto type = KeyframeType::KeyframeEnum(WunjoSettings::defaultkeyframeinterp());
    for (const auto &param : m_parameters) {
        QList<GenTime> list = param.second->getKeyframePos();
