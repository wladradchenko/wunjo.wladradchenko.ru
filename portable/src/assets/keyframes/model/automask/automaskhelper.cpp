...
void AutomaskHelper::addMonitorControlPoint(int position, const QSize frameSize, int xPos, int yPos, bool extend, bool exclude)
{
    ...
    pCore->getMonitor(Wunjo::ClipMonitor)->setUpEffectGeometry(points, pointsTypes, keyframes, box);
    generateImage();
}
...
void AutomaskHelper::addMonitorControlRect(int position, const QSize frameSize, const QRect rect, bool extend)
{
    ...
    pCore->getMonitor(Wunjo::ClipMonitor)->setUpEffectGeometry(points, pointsTypes, keyframes, rect);
    generateImage();
}
...
void AutomaskHelper::launchSam(const QDir &previewFolder, int offset, const ObjectId &ownerForFilter, bool autoAdd, int previewPos)
{
    ...
    if (!WunjoSettings::samDevice().isEmpty()) {
        args << QStringLiteral("-D") << WunjoSettings::samDevice();
    }
    if (WunjoSettings::sam_offload_video()) {
    ...
    if (WunjoSettings::maskBorderWidth() > 0) {
        args << QStringLiteral("--border") << QString::number(WunjoSettings::maskBorderWidth()) << QStringLiteral("--bordercolor")
             << QStringLiteral("%1,%2,%3,%4")
                    .arg(WunjoSettings::maskBorderColor().red())
                    .arg(WunjoSettings::maskBorderColor().green())
                    .arg(WunjoSettings::maskBorderColor().blue())
                    .arg(WunjoSettings::maskBorderColor().alpha());
    }
    args << QStringLiteral("--color")
         << QStringLiteral("%1,%2,%3,%4")
                .arg(WunjoSettings::maskColor().red())
                .arg(WunjoSettings::maskColor().green())
                .arg(WunjoSettings::maskColor().blue())
                .arg(WunjoSettings::maskColor().alpha());

    connect(&m_samProcess, &QProcess::stateChanged, this, [this](QProcess::ProcessState state) {
        if (state == QProcess::NotRunning) {
            qDebug() << "===== SAM SCRIPT TERMINATED ========";
            pCore->getMonitor(Wunjo::ClipMonitor)->abortPreviewMask();
    ...
    connect(&m_samProcess, &QProcess::readyReadStandardOutput, this, [this, autoAdd]() {
        ...
        if (command.startsWith(QLatin1String("preview ok"))) {
            ...
            pCore->getMonitor(Wunjo::ClipMonitor)->getControllerProxy()->m_previewOverlay = url;
            Q_EMIT pCore->getMonitor(Wunjo::ClipMonitor)->getControllerProxy()->previewOverlayChanged();
        } else if (command == QLatin1String("mask ok")) {
            m_jobStatus = QProcess::NotRunning;
            auto binClip = pCore->projectItemModel()->getClipByBinID(m_binId);
            Q_ASSERT(binClip);
            MaskTask::start(ObjectId(WunjoObjectType::BinClip, m_binId.toInt(), QUuid()), m_ownerForFilter, m_maskParams, binClip.get(), autoAdd);
...
bool AutomaskHelper::generateMask(const QString &binId, const QString &maskName, const QString &maskFile, const QPoint &zone)
{
    ...
    if (!m_includePoints.contains(0) && !m_boxes.contains(0)) {
        KMessageBox::information(pCore->getMonitor(Wunjo::ClipMonitor), i18n("You must define include points in the first video frame of the sequence."));
        pCore->getMonitor(Wunjo::ClipMonitor)->requestSeek(zone.x());
        return false;
    }
...
void AutomaskHelper::sceneUpdated(SceneType::MonitorSceneType sceneType)
{
    ...
    disconnect(pCore->getMonitor(Wunjo::ClipMonitor), &Monitor::sceneChanged, this, &AutomaskHelper::sceneUpdated);
}

void AutomaskHelper::monitorSeek(int pos)
{
    Monitor *mon = pCore->getMonitor(Wunjo::ClipMonitor);
