...
class MonitorProxy : public QObject
{
    ...
    /** @brief Face detection results for the current frame (normalized 0..1 rects) */
    Q_PROPERTY(QVariantList detectedFaces READ detectedFaces NOTIFY detectedFacesChanged)
...
public:
    ...
    QVariantList detectedFaces() const;
    /** @brief Update the face detection overlay; cleared automatically on seek. */
    void setDetectedFaces(const QVariantList &faces);
    /** @brief A detected face box was clicked in the monitor overlay. */
    Q_INVOKABLE void requestFaceMenu(int index);

Q_SIGNALS:
    void detectedFacesChanged();
    /** @brief Request the face actions menu for detectedFaces()[index] */
    void faceMenuRequested(int index);
