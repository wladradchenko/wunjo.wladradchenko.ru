...
#define check_error(fn)                                                                                                                                        \
    {                                                                                                                                                          \
        ...
        if (err != GL_NO_ERROR) {                                                                                                                              \
            qCritical(WUNJO_LOG) << "GL error" << hex << err << dec << "at" << __FILE__ << ":" << __LINE__;                                                 \
        }                                                                                                                                                      \
    }
#endif
    ...
    , m_offset(QPoint(0, 0))
{
    ...
    setAcceptDrops(true);
    setClearColor(WunjoSettings::window_background());

    if (m_id == Wunjo::ClipMonitor && !(WunjoSettings::displayClipMonitorInfo() & Monitor::InfoOverlay)) {
        m_rulerHeight = 0;
    } else if (!(WunjoSettings::displayProjectMonitorInfo() & Monitor::InfoOverlay)) {
        m_rulerHeight = 0;
    }
    ...
    m_blackClip->set("wunjo:id", "black");
...
{
    if (!WunjoSettings::gpu_accel()) return false;
...
void VideoWidget::requestSeek(int position, bool noAudioScrub)
{
    ...
    if (WunjoSettings::audio_scrub() && !noAudioScrub) {
        m_consumer->set("volume", WunjoSettings::volume() / 100.0);
...
void VideoWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    bool playMonitor = allowPlay && WunjoSettings::play_monitor_on_click() &&
...
int VideoWidget::setProducer(const std::shared_ptr<Mlt::Producer> &producer, bool isActive, int position)
{
    ...
    if (m_producer) {
        currentId = m_producer->parent().get("wunjo:id");
    ...
    if (position == -1 && m_producer->parent().get("wunjo:id") == currentId) {
...
int VideoWidget::reconfigure()
{
    ...
    if ((m_consumer == nullptr) || !m_consumer->is_valid() || strcmp(m_consumer->get("mlt_service"), "multi") == 0) {
        ...
        QString audioBackend = (WunjoSettings::external_display()) ? QStringLiteral("decklink:%1").arg(WunjoSettings::blackmagic_output_device())
                                                                      : WunjoSettings::audiobackend();
        if (m_consumer == nullptr || serviceName.isEmpty() || serviceName != audioBackend) {
            ...
            if (m_consumer->is_valid()) {
                ...
                for (const QString &bk : backends) {
                    ...
                    if (m_consumer->is_valid()) {
                        if (audioBackend == WunjoSettings::sdlAudioBackend()) {
                            // switch sdl audio backend
                            WunjoSettings::setSdlAudioBackend(bk);
                        }
                        WunjoSettings::setAudiobackend(bk);
                        serviceName = bk;
                        break;
                    } else {
                        m_consumer.reset();
            ...
            if (WunjoSettings::external_display()) {
    ...
    if (m_consumer->is_valid()) {
        ...
        int dropFrames = 1;
        if (!WunjoSettings::monitor_dropframes()) {
            dropFrames = -dropFrames;
        }
        m_consumer->set("real_time", dropFrames);
        m_consumer->set("channels", pCore->audioChannels());
        if (WunjoSettings::previewScaling() > 1) {
            m_consumer->set("scale", 1.0 / WunjoSettings::previewScaling());
        }
        // C & D
        if (m_glslManager) {
        ...
        int volume = WunjoSettings::volume();
        if (serviceName.startsWith(QLatin1String("sdl"))) {
            QString audioDevice = WunjoSettings::audiodevicename();
            if (!audioDevice.isEmpty()) {
                m_consumer->set("audio_device", audioDevice.toUtf8().constData());
            }

            QString audioDriver = WunjoSettings::audiodrivername();
            if (!audioDriver.isEmpty()) {
                m_consumer->set("audio_driver", audioDriver.toUtf8().constData());
            }
        }
        if (!pCore->getProjectProfile().progressive()) {
            m_consumer->set("progressive", WunjoSettings::monitor_progressive());
        }
        m_consumer->set("volume", volume / 100.0);
        // m_consumer->set("progressive", 1);
        m_consumer->set("rescale", WunjoSettings::mltinterpolation().toUtf8().constData());
        m_consumer->set("deinterlacer", WunjoSettings::mltdeinterlacer().toUtf8().constData());
        /*
#ifdef Q_OS_WIN
        ...
        */
        ...
        m_consumer->set("scrub_audio", WunjoSettings::audio_scrub());
...
bool VideoWidget::switchPlay(bool play, double speed)
{
    ...
    if (play) {
        if (m_consumer->position() >= m_maxProducerPosition && m_producer->position() >= m_maxProducerPosition && speed > 0) {
            ...
            if (m_id == Wunjo::ClipMonitor || (m_id == Wunjo::ProjectMonitor && WunjoSettings::jumptostart())) {
                m_producer->seek(0);
            } else {
                return false;
            }
        ...
        if (qFuzzyCompare(speed, 1.0) || speed < -6. || speed > 6.) {
            m_consumer->set("scrub_audio", 0);
        } else if (WunjoSettings::audio_scrub()) {
            m_consumer->set("scrub_audio", 1);
        }
        if (qFuzzyIsNull(current_speed)) {
            ...
            m_consumer->start();
            m_consumer->set("refresh", 1);
            m_consumer->set("volume", WunjoSettings::volume() / 100.);
        } else {
...
void VideoWidget::startConsumer()
{
    ...
    if (!restartConsumer()) {
        ...
        KMessageBox::error(
            qApp->activeWindow(),
            i18n("Could not create the video preview window.\nThere is something wrong with your Wunjo install or your driver settings, please fix it."));
...
void VideoWidget::setConsumerProperty(const QString &name, const QString &value)
{
    ...
    if (m_consumer) {
        ...
        if (m_consumer->start() == -1) {
            qCWarning(WUNJO_LOG) << "ERROR, Cannot start monitor";
        }
    }
}
