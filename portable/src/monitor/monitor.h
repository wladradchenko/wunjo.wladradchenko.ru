...
public:
    ...
    Monitor(Wunjo::MonitorId id, MonitorManager *manager, QWidget *parent = nullptr);
    ~Monitor() override;
...
private:
    ...
    /** @brief Effect stack of the clip the face overlay currently describes. */
    std::shared_ptr<EffectStackModel> faceEffectStack();

    std::shared_ptr<ProjectClip> m_controller;
    /** @brief Face detection overlay state (see refreshFaceDetection) */
    bool m_faceDetectEnabled{false};
    /** @brief Content shown by the face overlay (set in updateFaceOverlay) */
    QString m_faceOverlayBinId;
    int m_faceOverlayTimelineCid{-1};
    int m_faceOverlayClipIn{0};
    int m_faceOverlaySourcePos{0};
...
public Q_SLOTS:
    ...
    */
    ...
    /** @brief Redraw the face boxes for the current position from the data
     *  collected by FaceDetectTask (never re-detects). */
    void updateFaceOverlay();
    /** @brief Show the actions menu for a clicked face box. */
    void showFaceMenu(int index);
    /** @brief Hide the clicked face: a Motion Tracker effect pre-filled from
     *  the face track (blur type and strength adjustable in the stack). */
    void applyFaceEffect(int index);
    /** @brief Put an effect a face plugin brought on the clicked face (with the
     *  region effect it works inside), filling what it marked with the track. */
    void applyPluginFaceEffect(int index, const PluginManifest &plugin, const PluginEffect &effect);
    /** @brief Re-evaluate whether continuous face detection should run on
     *  this monitor (per-clip property, see FaceDataStore). */
    void refreshFaceDetection();
