...

{
    ...
    connect(&refreshTimer, &QTimer::timeout, this, [this]() {
        if (m_activeMonitor == m_clipMonitor) {
            activateMonitor(Wunjo::ProjectMonitor, true, true);
            refreshProjectMonitor(true);
            activateMonitor(Wunjo::ClipMonitor, true, true);
        }
    });
...
AbstractMonitor *MonitorManager::monitor(Wunjo::MonitorId monitorName)
{
...
void MonitorManager::lockMonitor(Wunjo::MonitorId name, bool lock)
{
    Q_UNUSED(name)
    if (lock) {
        m_refreshMutex.lock();
...
void MonitorManager::focusProjectMonitor()
{
    if (!m_projectMonitor->isActive()) {
        activateMonitor(Wunjo::ProjectMonitor);
...
bool MonitorManager::activateMonitor(Wunjo::MonitorId name, bool raiseMonitor, bool quickSwitch)
{
    ...
    if (m_activeMonitor) {
        if (name == Wunjo::ClipMonitor) {
            ...
            if (!quickSwitch) {
                ...
                Q_EMIT updateOverlayInfos(name, WunjoSettings::displayClipMonitorInfo());
        ...
        } else if (name == Wunjo::ProjectMonitor) {
            // Set guides list to show guides
            ...
            if (!quickSwitch) {
                ...
                Q_EMIT updateOverlayInfos(name, WunjoSettings::displayProjectMonitorInfo());
...
bool MonitorManager::isActive(Wunjo::MonitorId id) const
{
...
{
    if (activateClip) {
        activateMonitor(Wunjo::ClipMonitor);
    } else {
        activateMonitor(Wunjo::ProjectMonitor);
    }
}
...
void MonitorManager::setupActions()
{
    ...
    enableEditmode->setChecked(WunjoSettings::showOnMonitorScene());
    ...
    if (WunjoSettings::mltdeinterlacer() == QLatin1String("linearblend")) {
        interlace->setCurrentItem(1);
    } else if (WunjoSettings::mltdeinterlacer() == QLatin1String("yadif-nospatial")) {
        interlace->setCurrentItem(2);
    } else if (WunjoSettings::mltdeinterlacer() == QLatin1String("yadif")) {
    ...
    if (WunjoSettings::mltinterpolation() == QLatin1String("bilinear")) {
        interpol->setCurrentItem(1);
    } else if (WunjoSettings::mltinterpolation() == QLatin1String("bicubic")) {
        interpol->setCurrentItem(2);
    } else if (WunjoSettings::mltinterpolation() == QLatin1String("hyper")) {
    ...
    progressive->setChecked(WunjoSettings::monitor_progressive());

    QAction *audioScrub = new QAction(i18n("Audio Scrubbing"), this);
    connect(audioScrub, &QAction::triggered, this, [&](bool enable) { WunjoSettings::setAudio_scrub(enable); });
    pCore->window()->addAction(QStringLiteral("mlt_scrub"), audioScrub);
    audioScrub->setCheckable(true);
    audioScrub->setChecked(WunjoSettings::audio_scrub());
...
void MonitorManager::slotSetDeinterlacer(int ix)
{
    ...
    WunjoSettings::setMltdeinterlacer(value);
...
void MonitorManager::slotSetInterpolation(int ix)
{
    ...
    WunjoSettings::setMltinterpolation(value);
...
void MonitorManager::slotProgressivePlay(bool active)
{
    ...
    WunjoSettings::setMonitor_progressive(active);
    if (m_clipMonitor) {
...
void MonitorManager::markMonitorDirty(Wunjo::MonitorId name, const QUuid uuid)
{
    if (name == Wunjo::ClipMonitor) {
