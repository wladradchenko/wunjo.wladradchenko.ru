...
Item {
    ...
    property bool autoKeyframe: K.WunjoSettings.autoKeyframe
    ...
    onIsKeyframeChanged: {
        if (root.displayResize && !K.WunjoSettings.autoKeyframe) {
    ...
    MouseArea {
        ...
        onDoubleClicked: {
            if (root.isDefined) {
                if (root.controller.isKeyframe == false && K.WunjoSettings.autoKeyframe) {
        ...
        onPositionChanged: mouse => {
            if (pressed) {
                ...
                if (centerContainsMouse) {
                    // moving shape
                    if (root.controller.position == lastMousePos && K.WunjoSettings.autoKeyframe && root.controller.speed > 0) {
            ...
            } else if ((root.controller.isKeyframe || K.WunjoSettings.autoKeyframe) && root.centerPoints.length > 0) {
