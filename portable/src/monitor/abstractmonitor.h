...
class AbstractRender : public QObject
{
    Q_OBJECT public :
        ...
        explicit AbstractRender(Wunjo::MonitorId name, QWidget *parent = nullptr)
        : QObject(parent)
    ...
    /** @brief This property is used to decide if the renderer should convert it's frames to QImage for use in other Wunjo widgets. */
    bool sendFrameForAnalysis;

    /** @brief This property is used to decide if the renderer should send audio data for monitoring. */
    bool analyseAudio;

    Wunjo::MonitorId id() const { return m_id; }

    /** @brief Someone needs us to send again a frame. */
    virtual void sendFrameUpdate() = 0;

private:
    Wunjo::MonitorId m_id;
...
class AbstractMonitor : public QWidget
{
    Q_OBJECT
public:
    AbstractMonitor(Wunjo::MonitorId id, MonitorManager *manager, QWidget *parent = nullptr);
    Wunjo::MonitorId id() { return m_id; }
