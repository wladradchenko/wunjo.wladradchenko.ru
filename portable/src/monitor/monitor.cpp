...
QWidget *VolumeAction::createWidget(QWidget *parent)
{
    ...
    slider->setValue(WunjoSettings::volume());
...
Monitor::Monitor(Wunjo::MonitorId id, MonitorManager *manager, QWidget *parent)
    ...
    , m_lastMonitorSceneType(SceneType::MonitorSceneDefault)
{
        ...
        if (restoreVars) {
            ...
            if (m_id == Wunjo::ClipMonitor) {
                ...
                if (m_glMonitor->rootObject()) {
                    updateQmlDisplay(WunjoSettings::displayClipMonitorInfo());
                }
                ...
                if (m_glMonitor->rootObject()) {
                    updateQmlDisplay(WunjoSettings::displayProjectMonitorInfo());
                }
            }
        }
    ...
    connect(scalingAction, QOverload<int>::of(&QComboBox::activated), this, [this](int index) {
        switch (index) {
        case 1:
            WunjoSettings::setPreviewScaling(1);
            break;
        case 2:
            WunjoSettings::setPreviewScaling(2);
            break;
        case 3:
            WunjoSettings::setPreviewScaling(4);
            break;
        case 4:
            WunjoSettings::setPreviewScaling(8);
            break;
        case 5:
            WunjoSettings::setPreviewScaling(16);
            break;
        default:
            WunjoSettings::setPreviewScaling(0);
        }
    ...
    if (id == Wunjo::ClipMonitor) {
        ...
        connect(m_audioChannels, &QMenu::triggered, this, [this](QAction *ac) {
            ...
            if (!enabledStreams.isEmpty()) {
                ...
                props.insert(QStringLiteral("wunjo:active_streams"), astreams.join(QLatin1Char(';')));
                ...
                props.insert(QStringLiteral("wunjo:active_streams"), QStringLiteral("-1"));
                m_controller->setProperties(props, true);
            }
        });
    } else if (id == Wunjo::ProjectMonitor) {
    ...
    m_markerMenu = new KActionMenu(id == Wunjo::ClipMonitor ? i18n("Go to Clip Marker…") : i18n("Go to Timeline Marker…"), this);
    ...
    if (m_id == Wunjo::ClipMonitor) {
        ...
        if (WunjoSettings::monitor_background() == whiteAction->data().toString()) {
            m_background->setCurrentAction(whiteAction);
        } else if (WunjoSettings::monitor_background() == pinkAction->data().toString()) {
            m_background->setCurrentAction(pinkAction);
        } else if (WunjoSettings::monitor_background() == checkerboardAction->data().toString()) {
        ...
        connect(m_background, &KSelectAction::actionTriggered, this, [this](QAction *a) {
            WunjoSettings::setMonitor_background(a->data().toString());
    ...
    connect(m_glMonitor->getControllerProxy(), &MonitorProxy::positionChanged, this, &Monitor::updateFaceOverlay);
    connect(m_glMonitor->getControllerProxy(), &MonitorProxy::faceMenuRequested, this, &Monitor::showFaceMenu);
    // an analysis that just finished must show up without seeking first
    connect(pCore.get(), &Core::faceDataChanged, this, [this](const QString &) { updateFaceOverlay(); });
    m_timePos = new TimecodeDisplay(this);
    if (id == Wunjo::ProjectMonitor) {
    ...
    if (!m_audioMeterWidget->isValid) {
        WunjoSettings::setMonitoraudio(0x01);
        m_audioMeterWidget->setVisibility(false);
    } else {
        m_audioMeterWidget->setVisibility((WunjoSettings::monitoraudio() & m_id) != 0);
        if (id == Wunjo::ProjectMonitor) {
    ...
    if (m_id == Wunjo::ProjectMonitor) {
        if (!WunjoSettings::project_monitor_fullscreen().isEmpty()) {
            slotSwitchFullScreen();
        }
    } else if (!WunjoSettings::clip_monitor_fullscreen().isEmpty()) {
        slotSwitchFullScreen();
    }
...
void Monitor::refreshFaceDetection()
{
    // Face boxes live in the Project Monitor only: that is where the clip is
    // seen in its timeline context, which is also the clip the box acts on.
    // Active while any clip has the property enabled.
    m_faceDetectEnabled = m_id == Wunjo::ProjectMonitor && FaceDataStore::instance().hasAnyEnabled();
    updateFaceOverlay();
}

void Monitor::updateFaceOverlay()
{
    if (!m_faceDetectEnabled) {
        if (!m_glMonitor->getControllerProxy()->detectedFaces().isEmpty()) {
            m_glMonitor->getControllerProxy()->setDetectedFaces({});
        }
        return;
    }
    const int position = m_glMonitor->getControllerProxy()->getPosition();
    ...
    m_faceOverlayBinId.clear();
    m_faceOverlayTimelineCid = -1;
    m_faceOverlayClipIn = 0;
    m_faceOverlaySourcePos = position;
    if (pCore->window()->getCurrentTimeline() && pCore->window()->getCurrentTimeline()->model()) {
        // Find the clip that is actually on screen under the playhead: the
        // topmost one on a track that is not hidden. Whether it has faces is a
        // separate question, asked afterwards.
        //
        // Looking for the topmost clip *that has faces* instead — which is what
        // this did — skipped right past a clip covering the frame if nobody had
        // analysed it, and drew the boxes of the clip underneath over a picture
        // they had nothing to do with. A render placed on a track above its
        // source shows this immediately: the source's boxes float over a
        // different video, pointing at faces that are not where they say.
        auto model = pCore->window()->getCurrentTimeline()->model();
        int topTrackPosition = -1;
        int visibleCid = -1;
        const auto trackIds = model->getAllTracksIds();
        for (int trackId : trackIds) {
            if (model->isAudioTrack(trackId) || model->getTrackPosition(trackId) <= topTrackPosition) {
                continue;
            }
            // a hidden video track composites nothing, so it hides nothing either
            if (model->getTrackProperty(trackId, QStringLiteral("hide")).toInt() & 1) {
                continue;
            }
            const int cid = model->getClipByPosition(trackId, position);
            if (cid > -1) {
                topTrackPosition = model->getTrackPosition(trackId);
                visibleCid = cid;
            }
        }
        if (visibleCid > -1) {
            const QString binId = model->getClipBinId(visibleCid);
            if (store.isEnabled(binId)) {
                store.ensureLoaded(binId, dataFolder);
                const int sourcePos = position - model->getClipPosition(visibleCid) + model->getClipIn(visibleCid);
                faces = store.interpolatedFrameData(binId, sourcePos);
                m_faceOverlayBinId = binId;
                m_faceOverlayTimelineCid = visibleCid;
                m_faceOverlayClipIn = model->getClipIn(visibleCid);
                m_faceOverlaySourcePos = sourcePos;
            }
        }
    }
    QVariantList list;
    for (const QRectF &rect : faces) {
        list << rect;
    }
    if (list != m_glMonitor->getControllerProxy()->detectedFaces()) {
        m_glMonitor->getControllerProxy()->setDetectedFaces(list);
    }
}

void Monitor::showFaceMenu(int index)
{
    const QVariantList faces = m_glMonitor->getControllerProxy()->detectedFaces();
    if (index < 0 || index >= faces.size()) {
        return;
    }
    // The menu is the mount point for face-based tools: modules plug in here
    // and receive the face rect (normalized) + position from FaceDataStore
    QMenu menu(this);
    // one entry — it lands as a single Motion Tracker effect where the blur
    // type (blur / pixelate / opaque fill) and strength are switched anytime
    QAction *hideAction = menu.addAction(QIcon::fromTheme(QStringLiteral("view-hidden")), i18n("Hide Face"));
    connect(hideAction, &QAction::triggered, this, [this, index]() { applyFaceEffect(index); });
    QAction *maskAction = menu.addAction(QIcon::fromTheme(QStringLiteral("path-mask-edit")), i18n("Create Object Mask here…"));
    connect(maskAction, &QAction::triggered, this, []() {
        if (QAction *aiMask = pCore->window()->actionCollection()->action(QStringLiteral("ai_object_mask"))) {
            aiMask->trigger();
        }
    });
    // Face-target plugins plug in here — the menu was always meant as the mount
    // point for face tools. Each receives the normalized face rect and the
    // source frame position; the detection and Hide Face code stay untouched.
    const QList<PluginManifest> facePlugins = PluginManager::instance().pluginsForTarget(QStringLiteral("face"));
    if (!facePlugins.isEmpty() && !m_faceOverlayBinId.isEmpty()) {
        menu.addSeparator();
        const QRectF rect = faces.at(index).toRectF();
        const int position = m_faceOverlaySourcePos;
        const QString binId = m_faceOverlayBinId;
        for (const PluginManifest &plugin : facePlugins) {
...
std::shared_ptr<EffectStackModel> Monitor::faceEffectStack()
{
    // The boxes come from the timeline clip under the playhead (see
    // updateFaceOverlay), so the effect goes on that clip.
    if (m_faceOverlayTimelineCid > -1 && pCore->window()->getCurrentTimeline() && pCore->window()->getCurrentTimeline()->model()) {
        return pCore->window()->getCurrentTimeline()->model()->getClipEffectStackModel(m_faceOverlayTimelineCid);
    }
    return nullptr;
}

void Monitor::applyPluginFaceEffect(int index, const PluginManifest &plugin, const PluginEffect &effect)
{
    const QVariantList faces = m_glMonitor->getControllerProxy()->detectedFaces();
    if (index < 0 || index >= faces.size() || m_faceOverlayBinId.isEmpty()) {
        return;
    }
    std::shared_ptr<EffectStackModel> stack = faceEffectStack();
    if (stack == nullptr) {
        return;
    }
    const bool wholeSet = effect.id.isEmpty();
    QString results;
    if (wholeSet || PluginEffects::needsFace(plugin, effect)) {
        // The effect (or the region it works inside) declared where the face
        // goes: fill it with the same track Hide Face uses, so it follows the
        // picked face instead of standing on a static rectangle.
        const QRectF face = faces.at(index).toRectF();
        results = FaceEffect::buildTrackerResults(m_faceOverlayBinId, m_faceOverlaySourcePos, m_faceOverlayClipIn, face, pCore->getCurrentFrameSize());
        if (results.isEmpty()) {
            pCore->displayMessage(i18n("No face track for this clip — run face detection first"), ErrorMessage);
            return;
        }
    }
    const bool applied = wholeSet ? PluginEffects::applyAll(stack, plugin, results) : PluginEffects::apply(stack, plugin, effect, results);
    if (applied) {
        pCore->displayMessage(i18n("%1 applied to the face — set it up in the effect stack", wholeSet ? plugin.name() : effect.name),
                              InformationMessage);
        refreshMonitor(true);
    }
}

void Monitor::applyFaceEffect(int index)
{
    const QVariantList faces = m_glMonitor->getControllerProxy()->detectedFaces();
    if (index < 0 || index >= faces.size() || m_faceOverlayBinId.isEmpty()) {
        return;
    ...
    // Shared approved logic (also used by the scripting/MCP entry point).
    const QString results =
        FaceEffect::buildTrackerResults(m_faceOverlayBinId, m_faceOverlaySourcePos, m_faceOverlayClipIn, face, profile);
    if (results.isEmpty()) {
        return;
    }
    std::shared_ptr<EffectStackModel> stack = faceEffectStack();
    ...
    // The Motion Tracker effect applies blur/pixelate/fill along an animated
    // rect; pre-filling `results` from the face data means no analysis pass.
    // The plain pixelate/blur effects stay available as generic effects —
    // this entry point only adds the face-driven variant.
    stringMap params;
    params.insert(QStringLiteral("results"), results);
    // pixelate by default; the type dropdown in the effect stack switches it
    params.insert(QStringLiteral("blur_type"), QStringLiteral("2"));
    params.insert(QStringLiteral("blur"), QString::number(FaceEffect::autoStrength(face, profile)));
    params.insert(QStringLiteral("shape_width"), QStringLiteral("0"));
    if (stack->appendEffect(QStringLiteral("opencv.tracker"), true, params)) {
        pCore->displayMessage(i18n("Face effect applied — tune strength or remove it in the effect stack"), InformationMessage);
        refreshMonitor(true);
    }
}
                        ...
                        QAction *loopClip)
{
    ...
    if (m_id == Wunjo::ProjectMonitor) {
        ...
        m_configMenuAction->addAction(createZoneMarkerQuick);
        m_contextMenu->addAction(createZoneMarkerQuick);
    } else if (m_id == Wunjo::ClipMonitor) {
        ...
        connect(alwaysShowAudio, &QAction::triggered, this, &WunjoSettings::setAlwaysShowMonitorAudio);
        alwaysShowAudio->setChecked(WunjoSettings::alwaysShowMonitorAudio());
    ...
    switchAudioMonitor->setChecked((WunjoSettings::monitoraudio() & m_id) != 0);
    ...
    if (m_id == Wunjo::ClipMonitor) {
        ...
        recordTimecode->setChecked(WunjoSettings::rectimecode());
...
void Monitor::buildBackgroundedProducer(int pos)
{
    ...
    if (WunjoSettings::monitor_background() == "black" || m_controller->clipType() == ClipType::Audio) {
        ...
        trac.set("wunjo:id", m_controller->clipId().toLatin1().constData());
        QString color = QStringLiteral("color:%1").arg(WunjoSettings::monitor_background());
...
void Monitor::updateMarkers()
{
    if (m_markerMenu) {
        ...
        if (m_id == Wunjo::ClipMonitor && m_controller) {
            model = m_controller->getMarkerModel();
        } else if (m_id == Wunjo::ProjectMonitor && pCore->currentDoc()) {
...
const QScreen *Monitor::getScreenForFullscreen(bool *multipleScreens)
{
    ...
    *multipleScreens = true;
    QString requestedMonitor = WunjoSettings::fullscreen_monitor();
    if (m_id == Wunjo::ProjectMonitor) {
        if (!WunjoSettings::project_monitor_fullscreen().isEmpty()) {
            requestedMonitor = WunjoSettings::project_monitor_fullscreen();
        }
    } else {
        if (!WunjoSettings::clip_monitor_fullscreen().isEmpty()) {
            requestedMonitor = WunjoSettings::clip_monitor_fullscreen();
        }
    }
    int ix = -1;
    if (!requestedMonitor.isEmpty()) {
        ...
        for (auto screen : screens) {
            ...
            if (match) {
                if (m_id == Wunjo::ProjectMonitor) {
                    WunjoSettings::setProject_monitor_fullscreen(screenId);
                } else {
                    WunjoSettings::setClip_monitor_fullscreen(screenId);
                }
                return screen;
            }
        }
        ...
        if (screens.count() > 2) {
            ...
            if (m_id == Wunjo::ProjectMonitor) {
                if (WunjoSettings::clip_monitor_fullscreen().isEmpty()) {
                    ...
                    if (WunjoSettings::clip_monitor_fullscreen() == screenId) {
                        continue;
                    }
                }
            } else {
                if (WunjoSettings::project_monitor_fullscreen().isEmpty()) {
                    ...
                    if (WunjoSettings::project_monitor_fullscreen() == screenId) {
                        continue;
                    }
                }
            }
        } else {
            // Move monitor widget to the second screen (one screen for Wunjo, the other one for the Monitor widget)
            if (m_id == Wunjo::ProjectMonitor) {
                WunjoSettings::setProject_monitor_fullscreen(screenId);
            } else {
                WunjoSettings::setClip_monitor_fullscreen(screenId);
...
void Monitor::createFullscreenMirror()
{
    ...
    auto rebuildViewConnection = [this]() {
        ...
        m_monitorMirror->setClearColor(WunjoSettings::window_background());
        // Enforce geometry recalculation
...
void Monitor::slotSwitchFullScreen(bool minimizeOnly)
{
    ...
    if (!monitorIsFullScreen() && !minimizeOnly) {
        ...
        if (WunjoSettings::mirrorMonitorOnFullscreen() && multipleScreen) {
        ...
        destroyFullscreenMirror();
        if (m_id == Wunjo::ProjectMonitor) {
            WunjoSettings::setProject_monitor_fullscreen(QString());
        } else {
            WunjoSettings::setClip_monitor_fullscreen(QString());
        }
        setFocus();
    }
}
...
void Monitor::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (event->button() != Qt::RightButton) {
        if (m_glMonitor->geometry().contains(event->pos())) {
            if (isActive() && WunjoSettings::play_monitor_on_click()) {
                slotPlay();
            } else {
                slotActivateMonitor();
            }
...
{
    if (m_id == Wunjo::ProjectMonitor || m_controller == nullptr || !m_activeSequence.isNull()) {
    ...
    if (dragType.isEmpty()) {
        ...
        if (dragType == QLatin1String("A")) {
            ...
            drag->setPixmap(QIcon::fromTheme("wunjo-show-video").pixmap(32, 32));
        }
    }
...
void Monitor::slotExtractCurrentZone()
{
    ...
    CutTask::start(ObjectId(WunjoObjectType::BinClip, m_controller->clipId().toInt(), QUuid()), getZoneStart(), getZoneEnd(), this);
}
...
void Monitor::slotExtractCurrentFrame(QString frameName, bool addToProject, bool toClipboard)
{
    ...
    if (toClipboard) {
        ...
        int previewScale = WunjoSettings::previewScaling();
        if (previewScale > 0) {
            WunjoSettings::setPreviewScaling(0);
            m_glMonitor->updateScaling();
        }
        ...
        m_captureConnection =
            connect(m_glMonitor, &VideoWidget::analyseFrame, this, [this, proxiedClips, existingProxies, analysisStatus, previewScale](const QImage &img) {
                ...
                if (previewScale > 0) {
                    WunjoSettings::setPreviewScaling(previewScale);
                    m_glMonitor->updateScaling();
                }
    ...
    QString framesFolder = KRecentDirs::dir(QStringLiteral(":WunjoFramesFolder"));
    ...
    QCheckBox *b = nullptr;
    if (m_id == Wunjo::ClipMonitor && m_controller && m_controller->clipType() != ClipType::Text) {
        ...
        if (fSize != pCore->getCurrentFrameSize()) {
            ...
            b->setChecked(WunjoSettings::exportframe_usingsourceres());
    ...
    if (dlg->exec() == QDialog::Accepted) {
        ...
        if (!selectedFile.isEmpty()) {
            if (b != nullptr) {
                WunjoSettings::setExportframe_usingsourceres(useSourceResolution);
            }
            KRecentDirs::add(QStringLiteral(":WunjoFramesFolder"), QUrl::fromLocalFile(selectedFile).adjusted(QUrl::RemoveFilename).toLocalFile());
            // check if we are using a proxy
            if ((m_controller != nullptr) && !m_controller->getProducerProperty(QStringLiteral("wunjo:proxy")).isEmpty() &&
                m_controller->getProducerProperty(QStringLiteral("wunjo:proxy")) != QLatin1String("-")) {
                    ...
                    m_captureConnection =
                        connect(m_glMonitor, &VideoWidget::analyseFrame, this,
                                [this, proxiedClips, selectedFile, existingProxies, addToProject, analysisStatus, previewScale](const QImage &img) {
                                    ...
                                    QObject::disconnect(m_captureConnection);
                                    connect(m_glMonitor, &VideoWidget::analyseFrame, this, &Monitor::frameUpdated);
                                    KRecentDirs::add(QStringLiteral(":WunjoFramesFolder"),
...
void Monitor::mute(bool mute)
{
    ...
    m_glMonitor->setVolume(mute ? 0 : WunjoSettings::volume() / 100.0);
}
...
void Monitor::refreshMonitor(bool directUpdate, bool slowRefresh)
{
    ...
    if (isActive()) {
        ...
        bool otherMonitorVisible = m_id == Wunjo::ClipMonitor ? m_monitorManager->projectMonitorVisible() : m_monitorManager->clipMonitorVisible();
        slotActivateMonitor();
        if (isActive()) {
            m_glMonitor->refresh();
            ...
            m_switchConnection = connect(m_glMonitor, &VideoWidget::frameDisplayed, this, [this, otherMonitorVisible]() {
                m_monitorManager->activateMonitor(m_id == Wunjo::ClipMonitor ? Wunjo::ProjectMonitor : Wunjo::ClipMonitor, otherMonitorVisible);
                QObject::disconnect(m_switchConnection);
            });
        }
    }
...
void Monitor::slotSwitchPlay()
{
    ...
    if (!play && WunjoSettings::rewindOnStop()) {
    ...
    if (m_id == Wunjo::ClipMonitor) {
        showDropped = WunjoSettings::displayClipMonitorInfo() & Monitor::PlaybackFpsOverlay;
    } else if (m_id == Wunjo::ProjectMonitor) {
        showDropped = WunjoSettings::displayProjectMonitorInfo() & Monitor::PlaybackFpsOverlay;
    }
    if (showDropped) {
...
bool Monitor::slotOpenClip(const std::shared_ptr<ProjectClip> &controller, int in, int out, const QUuid &sequenceUuid)
{
    ...
    if (m_controller) {
        ...
        if (zoomFactor != 1.) {
            ...
            m_controller->setProducerProperty(QStringLiteral("wunjo:thumbZoomFactor"), zoomFactor);
            m_controller->setProducerProperty(QStringLiteral("wunjo:thumbZoomStart"), zoomStart);
        } else {
            m_controller->resetProducerProperty(QStringLiteral("wunjo:thumbZoomFactor"));
            m_controller->resetProducerProperty(QStringLiteral("wunjo:thumbZoomStart"));
        }
        m_controller->setProducerProperty(QStringLiteral("wunjo:monitorPosition"), position());
    ...
    refreshFaceDetection();
        ...
        if (WunjoSettings::rectimecode()) {
            m_timePos->setFrameOffset(qMax(0, m_controller->getStartTimecode()));
        }
        if (m_controller->statusReady()) {
            int seekPos = in;
            double audioScale = m_controller->getProducerDoubleProperty(QStringLiteral("wunjo:thumbZoomFactor"));
            if (in == out && in == -1) {
                // Only apply on bin clip, not sub clips
                int lastPosition = m_controller->getProducerIntProperty(QStringLiteral("wunjo:monitorPosition"));
                ...
                if (audioScale > 0. && audioScale != 1.) {
                    double audioStart = m_controller->getProducerDoubleProperty(QStringLiteral("wunjo:thumbZoomStart"));
                    m_glMonitor->getControllerProxy()->resetTimeZoom();
            ...
            refreshFaceDetection();
            if (!m_controller->hasVideo() || WunjoSettings::displayClipMonitorInfo() & Monitor::AudioWaveformOverlay) {
...
void Monitor::switchMonitorInfo(int code)
{
    int currentOverlay;
    if (m_id == Wunjo::ClipMonitor) {
        currentOverlay = WunjoSettings::displayClipMonitorInfo();
        currentOverlay ^= code;
        WunjoSettings::setDisplayClipMonitorInfo(currentOverlay);
    } else {
        currentOverlay = WunjoSettings::displayProjectMonitorInfo();
        currentOverlay ^= code;
        WunjoSettings::setDisplayProjectMonitorInfo(currentOverlay);
    }
...
{
    WunjoSettings::setShowOnMonitorScene(enable);
...
void Monitor::slotShowEffectScene(SceneType::MonitorSceneType sceneType, bool temporary, const QVariant &sceneData)
{
    ...
    if (sceneType == SceneType::MonitorSceneDefault || WunjoSettings::showOnMonitorScene()) {
...
{
    WunjoSettings::setVolume(volume);
...
void Monitor::onFrameDisplayed(const SharedFrame &frame)
{
    ...
    if (!m_glMonitor->checkFrameNumber(frame.get_position(), wasPlaying)) {
        updatePlayAction(false);
        if (wasPlaying && WunjoSettings::rewindOnStop() && frame.get_position() >= m_glMonitor->duration() - 2) {
            m_glMonitor->getControllerProxy()->setCursorPosition(m_startPlaybackPos);
            m_glMonitor->requestSeek(m_startPlaybackPos, true);
        }
    }
...
{
    qCWarning(WUNJO_LOG) << " + + + + Error initializing Movit GLSL manager";
    ...
    if (m_id == Wunjo::ProjectMonitor) {
        if (enable) {
            ...
            if ((m_splitEffect != nullptr) && m_splitEffect->is_valid()) {
                ...
                warningMessage(i18n("The alphagrad filter is required for that feature, please install frei0r and restart Wunjo"));
                return;
            }
...
void Monitor::buildSplitEffect(Mlt::Producer *original)
{
    ...
    if ((m_splitEffect != nullptr) && m_splitEffect->is_valid()) {
        ...
        // frei0r.alphagrad is not available
        pCore->displayMessage(i18n("The alphagrad filter is required for that feature, please install frei0r and restart Wunjo"), ErrorMessage);
        return;
    }
    ...
    if (!t.is_valid()) {
        m_splitEffect.reset();
        pCore->displayMessage(i18n("The cairoblend transition is required for that feature, please install frei0r and restart Wunjo"), ErrorMessage);
        return;
    }
    Mlt::Tractor trac(pCore->getProjectProfile());
    ...
    while (filter != nullptr) {
        QString ix = QString::fromLatin1(filter->get("wunjo_id"));
        if (!ix.isEmpty()) {
...
void Monitor::slotSwitchTrimming(bool enable)
{
    ...
    if (enable) {
        ...
        m_trimmingbar->setVisible(false);
        m_toolbar->setVisible(true);
        m_glMonitor->switchRuler(WunjoSettings::displayClipMonitorInfo() & Monitor::InfoOverlay);
    }
}
...
void Monitor::slotSwitchAudioMonitor()
{
    ...
    int currentOverlay = WunjoSettings::monitoraudio();
    currentOverlay ^= m_id;
    WunjoSettings::setMonitoraudio(currentOverlay);
    if ((WunjoSettings::monitoraudio() & m_id) != 0) {
...
{
    bool enable = isActive && ((WunjoSettings::monitoraudio() & m_id) != 0 || (m_id == Wunjo::ProjectMonitor && pCore->audioMixerVisible));
    if (enable) {
...
void Monitor::processSeek(int pos, bool noAudioScrub)
{
    ...
    if (WunjoSettings::pauseonseek() && m_playAction->isActive()) {
        pause();
    }
...
void Monitor::slotTrimmingPos(int pos, int offset, int frames1, int frames2)
{
    ...
    QString tc(pCore->timecode().getDisplayTimecodeFromFrames(offset, WunjoSettings::frametimecode()));
...
{
    WunjoSettings::setRectimecode(enable);
    if (!enable) {
...
void Monitor::slotCreateRangeMarkerFromZone()
{
    ...
    CommentedTime tempMarker(startPos, QString(), WunjoSettings::default_marker_type(), duration);
