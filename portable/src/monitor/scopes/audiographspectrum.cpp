    ...
    , m_manager(manager)
{
    ...
    if (!m_filter->is_valid()) {
        WunjoSettings::setEnableaudiospectrum(false);
    ...
    a->setCheckable(true);
    a->setChecked(WunjoSettings::enableaudiospectrum());
    if (WunjoSettings::enableaudiospectrum() && isVisible()) {
...
{
    if (WunjoSettings::enableaudiospectrum()) {
        if (!visible) {
...
void AudioGraphSpectrum::activate(bool enable)
{
    ...
    WunjoSettings::setEnableaudiospectrum(enable);
}
