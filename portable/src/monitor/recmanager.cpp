    ...
    , m_recToolbar(new QToolBar(parent))
{
    ...
    m_audioCaptureSlider->setValue(WunjoSettings::audiocapturevolume());
    ...
    QIcon icon;
    if (WunjoSettings::audiocapturevolume() == 0) {
    ...
    selectedIndex = m_device_selector->findData(WunjoSettings::defaultcapture());
...
{
    Q_EMIT pCore->showConfigDialog(Wunjo::PageCapture, m_device_selector->currentData().toInt());
}
...
void RecManager::slotRecord(bool record)
{
    ...
    QString extension = WunjoSettings::grab_extension();
...
#ifdef Q_OS_WIN
    captureArgs << QStringLiteral("-f") << QStringLiteral("gdigrab") << QStringLiteral("-framerate") << QString::number(WunjoSettings::grab_fps())
                << QStringLiteral("-i") << QStringLiteral("desktop");
    if (WunjoSettings::grab_parameters().contains(QLatin1String("alsa"))) {
        // Add audio device
        QString params = captureArgs.join(QLatin1Char(' '));
        params.append(QStringLiteral(" ") + WunjoSettings::grab_parameters().simplified());
        ...
        tst.start(WunjoSettings::ffmpegpath(), {"-hide_banner", "-list_devices", "true", "-f", "dshow", "-i", "dummy"});
        tst.waitForStarted();
        tst.waitForFinished();
        ...
        if (dshowOutput.contains(QLatin1String("DirectShow audio devices"))) {
            ...
            qDebug() << WunjoSettings::ffmpegpath() << "=== GOT DSHOW DEVICES: " << dshowOutput;
            captureArgs << params.split(QLatin1Char(' '));
        }
        ...
        m_captureProcess->start(WunjoSettings::ffmpegpath(), {});
    } else if (!WunjoSettings::grab_parameters().simplified().isEmpty()) {
        captureArgs << WunjoSettings::grab_parameters().simplified().split(QLatin1Char(' '));
        captureArgs << path;
        m_captureProcess->start(WunjoSettings::ffmpegpath(), captureArgs);
    }
#else
    captureArgs << QStringLiteral("-f") << QStringLiteral("x11grab");
    if (WunjoSettings::grab_follow_mouse()) {
        captureArgs << QStringLiteral("-follow_mouse") << QStringLiteral("centered");
    }
    if (!WunjoSettings::grab_hide_frame()) {
        captureArgs << QStringLiteral("-show_region") << QStringLiteral("1");
    }
    QString captureSize = QStringLiteral(":0.0");
    if (WunjoSettings::grab_capture_type() == 0) {
        // Full screen capture
        ...
        captureArgs << QStringLiteral("-s")
                    << QString::number(WunjoSettings::grab_width()) + QLatin1Char('x') + QString::number(WunjoSettings::grab_height());
        captureSize.append(QLatin1Char('+') + QString::number(WunjoSettings::grab_offsetx()) + QLatin1Char(',') +
                           QString::number(WunjoSettings::grab_offsety()));
    }
    if (WunjoSettings::grab_hide_mouse()) {
        captureSize.append(QStringLiteral("+nomouse"));
    }
    // fps
    captureArgs << QStringLiteral("-r") << QString::number(WunjoSettings::grab_fps());
    captureArgs << QStringLiteral("-i") << captureSize;
    if (!WunjoSettings::grab_parameters().simplified().isEmpty()) {
        captureArgs << WunjoSettings::grab_parameters().simplified().split(QLatin1Char(' '));
    }
...
void RecManager::slotAudioDeviceChanged(int)
{
    ...
    WunjoSettings::setDefaultaudiocapture(currentDevice);
}

void RecManager::slotSetVolume(int volume)
{
    WunjoSettings::setAudiocapturevolume(volume);
    QIcon icon;
...
void RecManager::slotVideoDeviceChanged(int)
{
    ...
    WunjoSettings::setDefaultcapture(currentItem);
    switch (currentItem) {
    case Video4Linux:
