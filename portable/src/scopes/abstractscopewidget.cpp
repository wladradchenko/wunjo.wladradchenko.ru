...

{
    ...
    m_menu = new QMenu();
    // Disabled dark palette on menus since it breaks up with some themes: wunjo issue #2950
...
#ifdef DEBUG_ASW
        qCDebug(WUNJO_LOG) << "Scope " << m_widgetName << " is not visible. Not calculating HUD.";
#endif
    } else {
...
#ifdef DEBUG_ASW
            qCDebug(WUNJO_LOG) << "HUD thread started in " << m_widgetName;
#endif

        }
#ifdef DEBUG_ASW
        else {
            qCDebug(WUNJO_LOG) << "HUD semaphore locked, not prodding in " << m_widgetName << ". Thread running: " << m_threadHUD.isRunning();
        }
#endif
    }
}
...
void AbstractScopeWidget::prodScopeThread()
{
    ...
    if (this->visibleRegion().isEmpty()) {
#ifdef DEBUG_ASW
        qCDebug(WUNJO_LOG) << "Scope " << m_widgetName << " is not visible. Not calculating scope.";
#endif
    } else {
...
#ifdef DEBUG_ASW
            qCDebug(WUNJO_LOG) << "Scope thread started in " << m_widgetName;
#endif

        } else {
#ifdef DEBUG_ASW
            qCDebug(WUNJO_LOG) << "Scope semaphore locked, not prodding in " << m_widgetName << ". Thread running: " << m_threadScope.isRunning();
#endif
        }
    }
}
void AbstractScopeWidget::prodBackgroundThread()
{
    if (this->visibleRegion().isEmpty()) {
#ifdef DEBUG_ASW
        qCDebug(WUNJO_LOG) << "Scope " << m_widgetName << " is not visible. Not calculating background.";
#endif
    } else {
...
#ifdef DEBUG_ASW
            qCDebug(WUNJO_LOG) << "Background thread started in " << m_widgetName;
#endif

        } else {
#ifdef DEBUG_ASW
            qCDebug(WUNJO_LOG) << "Background semaphore locked, not prodding in " << m_widgetName << ". Thread running: " << m_threadBackground.isRunning();
#endif
        }
    }
}
...
{
#ifdef DEBUG_ASW
    qCDebug(WUNJO_LOG) << "Forced update called in " << widgetName() << ". Arg: " << doUpdate;
#endif
...
#ifdef DEBUG_ASW
                qCDebug(WUNJO_LOG) << "Diff is " << diff << "; chose " << directions[m_rescaleDirection] << " as direction";
#endif
...
{
#ifdef DEBUG_ASW
    qCDebug(WUNJO_LOG) << "HUD rendering has finished in " << mseconds << " ms, waiting for termination in " << m_widgetName;
#endif
...
#ifdef DEBUG_ASW
        qCDebug(WUNJO_LOG) << "Trying to start a new HUD thread for " << m_widgetName << ". New frames/updates: " << m_newHUDFrames << '/'
                              << m_newHUDUpdates;
#endif
        prodHUDThread();
    }
...
#ifdef DEBUG_ASW
    qCDebug(WUNJO_LOG) << "Scope rendering has finished in " << mseconds << " ms, waiting for termination in " << m_widgetName;
#endif
...
#ifdef DEBUG_ASW
        qCDebug(WUNJO_LOG) << "Trying to start a new scope thread for " << m_widgetName << ". New frames/updates: " << m_newScopeFrames << '/'
                              << m_newScopeUpdates;
#endif
        prodScopeThread();
    }
...
{
#ifdef DEBUG_ASW
    qCDebug(WUNJO_LOG) << "Background rendering has finished in " << mseconds << " ms, waiting for termination in " << m_widgetName;
#endif
...
#ifdef DEBUG_ASW
        qCDebug(WUNJO_LOG) << "Trying to start a new background thread for " << m_widgetName << ". New frames/updates: " << m_newBackgroundFrames << '/'
...
#ifdef DEBUG_ASW
    qCDebug(WUNJO_LOG) << "Data incoming at " << widgetName() << ". New frames total HUD/Scope/Background: " << m_newHUDFrames << '/' << m_newScopeFrames
                          << '/' << m_newBackgroundFrames;
#endif

    if (this->visibleRegion().isEmpty()) {
#ifdef DEBUG_ASW
        qCDebug(WUNJO_LOG) << "Scope of widget " << m_widgetName << " is not at the top, not rendering.";
#endif
    } else {
        prodHUDThread();
        prodScopeThread();
