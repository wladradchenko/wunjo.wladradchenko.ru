...
QImage Spectrogram::renderAudioScope(uint, const audioShortVector &audioFrame, const int freq, const int num_channels, const int num_samples, const int newData)
{
    if (audioFrame.size() > 63 && m_innerScopeRect.width() > 0 && m_innerScopeRect.height() > 0) {
...
#ifdef DEBUG_SPECTROGRAM
        qCDebug(WUNJO_LOG) << "New data for " << widgetName() << ": " << newDataAvailable << QStringLiteral(" (") << newData << " units)";
#endif
        ...
        if (newDataAvailable) {
            ...
            qCDebug(WUNJO_LOG) << widgetName() << ": Has no new data to Fourier-transform";
        }
#endif
...
#ifdef DEBUG_SPECTROGRAM
        qCDebug(WUNJO_LOG) << "Rendered " << y - topDist << "lines from " << m_fftHistory.size() << " available samples in " << start.elapsed() << " ms"
        ...
        qCDebug(WUNJO_LOG) << QStringLiteral("Total storage used: %1 kB").arg((double)storedBytes / 1000, 0, 'f', 2);
#endif
