...
#ifdef DEBUG_CPE
        qCDebug(WUNJO_LOG) << "File given: " << ok;
#endif
    }
...
{
#ifdef DEBUG_CPE
    qCDebug(WUNJO_LOG) << "Exporting plane now to " << kurlrequester->text();
#endif
    QString lower = kurlrequester->text().toLower();
#ifdef DEBUG_CPE
    qCDebug(WUNJO_LOG) << "Lower: " << lower;
#endif
...
{
#ifdef DEBUG_CPE
    qCDebug(WUNJO_LOG) << "Color mode changed to " << cbColorspace->itemData(cbColorspace->currentIndex()).toInt();
#endif
    ...
    case CPE_YUV_Y:
#ifdef DEBUG_CPE
        qCDebug(WUNJO_LOG) << "Changing slider range.";
#endif
        enableSliderScaling(true);
        enableSliderColor(true);
        enableCbVariant(false);
