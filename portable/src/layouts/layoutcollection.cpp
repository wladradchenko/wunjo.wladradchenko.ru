...
void LayoutCollection::addLayout(const LayoutInfo &layout)
{
    ...
    if (!WunjoSettings::layoutsOrder().contains(layout.internalId)) {
        QStringList order = WunjoSettings::layoutsOrder();
        order << layout.internalId;
        WunjoSettings::setLayoutsOrder(order);
    }
}
...
QList<LayoutInfo> LayoutCollection::getAllLayouts() const
{
    ...
    for (const QString &id : WunjoSettings::layoutsOrder()) {
        if (m_layouts.contains(id)) {
...
void LayoutCollection::resetDefaultLayouts()
{
    ...
    if (customLayoutsFolder.cd(QStringLiteral("layouts"))) {
        for (auto &id : WunjoSettings::defaultLayoutsOrderValue()) {
            if (!m_layouts.contains(id)) {
...
void LayoutCollection::loadLayouts()
{
    ...
    for (const QString &folderpath : layoutsFolders) {
        ...
        while (it.hasNext()) {
            ...
            QVariantList results = loadDoc["wunjoInfo"].toArray().toVariantList();
            if (results.isEmpty()) {
                qDebug() << "// No Wunjo data found in layout: " << fInfo.absoluteFilePath();
                continue;
            }
            if (layoutId.endsWith(QLatin1String("_vertical"))) {
    ...
    QStringList previousOrder = WunjoSettings::layoutsOrder();
    WunjoSettings::setLayoutsOrder({});
    if (previousOrder.isEmpty()) {
        // Default layout order
        previousOrder = WunjoSettings::defaultLayoutsOrderValue();
    }
