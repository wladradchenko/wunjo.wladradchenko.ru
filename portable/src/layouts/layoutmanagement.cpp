    ...
    : QObject(parent)
{
    ...
    // The switcher pills float directly on the menu bar (styled via
    // "LayoutSwitcher QPushButton" rules in style.qss), no tinted box behind.
...
bool LayoutManagement::slotLoadLayoutById(const QString &layoutId)
{
    ...
    if (!layout.isValid()) {
        // stale id (e.g. a deleted custom layout) — fall back to the default workspace
        layout = m_layoutCollection.getLayout(QStringLiteral("editing"));
    }
...
bool LayoutManagement::slotLoadLayout(LayoutInfo layout, bool onlyIfNoPrevious)
{
    ...
    WunjoSettings::setActiveLayout(layout.internalId);
    ...
    if (!WunjoSettings::showtitlebars()) {
        Q_EMIT pCore->hideBars(!WunjoSettings::showtitlebars());
    }
    return true;
}
...
bool LayoutManagement::slotLoadLayoutFromData(const QString &layoutData, bool onlyIfNoPrevious)
{
    ...
    // Loaded a layout from Wunjo settings or document
    m_currentLayoutId.clear();
    m_layoutSwitcher->setCurrentLayout(QString());
    if (!WunjoSettings::showtitlebars() && m_firstLayoutLoaded) {
        Q_EMIT pCore->hideBars(!WunjoSettings::showtitlebars());
    }
    m_firstLayoutLoaded = true;
    return true;
...
void LayoutManagement::slotSaveLayout()
{
    ...
    QJsonArray info;
    QJsonObject wunjoData;
    wunjoData.insert(QLatin1String("displayName"), QJsonValue(saveName));
    wunjoData.insert(QLatin1String("layoutVersion"), QJsonValue(1));
    wunjoData.insert(QLatin1String("vertical"), QJsonValue(pCore->isVertical()));
    info.push_back(wunjoData);
    bool ok;
    if (doc.isObject()) {
        QJsonObject updatedObject = doc.object();
        qDebug() << ":::: JSON INSERTED INFO: " << info.toVariantList();
        updatedObject.insert(QStringLiteral("wunjoInfo"), info);
...
void LayoutManagement::slotUpdatePalette()
{
    ...
    QPalette pal = m_autosaveContainer->palette();
