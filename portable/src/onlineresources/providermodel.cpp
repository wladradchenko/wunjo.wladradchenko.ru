    ...
    , m_invalid(false)
{
    ...
    if (!file.exists()) {
        qCWarning(WUNJO_LOG) << "WARNING, can not find provider configuration file at" << path << ".";
        m_invalid = true;
    } else {
    ...
    if (!m_invalid) {
        ...
        qCWarning(WUNJO_LOG) << "The provider config file at " << path << " is invalid. ";
    }
}
...
void ProviderModel::initOAuth2()
{
    ...
    connect(&m_oauth2, &QOAuth2AuthorizationCodeFlow::error, this, [this](const QString &error, const QString &errorDescription) {
        m_replyHandler->close();
        qCWarning(WUNJO_LOG) << "Error in authorization flow. " << error << " " << errorDescription;
...
void ProviderModel::validate()
{
    ...
    if (m_doc.isNull() || m_doc.isEmpty() || !m_doc.isObject()) {
        qCWarning(WUNJO_LOG) << "Root object missing or invalid";
        return;
    }

    if (!m_doc["integration"].isString() || m_doc["integration"].toString() != "buildin") {
        qCWarning(WUNJO_LOG) << "Currently only integration type \"buildin\" is supported";
        return;
    }

    if (!m_doc["name"].isString()) {
        qCWarning(WUNJO_LOG) << "Missing key name of type string ";
        return;
    }

    if (!m_doc["homepage"].isString()) {
        qCWarning(WUNJO_LOG) << "Missing key homepage of type string ";
        return;
    }

    if (!m_doc["type"].isString()) {
        qCWarning(WUNJO_LOG) << "Missing key type of type string ";
        return;
    }

    if (!m_doc["api"].isObject() || !m_doc["api"].toObject()["search"].isObject()) {
        qCWarning(WUNJO_LOG) << "Missing api of type object or key search of type object";
        return;
    }
    if (downloadOAuth2()) {
        if (!m_doc["api"].toObject()["oauth2"].isObject()) {
            qCWarning(WUNJO_LOG) << "Missing OAuth2 configuration (required)";
            return;
        }
        if (m_doc["api"].toObject()["oauth2"].toObject()["authorizationUrl"].toString().isEmpty()) {
            qCWarning(WUNJO_LOG) << "Missing authorizationUrl for OAuth2";
            return;
        }
        if (m_doc["api"].toObject()["oauth2"].toObject()["accessTokenUrl"].toString().isEmpty()) {
            qCWarning(WUNJO_LOG) << "Missing accessTokenUrl for OAuth2";
            return;
        }
        if (m_doc["api"].toObject()["oauth2"].toObject()["clientId"].toString().isEmpty()) {
            qCWarning(WUNJO_LOG) << "Missing clientId for OAuth2";
            return;
        }
    }
...
void ProviderModel::slotStartSearch(const QString &searchText, const int page)
{
    ...
    if (m_search.value("req").toObject().value("method").toString() == "GET") {
        ...
        qCDebug(WUNJO_LOG) << "Only GET is implemented yet";
    }
}
...
std::pair<QStringList, QStringList> ProviderModel::parseFilesResponse(const QByteArray &data, const QString &id)
{
    ...
    if (keys["format"].toString() == "json") {
        ...
        qCWarning(WUNJO_LOG) << "WARNING fetch files: unknown response format: " << keys["format"];
    }
