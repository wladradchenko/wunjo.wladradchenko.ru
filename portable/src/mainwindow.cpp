...
static const char version[] = WUNJO_VERSION;
namespace Mlt {
class Producer;
}
    ...
    , m_effectBasket(nullptr)
{
    ...
    wunjoCategoryMap.insert(QStringLiteral("monitor"), category);
    category = new KActionCategory(i18n("Add Clip"), actionCollection());
    wunjoCategoryMap.insert(QStringLiteral("addclip"), category);
    category = new KActionCategory(i18n("Add Marker by Category Number"), actionCollection());
    wunjoCategoryMap.insert(QStringLiteral("guidecategorynumber"), category);
    category = new KActionCategory(i18n("Navigation and Playback"), actionCollection());
    wunjoCategoryMap.insert(QStringLiteral("navandplayback"), category);
    category = new KActionCategory(i18n("Bin Tags"), actionCollection());
    wunjoCategoryMap.insert(QStringLiteral("bintags"), category);
    ...
    // Panels stay put (no accidental tear-offs): dragging a panel or tab
    // requires holding Ctrl; the title bar float button still works
    KDDockWidgets::Config::self().setDragAboutToStartFunc(
        [](KDDockWidgets::Core::Draggable *) { return bool(QGuiApplication::keyboardModifiers() & Qt::ControlModifier); });
    ...
    mainDockWindow = new KDDockWidgets::QtWidgets::MainWindow(QStringLiteral("WunjoKDDock"));
...
void MainWindow::init()
{
    ...
    // Load themes
    // [Wunjo] color-scheme menu removed: auto themeManager = KColorSchemeManager::instance();
    ...
    // The scripting API the assistant drives the editor through: every
    // Q_SCRIPTABLE slot of this window, served over a local socket. This was
    // D-Bus, which exists on Linux and not on the other two platforms the
    // application is going to; the socket is the same set of methods reached
    // the same way, and the render process already talks to us over one.
    m_scriptingServer = new ScriptingServer(this, this);
    m_scriptingServer->start();


    QString defaultProfile = WunjoSettings::default_profile();
    ...
    if (currentProfilePath.startsWith(QLatin1Char('/')) && !QFile::exists(currentProfilePath)) {
        ...
        WunjoSettings::setDefault_profile(QStringLiteral("atsc_1080p_25"));
    }

    // Disable movit until it's stable
    m_gpuAllowed = false;
    WunjoSettings::setGpu_accel(false);
    ...
    m_clipMonitor = new Monitor(Wunjo::ClipMonitor, pCore->monitorManager(), this);
    ...
    m_projectMonitor = new Monitor(Wunjo::ProjectMonitor, pCore->monitorManager(), this);
    ...
    if (screens > 1) {
        ...
        screenCombo->setEnabled(WunjoSettings::grab_capture_type() == 0);
    }
    // Audio record action
    ...
    connect(recConfig, &QAction::triggered, [&]() { Q_EMIT pCore->showConfigDialog(Wunjo::PageCapture, 0); });
    ...
    connect(m_timelineTabs, &TimelineTabs::showMixModel, this, [&](int cid, std::shared_ptr<AssetParameterModel> model, bool refreshOnly) {
        ...
        if (WunjoSettings::raisepropsmixes()) {
    ...
    connect(m_timelineTabs, &TimelineTabs::showTransitionModel, this, [&](int tid, std::shared_ptr<AssetParameterModel> model) {
        ...
        if (WunjoSettings::raisepropscompositions()) {
            m_effectStackDock->setAsCurrentTab();
        }
    });
    connect(m_timelineTabs, &TimelineTabs::showItemEffectStack, this,
            [&](const QString &clipName, std::shared_ptr<EffectStackModel> model, QSize size, bool showKeyframes) {
                if (model == nullptr && m_assetPanel->effectStackOwner().type == WunjoObjectType::BinClip) {
                ...
                bool isClip = model && model->getOwnerId().type == WunjoObjectType::TimelineClip;
                bool isTrack = model && model->getOwnerId().type == WunjoObjectType::TimelineTrack;
                if ((isClip && WunjoSettings::raisepropsclips()) || (isTrack && WunjoSettings::raisepropstracks())) {
                    m_effectStackDock->setAsCurrentTab();
                }
            });
    ...
    connect(m_assetPanel, &AssetPanel::seekToPos, this, [this](int pos) {
        ...
        switch (oId.type) {
        case WunjoObjectType::TimelineTrack:
        case WunjoObjectType::TimelineClip:
        case WunjoObjectType::TimelineComposition:
        case WunjoObjectType::Master:
        case WunjoObjectType::TimelineMix:
            m_projectMonitor->requestSeek(pos);
            break;
        case WunjoObjectType::BinClip:
    ...
    includeList->setChecked(WunjoSettings::enableAssetsIncludeList());
    // 10 bit support
    ...
    tenBit->setChecked(WunjoSettings::tenbitpipeline());
    ...
    // Installing or removing a plugin adds or drops the effects it brings
    connect(&PluginManager::instance(), &PluginManager::pluginEffectsChanged, this, &MainWindow::slotUpdatePluginEffects);
    ...
    switchAction->setChecked(WunjoSettings::showtitlebars());
    addAction(QStringLiteral("show_titlebars"), switchAction);
    connect(switchAction, &QAction::triggered, this, [](bool checked) {
        WunjoSettings::setShowtitlebars(checked);
        Q_EMIT pCore->hideBars(!checked);
    });
    connect(pCore.get(), &Core::switchTitleBars, this, [switchAction]() { switchAction->trigger(); });

    // [Wunjo] Theme menu — replaces the framework application-style (Breeze/Fusion)
    // chooser. Two brand themes (dark default / light) + a primary accent color,
    // all driving WunjoTheme. Occupies the "styles_menu" slot in the Settings menu.
    auto *themeMenu = new KActionMenu(QIcon::fromTheme(QStringLiteral("color-picker")), i18n("Theme"), this);
    QMenu *tm = themeMenu->menu();

    auto *themeGroup = new QActionGroup(this);
    QAction *darkAction = tm->addAction(i18n("Dark (default)"));
    QAction *lightAction = tm->addAction(i18n("Light"));
    for (QAction *a : {darkAction, lightAction}) {
        a->setCheckable(true);
        a->setActionGroup(themeGroup);
    }
    darkAction->setChecked(WunjoTheme::instance()->isDark());
    lightAction->setChecked(!WunjoTheme::instance()->isDark());
    connect(darkAction, &QAction::triggered, this, []() { WunjoTheme::instance()->setDark(true); });
    connect(lightAction, &QAction::triggered, this, []() { WunjoTheme::instance()->setDark(false); });
    connect(WunjoTheme::instance(), &WunjoTheme::themeChanged, this, [darkAction, lightAction](bool dark) {
        darkAction->setChecked(dark);
        lightAction->setChecked(!dark);
    });

    tm->addSeparator();
    QMenu *colorMenu = tm->addMenu(i18n("Primary Color"));
    struct ThemeSwatch
    {
        const char *name;
        QColor color;
    };
    const QList<ThemeSwatch> themeSwatches = {
    ...
    for (const ThemeSwatch &s : themeSwatches) {
        QPixmap pix(16, 16);
        pix.fill(s.color);
        QAction *act = colorMenu->addAction(QIcon(pix), QString::fromLatin1(s.name));
        const QColor c = s.color;
        connect(act, &QAction::triggered, this, [c]() { WunjoTheme::instance()->setAccent(c); });
    }
    colorMenu->addSeparator();
    QAction *customColor = colorMenu->addAction(i18n("Custom…"));
    connect(customColor, &QAction::triggered, this, [this]() {
        const QColor c = QColorDialog::getColor(WunjoTheme::instance()->accent(), this, i18n("Primary Color"));
        if (c.isValid()) {
            WunjoTheme::instance()->setAccent(c);
        }
    });

    addAction(QStringLiteral("styles_menu"), themeMenu);
    ...
    if (WunjoSettings::lastCacheCheck().isNull()) {
        // Define a date for first check
        WunjoSettings::setLastCacheCheck(QDateTime::currentDateTime());
    }
    ...
    renderButtonAction->setIcon(QIcon::fromTheme(QStringLiteral("wunjo-render")));
    ...
    // The file is named explicitly: left to itself KXmlGui builds the name from
    // the component — "wunjoui.rc" — and ours is simply ui.rc, inside the
    // application's own kxmlgui5/wunjo folder where nothing else can collide.
    setupGUI(KXmlGuiWindow::ToolBar | KXmlGuiWindow::StatusBar | KXmlGuiWindow::Create, QStringLiteral("ui.rc"));

    // Wunjo: flush toolbars to the window edges. Unlocked toolbars reserve
    // space for a drag handle even when the style draws none, which pushed
    // the render button away from the left border.
    KToolBar::setToolBarsLocked(true);
    const QList<KToolBar *> allToolBars = toolBars();
    for (KToolBar *bar : allToolBars) {
    ...
    // The About entry in the help menu is built by KHelpMenu, which gives it the
    // window icon — the launcher tile, mint on near-black, next to a column of
    // monochrome menu icons. Give it the mark the About dialog shows, in the ink
    // of the current theme.
    if (QAction *aboutApp = actionCollection()->action(KStandardAction::name(KStandardAction::AboutApp))) {
        const auto applyAboutIcon = [aboutApp]() {
            aboutApp->setIcon(QIcon(WunjoTheme::instance()->isDark() ? QStringLiteral(":/pics/logo-mark-white.svg")
    ...
    separate_channels->setChecked(WunjoSettings::displayallchannels());
    ...
    normalize_channels->setChecked(WunjoSettings::normalizechannels());
    ...
    proxyRender->setChecked(WunjoSettings::proxypreview());
    connect(proxyRender, &QAction::triggered, this, [&](bool checked) { WunjoSettings::setProxypreview(checked); });
    ...
    autoRender->setChecked(WunjoSettings::autopreview());
    ...
    if (WunjoSettings::grab_parameters().isEmpty() || WunjoSettings::grab_extension().isEmpty()) {
        ...
        if (i.hasNext()) {
            ...
            WunjoSettings::setGrab_parameters(grabstring.section(QLatin1Char(';'), 0, 0));
            WunjoSettings::setGrab_extension(grabstring.section(QLatin1Char(';'), 1, 1));
        }
    }
    if (WunjoSettings::decklink_parameters().isEmpty() || WunjoSettings::decklink_extension().isEmpty()) {
        ...
        if (i.hasNext()) {
            ...
            WunjoSettings::setDecklink_parameters(decklinkstring.section(QLatin1Char(';'), 0, 0));
            WunjoSettings::setDecklink_extension(decklinkstring.section(QLatin1Char(';'), 1, 1));
        }
    }
    if (!QDir(WunjoSettings::currenttmpfolder()).isReadable())
        WunjoSettings::setCurrenttmpfolder(QStandardPaths::writableLocation(QStandardPaths::TempLocation));
...
void MainWindow::loadContainerActions()
{
    ...
    if (helpMenu) {
        ...
        // Wunjo: retarget the standard KDE "Donate" entry to our community/Patreon page
        const QList<QAction *> helpActions = helpMenu->actions();
        for (QAction *a : helpActions) {
            if (a->objectName() == QLatin1String("help_donate")
                || QString(a->text()).remove(QLatin1Char('&')).contains(QStringLiteral("Donate"), Qt::CaseInsensitive)) {
                a->setText(i18n("Community"));
                a->disconnect();
                connect(a, &QAction::triggered, this, []() {
                    QDesktopServices::openUrl(QUrl(QStringLiteral("https://www.patreon.com/wladradchenko")));
                });
                break;
            }
        }
    ...
    Generators::getGenerators(WunjoSettings::producerslist(), addMenu);
    connect(addMenu, &QMenu::triggered, this, &MainWindow::buildGenerator);
}

QString MainWindow::scriptingSocketName() const
{
    return m_scriptingServer != nullptr ? m_scriptingServer->name() : QString();
}
...
bool MainWindow::queryClose()
{
    ...
    WunjoSettings::setKdockLayout(QString(dockLayout.serializeLayout()));
...
void MainWindow::saveProperties(KConfigGroup &config)
{
    ...
    if (qApp->isSavingSession() && pCore->projectManager()) {
        if (pCore->currentDoc() && !pCore->currentDoc()->url().isEmpty()) {
            config.writeEntry("wunjo_lastUrl", pCore->currentDoc()->url().toLocalFile());
        }
    }
}
...
void MainWindow::slotUpdatePluginEffects(const QStringList &addedFiles, const QStringList &removedIds)
{
    for (const QString &id : removedIds) {
        m_effectList2->removePluginEffect(id);
    }
    for (const QString &path : addedFiles) {
        m_effectList2->reloadCustomEffect(path);
    }
    m_effectList2->reloadEffectMenu(m_effectsMenu, m_effectActions);
}
...
{
    if (m_assetPanel->effectStackOwner().type == WunjoObjectType::TimelineClip) {
...
{
    addAction(name, action, shortcut, wunjoCategoryMap.value(category, nullptr));
}
...
{
    return addAction(name, text, receiver, member, icon, shortcut, wunjoCategoryMap.value(category, nullptr));
}
...
void MainWindow::setupActions()
{
    ...
    QString normalEditIconName =
        QIcon::hasThemeIcon(QStringLiteral("timeline-mode-normal")) ? QStringLiteral("timeline-mode-normal") : QStringLiteral("wunjo-normal-edit");
    ...
    QString overwriteEditIconName =
        QIcon::hasThemeIcon(QStringLiteral("timeline-mode-overwrite")) ? QStringLiteral("timeline-mode-overwrite") : QStringLiteral("wunjo-overwrite-edit");
    ...
    QString insertEditIconName =
        QIcon::hasThemeIcon(QStringLiteral("timeline-mode-insert")) ? QStringLiteral("timeline-mode-insert") : QStringLiteral("wunjo-insert-edit");
    ...
    splitView->setChecked(WunjoSettings::audiotracksbelow() == 1);
    ...
    splitView2->setChecked(WunjoSettings::audiotracksbelow() == 2);
    ...
    mixedView->setChecked(WunjoSettings::audiotracksbelow() == 0);
    ...
    if (WunjoSettings::frametimecode()) {
    ...
    m_buttonRippleTool = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-ripple")), i18n("Ripple Tool"), this);
    ...
    /* TODO Implement Roll
    m_buttonRollTool = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-rolling")), i18n("Roll Tool"), this);

    m_buttonRollTool->setCheckable(true);
    m_buttonRollTool->setChecked(false);*/

    m_buttonSlipTool = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-slip")), i18n("Slip Tool"), this);
    ...
    m_buttonSlideTool = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-slide")), i18n("Slide Tool"), this);
    ...
    m_buttonVideoThumbs = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-show-video")), i18n("Show Video Thumbnails"), this);
    ...
    m_buttonVideoThumbs->setChecked(WunjoSettings::videothumbnails());
    connect(m_buttonVideoThumbs, &QAction::triggered, this, &MainWindow::slotSwitchVideoThumbs);

    // TODO: remove icon check ones we require KF > 6.1
    QString waveformIconName = QIcon::hasThemeIcon(QStringLiteral("waveform")) ? QStringLiteral("waveform") : QStringLiteral("wunjo-show-audiothumb");
    ...
    m_buttonAudioThumbs->setChecked(WunjoSettings::audiothumbnails());
    ...
    m_statusZoomLevelButton = new QPushButton(i18n("%1×", WunjoSettings::waveformScaler()));
    ...
    statusZoomInButton->setEnabled(WunjoSettings::waveformScaler() < 8);
    statusZoomOutButton->setEnabled(WunjoSettings::waveformScaler() > 1);
    // Helper function to update button state based on zoom level (click to reset zoom)
    auto updateZoomButtonState = [this]() {
        if (WunjoSettings::waveformScaler() > 1) {
    ...
    connect(WunjoSettings::self(), &WunjoSettings::waveformScalerChanged, this, [this, statusZoomInButton, statusZoomOutButton, updateZoomButtonState]() {
        m_statusZoomLevelButton->setText(i18n("%1×", WunjoSettings::waveformScaler()));
    ...
    m_buttonShowMarkers = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-show-markers")), i18n("Show Markers Comments"), this);

    m_buttonShowMarkers->setCheckable(true);
    m_buttonShowMarkers->setChecked(WunjoSettings::showmarkers());
    ...
    m_buttonSnap->setChecked(WunjoSettings::snaptopoints());
    ...
    m_buttonHideClipOverlays->setChecked(WunjoSettings::showClipOverlays());
    ...
    m_buttonMouseZoomOnPlayhead->setChecked(WunjoSettings::timelinemousezoomonplayhead());
    ...
    m_buttonTimelineTags->setChecked(WunjoSettings::tagsintimeline());
...
#if defined(Q_OS_WIN)
    int glBackend = WunjoSettings::opengl_backend();
    ...
    addAction(QStringLiteral("project_render"), i18n("Render…"), this, SLOT(slotRenderProject()), QIcon::fromTheme(QStringLiteral("wunjo-render")),
              Qt::CTRL | Qt::Key_Return);

    addAction(QStringLiteral("stop_project_render"), i18n("Stop Render"), this, SLOT(slotStopRenderProject()),
              QIcon::fromTheme(QStringLiteral("wunjo-render")));
    ...
    m_scaleGroup->setEnabled(!WunjoSettings::external_display());
    ...
    connect(pCore->monitorManager(), &MonitorManager::scalingChanged, this, [scale_1, scale_2, scale_4, scale_8, scale_16, scale_no]() {
        switch (WunjoSettings::previewScaling()) {
        case 1:
    ...
    connect(m_scaleGroup, &QActionGroup::triggered, this, [](QAction *ac) {
        ...
        WunjoSettings::setPreviewScaling(scaling);
    ...
    dropFrames->setChecked(WunjoSettings::monitor_dropframes());
    ...
    QAction *insertBinZone = addAction(QStringLiteral("insert_project_tree"), i18n("Insert Zone in Project Bin"), this, SLOT(slotInsertZoneToTree()),
                                       QIcon::fromTheme(QStringLiteral("wunjo-add-clip")), Qt::CTRL | Qt::Key_I);
    ...
    wunjoCategoryMap.insert(QStringLiteral("timelineselection"), clipActionCategory);
    ...
    autoTrackHeight->setChecked(WunjoSettings::autotrackheight());
    ...
    connect(masterEffectStack, &QAction::triggered, this, [&]() {
        pCore->monitorManager()->activateMonitor(Wunjo::ProjectMonitor);
    ...
    wunjoCategoryMap.insert(QStringLiteral("timeline"), timelineActions);
    ...
    lockGuides->setChecked(WunjoSettings::lockedGuides());
    ...
    // AI tools entry point: opens the object mask (SAM) panel for the
    // selected clip. Grouped with the other AI actions in the clip menu.
    QAction *aiObjectMask = new QAction(QIcon::fromTheme(QStringLiteral("path-mask-edit")), i18n("Object Mask (select && track objects)"), this);
    addAction(QStringLiteral("ai_object_mask"), aiObjectMask);
    connect(aiObjectMask, &QAction::triggered, this, [this]() {
        if (m_effectStackDock) {
            m_effectStackDock->open();
        }
        Q_EMIT pCore->switchMaskPanel(true);
    });

    // Per-clip face detection property (bundled RetinaFace model, CPU, no
    // installation needed). While enabled, previewed frames of the clip run
    // through the detector: the monitor shows the boxes and the positions
    // are collected in FaceDataStore for future face-based features. The
    // action's data() carries the bin id of the right-clicked clip (set in
    // TimelineWidget::showClipMenu).
    QAction *aiDetectFaces = new QAction(QIcon::fromTheme(QStringLiteral("wunjo-detect-faces")), i18n("Detect Faces"), this);
    aiDetectFaces->setCheckable(true);
    addAction(QStringLiteral("ai_detect_faces"), aiDetectFaces);
    connect(aiDetectFaces, &QAction::toggled, this, [this, aiDetectFaces](bool enabled) {
        // From the timeline the action carries the clip's range as well: that
        // part is analysed first, the rest of the file after it.
        const QVariantList context = aiDetectFaces->data().toList();
        const QString binId = context.isEmpty() ? aiDetectFaces->data().toString() : context.constFirst().toString();
        const int from = context.size() > 1 ? context.at(1).toInt() : -1;
        const int to = context.size() > 2 ? context.at(2).toInt() : -1;
        if (binId.isEmpty()) {
            return;
        }
        ...
        if (enabled) {
            // draw from stored data when it exists, analyse the rest in the
            // background (per-clip json in <projectDataFolder>/faces/)
    ...
    QAction *configNotifAction = KStandardAction::configureNotifications(this, &MainWindow::configureNotifications, actionCollection());
...
bool MainWindow::readOptions()
{
    ...
    if (WunjoSettings::defaultprojectfolder().isEmpty()) {
        QDir dir(QStandardPaths::writableLocation(QStandardPaths::MoviesLocation));
        dir.mkpath(QStringLiteral("."));
        WunjoSettings::setDefaultprojectfolder(dir.absolutePath());
    }

    if (WunjoSettings::trackheight() == 0) {
        ...
        WunjoSettings::setTrackheight(trackHeight);
    }
    bool firstRun = false;
    KConfigGroup initialGroup(config, "version");
    if (!initialGroup.exists() || WunjoSettings::sdlAudioBackend().isEmpty() || WunjoSettings::wunjorendererpath().isEmpty()) {
        // First run, check if user is on a KDE Desktop
        firstRun = true;
        // Define default video location for first run
        KRecentDirs::add(QStringLiteral(":WunjoClipFolder"), QStandardPaths::writableLocation(QStandardPaths::MoviesLocation));
        KRecentDirs::add(QStringLiteral(":WunjoProjectsFolder"), QStandardPaths::writableLocation(QStandardPaths::MoviesLocation));
    ...
    } else if (!WunjoSettings::ffmpegpath().isEmpty() && !QFile::exists(WunjoSettings::ffmpegpath())) {
    ...
    if (firstRun) {
        if (TransitionsRepository::get()->exists(QStringLiteral("qtblend")) && TransitionsRepository::get()->getVersion(QStringLiteral("qtblend")) > 200) {
            WunjoSettings::setPreferredcomposite(QStringLiteral("qtblend"));
        }
    }
    initialGroup.writeEntry("version", version);
    if (WunjoSettings::guidesCategories().isEmpty()) {
        WunjoSettings::setGuidesCategories(WunjoDoc::getDefaultGuideCategories());
    }
    return firstRun;
}
...
{
    WunjoSettingsDialog *d = static_cast<WunjoSettingsDialog *>(KConfigDialog::exists(QStringLiteral("settings")));
    if (d) {
        d->checkProfile();
    }
}
...
void MainWindow::slotEditProjectSettings(int ix)
{
    ...
    if (w->exec() == QDialog::Accepted) {
        ...
        if (WunjoSettings::videothumbnails() != w->enableVideoThumbs()) {
            slotSwitchVideoThumbs();
        }
        if (WunjoSettings::audiothumbnails() != w->enableAudioThumbs()) {
...
void MainWindow::connectDocument()
{
    WunjoDoc *project = pCore->currentDoc();
    if (m_chatWidget) {
        m_chatWidget->setProjectFolder(project->projectDataFolder());
    }
    connect(project, &WunjoDoc::startAutoSave, pCore->projectManager(), &ProjectManager::slotStartAutoSave);
    connect(project, &WunjoDoc::reloadEffects, this, &MainWindow::slotReloadEffects);
    WunjoSettings::setProject_fps(pCore->getCurrentFps());
    ...
    connect(project, &WunjoDoc::docModified, this, &MainWindow::slotUpdateDocumentState);
...
void MainWindow::slotEditKeys()
{
    ...
    KNSWidgets::Action *downloadKeybordSchemes =
        new KNSWidgets::Action(i18n("Download New Keyboard Schemes…"), QStringLiteral(":data/wunjo_keyboardschemes.knsrc"), this);
            ...
            for (auto &entry : changedEntries) {
                ...
                for (auto &file : files) {
                    ...
                    if (group.exists()) {
                        ...
                        node.setAttribute(QStringLiteral("name"), QStringLiteral("wunjo"));
...
void MainWindow::slotShowPreferencePage(Wunjo::ConfigPage page, int option)
{
    /*
    ...
    auto *dialog = new WunjoSettingsDialog(actions, m_gpuAllowed, this);
    dialog->setAttribute(Qt::WA_DeleteOnClose);
    ...
    connect(dialog, &WunjoSettingsDialog::doResetConsumer, this, [this](bool fullReset) {
        m_scaleGroup->setEnabled(!WunjoSettings::external_display());
        pCore->projectManager()->slotResetConsumers(fullReset);
    });
    connect(dialog, &WunjoSettingsDialog::checkTabPosition, this, &MainWindow::slotCheckTabPosition);
    connect(dialog, &WunjoSettingsDialog::restartWunjo, this, &MainWindow::slotRestart);
    connect(dialog, &WunjoSettingsDialog::updateLibraryFolder, pCore.get(), &Core::updateLibraryPath);
    connect(dialog, &WunjoSettingsDialog::resetView, this, &MainWindow::resetTimelineTracks);
    connect(WunjoSettings::self(), &WunjoSettings::window_backgroundChanged, pCore->monitorManager(), &MonitorManager::updateBgColor);
...
void MainWindow::showPluginSettings(const QString &pluginId)
{
    slotShowPreferencePage(Wunjo::PageSpeech, -1);
    if (auto *dialog = static_cast<WunjoSettingsDialog *>(KConfigDialog::exists(QStringLiteral("settings")))) {
        dialog->showPluginTab(pluginId);
    }
}
...
void MainWindow::slotCheckTabPosition()
{
    ...
    if (WunjoSettings::tabposition() != pos) {
...
void MainWindow::slotRestart(bool clean)
{
    if (clean) {
        if (KMessageBox::warningContinueCancel(this,
                                               i18n("This will delete Wunjo's configuration file and restart the application. Do you want to proceed?"),
...
{
    WunjoSettings::setVideothumbnails(!WunjoSettings::videothumbnails());
    m_buttonVideoThumbs->setChecked(WunjoSettings::videothumbnails());
}

void MainWindow::slotSwitchAudioThumbs()
{
    WunjoSettings::setAudiothumbnails(!WunjoSettings::audiothumbnails());
...
{
    WunjoSettings::setShowmarkers(!WunjoSettings::showmarkers());
    m_buttonShowMarkers->setChecked(WunjoSettings::showmarkers());
}

void MainWindow::slotSwitchSnap()
{
    WunjoSettings::setSnaptopoints(!WunjoSettings::snaptopoints());
    m_buttonSnap->setChecked(WunjoSettings::snaptopoints());
}

void MainWindow::slotSwitchClipOverlays()
{
    WunjoSettings::setShowClipOverlays(!WunjoSettings::showClipOverlays());
    m_buttonHideClipOverlays->setChecked(WunjoSettings::showClipOverlays());
}

void MainWindow::slotShowTimelineTags()
{
    WunjoSettings::setTagsintimeline(!WunjoSettings::tagsintimeline());
    m_buttonTimelineTags->setChecked(WunjoSettings::tagsintimeline());
...
{
    WunjoSettings::setTimelinemousezoomonplayhead(!WunjoSettings::timelinemousezoomonplayhead());
    m_buttonMouseZoomOnPlayhead->setChecked(WunjoSettings::timelinemousezoomonplayhead());
}
...
void MainWindow::slotAddMarkerWithCategory()
{
    ...
    int currentCategory = WunjoSettings::default_marker_type();

    WunjoSettings::setDefault_marker_type(category);
    if (m_clipMonitor->isActive()) {
        QMap<int, QString> marker;
        marker.insert(m_clipMonitor->position(), QString());
        pCore->bin()->addClipMarker(m_clipMonitor->activeClipId(), marker);
    ...
    WunjoSettings::setDefault_marker_type(currentCategory);
}
...
{
    WunjoSettings::setDisplayallchannels(!WunjoSettings::displayallchannels());
}

void MainWindow::slotAutoTrackHeight(bool enable)
{
    WunjoSettings::setAutotrackheight(enable);
    Q_EMIT pCore->autoTrackHeight(enable);
}

void MainWindow::slotNormalizeAudioChannel(bool normalize)
{
    WunjoSettings::setNormalizechannels(normalize);
}
...
{
    WunjoSettings::setLockedGuides(lock);
}
...
{
    getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineClip, true);
}
...
{
    bool res = getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineComposition, true, false, false);
    if (!res) {
        getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineMix, true);
    }
}

void MainWindow::slotDeselectTimelineClip()
{
    getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineClip, false);
}

void MainWindow::slotDeselectTimelineTransition()
{
    bool res = getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineComposition, false, false, false);
    if (!res) {
        getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineMix, false);
    }
}

void MainWindow::slotSelectAddTimelineClip()
{
    getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineClip, true, true);
}

void MainWindow::slotSelectAddTimelineTransition()
{
    getCurrentTimeline()->controller()->selectCurrentItem(WunjoObjectType::TimelineComposition, true, true);
}
...
{
    if (m_assetPanel->effectStackOwner().type == WunjoObjectType::BinClip || m_clipMonitor->isActive()) {
        // Pass the command to bin
        pCore->activeBin()->slotAddEffect({}, {effectId});
    } else if (m_assetPanel->effectStackOwner().type == WunjoObjectType::TimelineTrack ||
               m_assetPanel->effectStackOwner().type == WunjoObjectType::Master) {
        ...
        effectData.insert(QStringLiteral("wunjo/effect"), effectId);
...
{
    slotSetZoom(m_zoomSlider->value() - 1, WunjoSettings::timelinemousezoomonplayhead() ? false : zoomOnMouse);
...
void MainWindow::slotClipInTimeline(const QString &clipId, const QList<int> &ids)
{
    ...
    for (int i = 0; i < ids.count(); ++i) {
        ObjectId oid(WunjoObjectType::TimelineClip, ids.at(i), pCore->currentTimelineId());
...
{
    QString binId;
    if (ownerId.type != WunjoObjectType::TimelineClip) {
        int cid = getCurrentTimeline()->controller()->getMainSelectedClip();
        if (cid == -1) {
            return;
        }
        ownerId = ObjectId(WunjoObjectType::TimelineClip, cid, pCore->currentTimelineId());
...
void MainWindow::slotUpdateTimelineView(QAction *action)
{
    ...
    WunjoSettings::setAudiotracksbelow(viewMode);
    getCurrentTimeline()->model()->_resetView();
}
...
{
    QList<QAction *> list = wunjoCategoryMap.value(QStringLiteral("interface"))->actions();
    // Sort actions
...
{
    if (wunjoCategoryMap.contains(QStringLiteral("clipjobs"))) {
        auto ts = wunjoCategoryMap.take(QStringLiteral("clipjobs"));
        QList<QAction *> acs = ts->actions();
        qDeleteAll(acs);
        delete ts;
    }
    if (wunjoCategoryMap.contains(QStringLiteral("transcoderslist"))) {
        auto ts = wunjoCategoryMap.take(QStringLiteral("transcoderslist"));
        QList<QAction *> acs = ts->actions();
        qDeleteAll(acs);
        delete ts;
    }
    if (wunjoCategoryMap.contains(QStringLiteral("audiotranscoderslist"))) {
        auto ts = wunjoCategoryMap.take(QStringLiteral("audiotranscoderslist"));
        QList<QAction *> acs = ts->actions();
        qDeleteAll(acs);
        delete ts;
    }
    ...
    wunjoCategoryMap.insert(QStringLiteral("clipjobs"), cjobs);
            ...
            for (const QString &id : ids) {
                ...
                if (clipStreamSelection.contains(clipId)) {
                    ...
                    for (auto &ix : selectedStreams) {
                        ...
                        args.append(tData);
                        TranscodeTask::start(ObjectId(WunjoObjectType::BinClip, clipId.toInt(), QUuid()), suffix, QString(), args, info, clipIn, clipOut,
                                             false, clip.get());
                    }
                } else {
                    TranscodeTask::start(ObjectId(WunjoObjectType::BinClip, clipId.toInt(), QUuid()), QString(), QString(), tData, info, clipIn, clipOut,
                                         false, clip.get());
                }
            }
        });
    ...
    wunjoCategoryMap.insert(QStringLiteral("transcoderslist"), ts);
    wunjoCategoryMap.insert(QStringLiteral("audiotranscoderslist"), ats);
    updateDockMenu();
}
...
void MainWindow::updateDockMenu()
{
    ...
    if (wunjoCategoryMap.contains(QStringLiteral("interface"))) {
        guiActions = wunjoCategoryMap.value(QStringLiteral("interface"));
    ...
    wunjoCategoryMap.insert(QStringLiteral("interface"), guiActions);
    loadDockActions();
}

QList<QAction *> MainWindow::getExtraActions(const QString &name)
{
    if (!wunjoCategoryMap.contains(name)) {
        return QList<QAction *>();
    }
    return wunjoCategoryMap.value(name)->actions();
}
...
void MainWindow::slotFriendlyTranscode(const QString &binId, bool checkProfile)
{
    ...
    sourceProps.insert(QStringLiteral("wunjo:originalurl"), clip->url());
    sourceProps.insert(QStringLiteral("wunjo:clipname"), clip->clipName());
    sourceProps.insert(QStringLiteral("wunjo:proxy"), clip->getProducerProperty(QStringLiteral("wunjo:proxy")));
    ...
    connect(d, &ClipTranscode::addClip, [&, binId, sourceProps](const QUrl &url, const QString & /*folderInfo*/) {
        ...
        newProps.insert(QStringLiteral("wunjo:originalurl"), url.toLocalFile());
        newProps.insert(QStringLiteral("wunjo:clipname"), url.fileName());
        newProps.insert(QStringLiteral("wunjo:proxy"), QStringLiteral("-"));
...
void MainWindow::slotTranscodeClip()
{
    ...
    QString clipFolder = KRecentDirs::dir(QStringLiteral(":WunjoClipFolder"));
...
{
    WunjoSettings::setFrametimecode(ix == 1);
...
void MainWindow::slotShutdown()
{
    ...
    // Ask the session to shut the machine down. Every desktop exposes this over
    // D-Bus and each one differently; the tools below reach the same services
    // without the application having to speak a bus, and cover the three
    ...
    QProcess::startDetached(QStringLiteral("osascript"), {QStringLiteral("-e"), QStringLiteral("tell app \"System Events\" to shut down")});
#else
    // systemctl asks logind, which is what the desktops' own prompts end up
    // calling. It refuses politely when the user may not power the machine off.
    if (!QProcess::startDetached(QStringLiteral("systemctl"), {QStringLiteral("poweroff")})) {
        pCore->displayMessage(i18n("Could not ask the system to shut down."), ErrorMessage);
    }
#endif
}
...
{
    if (pCore->monitorManager()->isActive(Wunjo::ClipMonitor)) {
...
{
    WunjoSettings::setMonitor_dropframes(drop);
...
void MainWindow::triggerKey(QKeyEvent *ev)
{
    ...
    if (WunjoKeySequence::isShiftAsModifierAllowed(ev->key())) {
                                                          ...
                                                          KDDockWidgets::QtWidgets::DockWidget *otherDockWidget, const QSize preferredSize)
{
    ...
    KActionCategory *guiActions = nullptr;
    if (wunjoCategoryMap.contains(QStringLiteral("interface"))) {
        guiActions = wunjoCategoryMap.take(QStringLiteral("interface"));
    } else {
        guiActions = new KActionCategory(i18n("Interface"), actionCollection());
    }
    auto dockAction = dock->toggleAction();
    connect(dockAction, &QAction::triggered, this, [dock](bool dockVisible) {
        if (!dockVisible && !WunjoSettings::showtitlebars()) {
...
void MainWindow::slotUpdateMonitorOverlays(int id, int code)
{
    ...
    for (QAction *ac : std::as_const(actions)) {
        ...
        if (mid == Monitor::InfoOverlay || mid == Monitor::ClipJobsOverlay) {
            ac->setVisible(id == Wunjo::ClipMonitor);
        }
...
{
    WunjoSettings::setAutopreview(enable);
...
{
    if (WunjoSettings::normalizechannels()) {
        WunjoSettings::setNormalizechannels(false);
    }
    if (WunjoSettings::waveformScaler() < 5) {
        WunjoSettings::setWaveformScaler(WunjoSettings::waveformScaler() * 2);
    }
...
void MainWindow::slotAudioZoomReset()
{
    ...
    WunjoSettings::setWaveformScaler(1);
    slotNormalizeAudioChannel(true);
...
void MainWindow::updateAudioWaveformActionIcon()
{
    ...
    QString waveformIconName = QIcon::hasThemeIcon(QStringLiteral("waveform")) ? QStringLiteral("waveform") : QStringLiteral("wunjo-show-audiothumb");
    if (WunjoSettings::waveformScaler() > 1 && WunjoSettings::audiothumbnails()) {
...
void MainWindow::resetSubtitles(const QUuid &uuid)
{
    // Hide subtitle track
    m_buttonSubtitleEditTool->setChecked(false);
    WunjoSettings::setShowSubtitles(false);
...
void MainWindow::slotShowSubtitles(bool show)
{
    ...
    WunjoSettings::setShowSubtitles(show);
...
void MainWindow::slotEditSubtitle(const QMap<QString, QString> &subProperties)
{
    ...
    if (!hasSubtitleModel) {
        ...
        WunjoSettings::setShowSubtitles(m_buttonSubtitleEditTool->isChecked());
        getCurrentTimeline()->connectSubtitleModel(false);
    }
}
...
void MainWindow::slotCopyDebugInfo()
{
    ...
    debuginfo.append(QStringLiteral("Movit (GPU): %1\n").arg(WunjoSettings::gpu_accel() ? QStringLiteral("enabled") : QStringLiteral("disabled")));
...
bool MainWindow::eventFilter(QObject *object, QEvent *event)
{
    ...
    case QEvent::ApplicationPaletteChange:
        // The bundled icons use stroke:currentColor + a ColorScheme-Text style
        // that KIconLoader rewrites from the active scheme. Drop cached renders
        // and notify consumers so icons recolor (white↔dark) on a theme switch.
        QPixmapCache::clear();
        for (auto grp : {KIconLoader::Desktop, KIconLoader::Toolbar, KIconLoader::MainToolbar, KIconLoader::Small, KIconLoader::Panel, KIconLoader::Dialog}) {
            KIconLoader::global()->emitChange(grp);
        }
        if (m_assetPanel) {
            m_assetPanel->clear();
        }
...
void MainWindow::slotRemoveBinDock(const QString &name)
{
    ...
    if (!m_windowClosing) {
        WunjoSettings::setBinsCount(m_binWidgets.size());
        updateDockMenu();
    }
}
...
void MainWindow::cleanBins()
{
    ...
    if (!binHasFocus) {
        pCore->getMonitor(Wunjo::ClipMonitor)->refreshMonitor();
    }
}
...
{
    if (!WunjoSettings::checkForUpdate()) {
        return;
    }
    // At most once a week. The interval is deliberately long: this exists so
    // people hear about a release eventually, not so the application phones
    // home on every launch.
    const QDateTime last = WunjoSettings::lastUpdateCheck();
    if (last.isValid() && last.daysTo(QDateTime::currentDateTime()) < 7) {
        return;
    }
    // Written before the answer arrives, not after: a machine that is offline
    // every Monday would otherwise retry on every single startup.
    WunjoSettings::setLastUpdateCheck(QDateTime::currentDateTime());

    static const QString releasesApi = QStringLiteral("https://api.github.com/repos/wladradchenko/wunjo.wladradchenko.ru/releases/latest");
    auto *manager = new QNetworkAccessManager(this);
    QNetworkRequest request{QUrl(releasesApi)};
    request.setRawHeader("Accept", "application/vnd.github+json");
    // GitHub refuses requests without one, and an honest agent is how they can
    // tell this traffic apart if it ever misbehaves.
    request.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("WunjoMake/%1").arg(QStringLiteral(WUNJO_VERSION)));
    QNetworkReply *reply = manager->get(request);
    connect(reply, &QNetworkReply::finished, this, [this, reply, manager]() {
        reply->deleteLater();
        manager->deleteLater();
        if (reply->error() != QNetworkReply::NoError) {
            // No network, rate limited, no releases yet: all of them mean "ask
            // again next week", and none of them are the user's problem.
            return;
        }
        ...
        // Dates, not version numbers. Whether a release is newer than this
        // build is a fact; whether "3.0.0" sorts before some future tag is a
        // promise about numbering that a re-tag, a hotfix branch or a change of
        // scheme would quietly break. Version numbers are only what gets shown.
        const QDate published = QDateTime::fromString(release.value(QStringLiteral("published_at")).toString(), Qt::ISODate).date();
        const QDate built = QDate::fromString(QStringLiteral(WUNJO_BUILD_DATE), Qt::ISODate);
        if (!published.isValid() || !built.isValid() || published <= built) {
            return;
        }
        // Tags are written "v3.1.0" as often as "3.1.0".
        QString latest = release.value(QStringLiteral("tag_name")).toString();
        if (latest.startsWith(QLatin1Char('v'), Qt::CaseInsensitive)) {
            latest.remove(0, 1);
        }
        const QString current = QStringLiteral(WUNJO_VERSION);
        if (latest.isEmpty() || latest == current) {
            // Same release, published after this build was made: somebody
            // re-uploaded it. Nothing for the user to go and get.
            return;
        }
        const QString page = release.value(QStringLiteral("html_url")).toString();
        QAction *updateAction = new QAction(i18n("Go to download page"), this);
        connect(updateAction, &QAction::triggered, this, [page]() {
            QDesktopServices::openUrl(QUrl(page.isEmpty() ? QStringLiteral("https://wunjo.online/download") : page));
        });
        QAction *abortAction = new QAction(i18n("Never check again"), this);
        connect(abortAction, &QAction::triggered, this, []() { WunjoSettings::setCheckForUpdate(false); });
        Q_EMIT pCore->displayBinMessage(i18n("Wunjo Make %1 is available — you are running %2.", latest, current), KMessageWidget::Information,
                                        {updateAction, abortAction}, true, BinMessage::BinCategory::UpdateMessage);
    });
}

void MainWindow::checkMaxCacheSize()
{
    if (WunjoSettings::lastCacheCheck().daysTo(QDateTime::currentDateTime()) < 12) {
        return;
    }
    WunjoSettings::setLastCacheCheck(QDateTime::currentDateTime());
    // Check cached data size
    if (WunjoSettings::maxcachesize() <= 0 || pCore->currentDoc() == nullptr) {
        return;
    }
    bool ok;
    ...
    while (!toAdd.isEmpty()) {
        ...
        connect(watcher, &QFutureWatcherBase::finished, this, [this, watcher] {
            ...
            m_totalCacheJobs--;
            watcher->deleteLater();
            if (m_totalCacheJobs == 0 && m_totalCacheSize > KIO::filesize_t(1048576) * WunjoSettings::maxcachesize()) {
                slotManageCache();
            }
        });
...
void MainWindow::connectTimeline()
{
    ...
    connect(pCore->library(), &LibraryWidget::saveTimelineSelection, getCurrentTimeline()->controller(), &TimelineController::saveTimelineSelection,
            Qt::UniqueConnection);
    getCurrentTimeline()->controller()->clipActions = wunjoCategoryMap.value(QStringLiteral("timelineselection"))->actions();
    ...
    WunjoSettings::setShowSubtitles(showSubs && hasSubtitleModel);
...
void MainWindow::disconnectTimeline(TimelineWidget *timeline, bool onClose)
{
    ...
    if (pCore->currentDoc()) {
        ...
        //  disconnect(pCore->currentDoc(), &WunjoDoc::docModified, this, &MainWindow::slotUpdateDocumentState);
    }
    if (!onClose) {
...
void MainWindow::reloadAssetPanel()
{
    ...
    if (owner.type == WunjoObjectType::NoItem) {
        return;
    }
...
void MainWindow::slotEditToolbars()
{
    ...
    // connect dialog changes to Wunjo's refresh/save function (Fixes visual toolbar updates)
