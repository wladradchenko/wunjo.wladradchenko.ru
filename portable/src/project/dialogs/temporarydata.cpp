...
TemporaryData::TemporaryData(WunjoDoc *doc, bool currentProjectOnly, QWidget *parent)
    : QDialog(parent)
    , m_doc(doc)
    , m_currentProjectOnly(currentProjectOnly)
{
    ...
    // Config cleanup age
    gCleanupSpin->setSuffix(i18np(" month", " months", WunjoSettings::cleanCacheMonths()));
    gCleanupSpin->setValue(WunjoSettings::cleanCacheMonths());
    connect(gCleanupSpin, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this, [&](int value) {
        WunjoSettings::setCleanCacheMonths(value);
        gCleanupSpin->setSuffix(i18np(" month", " months", WunjoSettings::cleanCacheMonths()));
    });
    ...
    cache_info->setText(i18n("Your cache and backup data exceeds %1, cleanup is recommended.", KIO::convertSize(1048576 * WunjoSettings::maxcachesize())));
...
void TemporaryData::cleanBackup()
{
    ...
    for (const QFileInfo &f : std::as_const(files)) {
        if (f.lastModified().addMonths(WunjoSettings::cleanCacheMonths()) < current) {
            oldFiles << f.fileName();
            totalSize += f.size();
        }
    }
    if (oldFiles.isEmpty()) {
        KMessageBox::information(this, i18n("No backup data older than %1 months was found.", WunjoSettings::cleanCacheMonths()));
        return;
    }
    if (KMessageBox::warningContinueCancelList(
            this,
            i18n("This will delete backup data (%1) for projects older than %2 months.", KIO::convertSize(totalSize), WunjoSettings::cleanCacheMonths()),
...
void TemporaryData::cleanCache()
{
    ...
    for (int i = 0; i < max; i++) {
        ...
        if (childSize < 200000 || child->data(2, Qt::UserRole).toDateTime().addMonths(WunjoSettings::cleanCacheMonths()) < current) {
            emptyDirs << child;
            total += childSize;
        }
    }
    ...
    if (folders.isEmpty()) {
        KMessageBox::information(this, i18n("No cache data older than %1 months was found.", WunjoSettings::cleanCacheMonths()));
        return;
    }
    ...
    if (KMessageBox::warningContinueCancelList(this,
                                               i18n("This will delete cache data (%1) for projects that were deleted, have very few cached data or older than "
                                                    ...
                                                    KIO::convertSize(total), WunjoSettings::cleanCacheMonths()),
...
void TemporaryData::updateGlobalInfo()
{
    ...
    // These are some KDE cache dirs related to Wunjo that don't manage ourselves
...
void TemporaryData::gotFolderSize(KIO::filesize_t total)
{
    ...
    if (!str.isEmpty()) {
        ...
        if (QFile::exists(path)) {
            item->setIcon(0, QIcon::fromTheme(QStringLiteral("wunjo")));
        } else {
        ...
        if (m_processingDirectory == QLatin1String("proxy")) {
            item->setIcon(0, QIcon::fromTheme(QStringLiteral("wunjo-show-video")));
        }
    }
...
{
    if (WunjoSettings::maxcachesize() > 0 &&
        ((m_totalGlobal + m_totalBackup + m_totalProxy) > KIO::filesize_t(1048576) * WunjoSettings::maxcachesize())) {
...
void TemporaryData::cleanProxy()
{
    ...
    if (oldFiles.isEmpty()) {
        KMessageBox::information(this, i18n("No proxy clip older than %1 months found.", WunjoSettings::cleanCacheMonths()));
        return;
    }
    if (KMessageBox::warningContinueCancelList(
