    ...
    , m_projectFolder(std::move(projectFolder))
{
    ...
    m_projectWildcard.append(QStringLiteral("-??"));
    m_projectWildcard.append(QStringLiteral("-??"));
    m_projectWildcard.append(QStringLiteral("-??.wmproj"));
...
void BackupWidget::slotParseBackupFiles()
{
    ...
    // Parse new XDG backup folder $HOME/.local/share/wunjo/.backup
    ...
    for (int i = 0; i < resultList.count(); ++i) {
        ...
        if (m_projectWildcard.startsWith(QLatin1Char('*'))) {
            ...
            label.prepend(resultList.at(i).fileName().section(QLatin1Char('-'), 0, -7) + QStringLiteral(".wmproj - "));
        }
    ...
    // Parse old $HOME/wunjo/.backup folder
    ...
    if (dir.exists()) {
        ...
        for (int i = 0; i < resultList2.count(); ++i) {
            ...
            if (m_projectWildcard.startsWith(QLatin1Char('*'))) {
                // Displaying all backup files, so add project name in the entries
                label.prepend(resultList2.at(i).fileName().section(QLatin1Char('-'), 0, -7) + QStringLiteral(".wmproj - "));
            }
            auto *item = new QListWidgetItem(label, backup_list);
