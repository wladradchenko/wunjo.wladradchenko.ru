...
{
    return {QStringLiteral("wunjo/clip")};
}

void GuidesProxyModel::switchThumbs()
{
    m_showThumbs = WunjoSettings::guidesShowThumbs();
    ...
    : QWidget(parent)
{
    ...
    slotShowThumbs(WunjoSettings::guidesShowThumbs());
    ...
    showThumbs->setChecked(WunjoSettings::guidesShowThumbs());
    ...
    selectZone->setChecked(WunjoSettings::guidesSelectZone());
    connect(selectZone, &QAction::toggled, this, [&](bool toggled) { WunjoSettings::setGuidesSelectZone(toggled); });
...
void GuidesList::addGuide()
{
    ...
    int frame = pCore->getMonitorPosition(m_displayMode == TimelineMarkers ? Wunjo::ProjectMonitor : Wunjo::ClipMonitor);
    if (frame >= 0) {
        GenTime pos(frame, pCore->getCurrentFps());
        if (auto markerModel = m_model.lock()) {
            ProjectClip *clip = nullptr;
    ...
    if (m_displayMode == AllMarkers) {
        ...
        if (sourceModel) {
            ...
            if (markerModel) {
                ...
                QPoint zone;
                if (WunjoSettings::guidesSelectZone()) {
        ...
        pCore->seekMonitor(m_displayMode == TimelineMarkers ? Wunjo::ProjectMonitor : Wunjo::ClipMonitor, pos);
        if (WunjoSettings::guidesSelectZone()) {
            int out = m_proxy->data(ix, MarkerListModel::EndPosRole).toInt();
            if (out > pos) {
                pCore->setMonitorZone(m_displayMode == TimelineMarkers ? Wunjo::ProjectMonitor : Wunjo::ClipMonitor, QPoint(pos, out));
            }
        }
    }
}
...
void GuidesList::rebuildCategories()
{
    ...
    if (markerDefaultMenu) {
        ...
        connect(markerDefaultMenu, &QMenu::triggered, this, [this](QAction *ac) {
            ...
            WunjoSettings::setDefault_marker_type(val);
    ...
    while (i.hasNext()) {
        ...
        if (i.key() == WunjoSettings::default_marker_type()) {
    ...
    if (!defaultCategoryFound) {
        ...
        if (ac) {
            default_category->setIcon(ac->icon());
            WunjoSettings::setDefault_marker_type(ac->data().toInt());
        }
    }
}
...
{
    int ix = WunjoSettings::default_marker_type();
...
{
    WunjoSettings::setGuidesShowThumbs(show);
    int fontHeight = QFontMetrics(font()).lineSpacing();
    ...
    if (m_displayMode == ClipMarkers) {
        if (auto markerModel = m_model.lock()) {
            ...
            if (missingFrames.size() > 0) {
                ...
                CacheTask::start(ObjectId(WunjoObjectType::BinClip, binId.toInt(), QUuid()), missingFrames, this);
            }
...
void GuidesList::rebuildThumbs()
{
    if (auto markerModel = m_model.lock()) {
        ...
        CacheTask::start(ObjectId(WunjoObjectType::BinClip, binId.toInt(), QUuid()), frames, this);
    }
}
