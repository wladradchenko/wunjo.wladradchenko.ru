    ...
    , m_thumbJob(nullptr)
{
    ...
    m_view.folder_url->setUrl(QUrl::fromLocalFile(KRecentDirs::dir(QStringLiteral(":WunjoSlideShowFolder"))));
    m_view.icon_list->setIconSize(QSize(50, 50));
    m_view.show_thumbs->setChecked(WunjoSettings::showslideshowthumbs());
    ...
    if (clip) {
        ...
        QString extension = WunjoSettings::slideshowmimeextension();
        if (file.isFile()) {
        ...
        m_view.method_mime->setChecked(WunjoSettings::slideshowbymime());
...
{
    if (state == Qt::Checked) {
        WunjoSettings::setShowslideshowthumbs(true);
        slotGenerateThumbs();
    } else {
        WunjoSettings::setShowslideshowthumbs(false);
        if (m_thumbJob) {
...
void SlideshowClip::slotMethodChanged(bool active)
{
    if (active) {
        ...
        m_view.clip_duration->setText(m_timecode.reformatSeparators(WunjoSettings::image_duration()));
        m_view.stackedWidget->setCurrentIndex(0);
        WunjoSettings::setSlideshowbymime(true);
    } else {
        // User wants pattern image sequence
        m_view.clip_duration->setText(m_timecode.reformatSeparators(WunjoSettings::sequence_duration()));
        m_view.stackedWidget->setCurrentIndex(1);
        WunjoSettings::setSlideshowbymime(false);
    }
    parseFolder();
}
