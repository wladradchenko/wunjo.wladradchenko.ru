...
ProjectSettings::ProjectSettings(WunjoDoc *doc, QMap<QString, QString> metadata, int videotracks, int audiotracks, int audiochannels,
    ...
    , m_newProject(doc == nullptr)
{
    ...
    audio_thumbs->setChecked(WunjoSettings::audiothumbnails());
    video_thumbs->setChecked(WunjoSettings::videothumbnails());
    ...
    if (doc) {
        ...
        currentProf = WunjoSettings::default_profile();
        proxy_box->setChecked(WunjoSettings::enableproxy());
        external_proxy->setChecked(WunjoSettings::externalproxy());
        qDebug() << "//// INITIAL REPORT; ENABLE EXT PROCY: " << WunjoSettings::externalproxy() << "\n++++++++";
        m_initialExternalProxyProfile = WunjoSettings::externalProxyProfile();
        generate_proxy->setChecked(WunjoSettings::generateproxy());
        proxy_minsize->setValue(WunjoSettings::proxyminsize());
        m_proxyparameters = WunjoSettings::proxyparams();
        generate_imageproxy->setChecked(WunjoSettings::generateimageproxy());
        proxy_imageminsize->setValue(WunjoSettings::proxyimageminsize());
        proxy_resize->setValue(WunjoSettings::proxyscale());
        m_proxyextension = WunjoSettings::proxyextension();
        m_previewparams = WunjoSettings::previewparams();
        m_previewextension = WunjoSettings::previewextension();
        if (!WunjoSettings::defaultprojectfolder().isEmpty()) {
            project_folder->setUrl(QUrl::fromLocalFile(WunjoSettings::defaultprojectfolder()));
        } else {
        ...
        if (WunjoSettings::customprojectfolder()) {
            custom_folder->setChecked(true);
        } else if (WunjoSettings::sameprojectfolder()) {
            same_folder->setChecked(true);
        } else {
            default_folder->setChecked(true);
        }
    ...
    for (int i = 0; i < files.count(); ++i) {
        ...
        if (type != QLatin1String("colour") && type != QLatin1String("color")) {
            ...
            if (!url.isEmpty() && url != QLatin1String("<producer>")) {
                ...
                if (url.endsWith(QLatin1String(".mlt")) || url.endsWith(QLatin1String(".wmproj"))) {
...
void ProjectSettings::slotExportToText()
{
    ...
    if (!tmpfile.open()) {
        qCWarning(WUNJO_LOG) << "/////  CANNOT CREATE TMP FILE in: " << tmpfile.fileName();
        return;
    }
    ...
    while (k.hasNext()) {
        ...
        if (!k.key().isEmpty()) {
            ...
            if (!itemCodec.isEmpty() && allHWCodecs.contains(itemCodec)) {
                if (WunjoSettings::supportedHWCodecs().contains(itemCodec)) {
