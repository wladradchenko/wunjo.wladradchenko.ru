    ...
    , m_originalProfile(QStringLiteral("invalid"))
{
    ...
    auto updateFps = [&]() {
        ...
        WunjoSettings::setProfile_fps_filter(m_fpsFilt->currentText());
    ...
    int ix = m_fpsFilt->findText(WunjoSettings::profile_fps_filter());
    if (ix > -1) {
    ...
    auto updateScanning = [&]() {
        ...
        WunjoSettings::setProfile_scanning_filter(m_scanningFilt->currentText());
    ...
    ix = m_scanningFilt->findText(WunjoSettings::profile_scanning_filter());
    if (ix > -1) {
