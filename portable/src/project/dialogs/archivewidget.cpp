    ...
    , m_missingClips(0)
{
    ...
    for (const std::shared_ptr<ProjectClip> &clip : std::as_const(clipList)) {
        ...
        if (t == ClipType::SlideShow) {
            ...
            if (t == ClipType::TextTemplate) {
                ...
                QString proxy = clip->getProducerProperty(QStringLiteral("wunjo:proxy"));
    ...
    if (tracks.size() == 0 || !xmlData.contains(QStringLiteral("wunjo:docproperties.version"))) {
        m_infoMessage->setMessageType(KMessageWidget::Warning);
...
void ArchiveWidget::openArchiveForExtraction()
{
    ...
    // Check that it is a wunjo project archive
    bool isProjectArchive = false;
    QStringList files = m_archive->directory()->entries();
    for (int i = 0; i < files.count(); ++i) {
        if (files.at(i).endsWith(QLatin1String(".wmproj"))) {
    ...
    for (int i = 0; i < files_list->topLevelItemCount(); ++i) {
        ...
        for (int j = 0; j < parentItem->childCount(); ++j) {
            ...
            if (parentItem->data(0, Qt::UserRole).toString() == QLatin1String("texts")) {
                // Special case: wunjo title files may contain urls that need to be replaced
...
bool ArchiveWidget::writeTmpFile(bool isArchive, const QString &playList, const QString &destPath, const QDir destUrl, const QString &filename)
{
    if (isArchive) {
        ...
        if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            qCWarning(WUNJO_LOG) << "//////  ERROR writing to file: " << file.fileName();
...
QString ArchiveWidget::processTitleFile(const QString &filename)
{
    ...
    return processWunjoTitleFile(doc, QStringLiteral("../"));
}
...
bool ArchiveWidget::processProjectFile()
{
    ...
    QString path = archive_url->url().toLocalFile() + QDir::separator() + m_name + QStringLiteral("-backup.wmproj");
    ...
    path = archive_url->url().toLocalFile() + QDir::separator() + m_name + QStringLiteral(".wmproj");
    QFile file(path);
...
void ArchiveWidget::processElement(QDomElement e, const QString root)
{
    ...
    src = Xml::getXmlProperty(e, QStringLiteral("wunjo:proxy"));
    if (src.size() > 2) {
        if (QFileInfo(src).isRelative()) {
            src.prepend(root);
        }
        ...
        QUrl dest = m_replacementList.value(srcUrl);
        if (!dest.isEmpty()) {
            Xml::setXmlProperty(e, QStringLiteral("wunjo:proxy"), dest.toLocalFile());
        }
    }
    propertyProcessUrl(e, QStringLiteral("wunjo:originalurl"), root);
...
QString ArchiveWidget::processWunjoTitleFile(const QDomDocument &doc, const QString &destPrefix)
{
...
void ArchiveWidget::createArchive()
{
    ...
    if (success) {
        success = m_archive->addLocalFile(m_temp->fileName(), m_name + QStringLiteral(".wmproj"));
        delete m_temp;
        m_temp = nullptr;
    }
...
void ArchiveWidget::slotExtractingFinished()
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        if (playList.isEmpty()) {
            ...
            if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
                qCWarning(WUNJO_LOG) << "//////  ERROR writing to file: ";
                error = true;
            } else {
                file.write(playList.toUtf8());
                if (file.error() != QFile::NoError) {
