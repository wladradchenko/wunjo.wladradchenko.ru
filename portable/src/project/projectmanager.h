...
public:
    ...
    */
    ...
    void testSetDocument(WunjoDoc *doc);
    ...
    /** @brief Check if a file has a wunjo mime type, returns true if it does.
     */
    static bool isWunjoProjectFile(const QUrl url);
...
public Q_SLOTS:
     ...
     * ~/.kde/data/stalefiles/wunjo/ \n
     * that will be actually written in WunjoDoc::slotAutoSave()
...
public:
    /** @brief Whether a question may be put to the user right now.
     *
     *  A project change asked for from outside the window — by the chat
     *  assistant, by an agent over the scripting interface — has nobody sitting
     *  in front of the dialog it would open. Worse than unanswered: the modal
     *  spins a nested event loop in the middle of tearing a document down, and
     *  that has taken the whole editor with it. While this is off, anything
     *  that would ask takes the safe course instead and says so in the log.
     */
    void setPrompting(bool allowed) { m_prompting = allowed; }
...
protected:
    ...
    WunjoDoc *m_project{nullptr};
    bool m_prompting{true};
...
private:
    ...
    void finalizeDocumentOpening(WunjoDoc *document);
};
