    ...
    , m_postParams(std::move(postParams))
{
    ...
    auto_add->setChecked(WunjoSettings::add_new_clip());
...
{
    WunjoSettings::setAdd_new_clip(auto_add->isChecked());
...
void ClipTranscode::slotStartTransCode()
{
    ...
    if (WunjoSettings::ffmpegpath().isEmpty()) {
        ...
        log_text->setPlainText(i18n("FFmpeg not found, please set path in Wunjo's settings Environment"));
    ...
    bool mltEncoding = s_url.endsWith(QLatin1String(".mlt")) || s_url.endsWith(QLatin1String(".wmproj"));
    ...
    if (mltEncoding) {
        ...
        m_transcodeProcess.start(WunjoSettings::meltpath(), parameters);
    ...
    buttonBox->button(QDialogButtonBox::Abort)->setText(i18n("Abort"));
    m_destination = destination + extension;
    m_transcodeProcess.start(WunjoSettings::ffmpegpath(), parameters);
    source_url->setEnabled(false);
    dest_url->setEnabled(false);
    button_start->setEnabled(false);
}
