    ...
    : QDialog(parent)
{
    ...
    if (WunjoSettings::transcodeFriendly().isEmpty()) {
        // Use GPU accel by default if possible
        if (WunjoSettings::supportedHWCodecs().contains(QLatin1String("h264_nvenc"))) {
            WunjoSettings::setTranscodeFriendly(QStringLiteral("Lossy x264 I frame only (NVidia GPU)"));
        } else if (WunjoSettings::supportedHWCodecs().contains(QLatin1String("h264_vaapi"))) {
            WunjoSettings::setTranscodeFriendly(QStringLiteral("Lossy x264 I frame only (VAAPI GPU)"));
        } else {
            WunjoSettings::setTranscodeFriendly(QStringLiteral("Lossy x264 I frame only"));
        }
    }
    setAttribute(Qt::WA_DeleteOnClose, false);
    setWindowTitle(i18nc("@title:window", "Transcode Clip"));
    KConfig conf(QStringLiteral("transcodingrc"), KConfig::CascadeConfig, QStandardPaths::AppDataLocation);
    KConfigGroup group(&conf, "intermediate");
    replace_original->setChecked(forceReplace || WunjoSettings::transcodingReplace());
    ...
    int ix = encodingprofiles->findText(WunjoSettings::transcodeFriendly());
    if (ix > -1) {
...
QString TranscodeSeek::params(int clipType, std::pair<int, int> fps_info) const
{
    ...
    case ClipType::Video: {
        if (m_encodeParams.value(encodingprofiles->currentText()).endsWith(QLatin1String(";av"))) {
            WunjoSettings::setTranscodeFriendly(encodingprofiles->currentText());
