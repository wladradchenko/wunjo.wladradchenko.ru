...
{
    QString filter = i18n("Wunjo Project") + QStringLiteral(" (*.wmproj)");
    if (ark) {
...
void ProjectManager::slotLoadOnOpen()
{
    if (m_startUrl.isValid()) {
        slotOpenFile();
    } else if (WunjoSettings::openlastproject()) {
        openLastFile();
    } else {
        newFile(false);
    }
    ...
    // Ask the release feed whether a newer build exists. Later than the cache
    // check and on its own timer: it goes to the network, and nothing about
    // opening a project should wait on that.
    QTimer::singleShot(5000, pCore->window(), &MainWindow::checkForNewVersion);
}
...
void ProjectManager::init(const QUrl &projectUrl, const QStringList &clipList)
{
    ...
    m_autoSaveTimer.setInterval(1000 * WunjoSettings::autosave_time());
...
{
    QString profileName = WunjoSettings::default_profile();
...
{
    QUrl startFile = QUrl::fromLocalFile(WunjoSettings::defaultprojectfolder() + QStringLiteral("/_untitled.wmproj"));
    Q_EMIT pCore->GUISetupDone();
    ...
    std::pair<int, int> projectTracks{WunjoSettings::videotracks(), WunjoSettings::audiotracks()};
    int audioChannels = 2;
    if (WunjoSettings::audio_channels() == 1) {
        audioChannels = 4;
    } else if (WunjoSettings::audio_channels() == 2) {
        audioChannels = 6;
    }
    ...
    bool sameProjectFolder = WunjoSettings::sameprojectfolder();
    if (!showProjectSettings) {
        ...
        QPointer<ProjectSettings> w = new ProjectSettings(nullptr, QMap<QString, QString>(), projectTracks.first, projectTracks.second, audioChannels,
                                                          WunjoSettings::defaultprojectfolder(), false, true, pCore->window());
    ...
    WunjoDoc *doc = new WunjoDoc(projectFolder, pCore->window()->m_commandStack, profileName, documentProperties, documentMetadata, projectTracks,
    ...
    m_project = doc;
    initSequenceProperties(m_project->uuid(), {WunjoSettings::audiotracks(), WunjoSettings::videotracks()});
...
void ProjectManager::testSetDocument(WunjoDoc *doc)
{
    m_project = doc;
}
        ...
        if (m_project->getTimeline(i.key(), true) == nullptr) {
            ...
            if (tc) {
                ...
                if (constructTimelineFromTractor(timelineModel, nullptr, *tc.get(), m_project->modifiedDecimalPoint(), chunks, dirty)) {
                    ...
                    prod->parent().set("wunjo:clipname", clip->clipName().toUtf8().constData());
                    prod->set("wunjo:description", clip->description().toUtf8().constData());
...
bool ProjectManager::closeCurrentDocument(bool saveChanges, bool quit)
{
    ...
    // the next project hands the same bin ids to different clips
    FaceDataStore::instance().clearSession();
...
bool ProjectManager::saveFileAs(const QString &outputFileName, bool saveOverExistingFile, bool saveACopy)
{
    ...
    if (!saveACopy) {
        ...
        // setting up autosave file in ~/.kde/data/stalefiles/wunjo/
        // saved under file name
        // actual saving by WunjoDoc::slotAutoSave() called by a timer 3 seconds after the document has been edited
        // This timer is set by WunjoDoc::setModified()
    ...
    KRecentDirs::add(QStringLiteral(":WunjoProjectsFolder"), saveFolder);
    saveRecentFiles();
    if (!saveACopy) {
...
bool ProjectManager::saveFileAs(bool saveACopy)
{
    ...
    if (m_project->url().isValid()) {
        ...
        fd.setDirectory(WunjoSettings::defaultprojectfolder());
    }
    fd.setNameFilter(getProjectNameFilters(false));
    fd.setAcceptMode(QFileDialog::AcceptSave);
    fd.setFileMode(QFileDialog::AnyFile);
    fd.setDefaultSuffix(QStringLiteral("wmproj"));
...
bool ProjectManager::saveFile()
{
    if (!m_project) {
        ...
        qCDebug(WUNJO_LOG) << "SaveFile called without project";
        return false;
    }
    if (m_project->url().isEmpty()) {
        return saveFileAs();
...
void ProjectManager::slotOpenFile()
{
    ...
    Q_EMIT pCore->GUISetupDone();
    QString projectFolder = KRecentDirs::dir(QStringLiteral(":WunjoProjectsFolder"));
    ...
    KRecentDirs::add(QStringLiteral(":WunjoProjectsFolder"), url.adjusted(QUrl::RemoveFilename).toLocalFile());
    m_recentFilesAction->addUrl(url);
    saveRecentFiles();
    openFile(url);
}
...
// to find autosaved files (in ~/.local/share/stalefiles/wunjo) and recover it
bool ProjectManager::checkForBackupFile(const QUrl &url, bool newFile)
{
    ...
    if (!m_prompting) {
        // A script asked for this project, not for a decision about somebody's
        // unsaved work. Nothing here is opened, so no lock is taken and no copy
        // is removed: an auto-save left by a machine that went down mid-edit
        // waits for the next time the project is opened by hand, when there is
        // someone to ask. Discarding it quietly is losing work nobody offered.
        if (!staleFiles.isEmpty()) {
            qWarning() << "an auto-saved copy of" << url.fileName()
                       << "was left alone: this project was opened from a script, which cannot be asked";
        }
    ...
    // What is still ours to dispose of. The list from staleFiles() cannot be
    // walked twice: the search below deletes what it rejects, and the cleanup
    // that followed re-opened every entry — including the freed ones. That read
    // of released memory is what took the editor down without a word whenever a
    // project was opened after a machine that had been switched off mid-edit.
    QList<KAutoSaveFile *> undecided;
    // Check if we can have a lock on one of the file,
    // meaning it is not handled by any Wunjo instance
    for (KAutoSaveFile *stale : staleFiles) {
        if (orphanedFile != nullptr) {
            // one candidate is enough; the rest are disposed of below
            undecided.append(stale);
            continue;
        }
    ...
    // Deleting an open KAutoSaveFile takes it off the disk, which is what
    // "do not recover" means and what the ones nobody asked about deserve.
    auto discard = [](const QList<KAutoSaveFile *> &files) {
        for (KAutoSaveFile *stale : files) {
    ...
    if (orphanedFile) {
        ...
        if (KMessageBox::questionTwoActions(nullptr, i18n("Auto-saved file exists. Do you want to recover now?"), i18n("File Recovery"),
            ...
            discard(undecided);
            doOpenFile(url, orphanedFile); // takes the file over
            return true;
        }
        undecided.append(orphanedFile);
    }
...
{
    // Make sure the url is a Wunjo project file
    ...
    if (!url.fileName().endsWith(".wmproj")) {
        if (!isWunjoProjectFile(url)) {
...
bool ProjectManager::isWunjoProjectFile(const QUrl url)
{
    QMimeDatabase db;
    QMimeType type = db.mimeTypeForUrl(url);
    if (type.name().contains(QLatin1String("wmproj"))) {
        return true;
    }
    return false;
}
...
void ProjectManager::finalizeDocumentOpening(WunjoDoc *document)
{
...
void ProjectManager::doOpenFile(const QUrl &url, KAutoSaveFile *stale, bool isBackup)
{
    ...
    DocOpenResult openResult =
        WunjoDoc::Open(stale ? QUrl::fromLocalFile(stale->fileName()) : url, QString(), pCore->window()->m_commandStack, false, pCore->window());

    WunjoDoc *doc = nullptr;
    if (!openResult.isSuccessful() && !openResult.isAborted()) {
        if (!isBackup) {
            ...
            if (answer == KMessageBox::PrimaryAction) { // Open Backup
                ...
                // if file was broken by Wunjo 0.9.4, we can try recovering it. If successful, continue through rest of this function.
                openResult =
                    WunjoDoc::Open(stale ? QUrl::fromLocalFile(stale->fileName()) : url, QString(), pCore->window()->m_commandStack, true, pCore->window());
    ...
    if (stale == nullptr) {
        ...
        if (url.fileName().contains(QStringLiteral("_untitled.wmproj"))) {
            doc->setUrl(QUrl());
    ...
    for (auto &id : sequenceIds) {
        ClipLoadTask::start(ObjectId(WunjoObjectType::BinClip, id.toInt(), QUuid()), QDomElement(), true, -1, -1, this);
    }
    if (!activeUuid.isNull()) {
...
void ProjectManager::doOpenFileHeadless(const QUrl &url)
{
    ...
    DocOpenResult openResult = WunjoDoc::Open(url, QString() /*QDir::temp().path()*/, undoGroup, false, nullptr);
...
WunjoDoc *ProjectManager::current()
{
    return m_project;
}
...
bool ProjectManager::slotOpenBackup(const QUrl &url)
{
    ...
    if (url.isValid()) {
        ...
        projectFolder = QUrl::fromLocalFile(WunjoSettings::defaultprojectfolder());
        projectFile = url;
    } else {
...
void ProjectManager::slotStartAutoSave()
{
    ...
    if (m_autoSaveChangeCount >= WunjoSettings::autosave_ops() && m_lastSave.elapsed() > 5000) {
...
{
    m_autoSaveTimer.setInterval(WunjoSettings::autosave_time() * 1000);
}
...
void ProjectManager::slotAutoSave()
{
    ...
    if (!pCore->window()->m_commandStack->activeStack()->canUndo()) {
        ...
        m_autoSaveChangeCount = WunjoSettings::autosave_ops();
        return;
    }
...
void ProjectManager::prepareSave()
{
    ...
    pCore->projectItemModel()->saveProperty(QStringLiteral("wunjo:documentnotes"), documentNotes());
    pCore->projectItemModel()->saveProperty(QStringLiteral("wunjo:documentnotesversion"), QStringLiteral("2"));
    pCore->projectItemModel()->saveProperty(QStringLiteral("wunjo:docproperties.opensequences"), pCore->window()->openedSequences().join(QLatin1Char(';')));
    pCore->projectItemModel()->saveProperty(QStringLiteral("wunjo:docproperties.activetimeline"), m_activeTimelineModel->uuid().toString());
    KDDockWidgets::LayoutSaver dockLayout(KDDockWidgets::RestoreOption_AbsoluteFloatingDockWindows);
    pCore->projectItemModel()->saveProperty(QStringLiteral("wunjo:docproperties.layout"), QString(dockLayout.serializeLayout()));
}
                                            ...
                                            std::shared_ptr<TimelineItemModel> timelineModel, TimelineWidget *timelineWidget)
{
    ...
    prod->set("wunjo:uuid", uuid.toString().toUtf8().constData());
    prod->set("wunjo:clipname", i18n("Sequence 1").toUtf8().constData());
    prod->set("wunjo:producer_type", ClipType::Timeline);
    prod->set("wunjo:sequenceproperties.tracksCount", tracks.first + tracks.second);
    prod->parent().set("wunjo:uuid", uuid.toString().toUtf8().constData());
    prod->parent().set("wunjo:clipname", i18n("Sequence 1").toUtf8().constData());
    prod->parent().set("wunjo:sequenceproperties.hasAudio", tracks.first > 0 ? 1 : 0);
    prod->parent().set("wunjo:sequenceproperties.hasVideo", tracks.second > 0 ? 1 : 0);
    if (m_project->hasSequenceProperty(uuid, QStringLiteral("activeTrack"))) {
        ...
        prod->parent().set("wunjo:sequenceproperties.activeTrack", activeTrack);
    }
    prod->parent().set("wunjo:sequenceproperties.tracksCount", tracks.first + tracks.second);
    prod->parent().set("wunjo:sequenceproperties.documentuuid", m_project->uuid().toString().toUtf8().constData());
    prod->parent().set("wunjo:control_uuid", m_project->uuid().toString().toUtf8().constData());
    if (tractor.property_exists("wunjo:duration")) {
        const QString duration(tractor.get("wunjo:duration"));
        const QString maxduration(tractor.get("wunjo:maxduration"));
        prod->parent().set("wunjo:duration", duration.toLatin1().constData());
        prod->parent().set("wunjo:maxduration", maxduration.toLatin1().constData());
    } else {
        ...
        prod->parent().set("wunjo:duration", timelineModel->tractor()->frames_to_time(projectDuration + 1));
        prod->parent().set("wunjo:maxduration", durations.first + 1);
        prod->parent().set("length", projectDuration + 1);
        prod->parent().set("out", projectDuration);
    }
    if (tractor.property_exists("wunjo:sequenceproperties.timelineHash")) {
        prod->parent().set("wunjo:sequenceproperties.timelineHash", tractor.get("wunjo:sequenceproperties.timelineHash"));
    }

    prod->parent().set("wunjo:producer_type", ClipType::Timeline);
}
...
void ProjectManager::updateSequenceDuration(const QUuid &uuid)
{
    ...
    if (mainClip && model) {
        ...
        properties.insert(QStringLiteral("wunjo:duration"), QString(model->tractor()->frames_to_time(newDuration)));
        properties.insert(QStringLiteral("wunjo:maxduration"), QString::number(durations.first));
...
{
    if (effectData.contains(QStringLiteral("wunjo/effect"))) {
        pCore->window()->addEffect(effectData.value(QStringLiteral("wunjo/effect")).toString());
    } else {
...
void ProjectManager::saveWithUpdatedProfile(const QString &updatedProfile)
{
    ...
    QTemporaryFile tmpFile(saveFolder + "/wunjo-XXXXXX.mlt");
    QTemporaryFile tmpFile2(saveFolder + "/wunjo-XXXXXX.mlt");
    ...
    for (int i = 0; i < playlists.count(); ++i) {
        ...
        if (e.attribute(QStringLiteral("id")) == QLatin1String("main_bin")) {
            Xml::setXmlProperty(e, QStringLiteral("wunjo:docproperties.profile"), updatedProfile);
            break;
        }
    }
    ...
    for (int i = 0; i < tractors.count(); ++i) {
        ...
        const QString &guidesData = Xml::getXmlProperty(e, QStringLiteral("wunjo:sequenceproperties.guides"));
        if (!guidesData.isEmpty()) {
            ...
            if (json.isArray()) {
                ...
                Xml::setXmlProperty(e, QStringLiteral("wunjo:sequenceproperties.guides"), QString::fromUtf8(updatedJSon.toJson()));
            }
        }
    }
    ...
    for (int i = 0; i < producers.count(); ++i) {
        ...
        if (Xml::getXmlProperty(e, QStringLiteral("mlt_service")) == QLatin1String("qimage")) {
            ...
            if (resource.contains(QLatin1Char('%')) || resource.contains(QStringLiteral("/.all.")) || resource.contains(QStringLiteral("\\.all."))) {
                ...
                Xml::removeXmlProperty(e, QStringLiteral("wunjo:duration"));
                                  ...
                                  bool openInMonitor, bool forceCompositingForExistingModel)
{
    ...
    if (tc != nullptr && tc->is_valid()) {
        ...
        if (xmlProd == nullptr || !xmlProd->is_valid()) {
            ...
            if (m_project->isModified() && WunjoSettings::crashrecovery()) {
    ...
    if (internalLoad) {
        ...
        sequenceProperties.pass_values(playlistProps, "wunjo:sequenceproperties.");
        ...
        prod->set("wunjo:duration", prod->frames_to_time(duration));
        prod->set("wunjo:maxduration", durations.first);
        prod->set("length", duration);
        prod->set("out", duration - 1);
        prod->set("wunjo:clipname", clip->clipName().toUtf8().constData());
        prod->set("wunjo:description", clip->description().toUtf8().constData());
        prod->set("wunjo:uuid", uuid.toString().toUtf8().constData());
        prod->set("wunjo:producer_type", ClipType::Timeline);

        prod->parent().set("wunjo:duration", prod->frames_to_time(duration));
        prod->parent().set("wunjo:maxduration", durations.first);
        prod->parent().set("length", duration);
        prod->parent().set("out", duration - 1);
        prod->parent().set("wunjo:clipname", clip->clipName().toUtf8().constData());
        prod->parent().set("wunjo:description", clip->description().toUtf8().constData());
        prod->parent().set("wunjo:uuid", uuid.toString().toUtf8().constData());
        prod->parent().set("wunjo:producer_type", ClipType::Timeline);
        QObject::connect(timelineModel.get(), &TimelineModel::durationUpdated, this, &ProjectManager::updateSequenceDuration,
                         static_cast<Qt::ConnectionType>(Qt::DirectConnection | Qt::UniqueConnection));
        ...
        int duration = durations.second > 0 ? durations.second : durations.first + 1;
        std::shared_ptr<Mlt::Producer> prod = std::make_shared<Mlt::Producer>(timelineModel->tractor());
        prod->set("wunjo:duration", timelineModel->tractor()->frames_to_time(duration));
        prod->set("wunjo:maxduration", durations.first);
        prod->set("length", duration);
        prod->set("wunjo:producer_type", ClipType::Timeline);
        prod->set("out", duration - 1);
...
void ProjectManager::doSyncTimeline(std::shared_ptr<TimelineItemModel> model, bool refresh)
{
    if (model) {
        ...
        if (refresh) {
            ...
            sequenceProps.pass_values(*model->tractor(), "wunjo:sequenceproperties.");
...
void ProjectManager::updateSequenceOffset(const QUuid &uuid)
{
    if (uuid == m_project->activeUuid) {
        ...
        int offset = m_project->getSequenceProperty(uuid, QStringLiteral("wunjo:sequenceproperties.timecodeOffset")).toInt();
...
{
    QFile lockFile(QDir::temp().absoluteFilePath(QStringLiteral("wunjolock")));
    if (lockFile.exists()) {
        lockFile.remove();
    }
}
