...
{
    buildWhisperDeps(WunjoSettings::enableSeamless());
...
void SpeechToTextWhisper::buildWhisperDeps(bool enableSeamless)
{
    ...
    if (!scriptPath.isEmpty()) {
        ...
        // And the torch that suits this machine's driver. Without it pip brings
        // whatever is newest, which on an older driver loads and then reports no
        // GPU: transcription silently runs on the processor, ten times slower,
...
const QString SpeechToTextWhisper::modelFolder(bool mainFolder, bool create)
{
    QString folder;
    if (mainFolder) {
        folder = WunjoSettings::whisperModelFolder();
...
const QStringList SpeechToTextWhisper::getInstalledModels()
{
    ...
    for (auto &m : WunjoSettings::whisperAvailableModels()) {
