...
/** @brief Whether this machine has a card whisper can actually use.
 *
 * Two answers are consulted because they fail in different ways: the driver
 * knows a card exists before any environment is built, and the device list —
 * filled by checkgpu.py through torch — knows whether the installed wheels can
 * reach it. A CUDA build on a machine with no driver, or a processor-only
 * torch on a machine with a card, is exactly the mismatch worth catching.
 */
bool hasUsableGpu()
{
    if (WunjoSettings::whisperDevice().startsWith(QLatin1String("cuda"))) {
        return true;
    }
    return PluginManager::driverCudaVersion() > 0;
}

/** @brief Models that stay useful without a card.
 *
 * The line is drawn where transcription stops keeping up with playback: tiny,
 * base and small do, medium and the large family do not — on a processor they
 * take longer than the recording itself. The English-only variants (".en") sit
 * on the same side as the model they are cut from.
 */
bool cpuFriendly(const QString &model)
{
    const QString base = model.section(QLatin1Char('.'), 0, 0);
    return base == QLatin1String("tiny") || base == QLatin1String("base") || base == QLatin1String("small");
}
    ...
    , m_modelToInstall(modelName)
{
    ...
    if (!hasUsableGpu()) {
        ...
        cpuNote->setText(i18n("No graphics card was found, so only the light models are offered. "
                              "Bigger ones transcribe slower than the recording plays."));
        l->addWidget(cpuNote);
    }
...
void WhisperDownload::parseScriptFeedback(const QString &scriptName, const QStringList args, const QStringList jobData)
{
    ...
    if (scriptName.contains("whisperquery")) {
        ...
        const bool gpuAvailable = hasUsableGpu();
        ...
        for (auto &s : jobData) {
            ...
            if (name == QLatin1String("root_folder")) {
                WunjoSettings::setWhisperModelFolder(s.section(QLatin1Char(':'), 1).simplified());
                qDebug() << "+++\nSET WHISPER FOLDER: " << WunjoSettings::whisperModelFolder() << "\n\n+++++";
                continue;
            }
            ...
            // Without a card the big models are a trap: medium and up transcribe
            // slower than the recording plays, so an hour of footage takes more
            // than an hour and looks like the application has hung. They stay
            // visible — the machine may get a card later — but cannot be picked,
            // and the row says why instead of leaving the user to guess.
            if (!gpuAvailable && !cpuFriendly(name)) {
                item->setFlags(Qt::NoItemFlags);
                item->setText(i18n("%1 — needs a graphics card", displayName));
                item->setToolTip(i18n("On the processor this model runs slower than the audio it transcribes."));
                continue;
            }
        ...
        if (!updatedModelsList.isEmpty()) {
            WunjoSettings::setWhisperAvailableModels(updatedModelsList);
            WunjoSettings::setWhisperAvailableModelsHash(hashList);
        }
        if (itemToInstall) {
