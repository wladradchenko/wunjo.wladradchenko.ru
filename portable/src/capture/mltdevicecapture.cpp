...
MltDeviceCapture::MltDeviceCapture(const QString &profile, /*VideoSurface *surface, */ QWidget *parent)
    : AbstractRender(Wunjo::RecordMonitor, parent)
    , doCapture(0)
    ...
    , m_droppedFrames(0)
    , m_livePreview(WunjoSettings::enable_recording_preview())
{
    analyseAudio = WunjoSettings::monitor_audio();
    if (profile.isEmpty()) {
        // profile = WunjoSettings::current_profile();
    }
...
bool MltDeviceCapture::buildConsumer(const QString &profileName)
{
    ...
    QString videoDriver = WunjoSettings::videodrivername();
    ...
    // OpenGL monitor
    m_mltConsumer = new Mlt::Consumer(*m_mltProfile, WunjoSettings::audiobackend().toUtf8().constData());
    ...
    m_mltConsumer->set("window_background", WunjoSettings::window_background().name().toUtf8().constData());
...
bool MltDeviceCapture::slotStartCapture(const QString &params, const QString &path, const QString &playlist, bool livePreview, bool xmlPlaylist)
{
    ...
    if (m_livePreview) {
        ...
        // OpenGL monitor
        previewProps->set("mlt_service", WunjoSettings::audiobackend().toUtf8().constData());
        previewProps->set("preview_off", 1);
        previewProps->set("preview_format", mlt_image_rgb);
        previewProps->set("terminate_on_pause", 0);
        m_showFrameEvent = m_mltConsumer->listen("consumer-frame-show", this, mlt_listener(consumer_gl_frame_show));
        // m_mltConsumer->set("resize", 1);
        previewProps->set("window_background", WunjoSettings::window_background().name().toUtf8().constData());
        QString audioDevice = WunjoSettings::audiodevicename();
        if (!audioDevice.isEmpty()) {
...
void MltDeviceCapture::setOverlay(const QString &path)
{
    ...
    if (service.type() != mlt_service_tractor_type) {
        qCWarning(WUNJO_LOG) << "// TRACTOR PROBLEM";
        return;
    }
    Mlt::Tractor tractor(service);
    if (tractor.count() < 2) {
