...
void MediaCapture::initializeAudioSetup()
{
    ...
    format.setSampleRate(WunjoSettings::audiocapturesamplerate());
    format.setChannelCount(WunjoSettings::audiocapturechannels());
    format.setSampleFormat(static_cast<QAudioFormat::SampleFormat>(WunjoSettings::audiocapturesampleformat()));
...
void MediaCapture::switchMonitorState(bool run)
{
    ...
    if (run) {
        ...
        qreal linearVolume =
            QtAudio::convertVolume(WunjoSettings::audiocapturevolume() / 100.0, QtAudio::LogarithmicVolumeScale, QtAudio::LinearVolumeScale);
...
{
    if (allowCountDown && !WunjoSettings::disablereccountdown()) {
        pCore->getMonitor(Wunjo::ProjectMonitor)->startCountDown();
...
{
    QString deviceName = WunjoSettings::defaultaudiocapture();
...
void MediaCapture::setAudioVolume()
{
    ...
    qreal linearVolume = QtAudio::convertVolume(WunjoSettings::audiocapturevolume() / 100.0, QtAudio::LogarithmicVolumeScale, QtAudio::LinearVolumeScale);
    m_audioSource->setVolume(linearVolume);
}
