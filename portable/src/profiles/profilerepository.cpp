...
void ProfileRepository::refresh()
{
    ...
    // When Wunjo profiles share the same spec, the MLT entry is discarded.
    using MltEntry = std::pair<QString, std::unique_ptr<ProfileModel>>;
    std::map<QString, MltEntry> mltBySpec;

    QDir mltDir(WunjoSettings::mltpath());
    ...
    for (const auto &file : std::as_const(mltFiles)) {
        ...
        if (!profile->is_valid()) {
            qCWarning(WUNJO_LOG) << "//// WARNING: invalid profile found: " << file << ". Ignoring.";
            continue;
        }
    ...
    // Step 2: Load Wunjo profiles. For each valid profile, erase any MLT
    // profile with the same spec (keeping Wunjo's copy), then insert.
    ...
    for (const auto &dir : std::as_const(customProfilesDir)) {
        ...
        for (const auto &file : std::as_const(files)) {
            ...
            if (!profile->is_valid()) {
                qCWarning(WUNJO_LOG) << "//// WARNING: invalid profile found: " << fullPath << ". Ignoring.";
                continue;
            }
    ...
    // Step 3: Insert remaining MLT profiles that have no Wunjo equivalent.
...
std::unique_ptr<ProfileModel> &ProfileRepository::getProfile(const QString &path)
{
    ...
    if (m_profiles.count(path) == 0) {
        qCWarning(WUNJO_LOG) << "Profile " << path << "not found. Returning default profile instead.";
        QString default_profile = WunjoSettings::default_profile();
        if (default_profile.isEmpty()) {
            default_profile = QStringLiteral("dv_pal");
        }
        if (m_profiles.count(default_profile) == 0) {
            qCWarning(WUNJO_LOG) << "Default profile" << default_profile << "not found. Returning random profile instead.";
...
bool ProfileRepository::deleteProfile(const QString &path)
{
    ...
    if (!success) {
        qCDebug(WUNJO_LOG) << "//// Cannot delete profile " << path << ", does not seem to be custom one";
    } else {
        if (m_profiles.count(path) > 0) {
